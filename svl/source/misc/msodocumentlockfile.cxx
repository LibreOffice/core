...
namespace
{
bool isWordFormat(const OUString& aOrigURL)
{
    INetURLObject aDocURL = LockFileCommon::ResolveLinks(INetURLObject(aOrigURL));

    return aDocURL.GetFileExtension().equalsIgnoreAsciiCase("DOC")
           || aDocURL.GetFileExtension().equalsIgnoreAsciiCase("DOCX")
           || aDocURL.GetFileExtension().equalsIgnoreAsciiCase("RTF")
           || aDocURL.GetFileExtension().equalsIgnoreAsciiCase("ODT");
}

bool isExcelFormat(const OUString& aOrigURL)
{
    INetURLObject aDocURL = LockFileCommon::ResolveLinks(INetURLObject(aOrigURL));

    return //aDocURL.GetFileExtension().equalsIgnoreAsciiCase("XLS") || // MSO does not create lockfile for XLS
        aDocURL.GetFileExtension().equalsIgnoreAsciiCase("XLSX")
        || aDocURL.GetFileExtension().equalsIgnoreAsciiCase("ODS");
}

bool isPowerPointFormat(const OUString& aOrigURL)
{
    INetURLObject aDocURL = LockFileCommon::ResolveLinks(INetURLObject(aOrigURL));

    return aDocURL.GetFileExtension().equalsIgnoreAsciiCase("PPTX")
           || aDocURL.GetFileExtension().equalsIgnoreAsciiCase("PPT")
           || aDocURL.GetFileExtension().equalsIgnoreAsciiCase("ODP");
}
...
MSODocumentLockFile::AppType MSODocumentLockFile::getAppType(const OUString& sOrigURL)
{
    ...
    if (isWordFormat(sOrigURL))
        eResult = AppType::Word;
    else if (isExcelFormat(sOrigURL))
