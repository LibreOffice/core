...
{
    bool bError = false;
    const OpCode eOpCode = static_cast<OpCode>(_aToken.OpCode);      
...
bool FormulaMissingContext::AddMissing( FormulaTokenArray *pNewArr, const MissingConvention & rConv  ) const
{
    ...
    {
        case ocAddress:
            return AddDefaultArg( pNewArr, 2, 1.0 );    
        default:
            break;
    }
    ...
    else
    {
        ...
        switch (eOp)
        {
            ...
            case ocBW:      
            case ocZW:      
                bRet |= AddDefaultArg( pNewArr, 2, 0.0 );   
                bRet |= AddDefaultArg( pNewArr, 3, 0.0 );   
                break;
            case ocZins:    
                bRet |= AddDefaultArg( pNewArr, 1, 0.0 );   
                bRet |= AddDefaultArg( pNewArr, 3, 0.0 );   
                bRet |= AddDefaultArg( pNewArr, 4, 0.0 );   
                break;
            case ocExternal:
...
FormulaTokenArray * FormulaTokenArray::RewriteMissingToPof( const MissingConvention & rConv )
{
    ...
    int aOpCodeAddressStack[ nAlloc ];  
    const int nOmitAddressArg = 3;      
