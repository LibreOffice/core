...
def get_files_list(directory, extension):
    ...
    with os.scandir(directory) as dh:
        for entry in dh:
            if entry.is_dir():
                array_items += get_files_list(entry.path, extension)
            elif entry.is_file():
                if entry.name[-4:] == extension:
            ...
            if '<node oor:name=".uno:' in line:
                ...
                while '<value xml:lang="en-US">' not in line:
                    try:
                        ...
                        print("Couldn't find '<value xml:lang=\"en-US\">' line in %s" % filename,
                              file=sys.stderr)
                        break
...
def main():
    ...
    for sdi_file in sdi_files:
        sdi_file_basename, sdi_file_ext = os.path.splitext(os.path.basename(sdi_file))
        if sdi_file_basename == args.module and sdi_file_ext == sdi_ext:
