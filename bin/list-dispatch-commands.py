...
def print_output(all_commands):
    ...
    def sources(cmd):
        ...
        if cmd[hxxfile] >= 0:
            src += ' (' + str(cmd[hxxfile] + 1 + len(XCU_FILES) + len(SDI_FILES)) + '-' + str(cmd[hxxlinenumber]) + ')'
        return src.strip()
    #
    ...
    for cmd in sorted_by_module:
        ...
        if lastmodule != cmd[module]:
            ...
            print('! scope="col" | Internal<br>name')
            print('! scope="col" | Internal<br>value')
            print('! scope="col" | Mode')
            print('! scope="col" | Sources<br>(index)')
        print('|-\n')
