...
"""
while True:
    Get every screenshot_id posted in weblate and gather them in ids_to_delete
    if contain_next_page:
        current_screenshots_response = next_page_response.content
    else:
    ...
    if screenshots['next'] is not None:
        ...
        next_page_response = next_page_request(screenshots['next'],AUTH_HEADER)
    else:
        contain_next_page = False
        break
    if(int((next_page_response.headers)['X-RateLimit-Remaining'])) < 1000:
        ...
        if int((next_page_response.headers)['X-RateLimit-Remaining']) < 80:
            rate_limit_exceeded((next_page_response.headers)['X-RateLimit-Reset'])

for id in ids_to_delete:
    response = requests.delete(SCREENSHOTSLINK + '/' + id + '/', headers=AUTH_HEADER)
...
for screenshotpath in screenshot_to_strings.keys():
    ...
    if screenshotname in current_screenshots.keys():
        ...
        try:
            ...
            print('awwwwwwwwwwwwwwww')
            not_added_units.append([screenshotId , e])
    ...
    for ui in screenshot_to_strings[screenshotpath].keys():
        ...
        try:
            ...
            not_added_units.append((fileparameter,parameters))
with open(os.path.join(SCREENSHOTS_DIR_PATH
