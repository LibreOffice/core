    ...
    for ui in screenshotsWithUnitId[screenshotpath].keys():
        ...
        for i in range(len (uilist)):
            ...
            try:
                ...
                print('ratelimit remaining :' + (responseToAssociation.headers)['X-RateLimit-Remaining'] + 'time remaining ' +(CurrentscreenshotsRequest.headers)['X-RateLimit-Reset'])
...
with open( os.path.join(os.getcwd() , 'workdir','screenshots','exceptions.json'), 'w') as outfile :
    json.dump(exceptionsthrown , outfile)
    outfile.close
