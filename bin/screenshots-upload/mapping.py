...
LONG_OPTIONS = ["include_extended_tip"]
include_extended_tip = False
    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        for line in file:
            if line.__contains__('#.'):
                ...
                if((context.__contains__('extended_tip') and include_extended_tip)
                or (not context.__contains__('extended_tip'))):
...
# screenshot_to_strings contains:  screenshot_path -> {ui_path->{context, KeyId} }

argument_list = sys.argv[1:]
args, vals = getopt.getopt(argument_list, "", LONG_OPTIONS)
for argument in args:
    if(argument[0] in ['--include_extended_tip']):
        include_extended_tip = True
