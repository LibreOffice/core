...
rtl::OUString makeUrl(const rtl::OUString &rName)
{
    return rtl::OUString("file://") + TEST_DIR + "/" + rName;
}
...
void ImportTest::testWK3WithFM3()
{
    ...
    CPPUNIT_ASSERT_EQUAL(0x0000ff, nCharColor); // blue text
}
...
WpftLoader ImportTest::createLoader(const rtl::OUString &rUrl, const rtl::OUString &rFactoryUrl) const
{
    ...
    aDesc[MediaDescriptor::PROP_URL()] <<= rUrl;
    aDesc[MediaDescriptor::PROP_READONLY()] <<= sal_True;
    uno::Sequence<beans::PropertyValue> lDesc(aDesc.getAsConstPropertyValueList());
    const rtl::OUString sType = m_xTypeDetection->queryTypeByDescriptor(lDesc, sal_True);
