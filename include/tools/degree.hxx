...
template <typename I, int N> char (&NofDegree(Degree<I, N>))[N];
...
template <class To, typename IFrom, int NFrom> inline To to(Degree<IFrom, NFrom> x)
{
    constexpr sal_Int64 m = Nof<To> / std::gcd(Nof<To>, NFrom);
    constexpr sal_Int64 d = NFrom / std::gcd(Nof<To>, NFrom);
