...
constexpr sal_Int64 MulDiv(I n)
{
    ...
    // This excludes sal_uInt64 as not representable in sal_Int64
    static_assert(sizeof(sal_Int64) >= sizeof(I)
                  && std::is_signed_v<std::common_type_t<sal_Int64, I>>);
    if constexpr (mul % div == 0)
    {
        assert(sal_Int64(n) <= std::numeric_limits<sal_Int64>::max() / (mul / div));
        return sal_Int64(n) * (mul / div);
    }
    else if constexpr (div % mul == 0)
        return sal_Int64(n) / (div / mul);
    else
    {
        assert(sal_Int64(n) <= (std::numeric_limits<sal_Int64>::max() - div / 2) / mul
               && sal_Int64(n) >= (std::numeric_limits<sal_Int64>::min() + div / 2) / mul);
        return (n >= 0 ? (sal_Int64(n) * mul + div / 2) : (sal_Int64(n) * mul - div / 2)) / div;
    }
}
