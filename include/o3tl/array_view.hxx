...
#if defined _MSC_VER
    /// VS2015 has quite limited support for expression SFINAE
    template<class P = array_view<value_type const>, typename V = std::enable_if_t<!std::is_same<P, value_type>::value>>
#else
    template<class P = array_view<value_type const>, typename = std::enable_if<!std::is_const<value_type>::value>>
#endif
    operator P() const
    {
