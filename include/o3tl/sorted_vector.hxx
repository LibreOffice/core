...
public:
    ...
    std::pair<const_iterator,bool> insert( const Value& x )
    {
        ...
        if (!ret.second)
        {
            const_iterator const it = m_vector.insert(begin() + (ret.first - begin()), x);
    ...
    size_type erase( const Value& x )
    {
        ...
        if (ret.second)
        {
            m_vector.erase(begin() + (ret.first - begin()));
            return 1;
        }
        return 0;
    }
    ...
    {   // C++98 has vector::erase(iterator), so call that
        m_vector.erase(begin() + (position - begin()));
    }

    void erase(const_iterator const& first, const_iterator const& last)
    {
        m_vector.erase(begin() + (first - begin()),
                       begin() + (last - begin()));
    }
    ...
    void insert(sorted_vector<Value,Compare,Find> const& rOther)
    {
       ...
       if ( empty() )
       {
           m_vector.insert(begin(), rOther.begin(), rOther.end());
       }
       else
       {
