...
// Similar to O[U]String::getToken, returning the first token of a std::string_view, starting at a
// given position (and if needed, it can be extended to return the n'th token instead of just the
// first, or support an initial position of npos):
