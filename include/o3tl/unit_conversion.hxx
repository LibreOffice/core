...
enum class Length
{
    ...
    in1000, // in/1000, corresponds to MapUnit::Map1000thInch
    in100, // in/100, corresponds to MapUnit::Map100thInch
    in10, // in/10, corresponds to MapUnit::Map10thInch
    in,
    ft,
    mi,
    master, // PPT Master Unit: 1/576 in
    px, // "Pixel" unit: 15 twip (96 ppi), corresponds to MeasureUnit::PIXEL
    ch, // "Char" unit: 14 px, corresponds to FieldUnit::CHAR
    line, // "Line" unit: 11/45 in (? FIXME), corresponds to FieldUnit::LINE
...
static constexpr m_and_d mdBaseLen[] = {
    { 1, 100 }, // mm100 => mm
    { 1, 10 }, // mm10 => mm
    { 1, 1 }, // mm => mm
    { 10, 1 }, // cm => mm
    ...
    { 254 * 15 * 14, 10 * 1440 }, // ch => mm
    { 254 * 11, 10 * 45 }, // line => mm
};
...
static constexpr auto mdLen = prepareMDArray(mdBaseLen);

// an overload taking Length
constexpr sal_Int64 md(Length i, Length j)
{
    const int nI = static_cast<int>(i), nJ = static_cast<int>(j);
    assert(nI >= 0 && static_cast<size_t>(nI) < mdLen.size());
    assert(nJ >= 0 && static_cast<size_t>(nJ) < mdLen.size());
    return mdLen[nI][nJ];
}
