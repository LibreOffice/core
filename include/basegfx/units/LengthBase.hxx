...
/** Base for the length unit template class
 *
...
public:
    ...
    // We can get away with default "rocket" operator
    constexpr auto operator<=>(LengthBase const&) const = default;

    // Return the value in its raw form for data access
    constexpr TYPE data() const { return m_nValue; }

    // Convert to a input unit
    ...
    constexpr double as_hmm() const { return m_nValue / double(constFactor_hmm_to_EMU); }
    constexpr double as_mm() const { return m_nValue / double(constFactor_mm_to_EMU); }
    constexpr double as_cm() const { return m_nValue / double(constFactor_cm_to_EMU); }
    constexpr double as_meter() const { return m_nValue / double(constFactor_meter_to_EMU); }
    constexpr double as_twip() const { return m_nValue / double(constFactor_twip_to_EMU); }
    constexpr double as_in() const { return m_nValue / double(constFactor_in_to_EMU); }
    constexpr double as_pt() const { return m_nValue / double(constFactor_pt_to_EMU); }
    constexpr double as_px() const { return m_nValue / double(constFactor_px_to_EMU); }
    constexpr double as_emu() const { return double(m_nValue); }
};
