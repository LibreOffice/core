...
{
    std::vector<T> vec(static_cast<size_t>(seq.getLength()));
    const T* elems = seq.getConstArray(); //Direct access to avoid [] access overhead
                ...
                toSequence(const std::vector<T>& vec)
{
    ...
    T* elems = seq.getArray(); //Direct access to avoid [] access overhead
    for (size_t i=0; i<vec.size(); ++i)
        elems[i] = vec[i];
    return seq;
}
