...
{
    HKEY uninstallKey;
    if (RegOpenKeyExW(HKEY_LOCAL_MACHINE, MAINT_UNINSTALL_KEY, 0, KEY_WRITE | KEY_WOW64_32KEY,
                      &uninstallKey)
        != ERROR_SUCCESS)
    {
        return FALSE;
...
static BOOL GetVersionNumberFromPath(LPWSTR path, DWORD& A, DWORD& B, DWORD& C, DWORD& D)
{
    ...
    {
        LOG_WARN(("Could not obtain file info of old service.  (%lu)", GetLastError()));
        return FALSE;
    }
    ...
    if (!VerQueryValueW(fileVersionInfo.get(), L"\\", reinterpret_cast<LPVOID*>(&fixedFileInfo),
                        &size))
    {
        LOG_WARN(("Could not query file version info of old service.  (%lu)", GetLastError()));
        return FALSE;
    }
...
BOOL UpdateServiceDescription(SC_HANDLE serviceHandle)
{
    ...
    int bufferSize
        = MultiByteToWideChar(CP_UTF8, 0, serviceStrings.serviceDescription.get(), -1, nullptr, 0);
    mozilla::UniquePtr<WCHAR[]> serviceDescription = mozilla::MakeUnique<WCHAR[]>(bufferSize);
    if (!MultiByteToWideChar(CP_UTF8, 0, serviceStrings.serviceDescription.get(), -1,
                             serviceDescription.get(), bufferSize))
    {
        LOG_WARN(("Could not convert description to wide string format.  (%lu)", GetLastError()));
        return FALSE;
    }
...
BOOL SvcInstall(SvcInstallAction action)
{
    ...
    nsAutoServiceHandle schSCManager(OpenSCManager(nullptr, nullptr, SC_MANAGER_ALL_ACCESS));
    if (!schSCManager)
    {
    ...
    nsAutoServiceHandle schService(OpenServiceW(schSCManager, SVC_NAME, SERVICE_ALL_ACCESS));
    ...
    if (schService)
    {
        ...
        {
            LOG_WARN(("Could not determine buffer size for query service config.  (%lu)",
                      GetLastError()));
            return FALSE;
        }
        ...
        if (!QueryServiceConfigW(
                schService, reinterpret_cast<QUERY_SERVICE_CONFIGW*>(serviceConfigBuffer.get()),
                bytesNeeded, &bytesNeeded))
        {
            LOG_WARN(
                ("Could open service but could not query service config.  (%lu)", GetLastError()));
            return FALSE;
        }
        ...
        {
            if (!FixServicePath(schService, serviceConfig.lpBinaryPathName, servicePathWasWrong))
            {
                LOG_WARN(("Could not fix service path. This should never happen.  (%lu)",
                          GetLastError()));
    ...
    schService.own(CreateServiceW(schSCManager, SVC_NAME, SVC_DISPLAY_NAME, SERVICE_ALL_ACCESS,
                                  SERVICE_WIN32_OWN_PROCESS, SERVICE_DEMAND_START,
                                  SERVICE_ERROR_NORMAL, newServiceBinaryPath, nullptr, nullptr,
...
SetUserAccessServiceDACL(SC_HANDLE hService, PACL& pNewAcl, PSECURITY_DESCRIPTOR psd)
{
    ...
    if (!QueryServiceObjectSecurity(hService, DACL_SECURITY_INFORMATION, &psd, 0, &needed))
    {
        ...
        {
            LOG_WARN(("Could not query service object security size.  (%lu)", GetLastError()));
            return GetLastError();
        }
        ...
        if (!psd)
        {
            LOG_WARN(("Could not allocate security descriptor.  (%lu)", GetLastError()));
    ...
    if (!CreateWellKnownSid(WinBuiltinUsersSid, nullptr, sidBuiltinUsers, &SIDSize))
    {
        DWORD lastError = GetLastError();
    ...
    if (!CreateWellKnownSid(WinInteractiveSid, nullptr, sidInteractive, &SIDSize))
    {
        DWORD lastError = GetLastError();
    ...
    {
        LOG_WARN(("Could not initialize security descriptor.  (%lu)", GetLastError()));
        return GetLastError();
    }
    ...
    {
        LOG_WARN(("Could not set security descriptor DACL.  (%lu)", GetLastError()));
        return GetLastError();
    }
