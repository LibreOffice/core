...
CheckCertificateForPEFile(LPCWSTR filePath, CertificateCheckInfo& infoToMatch)
{
    ...
    BOOL result
        = CryptQueryObject(CERT_QUERY_OBJECT_FILE, filePath,
    ...
    result = CryptMsgGetParam(cryptMsg, CMSG_SIGNER_INFO_PARAM, 0, nullptr, &signerInfoSize);
    if (!result)
    {
        lastError = GetLastError();
    ...
    result
        = CryptMsgGetParam(cryptMsg, CMSG_SIGNER_INFO_PARAM, 0, (PVOID)signerInfo, &signerInfoSize);
    if (!result)
    {
        lastError = GetLastError();
        LOG_WARN(("CryptMsgGetParam failed.  (%d)", lastError));
    ...
    certContext = CertFindCertificateInStore(certStore, ENCODING, 0, CERT_FIND_SUBJECT_CERT,
...
BOOL DoCertificateAttributesMatch(PCCERT_CONTEXT certContext, CertificateCheckInfo& infoToMatch)
{
    ...
    if (infoToMatch.issuer)
    {
        ...
        if (!szName)
        {
            LOG_WARN(("Unable to allocate memory for issuer name.  (%d)", GetLastError()));
            return FALSE;
        }

        // Get Issuer name.
        if (!CertGetNameString(certContext, CERT_NAME_SIMPLE_DISPLAY_TYPE, CERT_NAME_ISSUER_FLAG,
    ...
    if (infoToMatch.name)
    {
        ...
        dwData
            = CertGetNameString(certContext, CERT_NAME_SIMPLE_DISPLAY_TYPE, 0, nullptr, nullptr, 0);
        if (!dwData)
        {
            LOG_WARN(("CertGetNameString failed.  (%d)", GetLastError()));
            return FALSE;
        ...
        if (!szName)
        {
            LOG_WARN(("Unable to allocate memory for subject name.  (%d)", GetLastError()));
            return FALSE;
        }

        // Obtain the name.
        if (!(CertGetNameString(certContext, CERT_NAME_SIMPLE_DISPLAY_TYPE, 0, nullptr, szName,
                                dwData)))
        {
            LOG_WARN(("CertGetNameString failed.  (%d)", GetLastError()));
            LocalFree(szName);
...
VerifyCertificateTrustForFile(LPCWSTR filePath)
{
    ...
    if (ERROR_SUCCESS == ret)
    {
        ...
        LOG(("The file \"%ls\" is signed and the signature was verified.", filePath));
