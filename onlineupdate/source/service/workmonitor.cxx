...
static BOOL IsStatusApplying(LPCWSTR patchDirPath, BOOL& isApplying)
{
    ...
    nsAutoHandle statusFile(CreateFileW(statusFilePath, GENERIC_READ,
...
BOOL StartUpdateProcess(int argc, LPWSTR* argv, LPCWSTR installDir, BOOL& processStarted)
{
    ...
    processStarted = CreateProcessW(argv[0], cmdLine.get(), nullptr, nullptr, FALSE,
...
static bool UpdaterIsValid(LPWSTR updater, LPWSTR installDir, LPWSTR updateDir)
{
    ...
    {
        LOG_WARN(("Filesystem in path %ls is not supported (%lu)", updater, GetLastError()));
        if (!WriteStatusFailure(updateDir, SERVICE_UPDATER_NOT_FIXED_DRIVE))
        {
            LOG_WARN(
                ("Could not write update.status service update failure.  (%lu)", GetLastError()));
        }
        return false;
    }

    nsAutoHandle noWriteLock(
        CreateFileW(updater, GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, 0, nullptr));
    if (INVALID_HANDLE_VALUE == noWriteLock)
    {
        LOG_WARN(("Could not set no write sharing access on file.  (%lu)", GetLastError()));
    ...
    bool result = true;
    HMODULE updaterModule = LoadLibraryEx(updater, nullptr, LOAD_LIBRARY_AS_DATAFILE);
    if (!updaterModule)
    {
...
BOOL ProcessSoftwareUpdateCommand(DWORD argc, LPWSTR* argv)
{
    ...
    if (UpdaterIsValid(argv[0], installDir, argv[1]))
    {
        ...
        else
        {
            result = FALSE;
            LOG_WARN(
                ("Error running update process. Updating update.status  (%lu)", GetLastError()));
            LogFlush();
    ...
    else
    {
        ...
        {
            LOG_WARN(("Could not write pending state to update.status.  (%lu)", GetLastError()));
        }
    }
...
BOOL ExecuteServiceCommand(int argc, LPWSTR* argv)
{
    ...
    if (!lstrcmpi(argv[2], L"software-update"))
    {
        ...
        {
            LOG_WARN(("The patch directory path length is not valid for this "
                      "application."));
            return FALSE;
        }
        ...
        {
            LOG_WARN(("The patch directory path subdirectory is not valid for this "
                      "application."));
            return FALSE;
        }
        ...
        if (!IsOldCommandline(argc - 3, argv + 3))
        {
            ...
            {
                if (argc <= 7 || (wcscmp(argv[7], L"-1") != 0 && !wcsstr(argv[7], L"/replace")))
                {
                ...
                NS_tsnprintf(workingDirParent,
                             sizeof(workingDirParent) / sizeof(workingDirParent[0]), NS_T("%s"),
                             argv[6]);
        ...
        if (!result)
        {
            LOG_WARN(("Could not copy path to secure location.  (%lu)", GetLastError()));
