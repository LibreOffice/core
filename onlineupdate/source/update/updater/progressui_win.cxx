...
#define RESIZE_WINDOW(hwnd, extrax, extray)                                                        \
    {                                                                                              \
        ...
        SetWindowPos(hwnd, 0, 0, 0, windowSize.right - windowSize.left + extrax,                   \
                     windowSize.bottom - windowSize.top + extray, SWP_NOMOVE | SWP_NOZORDER);      \
    }
...
static void CenterDialog(HWND hDlg)
{
    ...
    SetWindowPos(hDlg, HWND_TOP, rcOwner.left + (rc.right / 2), rcOwner.top + (rc.bottom / 2), 0,
...
static void InitDialog(HWND hDlg)
{
    ...
    int bufferSize = MultiByteToWideChar(CP_UTF8, 0, sUIStrings.title.get(), -1, nullptr, 0);
    szwTitle = mozilla::MakeUnique<WCHAR[]>(bufferSize);
    MultiByteToWideChar(CP_UTF8, 0, sUIStrings.title.get(), -1, szwTitle.get(), bufferSize);
    bufferSize = MultiByteToWideChar(CP_UTF8, 0, sUIStrings.info.get(), -1, nullptr, 0);
    szwInfo = mozilla::MakeUnique<WCHAR[]>(bufferSize);
    MultiByteToWideChar(CP_UTF8, 0, sUIStrings.info.get(), -1, szwInfo.get(), bufferSize);
    ...
    // nothing is drawn.
    if (DrawText(hDCInfo, szwInfo.get(), -1, &textSize, DT_CALCRECT | DT_NOCLIP | DT_SINGLELINE))
    {
...
int ShowProgressUI(bool indeterminate, bool initUIStrings)
{
    ...
    DialogBox(GetModuleHandle(nullptr), MAKEINTRESOURCE(IDD_DIALOG), nullptr, (DLGPROC)DialogProc);
