...
UmaskContext::UmaskContext(mode_t umaskToSet) { mPreviousUmask = umask(umaskToSet); }
...
public:
    int Run(ThreadFunc func, void* param)
    {
        ...
        mThread = (HANDLE)_beginthreadex(nullptr, 0, ThreadMain, this, 0, &threadID);
...
static void output_finish()
{
    ...
    if (gCopyOutputFiles)
    {
        ...
        if (GetSecureOutputFilePath(gPatchDirPath, L".status", srcStatusPath))
        {
            ...
            NS_tsnprintf(dstStatusPath, sizeof(dstStatusPath) / sizeof(dstStatusPath[0]),
...
static int ensure_copy(const NS_tchar* path, const NS_tchar* dest)
{
    ...
    if (!result)
    {
        LOG(("ensure_copy: failed to copy the file " LOG_S " over to " LOG_S ", lasterr: %lx", path,
             dest, GetLastError()));
...
static int remove_recursive_on_reboot(const NS_tchar* path, const NS_tchar* deleteDir)
{
    ...
    if (!S_ISDIR(sInfo.st_mode))
    {
        ...
        {
            LOG(("remove_recursive_on_reboot: failed to remove temporary file: " LOG_S ", err: %d",
                 tmpDeleteFile, errno));
        }
        rv = rename_file(path, tmpDeleteFile, false);
        if (MoveFileEx(rv ? path : tmpDeleteFile, nullptr, MOVEFILE_DELAY_UNTIL_REBOOT))
        {
            LOG(("remove_recursive_on_reboot: file will be removed on OS "
                 "reboot: " LOG_S,
...
void AddFile::Finish(int status)
{
    ...
    if (!sStagedUpdate)
    {
        ...
        if (status && mAdded)
        {
            ...
            {
                LOG(("non-fatal error after update failure removing added file: " LOG_S ", err: %d",
                     mFile.get(), errno));
            }
        }
        backup_finish(mFile.get(), mRelPath.get(), status);
...
int PatchFile::LoadSourceFile(FILE* ofile)
{
    ...
    {
        LOG(("LoadSourceFile: destination file size %d does not match expected "
             "size %d",
...
int PatchFile::Prepare()
{
    ...
    NS_tsnprintf(spath, sizeof(spath) / sizeof(spath[0]), NS_T("%s/updating/%d.patch"),
                 gWorkingDirPath, mPatchIndex);
    ...
    {
        LOG(("failure removing pre-existing patch file: " LOG_S ", err: %d", spath, errno));
        return WRITE_ERROR;
    }
...
bool LaunchWinPostProcess(const WCHAR* installationDir, const WCHAR* updateInfoDir)
{
    ...
    {
        LOG(("LaunchWinPostProcess failed because PathAppendSafe failed when "
             "getting INI path"));
        return false;
    }
    ...
    {
        LOG(("LaunchWinPostProcess failed because executable path contains "
    ...
    {
        LOG(("LaunchWinPostProcess failed because executable path is not a valid, "
             "full path"));
        return false;
    }
    ...
    {
        LOG(("LaunchWinPostProcess failed because the binary doesn't match the "
    ...
    if (gCopyOutputFiles)
    {
        ...
        {
            LOG(("LaunchWinPostProcess failed because a secure slogFile path is "
                 "unavailable"));
            return false;
        }
    }
    ...
    if (!cmdline)
    {
        LOG(("LaunchWinPostProcess failed due to failure to allocate %zu wchars "
             "for cmdline",
             len + 1));
        return false;
    }
...
static bool WriteToFile(const NS_tchar* aFilename, const char* aStatus)
{
    ...
    {
        NS_tsnprintf(statusFilePath, sizeof(statusFilePath) / sizeof(statusFilePath[0]),
                     NS_T("%s\\%s"), gPatchDirPath, aFilename);
    }
#else
    NS_tsnprintf(statusFilePath, sizeof(statusFilePath) / sizeof(statusFilePath[0]), NS_T("%s/%s"),
                           ...
                           mozilla::Maybe<int>* errorCode = nullptr)
{
    ...
    bool statusMatch = statusLen == expectedStatusLen
    ...
    if (errorCodeEnd != errorCodeStart && longErrorCode < INT_MAX && longErrorCode > INT_MIN)
    {
 ...
 */
static bool CompareSecureUpdateStatus(const char* expectedStatus, bool& statusMatch,
                                      mozilla::Maybe<int>* errorCode = nullptr)
{
    NS_tchar statusFilePath[MAX_PATH + 1] = { L'\0' };
    if (!GetSecureOutputFilePath(gPatchDirPath, L".status", statusFilePath))
    {
        LOG(("CompareSecureUpdateStatus failed due to GetSecureOutputFilePath "
             "failure"));
        return false;
    }
    ...
    if (file == nullptr)
    {
        LOG(("CompareSecureUpdateStatus failed to open the secure status file: %d", errno));
        return false;
    }
    ...
    LOG(("CompareSecureUpdateStatus %s %s %s", buf, statusMatch ? "matches" : "does not match",
         expectedStatus));
    return true;
}
...
static bool IsUpdateStatusPendingService()
{
    ...
    return UpdateStatusIs(buf, "pending-service") || UpdateStatusIs(buf, "applied-service");
}
 ...
 * @return true if the information was retrieved successfully.
 */
static bool IsSecureUpdateStatusFailed(bool& isFailed, mozilla::Maybe<int>* errorCode = nullptr)
{
...
{
    return ((errorCode >= 24 && errorCode <= 33) || (errorCode >= 49 && errorCode <= 58));
}
#endif
...
// 3. Delete tmpDir (or defer it to the next reboot).
    ...
    NS_tsnprintf(tmpLog, sizeof(tmpLog) / sizeof(tmpLog[0]), NS_T("%s/updates/last-update.log"),
                 tmpDir);
    if (!NS_taccess(tmpLog, F_OK))
    {
        ...
        {
            LOG(("non-fatal error removing log file: " LOG_S ", err: %d", destLog, errno));
        }
...
static int ReadMARChannelIDs(const NS_tchar* path, MARChannelStringTable* results)
{
    ...
    int result = ReadStrings(path, kUpdaterKeys, kNumStrings, &results->MARChannelID, "Settings");
...
static void UpdateThreadFunc(void* param)
{
    ...
    else
    {
        ...
        if (rv == OK)
        {
            ...
            {
                rv = gArchiveReader.VerifyProductInformation(MARStrings.MARChannelID.get(),
                                                             MOZ_APP_VERSION);
            }
        }
#endif
...
#elif XP_MACOSX
                                     ,
                                     mozilla::UniquePtr<UmaskContext> umaskContext
#endif
)
{
    ...
    { // isElevated
        LOG(("Not elevated. Skipping LaunchMacPostProcess and "
             "LaunchCallbackApp"));
    }
#endif /* XP_MACOSX */
}
...
{
    LOG(("No callback arg. Skipping LaunchWinPostProcess and "
         "LaunchCallbackApp"));
}
return 0;
}
...
int NS_main(int argc, NS_tchar** argv)
{
    ...
    gUseSecureOutputPath = sUsingService || (NS_tremove(elevatedLockFilePath) && errno != ENOENT);
    ...
    gIsElevated = GetFileAttributesW(elevatedLockFilePath) != INVALID_FILE_ATTRIBUTES;
#elif defined(XP_MACOSX)
    ...
    // the maintenance service.
        ...
        // Remove everything except close window from the context menu
        {
            ...
            RegCloseKey(hkApp);
            if (RegCreateKeyExW(HKEY_CURRENT_USER, L"Software\\Classes\\Applications\\updater.exe",
                                0, nullptr, REG_OPTION_VOLATILE, KEY_SET_VALUE, nullptr, &hkApp,
                                nullptr)
                == ERROR_SUCCESS)
            {
    ...
    if (!isDMGInstall)
    {
        ...
        if (argc > callbackIndex)
        {
            ...
            sUpdateSilently = ShouldRunSilently(argc - callbackIndex, argv + callbackIndex);
        }
    ...
    if (!isElevated && (!IsRecursivelyWritable(argv[2]) || isDMGInstall))
    {
        ...
        if (sUpdateSilently)
        {
            ...
            fprintf(stderr, "Skipping update to avoid elevation prompt from silent update.");
        }
        else
        {
        ...
        LaunchCallbackAndPostProcessApps(argc, argv, callbackIndex, std::move(umaskContext));
    ...
    if (!isDMGInstall)
    {
        ...
        {
            NS_tsnprintf(logFilePath, sizeof(logFilePath) / sizeof(logFilePath[0]), NS_T("%s\\%s"),
                         gPatchDirPath, UpdateLogFilename());
        }
#else
            NS_tsnprintf(logFilePath, sizeof(logFilePath) / sizeof(logFilePath[0]), NS_T("%s/%s"),
                         gPatchDirPath, UpdateLogFilename());
        ...
        if (!sUsingService && (argc > callbackIndex || sStagedUpdate || sReplaceRequest))
        {
            ...
            updateLockFileHandle
                = CreateFileW(updateLockFilePath, GENERIC_READ | GENERIC_WRITE, 0, nullptr,
...
// be invoked directly without going through the updater.
                ...
                if (useService)
                {
                    ...
                    DWORD ret = LaunchServiceSoftwareUpdateCommand(serviceArgc, (LPCWSTR*)argv);
                    ...
                    if (useService)
                    {
                        ...
                        {
                            Thread t1;
                            if (t1.Run(WaitForServiceFinishThread, nullptr) == 0 && showProgressUI)
                            {
                        ...
                        else
                        {
                            ...
                            if (checkID && GetSecureID(uuidStringAfter)
                                && strncmp(uuidStringBefore, uuidStringAfter,
                                           sizeof(uuidStringBefore))
                                       == 0)
                            {
                                LOG(("The secure ID hasn't changed after launching the updater "
                                     "using the service"));
                ...
                if (!useService && sStagedUpdate)
                {
                    ...
                    LOG(("Non-critical update staging error! Falling back to non-staged "
                         "updates and exiting"));
                    output_finish();
                ...
                if (!useService && !noServiceFallback
                    && (updateLockFileHandle == INVALID_HANDLE_VALUE || forceServiceFallback))
                {
                    ...
                    sinfo.fMask
                        = SEE_MASK_FLAG_NO_UI | SEE_MASK_FLAG_DDEWAIT | SEE_MASK_NOCLOSEPROCESS;
                    sinfo.hwnd = nullptr;
                    sinfo.lpFile = argv[0];
                    ...
                    if (result)
                    {
                        ...
                        char uuidStringAfter[UUID_LEN] = { '\0' };
                        if (checkID && GetSecureID(uuidStringAfter)
                            && strncmp(uuidStringBefore, uuidStringAfter, sizeof(uuidStringBefore))
                                   == 0)
                        {
                            LOG(("The secure ID hasn't changed after launching the updater "
                                 "using runas"));
                ...
                else
                {
                    ...
                    LOG(("forceServiceFallback=%s", forceServiceFallback ? "true" : "false"));
                }
                ...
                if (!sStagedUpdate)
                {
                    ...
                    {
                        LOG(("Not running LaunchWinPostProcess because update status is not"
                             "'succeeded'."));
                    }
                }
        ...
        if (argc > callbackIndex)
        {
            ...
            if (sReplaceRequest)
            {
                ...
                *p = NS_T('\0');
                ...
                NS_tstrncpy(
                    p, argv[callbackIndex] + std::max(callbackPrefixLength, commonPrefixLength),
                    bufferLeft);
                targetPath = buffer;
            }
                ...
                *d = NS_T('\0');
                ...
                if (callbackFile == INVALID_HANDLE_VALUE)
                {
                    ...
                    if (!proceedWithoutExclusive)
                    {
                        ...
                        {
                            LOG(("NS_main: unable to remove backup of callback app file, "
                                 "path: " LOG_S,
                                 gCallbackBackupPath));
                        }
                        output_finish();
        ...
        if (argc > callbackIndex && !sReplaceRequest)
        {
            ...
            if (NS_tremove(gCallbackBackupPath) && errno != ENOENT)
            {
                LOG(("NS_main: non-fatal error removing backup of callback app file, "
                     "path: " LOG_S,
                     gCallbackBackupPath));
            }
        }
...
int add_dir_entries(const NS_tchar* dirpath, ActionList* list)
{
    ...
    int add_dir_entries(const NS_tchar* dirpath, ActionList* list)
    {
        ...
        // returned.
        if (!(ftsdir
              = fts_open(pathargv, FTS_PHYSICAL | FTS_NOSTAT | FTS_XDEV | FTS_NOCHDIR, nullptr)))
        {
...
static NS_tchar* GetManifestContents(const NS_tchar* manifest)
{
    ...
    *rb = '\0';
    ...
    return mbuf;
        ...
        if (!MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, mbuf, -1, wrb, ms.st_size + 1))
        {
            LOG(("GetManifestContents: error converting utf8 to utf16le: %lu", GetLastError()));
            free(mbuf);
            free(wrb);
            return nullptr;
        }
...
#else
        mozilla::UniquePtr<NS_tchar[]> manifestPath(get_full_path(NS_T("precomplete")));
#endif
