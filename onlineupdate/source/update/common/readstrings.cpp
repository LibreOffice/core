...
int ReadStrings(const NS_tchar* path, const char* keyList, unsigned int numStrings,
                mozilla::UniquePtr<char[]>* results, const char* section)
{
...
IniReader::IniReader(const NS_tchar* iniPath, const char* section /* = nullptr */)
{
    if (iniPath)
    {
...
bool IniReader::MaybeAddKey(const char* key, size_t& insertionIndex)
{
    ...
    auto existingKey
        = std::find_if(mKeys.begin(), mKeys.end(), [=](mozilla::UniquePtr<char[]>& searchKey) {
...
#ifdef XP_WIN
void IniReader::AddKey(const char* key, mozilla::UniquePtr<wchar_t[]>* outputPtr)
{
    size_t insertionIndex;
    if (!MaybeAddKey(key, insertionIndex))
    {
...
static bool ConvertToWide(const char* toConvert, mozilla::UniquePtr<wchar_t[]>* result)
{
    int bufferSize = MultiByteToWideChar(CP_UTF8, 0, toConvert, -1, nullptr, 0);
    *result = mozilla::MakeUnique<wchar_t[]>(bufferSize);
    int charsWritten = MultiByteToWideChar(CP_UTF8, 0, toConvert, -1, result->get(), bufferSize);
...
int IniReader::Read()
{
    ...
    *keyListPtr = '\0';
    ...
    int statusCode
        = ReadStrings(mPath.get(), keyList.get(), mKeys.size(), results.get(), mSection.get());
    ...
    for (const auto output : mNarrowOutputs)
    {
        ...
        if (valueBuffer)
        {
            *(output.outputPtr) = mozilla::MakeUnique<char[]>(strlen(valueBuffer) + 1);
