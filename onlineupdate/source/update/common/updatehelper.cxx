...
BOOL GetSecureOutputDirectoryPath(LPWSTR outBuf)
{
    PWSTR progFilesX86;
    if (FAILED(
            SHGetKnownFolderPath(FOLDERID_ProgramFilesX86, KF_FLAG_CREATE, nullptr, &progFilesX86)))
    {
        return FALSE;
    }
    ...
    if (!CreateDirectoryW(outBuf, nullptr) && GetLastError() != ERROR_ALREADY_EXISTS)
    {
        return FALSE;
    }
 ...
 * @return TRUE if successful
 */
BOOL GetSecureOutputFileName(LPCWSTR patchDirPath, LPCWSTR fileExt, LPWSTR outBuf)
{
    ...
    // service.
    if (_wcsnicmp(patchDirPath + fullPathLen - relPathLen, PATCH_DIR_PATH, relPathLen) != 0)
    {
        return FALSE;
    }
...
BOOL WriteStatusFailure(LPCWSTR patchDirPath, int errorCode)
{
    ...
    HANDLE hStatusFile
        = CreateFileW(statusFilePath, GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS, 0, nullptr);
