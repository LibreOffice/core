    ...
    while (nextToken)
    {
        if ((GetFileAttributesW(partialPath) & FILE_ATTRIBUTE_REPARSE_POINT) != 0)
        {
            nsAutoHandle h(CreateFileW(
                partialPath, 0, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, nullptr,
                ...
                case IO_REPARSE_TAG_MOUNT_POINT:
                    reparseTarget = buffer->MountPointReparseBuffer.PathBuffer
                                    + (buffer->MountPointReparseBuffer.SubstituteNameOffset
                                       / sizeof(wchar_t));
                    if (buffer->MountPointReparseBuffer.SubstituteNameLength < ARRAYSIZE(L"\\??\\"))
                    {
                        return false;
                    }
                    break;
...
bool IsProgramFilesPath(NS_tchar* fullPath)
{
    ...
    if (!GetLongPathNameW(fullPath, longInstallPath.get(), longInstallPathChars))
    {
        return false;
    }
    ...
    // First check for Program Files (x86).
    {
        ...
        // bitness.
        if (FAILED(
                SHGetKnownFolderPath(FOLDERID_ProgramFilesX86, 0, nullptr, &programFiles32PathRaw)))
        {
            // That call should never fail on any supported OS version.
            return false;
        }
        mozilla::UniquePtr<wchar_t, mozilla::CoTaskMemFreeDeleter> programFiles32Path(
        ...
        {
            if (wcsnicmp(longInstallPath.get(), programFiles32Path.get(), length) == 0)
            {
                return true;
            }
        }
        else
        {
            ...
            if (wcsnicmp(longInstallPath.get(), programFiles32PathWithSlash.get(), length) == 0)
            {
                return true;
            }
        }
    ...
    // If we didn't find (x86), check for the native Program Files.
    {
        ...
        if (RegGetValueW(HKEY_LOCAL_MACHINE, L"Software\\Microsoft\\Windows\\CurrentVersion",
                         L"ProgramFilesDir", RRF_RT_REG_SZ | RRF_SUBKEY_WOW6464KEY, nullptr,
                         programFilesNativePath.get(), &length)
            != ERROR_SUCCESS)
        {
            return false;
        }
        size_t nativePathStrLen = wcsnlen_s(programFilesNativePath.get(), lengthChars);
        ...
        {
            if (wcsnicmp(longInstallPath.get(), programFilesNativePath.get(), nativePathStrLen)
                == 0)
            {
                return true;
            }
        ...
        else
        {
            ...
            NS_tsnprintf(programFilesNativePathWithSlash.get(), nativePathStrLen + 1, NS_T("%s\\"),
                         programFilesNativePath.get());
...
bool IsValidFullPath(NS_tchar* origFullPath)
{
    ...
    if (invalidSuffixLen <= pathLen
        && NS_tstrncmp(origFullPath + pathLen - invalidSuffixLen, invalidSuffix, invalidSuffixLen)
               == 0)
    {
        return false;
    }
