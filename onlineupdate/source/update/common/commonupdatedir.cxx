...
public:
   ...
   */
    ...
    {
        int bufferSize = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, src, -1, nullptr, 0);
        if (bufferSize == 0)
        {
            Truncate();
        ...
        int charsWritten = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, src, -1,
                                               mString.get(), bufferSize);
        if (charsWritten == 0)
        {
            Truncate();
            return HRESULT_FROM_WIN32(GetLastError());
...
static bool GetCachedHash(const char16_t* installPath, HKEY rootKey,
                          const SimpleAutoString& regPath, mozilla::UniquePtr<NS_tchar[]>& result);
static HRESULT GetUpdateDirectory(const wchar_t* installPath, WhichUpdateDir whichDir,
...
bool GetInstallHash(const char16_t* installPath, mozilla::UniquePtr<NS_tchar[]>& result)
{
    ...
    int charsWritten = NS_tsnprintf(result.get(), hashStrSize, NS_T("%") NS_T(PRIX64), hash);
    return !(charsWritten < 1 || static_cast<size_t>(charsWritten) > hashStrSize - 1);
}
                          ...
                          const SimpleAutoString& regPath, mozilla::UniquePtr<NS_tchar[]>& result)
{
    ...
    LSTATUS lrv
        = RegGetValueW(rootKey, regPath.String(), reinterpret_cast<const wchar_t*>(installPath),
    ...
    lrv = RegGetValueW(rootKey, regPath.String(), reinterpret_cast<const wchar_t*>(installPath),
 ...
 */
HRESULT
GetCommonUpdateDirectory(const wchar_t* installPath, mozilla::UniquePtr<wchar_t[]>& result)
{
    return GetUpdateDirectory(installPath, WhichUpdateDir::CurrentUpdateDir, result);
}
 ...
 */
HRESULT
GetOldUpdateDirectory(const wchar_t* installPath, mozilla::UniquePtr<wchar_t[]>& result)
{
    return GetUpdateDirectory(installPath, WhichUpdateDir::UnmigratedUpdateDir, result);
}
 ...
 * MAX_PATH + 1
 */
extern "C" HRESULT get_common_update_directory(const wchar_t* installPath, wchar_t* result)
{
...
static HRESULT GetUpdateDirectory(const wchar_t* installPath, WhichUpdateDir whichDir,
                                  mozilla::UniquePtr<wchar_t[]>& result)
{
    MOZ_ASSERT(installPath != nullptr, "Install path must not be null in GetUpdateDirectory");
    ...
    hrv = SHGetKnownFolderPath(FOLDERID_ProgramData, KF_FLAG_CREATE, nullptr, &baseDirParentPath);
    // Free baseDirParentPath when it goes out of scope.
    mozilla::UniquePtr<wchar_t, CoTaskMemFreeDeleter> baseDirParentPathUnique(baseDirParentPath);
    if (FAILED(hrv))
    {
        return hrv;
    }
    ...
    size_t basePathLen = wcslen(baseDirParentPath) + 1 /* path separator */ + baseDir.Length();
    basePath.AllocAndAssignSprintf(basePathLen, L"%s\\%s", baseDirParentPath, baseDir.String());
    ...
    size_t midPathLen = basePath.Length() + 1 /* path separator */ + wcslen(midPathDirName);
    SimpleAutoString midPath;
    midPath.AllocAndAssignSprintf(midPathLen, L"%s\\%s", basePath.String(), midPathDirName);
    ...
    regPath.AutoAllocAndAssignSprintf(L"SOFTWARE\\Mozilla\\%S\\TaskBarIDs", MOZ_APP_BASENAME);
    if (regPath.Length() != 0)
    {
        gotHash = GetCachedHash(reinterpret_cast<const char16_t*>(installPath), HKEY_LOCAL_MACHINE,
                                regPath, hash);
        if (!gotHash)
        {
    ...
    if (!gotHash)
    {
        bool success = GetInstallHash(reinterpret_cast<const char16_t*>(installPath), hash);
        if (!success)
        {
            return E_FAIL;
        }
    ...
    size_t updatePathLen = midPath.Length() + 1 /* path separator */ + wcslen(hash.get());
    SimpleAutoString updatePath;
    updatePath.AllocAndAssignSprintf(updatePathLen, L"%s\\%s", midPath.String(), hash.get());
...
static HRESULT GeneratePermissions(AutoPerms& result)
{
    ...
    BOOL success
        = AllocateAndInitializeSid(&result.sidIdentifierAuthority, 2, SECURITY_BUILTIN_DOMAIN_RID,
    ...
    success = AllocateAndInitializeSid(&result.sidIdentifierAuthority, 1, SECURITY_LOCAL_SYSTEM_RID,
    ...
    success = InitializeSecurityDescriptor(result.securityDescriptor, SECURITY_DESCRIPTOR_REVISION);
    if (!success)
    {
        return HRESULT_FROM_WIN32(GetLastError());
    }

    success = SetSecurityDescriptorDacl(result.securityDescriptor, TRUE, acl, FALSE);
    if (!success)
    {
        return HRESULT_FROM_WIN32(GetLastError());
    }
...
static HRESULT MakeDir(const SimpleAutoString& path, const AutoPerms& perms)
{
    BOOL success = CreateDirectoryW(path.String(),
