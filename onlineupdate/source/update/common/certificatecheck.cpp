...
BOOL DoCertificateAttributesMatch(PCCERT_CONTEXT certContext, CertificateCheckInfo& infoToMatch)
{
    ...
    if (infoToMatch.issuer)
    {
        ...
        if (!szName)
        {
            LOG_WARN(("Unable to allocate memory for issuer name.  (%lu)", GetLastError()));
            return FALSE;
        }

        // Get Issuer name.
        if (!CertGetNameStringW(certContext, CERT_NAME_SIMPLE_DISPLAY_TYPE, CERT_NAME_ISSUER_FLAG,
    ...
    if (infoToMatch.name)
    {
        ...
        if (!szName)
        {
            LOG_WARN(("Unable to allocate memory for subject name.  (%lu)", GetLastError()));
            return FALSE;
        }

        // Obtain the name.
        if (!(CertGetNameStringW(certContext, CERT_NAME_SIMPLE_DISPLAY_TYPE, 0, nullptr, szName,
                                 dwData)))
        {
            LOG_WARN(("CertGetNameString failed.  (%lu)", GetLastError()));
            LocalFree(szName);
