...
static int32_t search(int32_t* I, unsigned char* old, int32_t oldsize, unsigned char* newbuf,
                      int32_t newsize, int32_t st, int32_t en, int32_t* pos)
{
    int32_t x, y;
...
int main(int argc, char* argv[])
{
    ...
    MBSPatchHeader header = { { 'M', 'B', 'D', 'I', 'F', 'F', '1', '0' }, 0, 0, 0, 0, 0, 0 };
    ...
    if (((fd = open(argv[1], O_RDONLY | _O_BINARY, 0)) < 0)
        ...
        || ((old = (unsigned char*)malloc(oldsize + 1)) == NULL) || (lseek(fd, 0, SEEK_SET) != 0)
    ...
    if (((fd = open(argv[2], O_RDONLY | _O_BINARY, 0)) < 0)
        ...
        || ((newbuf = (unsigned char*)malloc(newsize + 1)) == NULL) || (lseek(fd, 0, SEEK_SET) != 0)
        || (read(fd, newbuf, newsize) != newsize) || (close(fd) == -1))
        reporterr(1, "%s\n", argv[2]);
    ...
    while (scan < newsize)
    {
        ...
        {
            len = search(I, old, oldsize, newbuf + scan, newsize - scan, 0, oldsize, &pos);

            for (; scsc < scan + len; scsc++)
                if ((scsc + lastoffset < oldsize) && (old[scsc + lastoffset] == newbuf[scsc]))
                    oldscore++;

            if (((len == oldscore) && (len != 0)) || (len > oldscore + 10))
                break;
        ...
        if ((len != oldscore) || (scan == newsize))
        {
            ...
            printf("Writing a block:\n"
                   ...
                   "	Y: %u\n"
                   "	Z: %i\n",
                   (uint32_t)lenf, (uint32_t)((scan - lenb) - (lastscan + lenf)),
    ...
    if (lseek(fd, 0, SEEK_SET) == -1 || write(fd, &header, sizeof(header)) != sizeof(header)
        || close(fd) == -1)
        reporterr(1, NULL);
