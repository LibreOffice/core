                                 ...
                                 SECKEYPublicKey** publicKey)
{
    ...
    cert = CERT_NewTempCertificate(CERT_GetDefaultCertDB(), &certDataItem, NULL, PR_FALSE, PR_TRUE);
...
CryptoX_Result NSS_VerifyBegin(VFYContext** ctx, SECKEYPublicKey* const* publicKey)
{
    ...
    if ((SECKEY_PublicKeyStrength(*publicKey) * 8) < XP_MIN_SIGNATURE_LEN_IN_BYTES)
    {
        fprintf(stderr, "ERROR: Key length must be >= %d bytes\n", XP_MIN_SIGNATURE_LEN_IN_BYTES);
        return CryptoX_Error;
    }

    *ctx = VFY_CreateContext(*publicKey, NULL, SEC_OID_ISO_SHA1_WITH_RSA_SIGNATURE, NULL);
    if (*ctx == NULL)
    {
        return CryptoX_Error;
    }
 ...
 */
    ...
    result = CryptVerifySignature(*hash, signatureReversed, signatureLen, *pubKey, NULL, 0);
...
*/
CryptoX_Result CryptoAPI_LoadPublicKey(HCRYPTPROV provider, BYTE* certData, DWORD sizeOfCertData,
                                       HCRYPTKEY* publicKey)
{
    ...
    if (!CryptQueryObject(CERT_QUERY_OBJECT_BLOB, &blob, CERT_QUERY_CONTENT_FLAG_CERT,
                          CERT_QUERY_FORMAT_FLAG_BINARY, 0, NULL, NULL, NULL, NULL, NULL,
                          (const void**)&context))
    {
        return CryptoX_Error;
    }

    if (!CryptImportPublicKeyInfo(provider, PKCS_7_ASN_ENCODING | X509_ASN_ENCODING,
...
CryptoX_Result CryptoAPI_InitCryptoContext(HCRYPTPROV* provider)
{
    ...
    {
        if (!CryptAcquireContext(provider, NULL, MS_ENHANCED_PROV, PROV_RSA_FULL,
