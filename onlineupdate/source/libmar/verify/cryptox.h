...
extern "C" {
#endif
CryptoX_Result NSS_LoadPublicKey(const unsigned char* certData, unsigned int certDataSize,
                                 SECKEYPublicKey** publicKey);
CryptoX_Result NSS_VerifyBegin(VFYContext** ctx, SECKEYPublicKey* const* publicKey);
CryptoX_Result NSS_VerifySignature(VFYContext* const* ctx, const unsigned char* signature,
...
#define CryptoX_FreeSignatureHandle(SignatureHandle) VFY_DestroyContext(*SignatureHandle, PR_TRUE)
...
CryptoX_Result CryptoMac_VerifyUpdate(CryptoX_SignatureHandle* aInputData, void* aBuf,
                                      unsigned int aLen);
CryptoX_Result CryptoMac_LoadPublicKey(const unsigned char* aCertData, unsigned int aDataSize,
...
#define CryptoX_InitCryptoProvider(aProviderHandle) CryptoMac_InitCryptoProvider()
#define CryptoX_VerifyBegin(aCryptoHandle, aInputData, aPublicKey) CryptoMac_VerifyBegin(aInputData)
#define CryptoX_VerifyUpdate(aInputData, aBuf, aLen) CryptoMac_VerifyUpdate(aInputData, aBuf, aLen)
#define CryptoX_LoadPublicKey(aProviderHandle, aCertData, aDataSize, aPublicKey)                   \
    CryptoMac_LoadPublicKey(aCertData, aDataSize, aPublicKey)
#define CryptoX_VerifySignature(aInputData, aPublicKey, aSignature, aSignatureLen)                 \
    CryptoMac_VerifySignature(aInputData, aPublicKey, aSignature, aSignatureLen)
#define CryptoX_FreeSignatureHandle(aInputData) CryptoMac_FreeSignatureHandle(aInputData)
...
CryptoX_Result CryptoAPI_LoadPublicKey(HCRYPTPROV hProv, BYTE* certData, DWORD sizeOfCertData,
                                       HCRYPTKEY* publicKey);
...
#define CryptoX_InitCryptoProvider(CryptoHandle) CryptoAPI_InitCryptoContext(CryptoHandle)
#define CryptoX_VerifyBegin(CryptoHandle, SignatureHandle, PublicKey)                              \
...
#define CryptoX_FreeCertificate(cert) CertCloseStore(*(cert), CERT_CLOSE_STORE_FORCE_FLAG);
