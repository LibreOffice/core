...
int mar_extract_and_verify_signatures(MarFile* mar, CryptoX_ProviderHandle provider,
                                      CryptoX_PublicKey* keys, uint32_t keyCount);
int mar_verify_extracted_signatures(MarFile* mar, CryptoX_ProviderHandle provider,
                                    CryptoX_PublicKey* keys,
                                    const uint8_t* const* extractedSignatures, uint32_t keyCount,
 ...
 */
CryptoX_Result ReadAndUpdateVerifyContext(MarFile* mar, size_t* mar_position, void* buffer,
                                          uint32_t size, CryptoX_SignatureHandle* ctxs,
                          ...
                          const uint32_t* certDataSizes, uint32_t certCount)
{
    ...
    {
        if (CryptoX_Failed(
                CryptoX_LoadPublicKey(provider, certData[k], certDataSizes[k], &keys[k])))
        {
                                      ...
                                      CryptoX_PublicKey* keys, uint32_t keyCount)
{
    ...
    if (mar_read_buffer(mar, &signatureCount, &mar_position, sizeof(signatureCount)) != 0)
    {
    ...
    for (i = 0; i < signatureCount; i++)
    {
        ...
        if (mar_read_buffer(mar, &signatureAlgorithmIDs[i], &mar_position, sizeof(uint32_t)) != 0)
        {
        ...
        if (mar_read_buffer(mar, &signatureLen, &mar_position, sizeof(uint32_t)) != 0)
        {
       ...
       isn't too big. */
        ...
        if (mar_read_buffer(mar, extractedSignatures[i], &mar_position, signatureLen) != 0)
        {
    ...
    if (mar_verify_extracted_signatures(mar, provider, keys,
                                        (const uint8_t* const*)extractedSignatures, signatureCount,
                                        &numVerified)
        == CryptoX_Error)
    {
        return CryptoX_Error;
 ...
 */
CryptoX_Result mar_verify_extracted_signatures(MarFile* mar, CryptoX_ProviderHandle provider,
                                               CryptoX_PublicKey* keys,
                                               const uint8_t* const* extractedSignatures,
                                               uint32_t signatureCount, uint32_t* numVerified)
{
   ...
   */
    ...
    for (i = 0; i < signatureCount; i++)
    {
        if (CryptoX_Failed(CryptoX_VerifyBegin(provider, &signatureHandles[i], &keys[i])))
        {
   ...
   */
    if (CryptoX_Failed(ReadAndUpdateVerifyContext(
            mar, &mar_position, buf, SIGNATURE_BLOCK_OFFSET + sizeof(uint32_t), signatureHandles,
    ...
    {
        /* Get the signature algorithm ID */
        if (CryptoX_Failed(ReadAndUpdateVerifyContext(mar, &mar_position, &buf, sizeof(uint32_t),
                                                      signatureHandles, signatureCount,
                                                      "signature algorithm ID")))
        {
            goto failure;
        }

        if (CryptoX_Failed(ReadAndUpdateVerifyContext(mar, &mar_position, &signatureLengths[i],
                                                      sizeof(uint32_t), signatureHandles,
    ...
    while (mar_position < mar->data_len)
    {
        ...
        for (i = 0; i < signatureCount; i++)
        {
            if (CryptoX_Failed(CryptoX_VerifyUpdate(&signatureHandles[i], buf, numRead)))
            {
    ...
    for (i = 0; i < signatureCount; i++)
    {
        if (CryptoX_Failed(CryptoX_VerifySignature(&signatureHandles[i], &keys[i],
                                                   extractedSignatures[i], signatureLengths[i])))
        {
