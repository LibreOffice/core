...
int mar_extract_and_verify_signatures_fp(FILE* fp, CryptoX_ProviderHandle provider,
                                         CryptoX_PublicKey* keys, uint32_t keyCount);
int mar_verify_signatures_for_fp(FILE* fp, CryptoX_ProviderHandle provider, CryptoX_PublicKey* keys,
...
*/
int ReadAndUpdateVerifyContext(FILE* fp, void* buffer, uint32_t size, CryptoX_SignatureHandle* ctxs,
                                         ...
                                         CryptoX_PublicKey* keys, uint32_t keyCount)
{
    ...
    rv = mar_verify_signatures_for_fp(fp, provider, keys,
                                 ...
                                 uint32_t* numVerified)
{
   ...
   */
    if (CryptoX_Failed(
            ReadAndUpdateVerifyContext(fp, buf, SIGNATURE_BLOCK_OFFSET + sizeof(uint32_t),
    ...
    {
        /* Get the signature algorithm ID */
        if (CryptoX_Failed(ReadAndUpdateVerifyContext(fp, &buf, sizeof(uint32_t), signatureHandles,
                                                      signatureCount, "signature algorithm ID")))
        {
            goto failure;
        }

        if (CryptoX_Failed(ReadAndUpdateVerifyContext(fp, &signatureLengths[i], sizeof(uint32_t),
