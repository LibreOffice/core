...
typedef SecTransformRef (*SecVerifyTransformCreateFunc)(SecKeyRef key, CFDataRef signature,
                                                        CFErrorRef* error);
SecVerifyTransformCreateFunc SecVerifyTransformCreatePtr = NULL;
typedef Boolean (*SecTransformSetAttributeFunc)(SecTransformRef transform, CFStringRef key,
...
static bool OnLionOrLater()
{
    if (sOnLionOrLater < 0)
    {
        ...
        CFURLRef url = CFURLCreateWithString(
            kCFAllocatorDefault, CFSTR("file:///System/Library/CoreServices/SystemVersion.plist"),
            NULL);
        ...
        CFArrayRef versions = CFStringCreateArrayBySeparatingStrings(kCFAllocatorDefault,
                                                                     versionString, CFSTR("."));
...
static const CSSM_GUID sMozCssmGuid
    = { 0x9243121f, 0x5820, 0x4b41, { 0xa6, 0x52, 0xba, 0xb6, 0x3f, 0x9d, 0x3d, 0x7f } };
...
static CSSM_API_MEMORY_FUNCS cssmMemFuncs
    = { &cssmMalloc, &cssmFree, &cssmRealloc, &cssmCalloc, NULL };
...
CryptoX_Result CryptoMac_InitCryptoProvider()
{
    ...
    if (!SecVerifyTransformCreatePtr)
    {
        SecVerifyTransformCreatePtr
            = (SecVerifyTransformCreateFunc)dlsym(RTLD_DEFAULT, "SecVerifyTransformCreate");
    }
    if (!SecTransformSetAttributePtr)
    {
        SecTransformSetAttributePtr
            = (SecTransformSetAttributeFunc)dlsym(RTLD_DEFAULT, "SecTransformSetAttribute");
    }
    ...
    if (!OnLionOrLater())
    {
        if (!sCspHandle)
        {
            ...
            if (!sCssmInitialized)
            {
                ...
                rv = CSSM_Init(&sCssmVersion, CSSM_PRIVILEGE_SCOPE_PROCESS, &sMozCssmGuid,
                               CSSM_KEY_HIERARCHY_NONE, &pvcPolicy, NULL);
                if (rv != CSSM_OK)
                {
                    return CryptoX_Error;
                }
            ...
            rv = CSSM_ModuleAttach(&gGuidAppleCSP, &sCssmVersion, &cssmMemFuncs, 0,
                                   CSSM_SERVICE_CSP, 0, CSSM_KEY_HIERARCHY_NONE, NULL, 0, NULL,
                                   &cspHandle);
            if (rv != CSSM_OK)
            {
                return CryptoX_Error;
...
{
    if (!aInputData || !*aInputData || !aPublicKey || !*aPublicKey || !aSignature
        || aSignatureLen == 0)
    {
        return CryptoX_Error;
    }
    ...
    if (!OnLionOrLater())
    {
        ...
        CSSM_KEY* publicKey;
        OSStatus status = SecKeyGetCSSMKey((SecKeyRef)*aPublicKey, (const CSSM_KEY**)&publicKey);
        if (status)
        {
            return CryptoX_Error;
        }
        ...
        if (CSSM_CSP_CreateSignatureContext(sCspHandle, CSSM_ALGID_SHA1WithRSA, NULL, publicKey,
                                            &ccHandle)
            != CSSM_OK)
        {
            return CryptoX_Error;
        ...
        CSSM_DATA inputData;
        inputData.Data
            = CFDataGetMutableBytePtr((CFMutableDataRef)(((CSSM_DATA_PTR)*aInputData)->Data));
        inputData.Length = ((CSSM_DATA_PTR)*aInputData)->Length;
        if (CSSM_VerifyData(ccHandle, &inputData, 1, CSSM_ALGID_NONE, &signatureData) == CSSM_OK)
        {
    ...
    CFDataRef signatureData = CFDataCreate(kCFAllocatorDefault, aSignature, aSignatureLen);
    if (!signatureData)
    {
        return CryptoX_Error;
    }
    ...
    SecTransformRef verifier
        = SecVerifyTransformCreatePtr((SecKeyRef)*aPublicKey, signatureData, &error);
    if (!verifier || error)
    {
    ...
    SecTransformSetAttributePtr(verifier, kSecTransformInputAttributeName, (CFDataRef)*aInputData,
                                &error);
    if (error)
    {
        CFRelease(signatureData);
    ...
    if (CFGetTypeID(rv) == CFBooleanGetTypeID() && CFBooleanGetValue((CFBooleanRef)rv) == true)
    {
        result = CryptoX_Success;
    }
