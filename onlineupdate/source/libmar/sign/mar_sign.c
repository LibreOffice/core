...
{
    SECStatus status = NSS_Initialize(NSSConfigDir, "", "", SECMOD_DB, NSS_INIT_READONLY);
 ...
 *         -1 on error
 */
int NSSSignBegin(const char* certName, SGNContext** ctx, SECKEYPrivateKey** privKey,
                 CERTCertificate** cert, uint32_t* signatureLength)
{
 ...
 */
int WriteAndUpdateSignatures(FILE* fpDest, void* buffer, uint32_t size, SGNContext** ctxs,
 ...
 *         -3 on signature update error
 */
int ReadWriteAndUpdateSignatures(FILE* fpSrc, FILE* fpDest, void* buffer, uint32_t size,
                                 SGNContext** ctxs, uint32_t ctxCount, const char* err)
{
    if (!size)
    {
        return 0;
...
{
    uint32_t offsetToIndex, dstOffsetToIndex, indexLength, numSignatures = 0, leftOver;
    int32_t stripAmount = 0;
    int64_t oldPos, numChunks, i, realSizeOfSrcMAR, numBytesToCopy, sizeOfEntireMAR = 0;
    ...
    /* Length of the index */
    if (ReadAndWrite(fpSrc, fpDest, &indexLength, sizeof(indexLength), "index length"))
    {
        goto failure;
    }
                     ...
                     const char* dest)
{
    ...
    FILE* fpSigFile = NULL;
    uint32_t i;
    uint32_t signatureCount, signatureLen, signatureAlgorithmID, numChunks, leftOver;
    char buf[BLOCKSIZE];
    ...
    if (ReadAndWrite(fpSrc, fpDest, buf, SIGNATURE_BLOCK_OFFSET, "signature block offset"))
    {
        goto failure;
    }

    /* Get the number of signatures */
    if (ReadAndWrite(fpSrc, fpDest, &signatureCount, sizeof(signatureCount), "signature count"))
    {
        goto failure;
    }
    signatureCount = ntohl(signatureCount);
    ...
    for (i = 0; i < signatureCount; i++)
    {
        ...
        if (ReadAndWrite(fpSrc, fpDest, &signatureAlgorithmID, sizeof(signatureAlgorithmID),
                         "sig algorithm ID"))
        {
            goto failure;
        }
        ...
        if (ReadAndWrite(fpSrc, fpDest, &signatureLen, sizeof(signatureLen), "sig length"))
        {
            goto failure;
        }
        signatureLen = ntohl(signatureLen);
        ...
        if (sigIndex == i)
        {
            ...
            if (fwrite(passedInSignatureRaw, passedInSignatureLenRaw, 1, fpDest) != 1)
            {
        ...
        {
            if (ReadAndWrite(fpSrc, fpDest, extractedMARSignature, signatureLen, "signature"))
            {
                goto failure;
            }
        }
 ...
 *         -1 on error
 */
int mar_repackage_and_sign(const char* NSSConfigDir, const char* const* certNames,
                           uint32_t certCount, const char* src, const char* dest)
{
    uint32_t offsetToIndex, dstOffsetToIndex, indexLength, leftOver, signatureAlgorithmID,
        numSignatures = 0, signatureSectionLength = 0;
    uint32_t signatureLengths[MAX_SIGNATURES];
    int64_t oldPos, numChunks, i, realSizeOfSrcMAR, signaturePlaceholderOffset, numBytesToCopy,
        sizeOfEntireMAR = 0;
    FILE *fpSrc = NULL, *fpDest = NULL;
    int rv = -1, hasSignatureBlock;
    ...
    {
        if (NSSSignBegin(certNames[k], &ctxs[k], &privKeys[k], &certs[k], &signatureLengths[k]))
        {
    ...
    /* MAR ID */
    if (ReadWriteAndUpdateSignatures(fpSrc, fpDest, buf, MAR_ID_SIZE, ctxs, certCount, "MAR ID"))
    {
        goto failure;
    }
    ...
    for (k = 0; k < certCount; k++)
    {
        signatureSectionLength
            += sizeof(signatureAlgorithmID) + sizeof(signatureLengths[k]) + signatureLengths[k];
    }
    dstOffsetToIndex = offsetToIndex;
    if (!hasSignatureBlock)
    {
    ...
    /* Write out the index offset */
    dstOffsetToIndex = htonl(dstOffsetToIndex);
    if (WriteAndUpdateSignatures(fpDest, &dstOffsetToIndex, sizeof(dstOffsetToIndex), ctxs,
                                 certCount, "index offset"))
    {
        goto failure;
    }
    dstOffsetToIndex = ntohl(dstOffsetToIndex);
    ...
    /* Write out the MAR size */
    sizeOfEntireMAR = HOST_TO_NETWORK64(sizeOfEntireMAR);
    if (WriteAndUpdateSignatures(fpDest, &sizeOfEntireMAR, sizeof(sizeOfEntireMAR), ctxs, certCount,
                                 "size of MAR"))
    {
        goto failure;
    }
    ...
    if (WriteAndUpdateSignatures(fpDest, &numSignatures, sizeof(numSignatures), ctxs, certCount,
                                 "num signatures"))
    {
        goto failure;
    }
    ...
    for (k = 0; k < certCount; k++)
    {
        ...
        if (WriteAndUpdateSignatures(fpDest, &signatureAlgorithmID, sizeof(signatureAlgorithmID),
        ...
        if (WriteAndUpdateSignatures(fpDest, &signatureLengths[k], sizeof(signatureLengths[k]),
    ...
    for (i = 0; i < numChunks; ++i)
    {
        if (ReadWriteAndUpdateSignatures(fpSrc, fpDest, buf, BLOCKSIZE, ctxs, certCount,
                                         "content block"))
        {
            goto failure;
        }
    ...
    /* Write out the left over */
    if (ReadWriteAndUpdateSignatures(fpSrc, fpDest, buf, leftOver, ctxs, certCount,
    ...
    /* Length of the index */
    if (ReadWriteAndUpdateSignatures(fpSrc, fpDest, &indexLength, sizeof(indexLength), ctxs,
                                     certCount, "index length"))
    {
        goto failure;
    }
    indexLength = ntohl(indexLength);
    ...
    if (WriteAndUpdateSignatures(fpDest, indexBuf, indexLength, ctxs, certCount, "index"))
    {
        goto failure;
    }
    ...
    for (k = 0; k < certCount; k++)
    {
        ...
        if (fseeko(fpDest, sizeof(signatureAlgorithmID) + sizeof(signatureLengths[k]), SEEK_CUR))
        {
            fprintf(stderr, "ERROR: Could not seek to signature offset\n");
            goto failure;
        }
