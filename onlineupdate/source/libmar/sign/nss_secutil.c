...
char* SECU_GetModulePassword(PK11SlotInfo* slot, PRBool retry, void* arg)
{
    ...
    {
        case PW_NONE:
            sprintf(prompt, "Enter Password or Pin for \"%s\":", PK11_GetTokenName(slot));
   ...
   */
            ...
            return pw;
        case PW_EXTERNAL:
            sprintf(prompt, "Press Enter, then enter PIN for \"%s\" on external device.\n",
