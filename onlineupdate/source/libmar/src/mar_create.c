 ...
 * @return 0 on success.
 */
static int mar_concat_product_info_block(FILE* fp, struct MarItemStack* stack,
                                         struct ProductInformationBlock* infoBlock)
{
    ...
    if (!fp || !infoBlock || !infoBlock->MARChannelID || !infoBlock->productVersion)
    {
        return -1;
    }
    ...
    infoBlockSize = sizeof(infoBlockSize) + sizeof(additionalBlockID) + PIB_MAX_MAR_CHANNEL_ID_SIZE
    ...
    if (fwrite(infoBlock->MARChannelID, strlen(infoBlock->MARChannelID) + 1, 1, fp) != 1)
    {
        return -1;
    }

    /* Write out the product version string and NULL terminator */
    if (fwrite(infoBlock->productVersion, strlen(infoBlock->productVersion) + 1, 1, fp) != 1)
    {
        return -1;
    }
    ...
    unused = infoBlockSize
             - (sizeof(infoBlockSize) + sizeof(additionalBlockID) + strlen(infoBlock->MARChannelID)
...
int refresh_product_info_block(const char* path, struct ProductInformationBlock* infoBlock)
{
    FILE* fp;
    int rv;
    uint32_t numSignatures, additionalBlockSize, additionalBlockID, offsetAdditionalBlocks,
        numAdditionalBlocks, i;
    int additionalBlocks, hasSignatureBlock;
    int64_t oldPos;

    rv = get_mar_file_info(path, &hasSignatureBlock, &numSignatures, &additionalBlocks,
               ...
               struct ProductInformationBlock* infoBlock)
{
    ...
    uint32_t offset_to_index = 0, size_of_index, numSignatures, numAdditionalSections;
    ...
    stack.last_offset = MAR_ID_SIZE + sizeof(offset_to_index) + sizeof(numSignatures)
                        + sizeof(numAdditionalSections) + sizeof(sizeOfEntireMAR);
    ...
    if (fwrite(&numAdditionalSections, sizeof(numAdditionalSections), 1, fp) != 1)
    {
        goto failure;
    }
    ...
    sizeOfEntireMAR = ((uint64_t)stack.last_offset) + stack.size_used + sizeof(size_of_index);
