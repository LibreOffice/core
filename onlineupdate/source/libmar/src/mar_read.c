...
static int mar_insert_item(MarFile* mar, const char* name, int namelen, uint32_t offset,
 ...
 */
int get_mar_file_info_fp(FILE* fp, int* hasSignatureBlock, uint32_t* numSignatures,
...
{
    uint32_t i, offsetAdditionalBlocks, numAdditionalBlocks, additionalBlockSize, additionalBlockID;
    int hasAdditionalBlocks;

    /* The buffer size is 97 bytes because the MAR channel name < 64 bytes, and
     product version < 32 bytes + 3 NULL terminator bytes. */
    char buf[97] = { '\0' };
    int ret = get_mar_file_info_fp(mar->fp, NULL, NULL, &hasAdditionalBlocks,
    ...
    for (i = 0; i < numAdditionalBlocks; ++i)
    {
        ...
        if (fread(&additionalBlockSize, sizeof(additionalBlockSize), 1, mar->fp) != 1)
        {
            return -1;
        }
        additionalBlockSize
                      ...
                      uint32_t* numAdditionalBlocks)
{
    ...
    rv = get_mar_file_info_fp(fp, hasSignatureBlock, numSignatures, hasAdditionalBlocks,
                              offsetAdditionalBlocks, numAdditionalBlocks);
