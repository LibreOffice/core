                                    ...
                                    sal_Int32 expectedCols)
{
    ...
    aArgs[0].Value = uno::makeAny(cellRange);

    aArgs[1].Name = "HasCategories";
    aArgs[1].Value = uno::makeAny(hasCategories);

    aArgs[2].Name = "FirstCellAsLabel";
    aArgs[2].Value = uno::makeAny(firstCellAsLabel);

    aArgs[3].Name = "DataRowSource";
    aArgs[3].Value = uno::makeAny(chart::ChartDataRowSource_COLUMNS);
...
void ScChart2DataProviderTest::testHeaderExpansion()
{
    ...
    lcl_createAndCheckDataProvider(rDoc, OUString("$Sheet1.$A$1:$D$4"), false, false, 4, 4);
    lcl_createAndCheckDataProvider(rDoc, OUString("$Sheet1.$A$1:$D$4"), true, true, 4, 3);

    lcl_createAndCheckDataProvider(rDoc, OUString("$Sheet1.$A$9:$D$12"), true, true, 1, 2);
