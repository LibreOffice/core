...
void ScTiledRenderingTest::testRowColumnSelections()
{
    ...
    /* Currently getTextSelection seems to crash with ctrl modifier. Activate it
     * when it gets fixed.
     */
    /*
    ...
    // Check if selected range is 5-10 and 13
    aResult = pModelObj->getTextSelection("text/plain;charset=utf-8", aUsedMimeType);
    CPPUNIT_ASSERT_EQUAL(aExpected, aResult);
    ...
    // Check if selected data includes column data too
    aResult = pModelObj->getTextSelection("text/plain;charset=utf-8", aUsedMimeType);
    CPPUNIT_ASSERT_EQUAL(aExpected, aResult);
