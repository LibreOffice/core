...
void ScExportTest::testTdf124741()
{
    ...
    // Check that we export default width, otherwise the skipped columns would have wrong width
    // Previously defaultColWidth attribute was missing
    assertXPath(pDoc, "/x:worksheet/x:sheetFormatPr", "defaultColWidth", "10");
    assertXPath(pDoc, "/x:worksheet/x:cols/x:col", 1);
    assertXPath(pDoc, "/x:worksheet/x:cols/x:col[1]", "min", "2");
    assertXPath(pDoc, "/x:worksheet/x:cols/x:col[1]", "max", "2");
    assertXPath(pDoc, "/x:worksheet/x:cols/x:col[1]", "width", "14.08");
}
