...
void ScExportTest::testHeaderFontStyleXLSX()
{
    ...
    // third line is bold italic
    pTextObj->GetCharAttribs(2, rLst);
    {
        bool bHasBoldItalic = std::any_of(rLst.begin(), rLst.end(), [](const EECharAttrib& rAttrib) {
            return rAttrib.pAttr->Which() == EE_CHAR_WEIGHT &&
                static_cast<const SvxPostureItem&>(*rAttrib.pAttr).GetPosture() == WEIGHT_BOLD; });
        bool bHasItalicBold = std::any_of(rLst.begin(), rLst.end(), [](const EECharAttrib& rAttrib) {
            return rAttrib.pAttr->Which() == EE_CHAR_ITALIC &&
                static_cast<const SvxPostureItem&>(*rAttrib.pAttr).GetPosture() == ITALIC_NORMAL; });

        CPPUNIT_ASSERT_MESSAGE("Third line should be bold italic.", bHasBoldItalic && bHasItalicBold);
    }
