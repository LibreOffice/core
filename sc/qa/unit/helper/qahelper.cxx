...
ScRangeList getChartRanges(ScDocument& rDoc, const SdrOle2Obj& rChartObj)
{
    ...
    for (size_t i = 0, n = aRangeReps.size(); i < n; ++i)
    {
        ...
        else
        {
            ...
            CPPUNIT_ASSERT_MESSAGE("Failed to parse a range representation.", (nRes & static_cast<sal_uInt16>(ScAddr::VALID)));
...
ScTokenArray* getTokens(ScDocument& rDoc, const ScAddress& rPos)
{
    ...
    if (!pCell)
    {
        OUString aStr = rPos.Format(static_cast<sal_uInt16>(ScAddr::VALID));
...
bool checkFormulaPosition(ScDocument& rDoc, const ScAddress& rPos)
{
    ...
    if (pFC->aPos != rPos)
    {
        OUString aStr2(pFC->aPos.Format(static_cast<sal_uInt16>(ScAddr::VALID)));
    ...
    ScDocument& rDoc, SCTAB nTab, SCCOL nCol, const SCROW* pRows, size_t nRowCount)
{
    ...
    for (size_t i = 0; i < nRowCount; ++i)
    {
        ...
        {
            OUString aStr(aPos.Format(static_cast<sal_uInt16>(ScAddr::VALID)));
