...
void TestSharedFormula::testSharedFormulas()
{
    ...
    CPPUNIT_ASSERT_EQUAL_MESSAGE("The token is expected to be shared.", pFC->GetCode(),
                                 pFC->GetSharedCode());
    ...
    CPPUNIT_ASSERT_EQUAL_MESSAGE("This cell should have been emptied.", CELLTYPE_NONE,
                                 m_pDoc->GetCellType(aPos));
    aPos.SetRow(12); // B13
    pFC = m_pDoc->GetFormulaCell(aPos);
    CPPUNIT_ASSERT(pFC);
...
void TestSharedFormula::testSharedFormulasRefUpdate()
{
...

    {
        ...
        const std::vector<std::vector<const char*>> aData = { { "=A10" }, { "=A11" }, { "=A12" } };
...
void TestSharedFormula::testSharedFormulasRefUpdateMove()
{
    ...
    // Move B2:B4 to B1:B3.
    bool bMoved = m_xDocShell->GetDocFunc().MoveBlock(ScRange(1, 1, 0, 1, 3, 0), ScAddress(1, 0, 0),
...
void TestSharedFormula::testSharedFormulasRefUpdateMove2()
{
...

    {
        ...
        const std::vector<std::vector<const char*>> aData = { { "=RC[-1]" }, { "=RC[-1]" } };
    ...
    // Move B2:C3 to C3:D4.
    bool bMoved = m_xDocShell->GetDocFunc().MoveBlock(ScRange(1, 1, 0, 2, 2, 0), ScAddress(2, 2, 0),
                                                      true, true, false, true);
    CPPUNIT_ASSERT(bMoved);
...
void TestSharedFormula::testSharedFormulasRefUpdateRange()
{
...

    {
        ...
        const std::vector<std::vector<const char*>> aData
            = { { "=SUM($A$3:$A$5)" }, { "=SUM($A$3:$A$5)" }, { "=SUM($A$3:$A$5)" } };
...
void TestSharedFormula::testSharedFormulasRefUpdateRangeDeleteRow()
{
    ...
    const std::vector<std::vector<const char*>> aData = { { "1", "2", "=SUM(A1:B1)" },
                                                          { "3", "4", "=SUM(A2:B2)" },
    ...
    CPPUNIT_ASSERT_EQUAL_MESSAGE("There should only be 2 area listeners.", size_t(2),
                                 aListeners.size());
    // First one should be group-listening on A1:B2.
    CPPUNIT_ASSERT_EQUAL_MESSAGE("This listener should be listening on A1:B2.",
                                 ScRange(0, 0, 0, 1, 1, 0), aListeners[0].maArea);
    ...
    CPPUNIT_ASSERT_EQUAL_MESSAGE("There should only be 1 area listener.", size_t(1),
                                 aListeners.size());
    CPPUNIT_ASSERT_EQUAL_MESSAGE("This listener should be listening on A1:B4.",
                                 ScRange(0, 0, 0, 1, 3, 0), aListeners[0].maArea);
    CPPUNIT_ASSERT_MESSAGE("This listener should be group-listening.",
                           aListeners[0].mbGroupListening);
#endif
...
void TestSharedFormula::testSharedFormulasRefUpdateExternal()
{
    ...
    ASSERT_FORMULA_EQUAL(*m_pDoc, ScAddress(0, 6, 0), "'file:///extdata.fake'#$Data.A1",
                         "Wrong formula!");
    ASSERT_FORMULA_EQUAL(*m_pDoc, ScAddress(0, 7, 0), "'file:///extdata.fake'#$Data.A2",
                         "Wrong formula!");
    ASSERT_FORMULA_EQUAL(*m_pDoc, ScAddress(0, 8, 0), "'file:///extdata.fake'#$Data.A3",
                         "Wrong formula!");
    ASSERT_FORMULA_EQUAL(*m_pDoc, ScAddress(0, 9, 0), "COUNTA('file:///extdata.fake'#$Data.A1:A3)",
                         "Wrong formula!");
    ...
    ASSERT_FORMULA_EQUAL(*m_pDoc, ScAddress(0, 4, 0), "'file:///extdata.fake'#$Data.A1",
                         "Wrong formula!");
    ASSERT_FORMULA_EQUAL(*m_pDoc, ScAddress(0, 5, 0), "'file:///extdata.fake'#$Data.A2",
                         "Wrong formula!");
    ASSERT_FORMULA_EQUAL(*m_pDoc, ScAddress(0, 6, 0), "'file:///extdata.fake'#$Data.A3",
                         "Wrong formula!");
    ASSERT_FORMULA_EQUAL(*m_pDoc, ScAddress(0, 7, 0), "COUNTA('file:///extdata.fake'#$Data.A1:A3)",
                         "Wrong formula!");
...
void TestSharedFormula::testSharedFormulasDeleteRows()
{
...

    {
        ...
        const std::vector<std::vector<const char*>> aData
            = { { "0", "=RC[-1]+1" },   { "1", "=RC[-1]+1" },   { "2", "=RC[-1]+1" },
                { "3", "=RC[-1]+1" },   { "4", "=RC[-1]+1" },   { "5", "=RC[-1]+1" },
                { "6", "=RC[-1]+1" },   { "7", "=RC[-1]+1" },   { "8", "=RC[-1]+1" },
                { "9", "=RC[-1]+1" },   { "10", "=RC[-1]+11" }, { "11", "=RC[-1]+11" },
                { "12", "=RC[-1]+11" }, { "13", "=RC[-1]+11" }, { "14", "=RC[-1]+11" },
                { "15", "=RC[-1]+11" }, { "16", "=RC[-1]+11" }, { "17", "=RC[-1]+11" },
                { "18", "=RC[-1]+11" }, { "19", "=RC[-1]+11" } };
...
void TestSharedFormula::testSharedFormulasCopyPaste()
{
    ...
    std::unique_ptr<ScUndoPaste> pUndo(
        createUndoPaste(*m_xDocShell, aRange, ScDocumentUniquePtr(pUndoDoc)));
    ...
    for (SCROW i = 0; i <= 9; ++i)
        CPPUNIT_ASSERT_EQUAL_MESSAGE("Numeric cell was expected.", CELLTYPE_VALUE,
                                     m_pDoc->GetCellType(ScAddress(1, i, 0)));
...
void TestSharedFormula::testSharedFormulaAbsCellListener()
{
    ...
    const std::vector<std::vector<const char*>> aData = { { "=$A$1" }, { "=$A$1" }, { "=$A$1" } };
...
void TestSharedFormula::testSharedFormulaUnshareAreaListeners()
{
    ...
    const std::vector<std::vector<const char*>> aData = {
        { "=SUM(B1:B2)", "1" }, { "=SUM(B2:B3)", "2" }, { "=SUM(B3:B4)", "4" }, { nullptr, "8" }
    };
    ...
    for (int nRun = 0; nRun < 7; ++nRun)
    {
        ...
        const std::vector<std::vector<const char*>> aData2 = {
            { "=SUM(B2:C2)", "1", "2" },     { "=SUM(B3:C3)", "4", "8" },
            { "=SUM(B4:C4)", "16", "32" },   { "=SUM(B5:C5)", "64", "128" },
            { "=SUM(B6:C6)", "256", "512" },
        };
        ...
        CPPUNIT_ASSERT_EQUAL_MESSAGE("Shared length.", static_cast<SCROW>(5),
                                     pFC->GetSharedLength());
        ...
        CPPUNIT_ASSERT_EQUAL_MESSAGE("Shared length.", static_cast<SCROW>(2),
                                     pFC->GetSharedLength());
        aPos.IncRow(3);
        pFC = m_pDoc->GetFormulaCell(aPos);
        CPPUNIT_ASSERT(pFC);
    ...
    // cell vectors.
    {
        ...
        std::vector<ScFormulaCell*> aCells;
        const std::vector<std::vector<const char*>> aData2 = {
            { "=SUM(B3:C3)", "1", "2" },     { "=SUM(B4:C4)", "4", "8" },
            { "=SUM(B5:C5)", "16", "32" },   { "=SUM(B6:C6)", "64", "128" },
            { "=SUM(B7:C7)", "256", "512" },
        };
        ...
        CPPUNIT_ASSERT_EQUAL_MESSAGE("Shared top row.", aPos.Row(), pFC->GetSharedTopRow());
        CPPUNIT_ASSERT_EQUAL_MESSAGE("Shared length.", static_cast<SCROW>(7),
                                     pFC->GetSharedLength());
        ...
        CPPUNIT_ASSERT_EQUAL_MESSAGE("Shared top row.", aPos.Row(), pFC->GetSharedTopRow());
        CPPUNIT_ASSERT_EQUAL_MESSAGE("Shared length.", static_cast<SCROW>(4),
                                     pFC->GetSharedLength());
        ...
        CPPUNIT_ASSERT_MESSAGE("A5", pFC->IsSharedTop());
        CPPUNIT_ASSERT_EQUAL_MESSAGE("Shared top row.", aPos.Row(), pFC->GetSharedTopRow());
        CPPUNIT_ASSERT_EQUAL_MESSAGE("Shared length.", static_cast<SCROW>(3),
                                     pFC->GetSharedLength());
...
void TestSharedFormula::testSharedFormulaUpdateOnReplacement()
{
    ...
    const std::vector<std::vector<const char*>> aData = { { "1" },
                                                          { "=SUM($A$1:$A1)" },
                                                          { "=SUM($A$1:$A2)" },
                                                          { "=SUM($A$1:$A3)" },
                                                          { "=SUM($A$1:$A4)" },
    ...
    ScUndoDeleteContents aUndo(m_xDocShell.get(), aMark, aUndoRange, std::move(pUndoDoc), false,
...
void TestSharedFormula::testSharedFormulaDeleteTopCell()
{
    ...
    ScMarkData aMark(m_pDoc->GetSheetLimits());
    aMark.SelectOneTable(0);
    m_xDocShell->GetDocFunc().DeleteCell(ScAddress(0, 0, 0), aMark, InsertDeleteFlags::CONTENTS,
                                         false, /*bApi=*/true);
    // Check it's gone.
...
void TestSharedFormula::testSharedFormulaCutCopyMoveWithRef()
{
    ...
    // Data in A1:C4
    std::vector<std::vector<const char*>> aData = { { "", "", "=SUM(A1:B1)" },
                                                    { "", "", "=SUM(A2:B2)" },
                                                    { "1", "2", "=SUM(A3:B3)" },
                                                    { "4", "8", "=SUM(A4:B4)" } };
    const ScAddress aOrgPos(0, 0, 0);
    ...
    const std::u16string_view sForm[]
        = { u"=SUM(A1:B1)", u"=SUM(A2:B2)", u"=SUM(A1:B1)", u"=SUM(A4:B4)" };
    for (SCROW i = 0; i < 4; ++i)
    {
        OUString aFormula;
...
void TestSharedFormula::testSharedFormulaCutCopyMoveWithinRun()
{
    ...
    // Data in C3:E9
    const std::vector<std::vector<const char*>> aData
        = { { "2200", "", "=SUM(C$3:C3)-SUM(D$3:D3)" }, { "", "", "=SUM(C$3:C4)-SUM(D$3:D4)" },
            { "", "1900", "=SUM(C$3:C5)-SUM(D$3:D5)" }, { "", "", "=SUM(C$3:C6)-SUM(D$3:D6)" },
            { "1600", "", "=SUM(C$3:C7)-SUM(D$3:D7)" }, { "", "1000", "=SUM(C$3:C8)-SUM(D$3:D8)" },
    ...
    CPPUNIT_ASSERT(pFC);
    CPPUNIT_ASSERT_EQUAL_MESSAGE("Shared formula top row.", aFormulaPos.Row(),
                                 pFC->GetSharedTopRow());
    CPPUNIT_ASSERT_EQUAL_MESSAGE("Shared formula length.", static_cast<SCROW>(7),
                                 pFC->GetSharedLength());
...
void TestSharedFormula::testSharedFormulaInsertShift()
{
...

    {
        ...
        CPPUNIT_ASSERT_EQUAL_MESSAGE("Shared formula top row.", aFormulaPos.Row(),
                                     pFC->GetSharedTopRow());
        CPPUNIT_ASSERT_EQUAL_MESSAGE("Shared formula length.", static_cast<SCROW>(2),
                                     pFC->GetSharedLength());
    }
