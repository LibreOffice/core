...
void ScFiltersTest::testCommentSize()
{
    ...
    CPPUNIT_ASSERT_EQUAL(tools::Long(2899), rOldRect.getHalfOpenWidth());
    CPPUNIT_ASSERT_EQUAL(tools::Long(939), rOldRect.getHalfOpenHeight());

    pNote->SetText(aPos, "first\nsecond\nthird");

    const tools::Rectangle& rNewRect = pCaption->GetLogicRect();
    CPPUNIT_ASSERT_EQUAL(rOldRect.getHalfOpenWidth(), rNewRect.getHalfOpenWidth());
    CPPUNIT_ASSERT_EQUAL(tools::Long(1605), rNewRect.getHalfOpenHeight());

    rDoc.GetUndoManager()->Undo();

    CPPUNIT_ASSERT_EQUAL(rOldRect.getHalfOpenWidth(), pCaption->GetLogicRect().getHalfOpenWidth());
    CPPUNIT_ASSERT_EQUAL(rOldRect.getHalfOpenHeight(), pCaption->GetLogicRect().getHalfOpenHeight());
