...
void runTestMATCH(ScDocument* pDoc, const char* aData[_DataSize], StrStrCheck aChecks[_FormulaSize])
{
    ...
    for (size_t i = 0; i < _FormulaSize; ++i)
    {
        ...
        OUStringBuffer aBuf(
            "=MATCH(B"+static_cast<sal_Int32>(i+1));
            +";A1:A"+static_cast<sal_Int32>(nDataSize)
            +";"+static_cast<sal_Int32>(_Type)+")"
        );
...
void runTestHorizontalMATCH(ScDocument* pDoc, const char* aData[_DataSize], StrStrCheck aChecks[_FormulaSize])
{
    ...
    for (size_t i = 0; i < _FormulaSize; ++i)
    {
        ...
        OUStringBuffer aBuf("=MATCH("+static_cast<sal_Unicode>('A'+i)+"2;A1:"+static_cast<sal_Unicode>('A'+nDataSize)+"1;"+static_cast<sal_Int32>(_Type)+")");
        OUString aFormula = aBuf.makeStringAndClear();
