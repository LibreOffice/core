...
class tdf126248(UITestCase):
    ...
    def test_tdf126248(self):
        ...
        westFontName = xCellsDlg.getChild("westfontnamelb-cjk")
        self.assertEqual("Liberation Sans", get_state_as_dict(westFontName)['Text'])

        eastFontName = xCellsDlg.getChild("eastfontnamelb")
        self.assertEqual("Noto Sans CJK SC Regular", get_state_as_dict(eastFontName)['Text'])
        ...
        self.ui_test.wait_until_property_is_updated(fontName, "Text", "Liberation Sans")

        # Without the fix in place, this test would have failed with
        # AssertionError: 'Liberation Sans' != 'Noto Sans CJK SC Regular'
        self.assertEqual("Liberation Sans", get_state_as_dict(fontName)['Text'])

        enter_text_to_cell(gridwin, "B1", "測試")

        self.ui_test.wait_until_property_is_updated(fontName, "Text", "Noto Sans CJK SC Regular")
        self.assertEqual("Noto Sans CJK SC Regular", get_state_as_dict(fontName)['Text'])
