                ...
                with self.ui_test.execute_dialog_through_command(".uno:Save", close_button="") as xSaveDialog:
                    ...
                    with self.ui_test.execute_dialog_through_action(xOpen, "CLICK", close_button="") as xWarnDialog:
                        ...
                        with self.ui_test.execute_dialog_through_action(xSave, "CLICK") as xCsvDialog:
                            xCharsetDropdown = xCsvDialog.getChild("charsetdropdown")
                            xField = xCsvDialog.getChild("field")
                            xText = xCsvDialog.getChild("text")

                            self.assertEqual("Unicode (UTF-8)", get_state_as_dict(xCharsetDropdown)['DisplayText'])
                            self.assertEqual(",", get_state_as_dict(xField)['Text'])
                            self.assertEqual('"', get_state_as_dict(xText)['Text'])
