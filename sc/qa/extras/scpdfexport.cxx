...
private:
    ...
    static bool hasFontInPdf(const std::shared_ptr<utl::TempFile> & pXPathFile, const char * sFontName, bool & bFound);
    void setFont(ScFieldEditEngine& rEE, sal_Int32 nStart, sal_Int32 nEnd, const OUString& rFontName);
...
bool ScPDFExportTest::hasFontInPdf(const std::shared_ptr<utl::TempFile> & pXPathFile, const char * sFontName, bool & bFound)
{
    ...
    std::size_t nRead = pStream->ReadBytes(pBuffer, nFileSize);
    if (nRead == nFileSize)
    {
...
std::shared_ptr<utl::TempFile> ScPDFExportTest::exportToPdf(uno::Reference< frame::XModel > & xModel, const ScRange & range)
{
    ...
    const OUString sFileURL = pTempFile->GetURL().replace('\\', '/');
...
void ScPDFExportTest::setFont(ScFieldEditEngine& rEE, sal_Int32 nStart, sal_Int32 nEnd, const OUString& rFontName)
{
    ESelection aSel;
