...
class InsertRemoveCells(unittest.TestCase):
    ...
    def test_fdo74824_load(self):
        ...
        tdoc_dir = os.getenv('TDOC')
        url = 'file://' + quote(os.path.join(tdoc_dir, 'fdo74824.ods'))
        ...
        # the ENUM to put in it's place.  Corrections welcomed.
        sheet.insertCells( addr, 2 )

        # basically, the insertCells call is the test: it should not crash.
        # LibreOffice.  However, for thoroughness, we should test the cell
        # contents as well while we're here.
