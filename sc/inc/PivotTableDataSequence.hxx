...
struct PivotTableItem
{
    ...
    explicit PivotTableItem(double fValue, sal_uInt32 nNumberFormat)
        : m_fValue(fValue)
        , m_aString()
        , m_bIsValue(true)
    ...
    explicit PivotTableItem(OUString const & rString)
        : m_fValue(0.0)
        , m_aString(rString)
        , m_bIsValue(false)
        , m_nNumberFormat(0)
...
{
public:
    explicit PivotTableDataSequence(ScDocument* pDocument, OUString const & sPivotTableName, OUString const & sID,
                                    std::vector<PivotTableItem> const & rData);
...
private:
    ...
    OUString m_aID;
    std::vector<PivotTableItem> m_aData;
