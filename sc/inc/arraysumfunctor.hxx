...
// git history). SSE2 is guaranteed on x86_64 and we require it as baseline
// for x86, so SSE2 use is hardcoded on x86/x86_64. Whenever we raise baseline
// to e.g. AVX, this may get replaced with AVX code (get it from git history).
