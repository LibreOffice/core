...
#ifdef LO_AVX512F_AVAILABLE
    if (hasAVX512F)
        bOK &= sc::op::executeAVX512F(i, 9, pCurrent).get() == 42.0;
#endif
#ifdef LO_AVX_AVAILABLE
    if (hasAVX)
        bOK &= sc::op::executeAVX(i, 9, pCurrent).get() == 42.0;
#endif
#ifdef LO_SSE2_AVAILABLE
    if (hasSSE2)
        bOK &= sc::op::executeSSE2(i, 9, pCurrent).get() == 42.0;
#endif
    bOK &= sc::op::executeUnrolled(i, 9, pCurrent).get() == 42.0;
    assert(bOK);
}
#endif
