...
void ScFormulaListener::startListening(ScTokenArray const * pArr, const ScRange& rRange)
{
...
void ScFormulaListener::addTokenArray(ScTokenArray const * pArray, const ScRange& rRange)
{
...
void ScColorScaleEntry::UpdateReference( sc::RefUpdateContext const & rCxt )
{
    if (!mpCell)
        return;
...
void ScColorScaleEntry::UpdateMoveTab( sc::RefUpdateMoveTabContext const & rCxt )
{
    if (!mpCell)
        return;
...
double ScColorScaleFormat::CalcValue(double nMin, double nMax, ScColorScaleEntries::const_iterator const & itr) const
{
    switch((*itr)->GetType())
    {
...
double ScIconSetFormat::CalcValue(double nMin, double nMax, ScIconSetFormat::const_iterator const & itr) const
{
    switch ((*itr)->GetType())
    {
        case COLORSCALE_PERCENT:
