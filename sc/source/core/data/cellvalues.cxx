...
typedef std::vector<std::unique_ptr<CellValues>> TableType_new;
typedef std::vector<std::unique_ptr<TableType_new>> TablesType_new;

struct TableValues::Impl
{
    ScRange maRange;
    TablesType_new m_Tables;
    ...
    CellValues* getCellValues( SCTAB nTab, SCCOL nCol )
    {
        ...
        if (nTabOffset >= m_Tables.size())
            return NULL;
        std::unique_ptr<TableType_new>& rTab2 = m_Tables[nTab-maRange.aStart.Tab()];
