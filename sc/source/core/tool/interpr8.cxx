 ...
 * Addition to the above is that initial calculation of Seasonal effect
 * is correctd for trend, whereas the above uses mean base value for the
 * complete period.
 *
...
private:
    ...
    double* sBase;                      // calculated base data array (without periodical influence)
    double* sTrend;                     // calculated trend factor array
    double* sPeriodicalIdx;             // calculated periodical index array
    double* sForecast;                  // forecasted value (X, Y) array
    ...
    double fMSE;                        // mean squared error (variation)
    double fRMSE;                       // root mean squared error (standarddeviation)
    ...
    const SCSIZE cnScenarios = 1000;          // number of scenarios to calculate for Prediction Interval calculations
    ...
    double RandDev( double fPILevel );
...
public:
    ...
    bool PreprocessDataRange( ScMatrixRef rMatX, ScMatrixRef rMatY, int& rSamplesinPeriod,
                              ...
                              ScETSType eETSType, sal_uInt16& rPreprocError  );
...
bool ScETSForecastCalculation::PreprocessDataRange( ScMatrixRef rMatX, ScMatrixRef rMatY, int& rSamplesinPeriod,
                                                    ...
                                                    ScETSType eETSType, sal_uInt16& rPreprocError  )
{
    bAdditive = ( eETSType == etsAdd || eETSType == etsSeason ||
    ...
    if ( rTMat )
    {
        if ( eETSType != etsPIAdd and eETSType != etsPIMult )
        {
        ...
        {
            // Excel2016 doesn't accept X-values <= sRange[ nCount - 1 ].fX for etsPIAdd
            if ( rTMat->GetDouble( 0 ) < sRange[ nCount - 1 ].fX )
            {
                // target cannot be in or before time(X)-range
                rPreprocError = errIllegalFPOperation; // #NUM!
                return false;
            }
        }
...
bool ScETSForecastCalculation::initData()
{
    ...
    sBase = new double[ nCount ];
    sTrend = new double[ nCount ];
    sPeriodicalIdx = new double[ nCount ];
    sForecast = new double[ nCount ];
    sForecast[ 0 ] = sRange[ 0 ].fY;
...
{
    sBase[ 0 ] = sRange[ 0 ].fY / sPeriodicalIdx[ 0 ];
    return true;
}
...
void ScETSForecastCalculation::calcAccuracyIndicators()
{
    ...
    for ( SCSIZE i = 1; i < nCount; i++ )
    {
        double fError = sForecast[ i ] - sRange[ i ].fY;
        fSumAbsErr    += fabs( fError );
        fSumErrSq     += fError * fError;
        fSumAbsPercErr += fabs( fError ) / ( fabs( sForecast[ i ] ) + fabs( sRange[ i ].fY ) );
    }

    for ( SCSIZE i = 2; i < nCount; i++ )
        fSumDivisor += fabs( sRange[ i ].fY - sRange[ i - 1 ].fY );

    int nCalcCount = nCount - 1;
...
{
    // refill sBase, sTrend, sPeriodicalidx and sForecast with values
    ...
    for ( SCSIZE i = 1; i < nCount; i++ )
    {
        ...
        if ( bAdditive )
        {
            ...
            sBase[ i ] = fAlpha * ( sRange[ i ].fY - sPeriodicalIdx[ nIdx ] ) +
                         ( 1 - fAlpha ) * ( sBase[ i - 1 ] + sTrend[ i - 1 ] );
            sPeriodicalIdx[ i ] = fBeta * ( sRange[ i ].fY - sBase[ i ] ) +
        ...
        sTrend[ i ] = fGamma * ( sBase[ i ] - sBase[ i - 1 ] ) +
                      ( 1 - fGamma ) * sTrend[ i - 1 ];

        if ( bAdditive )
            sForecast[ i ] = sBase[ i - 1 ] + sTrend[ i - 1 ] + sPeriodicalIdx[ nIdx ];
        else
            sForecast[ i ] = ( sBase[ i - 1 ] + sTrend[ i - 1 ] ) * sPeriodicalIdx[ nIdx ];
    }
    calcAccuracyIndicators();
...
bool ScETSForecastCalculation::GetForecast( double fTarget, double& rForecast )
{
    ...
    else
    {
        ...
        if ( bAdditive )
            rForecast = sBase[ nCount - 1 ] + n * sTrend[ nCount - 1 ] +
        ...
        if ( fInterpolate >= cfMinABCResolution )
        {
            ...
            double fFc_1;
            if ( bAdditive )
                fFc_1 = sBase[ nCount - 1 ] + ( n + 1 ) * sTrend[ nCount - 1 ] +
...
// TODO : this is a (almost) copy of ScInterpreter::GetPercentile (nterpr3.cxx)
//        a small change in the original code (move check to before getperCentile call)
//        plus making it static may solve this redundancy problem.
double lcl_GetPercentile( vector<double> & rArray, double fPercentile )
{
...
double ScETSForecastCalculation::RandDev( double fPILevel )
{
    // return a random deviation given the standarddeviation and the probability (prediction interval)
    return ( ScInterpreter::gaussinv( fPILevel ) * fRMSE * ( ( ( double ) rand() / RAND_MAX ) - 0.5 ) );
}
...
bool ScETSForecastCalculation::GetPredictionIntervals( ScMatrixRef rTMat, ScMatrixRef rPIMat, double fPILevel )
{
    ...
    fMaxTarget -= sRange[ nCount - 1 ].fX;
    SCSIZE nSize = ( fMaxTarget / fStepSize );
    if ( fmod( fMaxTarget, fStepSize ) != 0.0 )
        nSize++;
    double sScenRange[ nSize ], sScenBase[ nSize ], sScenTrend[ nSize ], sScenPerIdx[ nSize ];
    vector< vector< double > >  sPredictions( nSize, vector< double >( cnScenarios ) );

    // fill scenarios
    for ( SCSIZE k = 0; k < cnScenarios; k++ )
    {
        // fill array with forecasts
        // first time, rand() is added to sPrediction, in loop rand() is added to sScenRange
        if ( bAdditive )
        {
// TODO : test code here
            sPredictions[ 0 ][ k ] = sBase[ nCount - 1 ] + sTrend[ nCount - 1 ] +
                                     sPeriodicalIdx[ nCount - nSamplesinPeriod ] +
                                     RandDev( fPILevel );
            sScenRange[ 0 ] = sPredictions[ 0 ][ k ] + RandDev( fPILevel );
            sScenBase[ 0 ] = fAlpha * ( sScenRange[ 0 ] - sPeriodicalIdx[ nCount - nSamplesinPeriod ] ) +
                             ( 1 - fAlpha ) * ( sBase[ nCount - 1 ] + sTrend[ nCount - 1 ] );
            sScenTrend[ 0 ] = fGamma * ( sScenBase[ 0 ] - sBase[ nCount - 1 ] ) +
                              ( 1 - fGamma ) * sTrend[ nCount - 1 ];
            sScenPerIdx[ 0 ] = fBeta * ( sScenRange[ 0 ] - sScenBase[ 0 ] ) +
                               ( 1 - fBeta ) * sPeriodicalIdx[ nCount - nSamplesinPeriod ];
            for ( SCSIZE i = 1; i < nSize; i++ )
            {
                double fPerIdx;
                if ( i < nSamplesinPeriod )
                    fPerIdx = sPeriodicalIdx[ nCount + i - nSamplesinPeriod ];
                else
                    fPerIdx = sScenPerIdx[ i - nSamplesinPeriod ];
                sPredictions[ i ][ k ] = sScenBase[ i - 1 ] + sScenTrend[ i - 1 ] + fPerIdx;
                sScenRange[ i ] = sPredictions[ i ][ k ] + RandDev( fPILevel );
                sScenBase[ i ] = fAlpha * ( sScenRange[ i ] - fPerIdx ) +
                                 ( 1 - fAlpha ) * ( sScenBase[ i - 1 ] + sScenTrend[ i - 1 ] );
                sScenTrend[ i ] = fGamma * ( sScenBase[ i ] - sScenBase[ i - 1 ] ) +
                                  ( 1 - fGamma ) * sScenTrend[ i - 1 ];
                sScenPerIdx[ i ] = fBeta * ( sScenRange[ i ] - sScenBase[ i ] ) +
                                   ( 1 - fBeta ) * fPerIdx;
            }
        }
        else
...
// TODO : test code here
    ...
    double sPercentile[ nSize ];
    for ( SCSIZE i = 0; i < nSize; i++ )
    {
        sPercentile[ i ] = lcl_GetPercentile( sPredictions[ i ], 1 - ( 1.0 - fPILevel ) / 2.0 ) -
                           lcl_GetPercentile( sPredictions[ i ], ( 1.0 - fPILevel ) / 2.0 );
    }
    ...
    for ( SCSIZE i = 0; i < nR; i++ )
    {
        for ( SCSIZE j = 0; j < nC; j++ )
        {
            double fTarget = rTMat->GetDouble( j, i ) - sRange[ nCount - 1 ].fX;
            SCSIZE nSteps = ( fTarget / fStepSize ) - 1;
            double fFactor = fmod( fTarget, fStepSize );
            double fPI = sPercentile[ nSteps ];
            if ( fFactor != 0.0 )
            {
                // interpolate
                double fPI1 = sPercentile[ nSteps + 1 ];
...
/* vim:set nu shiftwidth=2 softtabstop=2 expandtab: */
