 ...
 * is corrected for trend, whereas the above uses mean base value for the
 * complete period.
 *
...
private:
    ...
    SCSIZE mnSmplInPrd;                 // length of period
    double mfStepSize;                  // increment of X in maRange
    double mfAlpha, mfBeta, mfGamma;    // constants to minimise the MSE in the ETS-equations
    ...
    double mfRMSE;                      // root mean squared error (standarddeviation)
                                                    ...
                                                    ScETSType eETSType )
{
    ...
    if ( rTMat )
    {
        ...
        else
        {
            ...
            {
                // target cannot be in or before X-range
                mnErrorValue = errIllegalFPOperation;
                return false;
            }
        }
    ...
    Date aNullDate = *( pFmtr->GetNullDate() );
    ...
    for ( SCSIZE i = 1; i < mnCount; i++ )
    {
        ...
        if ( fStep == 0.0 )
        {
            if ( nAggregation == 0 )
            {
                // double X-values are not allowed
                mnErrorValue = errNoValue; // #VALUE!
                return false;
            }
...
double ScETSForecastCalculation::RandDev()
{
    ...
    return ( mfRMSE * ScInterpreter::gaussinv( static_cast< double >( ( 1.0 + rand() / 2.0 ) ) / RAND_MAX ) );
}
...
bool ScETSForecastCalculation::GetETSPredictionIntervals( ScMatrixRef rTMat, ScMatrixRef rPIMat, double fPILevel )
{
    ...
    double pScenRange[ nSize ], pScenBase[ nSize ], pScenTrend[ nSize ], pScenPerIdx[ nSize ];
    vector< vector< double > >  aPredictions( nSize, vector< double >( cnScenarios ) );

    // fill scenarios
    for ( SCSIZE k = 0; k < cnScenarios; k++ )
    {
        // fill array with forecasts, with RandDev() added to pScenRange
        if ( bAdditive )
        {
            // calculation based on additive model
            pScenRange[ 0 ] = mpBase[ mnCount - 1 ] + mpTrend[ mnCount - 1 ] +
                              mpPerIdx[ mnCount - mnSmplInPrd ] +
                              RandDev();
            aPredictions[ 0 ][ k ] = pScenRange[ 0 ];
            pScenBase[ 0 ] = mfAlpha * ( pScenRange[ 0 ] - mpPerIdx[ mnCount - mnSmplInPrd ] ) +
                             ( 1 - mfAlpha ) * ( mpBase[ mnCount - 1 ] + mpTrend[ mnCount - 1 ] );
            pScenTrend[ 0 ] = mfGamma * ( pScenBase[ 0 ] - mpBase[ mnCount - 1 ] ) +
                              ( 1 - mfGamma ) * mpTrend[ mnCount - 1 ];
            pScenPerIdx[ 0 ] = mfBeta * ( pScenRange[ 0 ] - pScenBase[ 0 ] ) +
            ...
            for ( SCSIZE i = 1; i < nSize; i++ )
            {
                ...
                if ( i < mnSmplInPrd )
                    ...
                    fPerIdx = pScenPerIdx[ i - mnSmplInPrd ];
                pScenRange[ i ] = pScenBase[ i - 1 ] + pScenTrend[ i - 1 ] + fPerIdx + RandDev();
                aPredictions[ i ][ k ] = pScenRange[ i ];
                pScenBase[ i ] = mfAlpha * ( pScenRange[ i ] - fPerIdx ) +
                                 ( 1 - mfAlpha ) * ( pScenBase[ i - 1 ] + pScenTrend[ i - 1 ] );
                pScenTrend[ i ] = mfGamma * ( pScenBase[ i ] - pScenBase[ i - 1 ] ) +
                                  ( 1 - mfGamma ) * pScenTrend[ i - 1 ];
                pScenPerIdx[ i ] = mfBeta * ( pScenRange[ i ] - pScenBase[ i ] ) +
                                   ( 1 - mfBeta ) * fPerIdx;
            }
        }
        else
    ...
    double pPercentile[ nSize ];
    for ( SCSIZE i = 0; i < nSize; i++ )
    {
        pPercentile[ i ] = ScInterpreter::GetPercentile( aPredictions[ i ], ( 1 + fPILevel ) / 2 ) -
    ...
    for ( SCSIZE i = 0; i < nR; i++ )
    {
        for ( SCSIZE j = 0; j < nC; j++ )
        {
            ...
            double fPI = pPercentile[ nSteps ];
            if ( fFactor != 0.0 )
            {
                // interpolate
                double fPI1 = pPercentile[ nSteps + 1 ];
...
void ScInterpreter::ScForecast_Ets( ScETSType eETSType )
{
         ...
         eETSType != etsSeason )
    {
        ...
        if ( fmod( fVal, 1.0 ) != 0 or fVal < 0.0 )
        {
