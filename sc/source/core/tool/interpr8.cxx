...
public:
    ...
    bool PreprocessDataRange( ScMatrixRef rMatX, ScMatrixRef rMatY, int& rSamplesInPeriod,
                              bool bDataCompletion, int nAggregation, ScMatrixRef rTMat, sal_uInt16& rPreprocError  );
...
void ScETSForecastCalculation::refill()
{
    ...
    for ( SCSIZE i = 1; i < nCount; i++ )
    {
        SCSIZE nIdx = ( i >= nSamplesinPeriod ? i - nSamplesinPeriod : i );
...
bool ScETSForecastCalculation::GetForecast( double fTarget, double& rForecast )
{
    ...
    else
    {
        ...
        {
            double fInterpolateFactor = fInterpolate / fStepSize;
            double fFc_1 = ( sSmoothed[ nCount - 1 ] + ( n + 1 ) * sTrend[ nCount - 1 ] ) *
...
void ScInterpreter::ScForecast_Ets( ScETSType eETSType )
{
    ...
    // required arguments
    double fConfLevel;
    if ( eETSType == etsConfInt )
    {
        fConfLevel = GetDouble();
        if ( fConfLevel < 0 || fConfLevel > 1 )
        {
            PushIllegalParameter();
            return;
        }
    ...
    if ( nSamplesinPeriod == 0 && eETSType != etsStat )
    {
        // linear forecast
    ...
    if ( !sETSCalc.PreprocessDataRange( pMatX, pMatY, nSamplesinPeriod, bDataCompletion,
                                        nAggregation, ( eETSType != etsStat ? pTMat : 0 ), nErrorValue ) )
    {
    ...
    switch ( eETSType )
    {
        ...
        case etsConfInt :
// TODO : provide code
//            SAL_ DEBUG( "ScForecast_Ets( etsConfInt ), under construction..." );
            PushNA();
            break;
        case etsStat:
            {
