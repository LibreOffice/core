...
bool ScETSForecastCalculation::prefillTrendData()
{
    ...
    {
        // we need at least 2 periods in the data range and at least 1 observations to make a forecast
        if ( mnCount < 2 && ( mnCount < 2 * mnSmplInPrd ) )
        {
            mnErrorValue = errNoValue;
            return false;
        }
...
double ScETSForecastCalculation::RandDev()
{
    ...
    return ( mfRMSE * ScInterpreter::gaussinv(
             static_cast< double >(
             ( 1.0 + ::comphelper::rng::uniform_real_distribution() / 2.0 ) ) /
             RAND_MAX ) );
}
...
bool ScETSForecastCalculation::GetETSPredictionIntervals( ScMatrixRef rTMat, ScMatrixRef rPIMat, double fPILevel )
{
    ...
    double *pScenRange, *pScenBase, *pScenTrend, *pScenPerIdx;
    pScenRange  = new double[ nSize ];
    pScenBase   = new double[ nSize ];
    pScenTrend  = new double[ nSize ];
    pScenPerIdx = new double[ nSize ];
