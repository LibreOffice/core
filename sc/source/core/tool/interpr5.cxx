        ...
        else
        {
                ...
                nFmt1 == NUMBERFORMAT_DATETIME || nFmt2 == NUMBERFORMAT_DATETIME )
            {
                if ( nFmt1 == NUMBERFORMAT_TIME || nFmt2 == NUMBERFORMAT_TIME )
                    nFuncFmt = NUMBERFORMAT_DATETIME;   
            }
        }
    }
}
...
void ScInterpreter::CalulateRGPRKP(bool _bRKP)
{
    ...
    if (nCase==1)
    {
        ...
        pResMat->PutDouble(_bRKP ? exp(fIntercept) : fIntercept, 1, 0); 
    ...
    else 
    {
        ...
        if (nCase ==2) 
        {
            ::std::vector< double> aVecR(N); 
            
            ScMatrixRef pMeans = GetNewMat(K, 1); 
            ScMatrixRef pMatZ; 
            if (bStats)
                pMatZ = pMatY->Clone(); 
            else
                pMatZ = pMatY; 
            ScMatrixRef pSlopes = GetNewMat(1,K); 
            ...
            if (bStats)
            {
                ...
                else
                {
                    ...
                    for (SCSIZE col = 0; col < K; col++)
                    {
                        ...
                        pMatZ->FillDouble(0.0,0,0,0,K-1); 
        ...
        else  
        {
            ...
            ScMatrixRef pMeans = GetNewMat(1, K); 
            ScMatrixRef pMatZ; 
            if (bStats)
                pMatZ = pMatY->Clone(); 
            else
                pMatZ = pMatY; 
            ScMatrixRef pSlopes = GetNewMat(K,1); 
            if (!pMeans || !pMatZ || !pSlopes)
            {
                PushError(errCodeOverflow);
                return;
            ...
            if (bStats)
            {
                ...
                else
                {
                    ...
                    for (SCSIZE row = 0; row < K; row++)
                    {
                        ...
                        pMatZ->FillDouble(0.0,0,0,K-1,0); 
