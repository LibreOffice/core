...
static const SfxItemPropertyMapEntry* lcl_GetViewOptPropertyMap()
{
    static const SfxItemPropertyMapEntry aViewOptPropertyMap_Impl[] =
    {
        ...
        {OUString(SC_UNO_SHOWCHARTS),   0,  getCppuType((sal_Int16*)0),    0, 0},
        {OUString(SC_UNO_SHOWDRAW),     0,  getCppuType((sal_Int16*)0),    0, 0},
        ...
        {OUString(SC_UNO_SHOWOBJ),      0,  getCppuType((sal_Int16*)0),    0, 0},
        ...
        {OUString(SC_UNO_ZOOMTYPE),     0,  getCppuType((sal_Int16*)0),    0, 0},
        {OUString(SC_UNO_ZOOMVALUE),    0,  getCppuType((sal_Int16*)0),    0, 0},
          ...
          std::exception)
{
    ...
    else if ( pShapeImp || xShapeColl.is() )            
    {
        ScDrawView* pDrawView = pViewSh->GetScDrawView();
        if (pDrawView)
        {
...
ScViewPaneObj* ScTabViewObj::GetObjectByIndex_Impl(sal_uInt16 nIndex) const
{
    ...
    if (pViewSh)
    {
        ScSplitPos eWhich = SC_SPLIT_BOTTOMLEFT;    
                                    ...
                                    throw (uno::RuntimeException)
{
    ...
    if ((aMouseClickHandlers.size() == 0) && (nCount > 0)) 
        EndMouseListening();
}
                                    ...
                                    throw (uno::RuntimeException)
{
    ...
    if (aListener.is())
    {
        ...
        if (aActivationListeners.size() == 1 && nCount == 0) 
                                    ...
                                    throw (uno::RuntimeException)
{
    ...
    if ((aActivationListeners.size() == 0) && (nCount > 0)) 
        EndActivationListening();
}
...
sal_Int16 ScTabViewObj::GetZoom(void) const
{
    ...
    if (pViewSh)
    {
        const Fraction& rZoomY = pViewSh->GetViewData()->GetZoomY();    
...
sal_Int32 SAL_CALL ScTabViewObj::getSplitRow() throw(uno::RuntimeException, std::exception)
{
    ...
    if (pViewSh)
    {
        ...
        if ( pViewData->GetVSplitMode() != SC_SPLIT_NONE )
        {
            ...
            ScSplitPos ePos = SC_SPLIT_TOPLEFT;     
            SCsCOL nCol;
            SCsROW nRow;
    ...
    throw (uno::RuntimeException, std::exception)
{
    ...
    {
        pViewSh->GetViewData()->ReadExtOptions(*pExtOpt);        
