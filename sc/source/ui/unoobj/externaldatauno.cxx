...
void ScExternalDataProvidersObj::insertByIndex(sal_Int32 nIndex, const css::uno::Any& rAny)
{
    ...
    mpDoc->GetExternalDataMapper().insertDataSource(
        sc::ExternalDataSource(xData->getURL(), xData->getProvider(), mpDoc));
}
...
css::uno::Type ScExternalDataProvidersObj::getElementType() { return css::uno::Type(); }
...
css::uno::Any ScExternalDataProvidersObj::getByIndex(sal_Int32 nIndex)
{
    ...
    css::uno::Any aRet;
    aRet <<= css::uno::Reference<css::sheet::XExternalDataProvider>(
        new ScExternalDataProviderObj(mpDoc, nIndex));
    return aRet;
}
...
uno::Sequence<::css::sheet::ExternalDataSourceInfo>
    SAL_CALL ScExternalDataProviderObj::getDataSourcesForURL(const OUString& rURL)
{
    // TODO
...
OUString SAL_CALL ScExternalDataProviderObj::getURL() { return getDataSource().getURL(); }

OUString SAL_CALL ScExternalDataProviderObj::getProvider() { return getDataSource().getProvider(); }
...
void ScExternalDataProviderObj::addTransformation(sal_Int32 /*nIndex*/,
                                                  const css::uno::Any& rTransformation)
{
