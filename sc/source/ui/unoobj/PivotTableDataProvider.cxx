...
void PivotTableDataProvider::Notify(SfxBroadcaster& /*rBC*/, const SfxHint& rHint)
{
...
{
    std::unique_ptr<PivotTableDataSequence> pSequence(new PivotTableDataSequence(
                                                    ...
                                                    std::vector<PivotTableItem> const & rLabel)
{
    ...
    std::unique_ptr<PivotTableDataSequence> pLabelSequence(new PivotTableDataSequence(
                                                             ...
                                                             bool bOrientCol)
{
    ...
    if (bOrientCol)
    {
        for (std::vector<PivotTableItem> const & rCategories : m_aCategoriesColumnOrientation)
        {
    ...
    {
        for (std::vector<PivotTableItem> const & rCategories : m_aCategoriesRowOrientation)
        {
            uno::Reference<chart2::data::XLabeledDataSequence> xResult = newLabeledDataSequence();
            setLabeledDataSequenceValues(xResult, "categories", "Categories", rCategories);
            aLabeledSequences.push_back(xResult);
...
void PivotTableDataProvider::collectPivotTableData()
{
    ...
    for (uno::Sequence<sheet::DataResult> const & xDataResults : xDataResultsSequence)
    {
        ...
        for (sheet::DataResult const & rDataResult : xDataResults)
        {
            if (rDataResult.Flags == 0 || rDataResult.Flags & css::sheet::DataResultFlags::HASDATA)
            {
                ...
                m_aDataRowVector[nIndex].push_back(PivotTableItem(rDataResult.Flags ? rDataResult.Value : fNan, 0));
            }
            nIndex++;
        }
    }
                ...
                switch (eDimOrient)
                {
                    case sheet::DataPilotFieldOrientation_COLUMN:
                    {
                        ...
                        for (sheet::MemberResult & rMember : aSequence)
                        {
                                ...
                                rMember.Flags & sheet::MemberResultFlags::CONTINUE)
                            {
                                ...
                                m_aLabels[i][nDimPos] = PivotTableItem(sCaption);
                    ...
                    case sheet::DataPilotFieldOrientation_ROW:
                    {
                        ...
                        for (sheet::MemberResult & rMember : aSequence)
                        {
                            ...
                            {
                                std::unique_ptr<PivotTableItem> pItem;
                                ...
                                if (rtl::math::isNan(fValue))
                                {
                                    ...
                                    pItem.reset(new PivotTableItem(sStringValue));
                                }
                                else
                                {
                                    if (bHasContinueFlag)
                                        pItem.reset(new PivotTableItem());
                                    else
                                        pItem.reset(new PivotTableItem(fValue, nNumberFormat));
                                }
    ...
    {
        for (std::vector<PivotTableItem> & rDataRow : m_aDataRowVector)
        {
            size_t i = 0;
            for (PivotTableItem & rItem : rDataRow)
            {
...
void PivotTableDataProvider::assignValuesToDataSequence(
                                    ...
                                    size_t nIndex)
{
    ...
    std::vector<PivotTableItem> const & rRowOfData = m_aDataRowVector[size_t(nIndex)];
...
void PivotTableDataProvider::assignLabelsToDataSequence(
                                    uno::Reference<chart2::data::XDataSequence> & rDataSequence,
                                    size_t nIndex)
{
    ...
    OUString aLabel;
    bool bFirst = true;
    for (PivotTableItem const & rItem : m_aLabels[size_t(nIndex)])
    {
        if (bFirst)
        {
    ...
    std::vector<PivotTableItem> aLabelVector { PivotTableItem(aLabel) };
...
PivotTableDataProvider::createValuesDataSource(OUString const & rRangeRepresentation)
{
    ...
    {
        std::vector<PivotTableItem> aFirstCategories;
    ...
    {
        int i = 0;
        for (std::vector<PivotTableItem> const & rRowOfData : m_aDataRowVector)
        {
            ...
            OUString aLabel;
            bool bFirst = true;
            for (PivotTableItem const & rItem : m_aLabels[i])
            {
                if (bFirst)
                {
                    aLabel += rItem.m_aString;
...
PivotTableDataProvider::createDataSequenceOfCategories()
{
    ...
    std::vector<PivotTableItem> const & rCategories = m_aCategoriesColumnOrientation[0];
...
void SAL_CALL PivotTableDataProvider::setPropertyValue(
...
uno::Any SAL_CALL PivotTableDataProvider::getPropertyValue(
