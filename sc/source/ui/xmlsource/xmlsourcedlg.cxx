...
{
    mxLbTree->set_size_request(mxLbTree->get_approximate_digit_width() * 16,
                               mxLbTree->get_height_rows(30));
...
{
    weld::TreeView& mrTree;
public:
    explicit UnhighlightEntry(weld::TreeView& rTree) : mrTree(rTree) {}

    void operator()(const std::unique_ptr<weld::TreeIter>& rEntry)
    {
        mrTree.unselect(*rEntry);
    }
};
...
void ScXMLSourceDlg::TreeItemSelected()
{
    ...
    if (!maHighlightedEntries.empty())
    {
        ...
        std::for_each(maHighlightedEntries.begin(), maHighlightedEntries.end(), UnhighlightEntry(*mxLbTree));
        maHighlightedEntries.clear();
    }
...
void ScXMLSourceDlg::RepeatElementSelected(weld::TreeIter& rEntry)
{
    ...
    if (!mxLbTree->is_selected(rEntry))
    {
        ...
        maHighlightedEntries.emplace_back(mxLbTree->make_iterator(&rEntry));
    }
...
void ScXMLSourceDlg::SelectAllChildEntries(weld::TreeIter& rEntry)
{
    ...
    do
    {
        ...
        maHighlightedEntries.emplace_back(mxLbTree->make_iterator(xChild.get()));
    } while (mxLbTree->iter_next_sibling(*xChild));
}
...
//  mxRefEdit->SetRefValid(bValid);
    ...
    if (bRepeatElem)
    {
        if (bValid)
            ...
            maRangeLinks.erase(mxCurRefEntry); //TODO
    }
    else
    {
        if (bValid)
