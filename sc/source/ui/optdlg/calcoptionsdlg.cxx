    ...
    void addTest(const Area &rArea)
    {
        ...
        mpDoc->SetString(ScAddress(1,0,0),
                        ...
                        ScRange(ScAddress(1,1,0),
                                ScAddress(1,1+mnTestAreas-1,0)).Format(SCA_VALID|SCA_COL_VALID|SCA_ROW_VALID) +
                        ")=0,\"PASS\",\"FAIL\")");
    }
};
...
struct UnOp : Op
{
    ...
    virtual void addRow(ScDocument *pDoc, int nRow, int nTab) const override
    {
        ...
        pDoc->SetString(ScAddress(1,1+nRow,nTab),
                        "=" + msOp + "(" + ScAddress(0,1+nRow,nTab).Format(SCA_COL_VALID|SCA_ROW_VALID) + ")");
        ...
        if (mnEpsilon < 0)
        {
            ...
            pDoc->SetString(ScAddress(3,1+nRow,nTab),
                            "=IF(ABS((" + ScAddress(1,1+nRow,nTab).Format(SCA_COL_VALID|SCA_ROW_VALID) +
                            "-" + ScAddress(2,1+nRow,nTab).Format(SCA_COL_VALID|SCA_ROW_VALID) +
                            ")/" + ScAddress(2,1+nRow,nTab).Format(SCA_COL_VALID|SCA_ROW_VALID) +
    ...
    virtual OUString getSummaryFormula(ScDocument *pDoc, int nTab) const override
    {
        return "=SUM(" +
            ScRange(ScAddress(3,1,nTab),
                    ScAddress(3,1+mnRows-1,nTab)).Format(SCA_VALID|SCA_TAB_3D|SCA_COL_VALID|SCA_ROW_VALID|SCA_TAB_VALID, pDoc) +
            ")";
    }
};
...
struct BinOp : Op
{
    ...
    virtual void addRow(ScDocument *pDoc, int nRow, int nTab) const override
    {
        ...
        pDoc->SetString(ScAddress(2,1+nRow,nTab),
                        "=" + ScAddress(0,1+nRow,nTab).Format(SCA_COL_VALID|SCA_ROW_VALID) +
                        msOp + ScAddress(1,1+nRow,nTab).Format(SCA_COL_VALID|SCA_ROW_VALID));

        pDoc->SetValue(ScAddress(3,1+nRow,nTab), mpFun(nLhs, nRhs));

        pDoc->SetString(ScAddress(4,1+nRow,nTab),
                        "=IF(ABS(" + ScAddress(2,1+nRow,nTab).Format(SCA_COL_VALID|SCA_ROW_VALID) +
                        "-" + ScAddress(3,1+nRow,nTab).Format(SCA_COL_VALID|SCA_ROW_VALID) +
                        ")<=" + OUString::number(mnEpsilon) +
                        ",0,1)");
    }
    ...
    virtual OUString getSummaryFormula(ScDocument *pDoc, int nTab) const override
    {
        return "=SUM(" +
            ScRange(ScAddress(4,1,nTab),
                    ScAddress(4,1+mnRows-1,nTab)).Format(SCA_VALID|SCA_TAB_3D|SCA_COL_VALID|SCA_ROW_VALID|SCA_TAB_VALID, pDoc) +
            ")";
    }
};
...
struct Round : Area
{
    ...
    virtual void addRow(ScDocument *pDoc, int nRow, int nTab) const override
    {
        ...
        pDoc->SetString(ScAddress(2,1+nRow,nTab),
                        "=ROUND(" + ScAddress(0,1+nRow,nTab).Format(SCA_COL_VALID|SCA_ROW_VALID) +
                        "," + ScAddress(1,1+nRow,nTab).Format(SCA_COL_VALID|SCA_ROW_VALID) +
                        ")");
...
struct Normdist : Area
{
    ...
    virtual void addRow(ScDocument *pDoc, int nRow, int nTab) const override
    {
        ...
        pDoc->SetString(ScAddress(4,1+nRow,nTab),
                        "=NORMDIST(" + ScAddress(0,1+nRow,nTab).Format(SCA_COL_VALID|SCA_ROW_VALID) +
                        "," + ScAddress(1,1+nRow,nTab).Format(SCA_COL_VALID|SCA_ROW_VALID) +
                        "," + ScAddress(2,1+nRow,nTab).Format(SCA_COL_VALID|SCA_ROW_VALID) +
                        "," + ScAddress(3,1+nRow,nTab).Format(SCA_COL_VALID|SCA_ROW_VALID) +
                        ")");
        ...
        pDoc->SetString(ScAddress(6,1+nRow,nTab),
                        "=IF(ABS(" + ScAddress(4,1+nRow,nTab).Format(SCA_COL_VALID|SCA_ROW_VALID) +
                        "-" + ScAddress(5,1+nRow,nTab).Format(SCA_COL_VALID|SCA_ROW_VALID) +
                        ")<=3e-10"
                        ",0,1)");
    }
    ...
    virtual OUString getSummaryFormula(ScDocument *pDoc, int nTab) const override
    {
        return "=SUM(" +
            ScRange(ScAddress(6,1,nTab),
                    ScAddress(6,1+mnRows-1,nTab)).Format(SCA_VALID|SCA_TAB_3D|SCA_COL_VALID|SCA_ROW_VALID|SCA_TAB_VALID, pDoc) +
            ")";
    }
        ...
        if (nRow >= mnRows/2-1)
        {
            pDoc->SetString(ScAddress(1,1+nRow-mnRows/2+1,nTab),
                            ...
                            ScRange(ScAddress(0,1+nRow-mnRows/2+1,nTab),
                                    ScAddress(0,1+nRow,nTab)).Format(SCA_VALID|SCA_TAB_3D|SCA_COL_VALID|SCA_ROW_VALID) +
                            ")");
            ...
            if (mnEpsilon != 0)
                pDoc->SetString(ScAddress(3,1+nRow-mnRows/2+1,nTab),
                                "=IF(ABS(" + ScAddress(1,1+nRow-mnRows/2+1,nTab).Format(SCA_COL_VALID|SCA_ROW_VALID) +
                                "-" + ScAddress(2,1+nRow-mnRows/2+1,nTab).Format(SCA_COL_VALID|SCA_ROW_VALID) +
                                ")<=" + OUString::number(mnEpsilon) +
                                ",0,1)");
            else
                pDoc->SetString(ScAddress(3,1+nRow-mnRows/2+1,nTab),
                                "=IF(" + ScAddress(1,1+nRow-mnRows/2+1,nTab).Format(SCA_COL_VALID|SCA_ROW_VALID) +
                                "=" + ScAddress(2,1+nRow-mnRows/2+1,nTab).Format(SCA_COL_VALID|SCA_ROW_VALID) +
                                ",0,1)");
        }
    }
    ...
    virtual OUString getSummaryFormula(ScDocument *pDoc, int nTab) const override
    {
        return "=SUM(" +
            ScRange(ScAddress(3,1+0,nTab),
                    ScAddress(3,1+mnRows-mnRows/2-1,nTab)).Format(SCA_VALID|SCA_TAB_3D|SCA_COL_VALID|SCA_ROW_VALID|SCA_TAB_VALID, pDoc) +
            ")";
    }
};
