...
void ScStatisticsInputOutputDialog::GetRangeFromSelection()
{
    ...
    OUString aCurrentString(mInputRange.Format(static_cast<sal_uInt16>(ScAddr::RANGE_ABS_3D), mDocument, mAddressDetails));
...
void ScStatisticsInputOutputDialog::SetReference( const ScRange& rReferenceRange, ScDocument* pDocument )
{
    if ( mpActiveEdit )
    {
        ...
        if ( mpActiveEdit == mpInputRangeEdit )
        {
            ...
            aReferenceString = mInputRange.Format(static_cast<sal_uInt16>(ScAddr::RANGE_ABS_3D), pDocument, mAddressDetails);
        ...
        else if ( mpActiveEdit == mpOutputRangeEdit )
        {
            ...
            sal_uInt16 nFormat = ( mOutputAddress.Tab() == mCurrentAddress.Tab() ) ? static_cast<sal_uInt16>(ScAddr::ADDR_ABS) : static_cast<sal_uInt16>(ScAddr::ADDR_ABS_3D);
