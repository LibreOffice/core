...
ScRange ScFourierAnalysisDialog::ApplyOutput(ScDocShell* pDocShell)
{
    ...
    aOutput.writeBoldString(mbInverse ? OUString(ScResId(STR_INVERSE_FOURIER_TRANSFORM))
                                      : OUString(ScResId(STR_FOURIER_TRANSFORM)));
    aOutput.newLine();
    OUString aLabel;
    getDataLabel(aLabel);
...
bool ScFourierAnalysisDialog::InputRangesValid()
{
    ...
    if (nComponents > 2)
    {
        OUString aMsg = mbGroupedByColumn ? OUString(ScResId(STR_MESSAGE_INVALID_NUMCOLS))
                                          : OUString(ScResId(STR_MESSAGE_INVALID_NUMROWS));
...
void ScFourierAnalysisDialog::getDataLabel(OUString& rLabel)
{
    ...
    rLabel = ScResId(STR_INPUT_DATA_RANGE) + OUString(" : ") + aDataSrc;
    return;
}
...
void ScFourierAnalysisDialog::genFormula(OUString& rFormula)
{
    ...
    if (!mbPolar)
    {
        rFormula = OUString("FOURIER(%INPUTRANGE%;") + OUString::boolean(mbGroupedByColumn) + aSep
                   + OUString::boolean(mbInverse) + OUString(")");
        return;
    }

    rFormula = OUString("FOURIER(%INPUTRANGE%;") + OUString::boolean(mbGroupedByColumn) + aSep
               + OUString::boolean(mbInverse) + aSep + OUString("true;")
               + OUString::number(mfMinMag) + OUString(")");
}
