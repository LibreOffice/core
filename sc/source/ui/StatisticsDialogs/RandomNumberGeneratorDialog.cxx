...
void ScRandomNumberGeneratorDialog::GetRangeFromSelection()
{
    ...
    OUString aCurrentString(maInputRange.Format(static_cast<sal_uInt16>(ScAddr::RANGE_ABS_3D), mpDoc, mpDoc->GetAddressConvention()));
...
void ScRandomNumberGeneratorDialog::SetReference( const ScRange& rReferenceRange, ScDocument* pDoc )
{
    if ( mpInputRangeEdit->IsEnabled() )
    {
        ...
        OUString aReferenceString(maInputRange.Format(static_cast<sal_uInt16>(ScAddr::RANGE_ABS_3D), pDoc, pDoc->GetAddressConvention()));
