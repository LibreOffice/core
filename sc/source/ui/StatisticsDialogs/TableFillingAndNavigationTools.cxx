...
{
    sal_uInt16 nFlag = b3D ? static_cast<sal_uInt16>(ScAddr::RANGE_ABS_3D) : static_cast<sal_uInt16>(ScAddr::RANGE_ABS);
...
{
    OUString aString;
    aRangeList.Format(aString, static_cast<sal_uInt16>(ScAddr::RANGE_ABS_3D), mpDoc, mpDoc->GetAddressConvention());
    mTemplate = mTemplate.replaceAll(aVariable, aString);
}

void FormulaTemplate::applyAddress(const OUString& aVariable, const ScAddress& aAddress, bool b3D)
{
    sal_uInt16 nFlag = b3D ? static_cast<sal_uInt16>(ScAddr::ADDR_ABS_3D) : static_cast<sal_uInt16>(ScAddr::ADDR_ABS);
