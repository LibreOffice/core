...
OUString ScRegressionDialog::GetVariableNameFormula(bool bXVar, size_t nIndex, bool bWithLog)
{
    ...
    OUString aDefaultVarName(bXVar ? "X" : "Y");

    if (bXVar)
        aDefaultVarName += OUString::number(nIndex);
    ...
    {
        {
            ...
            OUString("=CONCAT(\"") + ScResId(STR_LABEL_LOWER) +
            OUString(" \" ; INT(%CONFIDENCE_LEVEL_ADDR% * 100) ; \"%\")"),

            OUString("=CONCAT(\"") + ScResId(STR_LABEL_UPPER) +
            OUString(" \" ; INT(%CONFIDENCE_LEVEL_ADDR% * 100) ; \"%\")"),
        },
        ...
        // Following are matrix formulas of size numcols = 1, numrows = (mnNumIndependentVars + 1)
        {
            ...
            OUString("=INDEX(%COEFFICIENTS_REV_RANGE%; 1 ; ") + OUString::number(nLastRow + 2) + OUString(" - ROW())"),
            // This puts the standard errors in the reverse order compared to that in LINEST output.
            OUString("=INDEX(%SERRORSX_REV_RANGE%; 1 ; ") + OUString::number(nLastRow + 2) + OUString(" - ROW())"),
            // t-Statistic
