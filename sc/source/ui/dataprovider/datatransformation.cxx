...
Date getDate(double nDateTime, SvNumberFormatter* pFormatter)
{
    ...
    aDate.AddDays(static_cast<sal_Int32>(::rtl::math::approxFloor(nDateTime)) + 1);
    return aDate;
}
}
        ...
        switch (maType)
        {
            ...
            case DATETIME_TRANSFORMATION_TYPE::HOUR:
            {
                for (SCROW nRow = 0; nRow <= nEndRow; ++nRow)
                {
                    ...
                    {
                        double nVal = rDoc.GetValue(rCol, nRow, 0);
                        nVal -= ::rtl::math::approxFloor(nVal);
                        nVal = static_cast<long>(::rtl::math::approxFloor(nVal*DATE_TIME_FACTOR)) / ::tools::Time::secondPerHour;
                        rDoc.SetValue(rCol, nRow, 0, nVal);
                    }
                }
            }
            ...
            case DATETIME_TRANSFORMATION_TYPE::MINUTE:
            {
                for (SCROW nRow = 0; nRow <= nEndRow; ++nRow)
                {
                    ...
                    if (eType == CELLTYPE_VALUE)
                    {
                        double nVal = rDoc.GetValue(rCol, nRow, 0);
                        nVal -= ::rtl::math::approxFloor(nVal);
                        long nMin = static_cast<long>(::rtl::math::approxFloor(nVal*DATE_TIME_FACTOR)) % ::tools::Time::secondPerHour;
                        nMin = static_cast<long>(nMin / ::tools::Time::secondPerMinute);
                        rDoc.SetValue(rCol, nRow, 0, nMin);
                    }
                }
            }
            break;
            case DATETIME_TRANSFORMATION_TYPE::SECOND:
            {
                for (SCROW nRow = 0; nRow <= nEndRow; ++nRow)
                {
                    ...
                    if (eType == CELLTYPE_VALUE)
                    {
                        double nVal = rDoc.GetValue(rCol, nRow, 0);
                        nVal -= ::rtl::math::approxFloor(nVal);
                        long nSec = static_cast<long>(::rtl::math::approxFloor(nVal*DATE_TIME_FACTOR)) % ::tools::Time::secondPerMinute;
                        rDoc.SetValue(rCol, nRow, 0, nSec);
                    }
                }
            }
            break;
