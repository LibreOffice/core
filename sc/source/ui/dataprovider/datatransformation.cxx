...
DateTime getDateTime(double nDateTime)
{
    sal_Int32 nDays = std::trunc(nDateTime);
    DateTime aDate(DateTime::DateTimeInitEmpty::EMPTY);
  //  aDate.Year = 0;
  //  aDate.GetMonth() = 0;
   // aDate.GetDay() = 0;
   // aDate.Seconds = getSecond(nDateTime);
    //aDate.Minutes = getMinute(nDateTime);
    //aDate.Hours = getHour(nDateTime);
    // Add number of days
    aDate.AddDays(nDays);
    //aDate = mktime(aDate);
    return aDate;
        ...
        switch (maType)
        {
            case DATETIME_TRANSFORMATION_TYPE::DATE_STRING:
            {
                for (SCROW nRow = 0; nRow <= nEndRow; ++nRow)
                {
                    ...
                    {
                        double nVal = rDoc.GetValue(rCol, nRow, 0);
                        DateTime aDate = getDateTime(nVal);
            ...
            case DATETIME_TRANSFORMATION_TYPE::YEAR:
            {
                for (SCROW nRow = 0; nRow <= nEndRow; ++nRow)
                {
                    ...
                    if (eType == CELLTYPE_VALUE)
                    {
                        double nVal = rDoc.GetValue(rCol, nRow, 0);
                        DateTime aDate = getDateTime(nVal);
                        rDoc.SetValue(rCol, nRow, 0, 1900 + aDate.GetYear());
                    }
                }
            }
            break;
            case DATETIME_TRANSFORMATION_TYPE::START_OF_YEAR:
            {
                for (SCROW nRow = 0; nRow <= nEndRow; ++nRow)
                {
                    ...
                    if (eType == CELLTYPE_VALUE)
                    {
                        double nVal = rDoc.GetValue(rCol, nRow, 0);
                        DateTime aDate = getDateTime(nVal);
                        rDoc.SetString(rCol, nRow, 0, OUString("01-01-").concat(OUString::number(1900 + aDate.GetYear())));
                    }
                }
            }
            break;
            case DATETIME_TRANSFORMATION_TYPE::END_OF_YEAR:
            {
                for (SCROW nRow = 0; nRow <= nEndRow; ++nRow)
                {
                    ...
                    if (eType == CELLTYPE_VALUE)
                    {
                        double nVal = rDoc.GetValue(rCol, nRow, 0);
                        DateTime aDate = getDateTime(nVal);
                        rDoc.SetString(rCol, nRow, 0, OUString("31-12-").concat(OUString::number(1900 + aDate.GetYear())));
                    }
                }
            }
            break;
            case DATETIME_TRANSFORMATION_TYPE::MONTH:
            {
                for (SCROW nRow = 0; nRow <= nEndRow; ++nRow)
                {
                    ...
                    if (eType == CELLTYPE_VALUE)
                    {
                        double nVal = rDoc.GetValue(rCol, nRow, 0);
                        DateTime aDate = getDateTime(nVal);
                        rDoc.SetValue(rCol, nRow, 0, 1 + aDate.GetMonth());
                    }
                }
            }
            break;
            ...
            case DATETIME_TRANSFORMATION_TYPE::START_OF_MONTH:
            {
                for (SCROW nRow = 0; nRow <= nEndRow; ++nRow)
                {
                    ...
                    if (eType == CELLTYPE_VALUE)
                    {
                        ...
                        rDoc.SetString(rCol, nRow, 0, OUString("01-").concat(sMonth).concat("-").concat(OUString::number(1900 + aDate.GetYear())));
                    }
                }
            }
            break;
                ...
                for (SCROW nRow = 0; nRow <= nEndRow; ++nRow)
                {
                    ...
                    if (eType == CELLTYPE_VALUE)
                    {
                        ...
                        if(nMonth == 4 || nMonth == 6 || nMonth == 9 || nMonth == 11)
                            rDoc.SetString(rCol, nRow, 0, OUString("30-").concat(sMonth).concat("-").concat(OUString::number(1900 + aDate.GetYear())));

                        else if(nMonth == 1 || nMonth == 3 || nMonth == 5 || nMonth == 7 || nMonth == 8 || nMonth == 10 || nMonth ==12)
                           rDoc.SetString(rCol, nRow, 0, OUString("31-").concat(sMonth).concat("-").concat(OUString::number(1900 + aDate.GetYear())));

                        else if(nMonth == 2 && aDate.GetYear() % 4 != 0)
                            rDoc.SetString(rCol, nRow, 0, OUString("28-").concat(sMonth).concat("-").concat(OUString::number(1900 + aDate.GetYear())));

                        else if(nMonth == 2 && aDate.GetYear() % 4 == 0)
                            rDoc.SetString(rCol, nRow, 0, OUString("29-").concat(sMonth).concat("-").concat(OUString::number(1900 + aDate.GetYear())));
                ...
                for (SCROW nRow = 0; nRow <= nEndRow; ++nRow)
                {
                    ...
                    if (eType == CELLTYPE_VALUE)
                    {
                        ...
                        if(nMonth >= 1 && nMonth <=3)
                           ...
                           rDoc.SetString(rCol, nRow, 0, OUString("01-04-").concat(OUString::number(1900 + aDate.GetYear())));

                        else if(nMonth >= 7 && nMonth <=9)
                            rDoc.SetString(rCol, nRow, 0, OUString("01-07-").concat(OUString::number(1900 + aDate.GetYear())));

                        else if(nMonth >= 10 && nMonth <=12)
                            rDoc.SetString(rCol, nRow, 0, OUString("01-10-").concat(OUString::number(1900 + aDate.GetYear())));
                        else
                            rDoc.SetValue(rCol, nRow, 0, -1);
            ...
            case DATETIME_TRANSFORMATION_TYPE::END_OF_QUARTER:
            {
                for (SCROW nRow = 0; nRow <= nEndRow; ++nRow)
                {
                    ...
                    if (eType == CELLTYPE_VALUE)
                    {
                        ...
                        if(nMonth >= 1 && nMonth <=3)
                            rDoc.SetString(rCol, nRow, 0, OUString("31-03-").concat(OUString::number(1900 + aDate.GetYear())));

                        else if(nMonth >= 4 && nMonth <=6)
                           rDoc.SetString(rCol, nRow, 0, OUString("30-06-").concat(OUString::number(1900 + aDate.GetYear())));

                        else if(nMonth >= 7 && nMonth <=9)
                            rDoc.SetString(rCol, nRow, 0, OUString("30-09-").concat(OUString::number(1900 + aDate.GetYear())));
