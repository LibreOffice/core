...
void ScPivotLayoutDialog::SetupSource()
{
    ...
    if (maPivotTableObject.GetSheetDesc())
    {
        ...
        {
            OUString aSourceRangeName = aSourceRange.Format(static_cast<sal_uInt16>(ScAddr::RANGE_ABS_3D), mpDocument, maAddressDetails);
...
void ScPivotLayoutDialog::SetupDestination()
{
    ...
    else
    {
        if (maPivotParameters.nTab != MAXTAB + 1)
        {
            ...
            OUString aAddressString = aAddress.Format(static_cast<sal_uInt16>(ScAddr::ADDR_ABS_3D), mpDocument, maAddressDetails);
...
void ScPivotLayoutDialog::SetReference(const ScRange& rReferenceRange, ScDocument* pDocument)
{
    ...
    OUString aReferenceString = rReferenceRange.Format(static_cast<sal_uInt16>(ScAddr::RANGE_ABS_3D), pDocument, maAddressDetails);
...
void ScPivotLayoutDialog::UpdateSourceRange()
{
    ...
    else if (mpSourceRadioSelection->IsChecked())
    {
        ...
        bool bIsValid = (nResult & static_cast<sal_uInt16>(ScAddr::VALID)) == static_cast<sal_uInt16>(ScAddr::VALID); // aSourceString is valid
