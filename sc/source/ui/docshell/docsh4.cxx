        ...
        case SID_CHART_ADDSOURCE:
            if (pReqArgs)
            {
                ...
                if (!bValid)
                {
                    ...
                    {
                        bMultiRange = true;
                        aSingleRange = *aRangeListRef->front(); 
                        bValid = true;
                    }
                    else
                        aRangeListRef.Clear();
        ...
        case FID_CHG_RECORD:
            {
                ...
                if(pDoc!=NULL)
                {
                    ...
                    sal_Bool bActivateTracking = (pChangeTrack == 0);   
                    if ( pItem )
                        bActivateTracking = pItem->GetValue();      
                ...
                if ( pMed )     
                {
                    ...
                    if ( !pOtherDocSh->GetError() )                 
                    {
                        ...
                        if ( !IsDocShared() )
                        {
                            ...
                            if ( pViewFrm )
                            {
                                pViewFrm->ShowChildWindow( ScAcceptChgDlgWrapper::GetChildWindowId(), sal_True ); 
                            }
                            if ( pBindings )
                            {
...
void ScDocShell::DoHardRecalc( bool /* bApi */ )
{
    ...
    SCTAB nTabCount = aDocument.GetTableCount();
    SCTAB nTab;
    if (aDocument.HasAnySheetEventScript( SC_SHEETEVENT_CALCULATE, true )) 
    ...
    if ( pStyle->GetFamily() == SFX_STYLE_FAMILY_PAGE )
    {
        if ( nId == SFX_STYLESHEET_MODIFIED )
        {
            ...
            for (SCTAB nTab=0; nTab<nTabCount; nTab++)
                if (aDocument.GetPageStyle(nTab) == aNewName)   
                {
...
void ScDocShell::PageStyleModified( const OUString& rStyleName, bool bApi )
{
    ...
    if (ValidTab(nUseTab))      
    {
        ScPrintFunc aPrintFunc( this, GetPrinter(), nUseTab );  
        if (!aPrintFunc.UpdatePages())                          
            bWarn = sal_True;
...
void ScDocShell::Draw( OutputDevice* pDev, const JobSetup & /* rSetup */, sal_uInt16 nAspect )
{
    ...
    pDev->SetLayoutMode( TEXT_LAYOUT_DEFAULT );     
...
void ScDocShell::SnapVisArea( Rectangle& rRect ) const
{
    ...
    if ( bNegativePage )
        ScDrawLayer::MirrorRectRTL( rRect );        
