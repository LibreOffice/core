    ...
    mpDBCollection (pDocShell->GetDocument().GetDBCollection())
{
    ...
    if(!rNamedDBS.insert (aDBData))
        bSuccess = false;
    mpDBDataManager = std::unique_ptr<ScDBDataManager> (new ScDBDataManager(aDBData, bAllowResize));
...
bool ScDBDataManager::RequiresResize(sal_uInt32& RowDifference, sal_uInt32& ColDifference)
{
    sal_uInt32 nTotalSourceRows = maSourceRange.aStart.Row() - maSourceRange.aEnd.Row();
    sal_uInt32 nTotalSourceCols = maSourceRange.aStart.Col() - maSourceRange.aEnd.Col();

    sal_uInt32 nTotalDestinationRows = maDestinationRange.aStart.Row() - maDestinationRange.aEnd.Row();
    sal_uInt32 nTotalDestinationCols = maDestinationRange.aStart.Col() - maDestinationRange.aEnd.Col();
...
{
    sal_uInt32 RowDifference =0, ColDifference = 0;
