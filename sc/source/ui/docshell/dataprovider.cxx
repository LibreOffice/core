    ...
    mpDBCollection (pDocShell->GetDocument().GetDBCollection())
{
    ...
    if(!rNamedDBS.insert (aDBData))
        bSuccess = false;
    mpDBDataManager = new ScDBDataManager(aDBData, bAllowResize);
    mpDBDataManager->SetDestinationRange(maRange);

    mpDataProvider = std::unique_ptr<DataProvider> (new CSVDataProvider(mpDocShell, rURL, maRange, mpDBDataManager));
}

ExternalDataMapper::~ExternalDataMapper()
{
    delete  mpDBDataManager;
}
