...
ExternalDataMapper::ExternalDataMapper(ScDocShell* pDocShell, const OUString& rURL, const OUString& rName, SCTAB nTab,
    ...
    mpDataProvider (new CSVDataProvider::CSVDataProvider(mpDocShell, rURL, maRange)),
...
CSVFetchThread::CSVFetchThread(OUString& mrURL, size_t nColCount):
        Thread("ReaderThread"),
        mpStream(nullptr),
        maURL (mrURL),
    ...
    mbImportUnderway(false)
{
    ...
    maImportTimer.SetTimeoutHdl( LINK(this, CSVDataProvider, ImportTimerHdl) );
...
void CSVDataProvider::StartImport()
{
    ...
    {
        mxCSVFetchThread = new CSVFetchThread(maURL, mrRange.aEnd.Col() - mrRange.aStart.Col() + 1);
        maImportTimer.Start();
...
void CSVDataProvider::WriteToDoc()
{
    mnCurRow = mrRange.aStart.Row();
    ...
    do
    {
        ...
        for (auto& rCell : aLine.maCells)
        {
            if (rCell.mbValue)
            {
                mpDocument->SetValue(nCol, mnCurRow, mrRange.aStart.Tab(), rCell.mfValue);
            }
            else
            {
                mpDocument->SetString(nCol, mnCurRow, mrRange.aStart.Tab(), OUString(pLineHead+rCell.maStr.Pos, rCell.maStr.Size, RTL_TEXTENCODING_UTF8));
            }
            ++nCol;
        }
        ++mnCurRow;
