...
public:
    ...
    bool query(SCCOL nCol, SCROW nRow, const ScRefCellValue& rCell, MisspellType*& rpRanges) const
    {
        ...
        //SAL_DEEBUG("SpellCheckCache::query : nCol = " << nCol << " nRow = " << nRow << " : string-map-size = " << maStringMisspells.size() << " : edit-map-size = " << maEditTextMisspells.size() << " : lang-map-size = " << maCellLanguages.size());
        if (eType == CELLTYPE_STRING)
        {
            //SAL_DEEBUG("\ttype = STRING");
            ...
            //SAL_DEEBUG("\tHit! : " << rpRanges);
            return true;
        }

        if (eType == CELLTYPE_EDIT)
        {
            //SAL_DEEBUG("\ttype = EDIT");
    ...
    {
        //SAL_DEEBUG("SpellCheckCache::clear()");
    ...
    {
        //SAL_DEEBUG("SpellCheckCache::clearEditTextMap()");
        maEditTextMisspells.clear();
    }
    ...
    meLanguage(ScGlobal::GetEditDefaultLanguage())
{
    ...
    //SAL_DEEBUG("SpellCheckContext construct : meLanguage = " << LanguageTag::convertToBcp47(meLanguage));
}
...
void SpellCheckContext::ensureResults(SCCOL nCol, SCROW nRow)
{
    ...
    {
        //SAL_DEEBUG("ensureResults : Detected language change from " << LanguageTag::convertToBcp47(meLanguage) << " to " << LanguageTag::convertToBcp47(ScGlobal::GetEditDefaultLanguage()) << " :  resetting engine/cache.");
        reset();
    }

    //SAL_DEEBUG("SpellCheckCache::ensureResults : nCol = " << nCol << " nRow = " << nRow);
    ...
    //SAL_DEEBUG("\tmeLanguage = " << LanguageTag::convertToBcp47(meLanguage) << " cell-lang = " << LanguageTag::convertToBcp47(eCellLang));
    ...
    {
        //SAL_DEEBUG("SpellCheckCache::ensureResults : cell-lang change from " << LanguageTag::convertToBcp47(eCachedCellLang) << " to " << LanguageTag::convertToBcp47(eCellLang));
