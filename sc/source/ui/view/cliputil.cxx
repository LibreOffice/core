...
void ScClipUtil::PasteFromClipboard( ScViewData* pViewData, ScTabViewShell* pTabViewShell, bool bShowDialog )
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (rClipParam.isMultiRange() && rClipParam.maMatrixRanges.empty())
            ...
            if (!rClipParam.maOriginMatrixRanges.empty())
            {
                ...
                formula::FormulaGrammar::Grammar eGram = pThisDoc->GetGrammar();
                ...
                for (const auto& rInfo : aOriginInfos)
                {
                    ...
                    for (SCROW r = rDest.aStart.Row(); r <= rDest.aEnd.Row(); ++r)
                    {
                        ...
                        {
                            ScRange aExisting;
                            if (pThisDoc->GetMatrixFormulaRange(ScAddress(c, r, nThisTab), aExisting))
                            {
                                if (rDest.In(aExisting))
                                {
                                    if (!aMatricesToClear.In(aExisting))
                    ...
                    && SC_MOD()->GetInputOptions().GetReplaceCellsWarn())
                {
                    ...
                    bIsEmpty = pThisDoc->IsBlockEmpty( nThisTab,
                            nThisCol, nThisRow, nOrigEndCol, nOrigEndRow );
                    ...
                    for (size_t i = 0; i < aOriginInfos.size() && bIsEmpty; ++i)
                    {
                        ...
                        if (bIsEmpty && rDest.aEnd.Row() > nOrigEndRow)
                        {
                            ...
                            bIsEmpty = pThisDoc->IsBlockEmpty( nThisTab,
                                    ...
                                    nColEnd, rDest.aEnd.Row() );
                        }
                    }

                    if (!bIsEmpty)
                    {
                        ScReplaceWarnBox aBox(pViewData->GetDialogParent());
                        if (aBox.run() != RET_YES)
                            return;
                    }
                }
                ...
                for (size_t i = 0; i < aMatricesToClear.size(); ++i)
                {
                    ...
                    ScMarkData aClearMark(pThisDoc->MaxRow(), pThisDoc->MaxCol());
                ...
                for (const auto& rInfo : aOriginInfos)
                {
                    ...
                    const ScRange& rDest = rInfo.maDestRange;
                    ScMarkData aMark(pThisDoc->MaxRow(), pThisDoc->MaxCol());
                ...
                pViewData->GetMarkData().SetMarkArea(aDestRange);
