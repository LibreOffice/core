...
void ScPreview::CalcPages()
{
    ...
    for (SCTAB i=nStart; i<nTabCount; i++)
    {
        ...
        nFirstAttr[i] = aPrintFunc.GetFirstPageNo();    
...
void ScPreview::DoPrint( ScPreviewLocationData* pFillLocation )
{
    ...
    if ( bValidPage )
    {
        ...
        if ( bDoPrint && bPageMargin && pLocationData )     
        {
            Rectangle aPixRect;
    ...
    if ( bDoPrint )
    {
        ...
        if (bRight || bBottom)
        {
            ...
            {
                if (bRight)
                    DrawRect(Rectangle(0,nPageEndY, nPageEndX,aWinEnd.Y()));    
                else
