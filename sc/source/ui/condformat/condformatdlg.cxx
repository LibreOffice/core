...
IMPL_LINK_NOARG(ScCondFormatList, UpBtnHdl, Button*, void)
{
    ...
    for (size_t i = 0; i < maEntries.size(); i++)
    {
        ...
        if (widget->IsSelected())
        {
            if (i > 0)
            {
                auto curr = maEntries[i];
                maEntries[i] = maEntries[i - 1];
                maEntries[i - 1] = curr;
                index = i - 1;
                break;
            }
...
void ScCondFormatDlg::OnSelectionChange(size_t index, size_t size, bool selected)
{
    if (size <= 1 || !selected)
    {
        ...
        mpBtnUp->Enable(index != 0);
        mpBtnDown->Enable(index < size - 1);
    }
}
