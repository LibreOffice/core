...
ScCondFrmtEntry::ScCondFrmtEntry(ScCondFormatList* pParent, ScDocument* pDoc, const ScAddress& rPos)
    ...
    , mxEventBox(mxBuilder->weld_widget("eventbox"))
    ...
    , maPos(rPos)
{
    ...
    mxFtCondNr->SetControlBackground(aBack);
    mxFtCondNr->SetBackground(mxFtCondNr->GetControlBackground());

    mxFtCondition->SetControlBackground(aBack);
    mxFtCondition->SetBackground(mxFtCondition->GetControlBackground());
#endif

    mxLbType->connect_changed(LINK(pParent, ScCondFormatList, TypeListHdl));
    mxEventBox->connect_mouse_press(LINK(this, ScCondFrmtEntry, EntrySelectHdl));
...
{
#if 0
    const long nMaxHeight = mxGrid->get_preferred_size().Height();
    if (nMaxHeight > 0)
    {
...
void StyleSelect( weld::ComboBox& rLbStyle, const ScDocument* pDoc, FontPrevWindow& rWdPreview )
{
...
IMPL_LINK_NOARG(ScConditionFrmtEntry, StyleSelectHdl, weld::ComboBox&, void)
{
    ...
    StyleSelect( *mxLbStyle, mpDoc, maWdPreview );
    mbIsInStyleCreate = false;
}
