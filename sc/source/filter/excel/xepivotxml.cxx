...
void XclExpXmlPivotTables::SavePivotTableXml( XclExpXmlStream& rStrm, const ScDPObject& rDPObj, sal_Int32 nCacheId )
{
    ...
    for (size_t i = 0; i < nFieldCount; ++i)
    {
        ...
        for (size_t i = 0; i < rCacheFieldItems.size(); ++i)
        {
            if (aUsedCachePositions.find(i) == aUsedCachePositions.end())
                aMemberSequence.push_back(std::make_pair(i, true));
        }
        ...
        for (auto nMember : aMemberSequence)
        {
