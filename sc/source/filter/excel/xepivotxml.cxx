...
void XclExpXmlPivotCaches::SavePivotCacheXml( XclExpXmlStream& rStrm, const Entry& rEntry, sal_Int32 nCounter )
{
    ...
    for (size_t i = 0; i < nCount; ++i)
    {
        ...
        ScDPCache::ScDPItemDataVec::const_iterator iMin = rFieldItems.end(), iMax = rFieldItems.end();
        ...
        if (isContainsDate)
        {
            pAttList->add(XML_minDate, XclXmlUtils::ToOString(rCache.GetISO8601FormattedDate(i, *iMin)));
            pAttList->add(XML_maxDate,XclXmlUtils::ToOString(rCache.GetISO8601FormattedDate(i, *iMax)));
        }
        ...
        // Even if field contain mixed types (Date and Nuumbers), MS Excel is saving only "minDate" and "maxDate" and not "minValue" and "maxValue"
        // Example:
                    ...
                    case ScDPItemData::Value:
                        if (isContainsDate)
                        {
                            ...
                            pDefStrm->singleElement(XML_d,
                                XML_v, XclXmlUtils::ToOString(rCache.GetISO8601FormattedDate(i, rItem)),
                                FSEND);
                        }
                        else
