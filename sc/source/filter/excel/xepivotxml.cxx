...
void XclExpXmlPivotCaches::SavePivotCacheXml( XclExpXmlStream& rStrm, const Entry& rEntry, sal_Int32 nCounter )
{
    ...
    for (size_t i = 0; i < nCount; ++i)
    {
        ...
        if (isContainsDate)
        {
            pAttList->add(XML_minDate, XclXmlUtils::ToOString(rCache.GetISO8601FormattedDate(fMin)));
            pAttList->add(XML_maxDate, XclXmlUtils::ToOString(rCache.GetISO8601FormattedDate(fMax)));
        }
                    ...
                    case ScDPItemData::Value:
                        if (isContainsDate)
                        {
                            ...
                            pDefStrm->singleElement(XML_d,
                                XML_v, XclXmlUtils::ToOString(rCache.GetISO8601FormattedDate(rItem.GetValue())),
                                FSEND);
                        }
                        else
