...
OString lclGetSubtotalFuncName(ScGeneralFunction eFunc)
{
    switch (eFunc)
    {
...
sal_Int32 lclGetSubtotalAttrToken(ScGeneralFunction eFunc)
{
    switch (eFunc)
    {
...
void XclExpXmlPivotTables::SavePivotTableXml( XclExpXmlStream& rStrm, const ScDPObject& rDPObj, sal_Int32 nCacheId )
{
    ...
    for (size_t i = 0; i < nFieldCount; ++i)
    {
        ...
        for (auto& sMemberName : aMemberNames)
        {
        ...
        for (long nSubTotal = 0; nSubTotal < nSubTotalCount; ++nSubTotal)
        {
            ...
            aSubtotalSequence.push_back(lclGetSubtotalFuncName(eFunc));
            sal_Int32 nAttToken = lclGetSubtotalAttrToken(eFunc);
        ...
        for (OString& sSubtotal : aSubtotalSequence)
        {
            pPivotStrm->singleElement(XML_item,
                XML_t, sSubtotal,
                FSEND);
