...
void XclExpXmlPivotCaches::SavePivotCacheXml( XclExpXmlStream& rStrm, const Entry& rEntry, sal_Int32 nCounter )
{
    ...
    for (size_t i = 0; i < nCount; ++i)
    {
        ...
        pAttList->add(XML_containsBlank, ToPsz10(aDPTypes.find(ScDPItemData::Empty) != aDPTypeEnd));

        std::set<ScDPItemData::Type> aDPTypesWithoutBlank = aDPTypes.erase(ScDPItemData::Empty);
        ...
        pAttList->add(XML_containsMixedTypes, ToPsz10(aDPTypesWithoutBlank.size() > 1));
        ...
        pAttList->add(XML_containsSemiMixedTypes, ToPsz10((aDPTypes.find(ScDPItemData::String) != aDPTypeEnd) && (aDPTypes.size() > 1)));
        pAttList->add(XML_containsString, ToPsz10(aDPTypes.find(ScDPItemData::String) != aDPTypeEnd));
        pAttList->add(XML_containsNumber, XclXmlUtils::ToPsz10(aDPTypes.find(ScDPItemData::Value) != aDPTypeEnd));
