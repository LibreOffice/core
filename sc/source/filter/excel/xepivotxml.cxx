...
void XclExpXmlPivotCaches::SavePivotCacheXml( XclExpXmlStream& rStrm, const Entry& rEntry, sal_Int32 nCounter )
{
    ...
    auto WriteFieldGroup = [this, &rCache, pDefStrm](size_t i, size_t base) {
        ...
        if (nDatePart == sheet::DataPilotFieldGroupBy::YEARS && pGI->mbAutoStart)
            pGroupAttList->add(XML_autoStart, ToPsz10(true));
        else
            pGroupAttList->add(XML_startDate,
                               GetExcelFormattedDate(pGI->mfStart, rFormatter).toUtf8());
        if (nDatePart == sheet::DataPilotFieldGroupBy::YEARS && pGI->mbAutoEnd)
...
void XclExpXmlPivotTables::SavePivotTableXml( XclExpXmlStream& rStrm, const ScDPObject& rDPObj, sal_Int32 nCacheId )
{
    ...
    for (size_t i = 0; i < nFieldCount; ++i)
    {
        ...
        const bool bNonGroupField = i < rCache.GetFieldCount() && !rCache.GetGroupType(i);
        if (bNonGroupField)
        ...
        for (const auto & rMember : aMembers)
        {
            auto it = std::find_if(rCacheFieldItems.begin(), rCacheFieldItems.end(),
                [&rDPObj, &pDim, &rMember, &rCache, i, bNonGroupField](const ScDPItemData* pItem) {
