...
bool check_order(const std::vector<int>& items, const std::vector<size_t>& order)
{
    ...
    std::vector<size_t>::const_iterator it = order.begin();
...
bool dump_and_check(const field& fld, const std::vector<int>& original, bool dump_values)
{
    ...
    if (dump_values)
    {
        ...
        {
            std::vector<size_t>::const_iterator it = fld.order.begin(), it_end = fld.order.end();
            for (; it != it_end; ++it)
            {
...
void run1(const std::vector<int>& vals, bool dump_values)
{
    field fld;
    {
        ...
        std::vector<int>::const_iterator it = vals.begin(), it_end = vals.end();
        for (; it != it_end; ++it)
        {
            long index = 0;
