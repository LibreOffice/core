...
the end. The jump table points to the `ocLambda`, each `ocSep`, and the
`ocClose`, which allows the interpreter to find each parameter name (the
`ocLambda` and `ocSep`s except the last), the body (the last `ocSep`), and the
position where execution continues (`ocClose`).

* When interpreted, `ocLambda` causes a `ScFormulaFunction` to be built using
the contents of the jump table. At this point all parameters are counted, and
that information is passed on to the next step.
