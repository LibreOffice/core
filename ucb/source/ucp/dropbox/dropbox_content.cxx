...
#include "dropbox_provider.hxx"
#include "dropbox_datasupplier.hxx"
#include "dropbox_resultset.hxx"
...
Content::Content( const uno::Reference< uno::XComponentContext >& rxContext,
                  ContentProvider* pProvider,
                  const uno::Reference< ucb::XContentIdentifier >& Identifier,
                  const DropboxFileInfo& rFileInfo )
: ContentImplHelper( rxContext, pProvider, Identifier ),
  m_pProvider( pProvider ),
  ...
  m_bTransient( false )
{
    SAL_WARN("ucb.ucp.dropbox", "Content constructor with FileInfo - name: '" + m_aFileInfo.name +
...
{
    return u"com.sun.star.comp.ucb.DropboxContent"_ustr;
}

uno::Sequence< OUString > SAL_CALL Content::getSupportedServiceNames()
{
    return { DROPBOX_CONTENT_SERVICE_NAME };
}
...
{
    if ( isFolder( uno::Reference< ucb::XCommandEnvironment >() ) )
        return DROPBOX_FOLDER_TYPE;
    else
        return DROPBOX_FILE_TYPE;
}
...
{
    SAL_WARN("ucb.ucp.dropbox", "Content::execute called - Command: " + aCommand.Name +
    ...
    else if ( aCommand.Name == "open" )
    {
        ...
        SAL_WARN("ucb.ucp.dropbox", "Content::execute - open command, Environment available: " +
             ...
             ( aOpenCommand.Mode == ucb::OpenMode::DOCUMENTS ) )
        {
            ...
            catch ( const uno::Exception& e )
            {
                ...
                SAL_WARN("ucb.ucp.dropbox", "Failed to create dynamic result set: " + e.Message);
        ...
        else if ( aOpenCommand.Mode == ucb::OpenMode::DOCUMENT )
        {
            ...
            SAL_WARN("ucb.ucp.dropbox", "Open command - Sink provided: " + OUString::boolean(aOpenCommand.Sink.is()));
            if ( aOpenCommand.Sink.is() )
            {
                SAL_WARN("ucb.ucp.dropbox", "Opening document with file ID: " + m_aFileInfo.id);
                ...
                SAL_WARN("ucb.ucp.dropbox", "Sink types - XOutputStream: " + OUString::boolean(xOut.is()) +
                ...
                if (xDataStreamer.is()) {
                    ...
                    SAL_WARN("ucb.ucp.dropbox", "XActiveDataStreamer.getStream() available: " + OUString::boolean(xStream.is()));
                    if (xStream.is()) {
                        auto xOutputStream = xStream->getOutputStream();
                        SAL_WARN("ucb.ucp.dropbox", "Stream.getOutputStream() available: " + OUString::boolean(xOutputStream.is()));
                    }
                }
                ...
                {
                    SAL_WARN("ucb.ucp.dropbox", "No valid data sink interface available");
                    ucbhelper::cancelCommandExecution(
                ...
                {
                    SAL_WARN("ucb.ucp.dropbox", "Creating pipe stream for XActiveDataStreamer");
                    try {
                        ...
                        SAL_WARN("ucb.ucp.dropbox", "Pipe interfaces - XInputStream: " + OUString::boolean(xPipeIn.is()) +
                        ...
                        if (xPipeIn.is() && xPipeOut.is()) {
                            ...
                            SAL_WARN("ucb.ucp.dropbox", "Using pipe output stream for XActiveDataStreamer");
                            ...
                            if (xSinkInterface.is()) {
                                SAL_WARN("ucb.ucp.dropbox", "XActiveDataStreamer also implements XActiveDataSink, using that interface");
                                xDataSink = xSinkInterface;
                                xOut.clear(); // Will use XActiveDataSink path instead
                            }
                        } else {
                            SAL_WARN("ucb.ucp.dropbox", "Failed to get required pipe interfaces - XInputStream: " +
                                     OUString::boolean(xPipeIn.is()) + ", XOutputStream: " + OUString::boolean(xPipeOut.is()));
                        }
                    } catch (const uno::Exception& e) {
                        SAL_WARN("ucb.ucp.dropbox", "Exception creating pipe stream: " + e.Message);
                    }
                }
                ...
                if ( xDataSink.is() || xOut.is() )
                {
                    try {
                        if (!m_pApiClient) {
                            SAL_WARN("ucb.ucp.dropbox", "No API client available for download");
                            ucbhelper::cancelCommandExecution(
                                uno::Any( ucb::IOErrorCode_NOT_EXISTING ),
                                Environment );
                        }

                        SAL_WARN("ucb.ucp.dropbox", "Downloading file with ID: " + m_aFileInfo.id);
                        uno::Reference< io::XInputStream > xIn =
                            m_pApiClient->downloadFile( m_aFileInfo.id );

                        if ( xIn.is() ) {
                            SAL_WARN("ucb.ucp.dropbox", "Download successful, transferring data");

                            if ( xDataSink.is() )
                            {
                                SAL_WARN("ucb.ucp.dropbox", "Using XActiveDataSink interface");
                                if ( xPipeInputForSink.is() )
                                {
                                    SAL_WARN("ucb.ucp.dropbox", "Using pipe for XActiveDataSink - copying data through pipe");
                                ...
                                {
                                    SAL_WARN("ucb.ucp.dropbox", "Direct XActiveDataSink - setting input stream directly");
                                    xDataSink->setInputStream( xIn );
                                }
                            }
                            else if ( xOut.is() )
                            {
                                SAL_WARN("ucb.ucp.dropbox", "Using XOutputStream interface");
                                copyData( xIn, xOut );
                            }
                        } else {
                            SAL_WARN("ucb.ucp.dropbox", "Download failed - no input stream returned");
                            ucbhelper::cancelCommandExecution(
                                uno::Any( ucb::IOErrorCode_CANT_READ ),
                                Environment );
                        }
                    } catch (const uno::Exception& e) {
                        SAL_WARN("ucb.ucp.dropbox", "Exception during file download: " + e.Message);
                        ucbhelper::cancelCommandExecution(
                ...
                {
                    SAL_WARN("ucb.ucp.dropbox", "No data sink available");
                    ucbhelper::cancelCommandExecution(
            ...
            {
                SAL_WARN("ucb.ucp.dropbox", "No sink provided in open command");
            }
        }
        else
        {
...
{
    if ( !Info.Type.equals( DROPBOX_FILE_TYPE ) && !Info.Type.equals( DROPBOX_FOLDER_TYPE ) )
    ...
    try
    {
        ...
        DropboxFileInfo aInfo;
        aInfo.isFolder = Info.Type.equals( DROPBOX_FOLDER_TYPE );
...
void Content::setFileInfo( const DropboxFileInfo& rInfo )
{
    m_aFileInfo = rInfo;
    ...
    const uno::Reference< ucb::XCommandEnvironment >& /*xEnv*/ )
{
    ...
    SAL_WARN("ucb.ucp.dropbox", "getPropertyValues - Processing " + OUString::number(nCount) + " properties");

    // Process properties in the exact order requested to maintain column indices
    for ( sal_Int32 n = 0; n < nCount; ++n )
    {
        const beans::Property& rProp = rProperties[n];
        SAL_WARN("ucb.ucp.dropbox", "Property[" + OUString::number(n) + "] = " + rProp.Name);

        if ( rProp.Name == "Title" )
        {
            SAL_WARN("ucb.ucp.dropbox", "getPropertyValues - Title requested, returning: '" + m_aFileInfo.name + "'");
            if (m_aFileInfo.name.isEmpty()) {
                SAL_WARN("ucb.ucp.dropbox", "WARNING: Title is empty!");
        ...
        else if ( rProp.Name == "DateModified" )
        {
            ...
            util::DateTime aDateTime = DropboxJsonHelper::parseDateTime(m_aFileInfo.modifiedTime);
            xRow->appendTimestamp( rProp, aDateTime );
        }
        else
        {
            SAL_WARN("ucb.ucp.dropbox", "Unknown property requested: " + rProp.Name);
...
void Content::queryChildren( ContentRefList& rChildren )
{
    ...
    if ( m_pApiClient )
    {
        std::vector< DropboxFileInfo > aFileInfos =
        ...
        {
            OUString sContentId = u"dropbox://"_ustr + fileInfo.id;
            uno::Reference< ucb::XContentIdentifier > xId(
                       ...
                       const uno::Reference< ucb::XCommandEnvironment >& /*xEnv*/ )
{
    if (bDeletePhysical && m_pApiClient && !m_aFileInfo.id.isEmpty()) {
        SAL_WARN("ucb.ucp.dropbox", "Deleting file/folder: " + m_aFileInfo.name + " (ID: " + m_aFileInfo.id + ")");
...
{
    if (!m_pApiClient) {
        SAL_WARN("ucb.ucp.dropbox", "No API client available for transfer operation");
        return;
    }

    SAL_WARN("ucb.ucp.dropbox", "transfer() called - Operation: " <<
    ...
    if (sSourceFileId.isEmpty()) {
        SAL_WARN("ucb.ucp.dropbox", "Cannot extract source file ID from URL: " + rArgs.SourceURL);
        return;
    }
    ...
    if (sTargetParentId.isEmpty()) {
        SAL_WARN("ucb.ucp.dropbox", "Target parent ID is empty");
        return;
    }
    ...
    try {
        if (rArgs.MoveData) {
            // Move operation
            SAL_WARN("ucb.ucp.dropbox", "Moving file " + sSourceFileId + " to parent " + sTargetParentId);
            m_pApiClient->moveFile(sSourceFileId, sTargetParentId, rArgs.NewTitle);
        } else {
            // Copy operation
            SAL_WARN("ucb.ucp.dropbox", "Copying file " + sSourceFileId + " to parent " + sTargetParentId);
            rtl::OUString sNewFileId = m_pApiClient->copyFile(sSourceFileId, sTargetParentId, rArgs.NewTitle);
            if (!sNewFileId.isEmpty()) {
                SAL_WARN("ucb.ucp.dropbox", "Copy successful, new file ID: " + sNewFileId);
            }
        }
    } catch (const uno::Exception& e) {
        SAL_WARN("ucb.ucp.dropbox", "Exception during transfer: " + e.Message);
    }
}
...
void Content::updateFileInfo()
{
    ...
    {
        SAL_WARN("ucb.ucp.dropbox", "Updating file info for ID: " + m_aFileInfo.id);

        DropboxFileInfo updatedInfo = m_pApiClient->getFileInfo(m_aFileInfo.id);

        if (!updatedInfo.id.isEmpty()) {
            m_aFileInfo = updatedInfo;
            m_eResourceType = updatedInfo.isFolder ? FOLDER : FILE;
            SAL_WARN("ucb.ucp.dropbox", "Updated file info: " + m_aFileInfo.name +
                     " (type: " + (m_eResourceType == FOLDER ? u"folder" : u"file") + ")");
        } else {
            SAL_WARN("ucb.ucp.dropbox", "Failed to retrieve file info, assuming file exists");
    ...
    if ( m_eResourceType == FOLDER && m_pApiClient )
    {
        ...
        for ( const auto& fileInfo : aFileInfos )
        {
            ...
            if ( pContent.is() )
            {
                ...
                const DropboxFileInfo& rInfo = pContent->getFileInfo();

                aRow[0] <<= (rInfo.isFolder ? DROPBOX_FOLDER_TYPE : DROPBOX_FILE_TYPE);
                aRow[1] <<= rInfo.name;
                aRow[2] <<= (rInfo.size.isEmpty() ? sal_Int64(0) : rInfo.size.toInt64());
                aRow[3] <<= DropboxJsonHelper::parseDateTime(rInfo.modifiedTime);
