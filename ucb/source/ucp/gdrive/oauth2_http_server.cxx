...
bool OAuth2HttpServer::start()
{
    ...
    SAL_WARN("ucb.ucp.gdrive", "Starting OAuth2 HTTP server on port " + OUString::number(m_nPort));
    ...
    if (m_nSocketFd < 0) {
#endif
        SAL_WARN("ucb.ucp.gdrive", "Failed to create socket");
        return false;
    }
...
#endif
        SAL_WARN("ucb.ucp.gdrive", "Failed to set socket options");
#ifdef _WIN32
    ...
    if (bind(m_nSocketFd, (struct sockaddr*)&address, sizeof(address)) < 0) {
        SAL_WARN("ucb.ucp.gdrive", "Failed to bind socket to port " + OUString::number(m_nPort));
#ifdef _WIN32
        closesocket(m_nSocketFd);
#else
        close(m_nSocketFd);
    ...
    if (listen(m_nSocketFd, 3) < 0) {
        SAL_WARN("ucb.ucp.gdrive", "Failed to listen on socket");
#ifdef _WIN32
        closesocket(m_nSocketFd);
#else
        close(m_nSocketFd);
    ...
    try {
        ...
        SAL_WARN("ucb.ucp.gdrive", "OAuth2 HTTP server started successfully on port " + OUString::number(m_nPort));
        return true;
    } catch (...) {
        SAL_WARN("ucb.ucp.gdrive", "Failed to start server thread");
        m_bRunning = false;
#ifdef _WIN32
        closesocket(m_nSocketFd);
#else
...
void OAuth2HttpServer::stop()
{
    ...
    SAL_WARN("ucb.ucp.gdrive", "Stopping OAuth2 HTTP server");
    ...
    SAL_WARN("ucb.ucp.gdrive", "OAuth2 HTTP server stopped");
}

rtl::OUString OAuth2HttpServer::waitForAuthCode(sal_Int32 timeoutSeconds)
{
    SAL_WARN("ucb.ucp.gdrive", "Waiting for OAuth2 authorization code (timeout: " + OUString::number(timeoutSeconds) + " seconds)");
    ...
    while (!m_bCodeReceived.load() && m_bRunning.load()) {
        ...
        if (elapsed >= timeoutSeconds) {
            SAL_WARN("ucb.ucp.gdrive", "Timeout waiting for authorization code");
            return rtl::OUString();
        }
    }

    if (m_bCodeReceived.load()) {
        SAL_WARN("ucb.ucp.gdrive", "Authorization code received: " + m_sAuthCode.copy(0, 10) + "...");
        return m_sAuthCode;
    }

    SAL_WARN("ucb.ucp.gdrive", "Server stopped before receiving authorization code");
    return rtl::OUString();
}

void OAuth2HttpServer::serverLoop()
{
    SAL_WARN("ucb.ucp.gdrive", "OAuth2 server thread started");
    ...
    while (m_bRunning.load()) {
...
#endif
            if (m_bRunning.load()) {
                SAL_WARN("ucb.ucp.gdrive", "Accept failed");
            }
            break;
        }
        ...
        if (bytes_read > 0) {
            ...
            SAL_WARN("ucb.ucp.gdrive", "Received HTTP request: " + request.copy(0, 200));
            ...
            if (!authCode.isEmpty()) {
                ...
                SAL_WARN("ucb.ucp.gdrive", "Authorization code extracted successfully");
            } else {
                ...
                SAL_WARN("ucb.ucp.gdrive", "No authorization code found in request");
            }
            ...
            if (m_bCodeReceived.load()) {
                SAL_WARN("ucb.ucp.gdrive", "Authorization code received, stopping server");
                break;
            }
        } else {
#ifdef _WIN32
    ...
    SAL_WARN("ucb.ucp.gdrive", "OAuth2 server thread exiting");
}
...
rtl::OUString OAuth2HttpServer::parseAuthCodeFromRequest(const rtl::OUString& request)
{
    ...
    if (codeEnd > codeStart) {
        ...
        SAL_WARN("ucb.ucp.gdrive", "Extracted authorization code: " + authCode.copy(0, 10) + "...");
        return authCode;
    }
