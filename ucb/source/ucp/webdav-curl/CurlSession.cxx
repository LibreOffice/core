...
static auto ProcessHeaders(::std::vector<OString> const& rHeaders) -> ::std::map<OUString, OUString>
{
    ...
    for (OString const& rLine : rHeaders)
    {
        ...
        sal_Int32 nEnd(line.getLength() - 1);
        while (nStart < nEnd && line[nEnd] == ' ' && line[nEnd] == '\t')
        {
            --nEnd;
        }
...
struct CurlProcessor
{
    ...
    static auto MoveOrCopy(CurlSession& rSession, ::std::u16string_view const rSourceURIReference,
    ...
    -> void
{
    if (pRequestedHeaders)
    {
        pRequestedHeaders->second.uri = rURIReference;
    ...
    OString const utf8URIRef(::rtl::OUStringToOString(rURIReference, RTL_TEXTENCODING_UTF8));
    ...
    -> uno::Reference<io::XInputStream>
{
    ...
    uno::Reference<io::XInputStream> const xResponseInStream(io::Pipe::create(m_xContext));
    uno::Reference<io::XOutputStream> const xResponseOutStream(xResponseInStream, uno::UNO_QUERY);
    assert(xResponseInStream.is());
    assert(xResponseOutStream.is());
                      ...
                      DAVRequestEnvironment const& rEnv) -> void
{
    ...
    if (!token.isEmpty())
    {
        OString const utf8If(
            "If: <" + ::rtl::OUStringToOString(rURIReference, RTL_TEXTENCODING_ASCII_US) + "> (<"
                               ...
                               char const* const pMethod) -> void
{
    ...
    OString const utf8Destination(
        "Destination: " + ::rtl::OUStringToOString(rDestinationURI, RTL_TEXTENCODING_ASCII_US));
    ::std::unique_ptr<curl_slist, deleter_from_fn<curl_slist_free_all>> pList(
