...
static auto GetResponseCode(Headers const& rHeaders) -> ::std::optional<long>
{
                              ...
                              void* const userdata)
{
    ...
    try
    {
        ...
        else if (buffer[0] == ' ' || buffer[0] == '\t') // folded header field?
        {
            ...
            pHeaders->HeaderFields.back().first.back() += " " + OString(&buffer[i], nitems - i);
        }
        else
        {
            if (pHeaders->HeaderFields.empty() || pHeaders->HeaderFields.back().second)
...
static auto ProcessHeaders(::std::vector<OString> const& rHeaders) -> ::std::map<OString, OString>
{
    ...
    , m_Proxy(rProxyDecider.getProxy(m_URI.GetScheme(), m_URI.GetHost(), m_URI.GetPort()))
{
    ...
    OString const useragent("LibreOffice " LIBO_VERSION_DOTTED " curl/"
...
{
    static auto ProcessRequest(CurlSession& rSession, OUString const& rInPath,
...
auto CurlProcessor::ProcessRequest(CurlSession& rSession, OUString const& rInPath,
                                   DAVRequestEnvironment const& rEnv,
#if 0
        DownloadTarget * pTarget,
        UploadSource * pSource,
                                   ...
                                   ) -> void
{
    ...
    struct Auth
    {
        ...
        OUString PassWord;
        long AuthMask;
        Auth(OUString const& rUserName, OUString const& rPassword, long const& rAuthMask)
            : UserName(rUserName)
            , PassWord(rPassword)
            , AuthMask(rAuthMask)
        {
        ...
        if (rc != CURLE_OK)
        {
            ...
            SAL_WARN("ucb.ucp.webdav.curl",
                     "curl_easy_perform failed: "
                         << GetErrorString(rc, rSession.m_ErrorBuffer).getStr());
            switch (rc)
            {
    ...
    -> uno::Reference<io::XInputStream>
{
    ...
    CurlProcessor::ProcessRequest(*this, rInPath, rEnv, &xOut, nullptr);
                      ...
                      DAVRequestEnvironment const& rEnv) -> void
{
    ...
    CurlProcessor::ProcessRequest(*this, rInPath, rEnv, &rOutStream, nullptr);
}
