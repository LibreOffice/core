...
//typedef ::std::vector<::std::pair<::std::vector<OString>, ::std::optional<long>>> Headers;
...
static size_t write_callback(char* const ptr, size_t const size, size_t const nmemb,
                             void* const userdata)
{
    // FIXME: there could be an error status, and this would still be called?
    //
                              ...
                              void* const userdata)
{
    ...
    try
    {
        ...
        if (nitems == 0)
        {
            ...
            assert(rc == CURLE_OK);
            (void)rc;
            //pHeaders->back().oResponseCode.emplace(statusCode);
...
static auto ProcessHeaders(::std::vector<OString> const& rHeaders) -> ::std::map<OUString, OUString>
{
    ...
    for (OString const& rLine : rHeaders)
    {
        ...
        auto const name(::rtl::OStringToOUString(rLine.copy(0, nColon).toAsciiLowerCase(),
        ...
        auto const value(::rtl::OStringToOUString(rLine.subView(nStart, nEnd - nStart),
                                                  RTL_TEXTENCODING_ASCII_US));
    ...
    -> void
{
    ...
    if (pRequestedHeaders)
    {
        ...
        {
            auto const it(headerMap.find(rHeader));
            if (it != headerMap.end())
            {
...
struct CurlProcessor
{
    static auto ProcessRequest(
        CurlSession& rSession, OUString const& rInPath, DAVRequestEnvironment const* pEnv,
#if 0
        ...
        css::uno::Reference<css::io::XOutputStream> const* pxOutStream,
        css::uno::Reference<css::io::XInputStream> const* pxInStream,
        ::std::pair<::std::vector<OUString> const&, DAVResource&> const* pRequestedHeaders) -> void;

    static auto
    PropFind(CurlSession& rSession, OUString const& rInPathSource, Depth depth,
             ::std::pair<::std::vector<OUString> const&, ::std::vector<DAVResource>&> const* const
                 o_pRequestedProperties,
             ::std::vector<DAVResourceInfo>* const o_pResourceInfos,
             DAVRequestEnvironment const& rEnv) -> void;

    static auto MoveOrCopy(CurlSession& rSession, OUString const& rInPathSource,
                           ::std::u16string_view rInPathDestination,
                           DAVRequestEnvironment const& rEnv, bool isOverwrite, char const* pMethod)
        -> void;

    static auto ProcessLock(
        CurlSession& rSession, OUString const& rInPath, DAVRequestEnvironment const* pEnv,
        ::std::unique_ptr<curl_slist, deleter_from_fn<curl_slist_free_all>> pRequestHeaderList,
        css::uno::Reference<css::io::XInputStream> const* pxInStream)
        -> ::std::vector<::std::pair<ucb::Lock, sal_Int32>>;

    static auto Unlock(CurlSession& rSession, OUString const& rInPath,
                       DAVRequestEnvironment const* pEnv) -> void;
};

auto CurlProcessor::ProcessRequest(
    CurlSession& rSession, OUString const& rInPath, DAVRequestEnvironment const* const pEnv,
    ::std::unique_ptr<curl_slist, deleter_from_fn<curl_slist_free_all>> pRequestHeaderList,
#if 0
        DownloadTarget * pTarget,
        UploadSource * pSource,
#endif
    css::uno::Reference<css::io::XOutputStream> const* const pxOutStream,
    css::uno::Reference<css::io::XInputStream> const* const pxInStream,
    ::std::pair<::std::vector<OUString> const&, DAVResource&> const* const pRequestedHeaders)
    -> void
{
    if (pEnv)
    {
        for (auto const& rHeader : pEnv->m_aRequestHeaders)
        {
            OString const utf8Header(
                ::rtl::OUStringToOString(rHeader.first, RTL_TEXTENCODING_ASCII_US) + ": "
                + ::rtl::OUStringToOString(rHeader.second, RTL_TEXTENCODING_ASCII_US));
    ...
    auto uc = curl_url_set(pUrl.get(), CURLUPART_PATH, utf8Path.getStr(), 0);
    if (uc != CURLUE_OK)
    {
    ...
    struct Auth
    {
        OUString UserName;
        OUString PassWord;
        decltype(CURLAUTH_ANY) AuthMask;
    ...
    do
    {
        ...
        if (oAuth)
        {
            ...
            //rc = curl_easy_setopt(rSession.m_pCurl.get(), CURLOPT_HTTPAUTH, auth);
        ...
        //::std::vector<::std::tuple<::std::vector<OString>, bool, ::std::optional<long>>> headers;
        //::std::vector<::std::pair<::std::vector<OString>, ::std::optional<long>>> headers;
        ResponseHeaders headers(rSession.m_pCurl.get());
        rc = curl_easy_setopt(rSession.m_pCurl.get(), CURLOPT_HEADERDATA, &headers);
        assert(rc == CURLE_OK);
#if 0
        DownloadTarget downloadTarget(uno::Reference<io::XOutputStream>(xRet, uno::UNO_QUERY_THROW),
                                      headers);
...
auto CurlProcessor::PropFind(
    CurlSession& rSession, OUString const& rInPath, Depth const nDepth,
    ::std::pair<::std::vector<OUString> const&, ::std::vector<DAVResource>&> const* const
        o_pRequestedProperties,
    ...
    -> void
{
    ...
    ::std::scoped_lock const g(rSession.m_Mutex);
    ...
    ::comphelper::ScopeGuard const gg([&]() {
        ...
        (void)rc;
#if 0
        rc = curl_easy_setopt(m_pCurl.get(), CURLOPT_UPLOAD, 0L);
        assert(rc == CURLE_OK);
        (void)rc;
#endif
    });
...
#if 0
    // needed or not???
    rc = curl_easy_setopt(m_pCurl.get(), CURLOPT_UPLOAD, 1L);
    assert(rc == CURLE_OK);
#endif
    ...
    {
        if (o_pRequestedProperties->first.empty())
        {
            xWriter->startElement("allprop", nullptr);
            xWriter->endElement("allprop");
        }
        else
            for (OUString const& rName : o_pRequestedProperties->first)
            {
    ...
    CurlProcessor::ProcessRequest(rSession, rInPath, &rEnv, ::std::move(pList), &xResponseOutStream,
    ...
    {
        o_pRequestedProperties->second = parseWebDAVPropFindResponse(xResponseInStream);
    }
}

// DAV methods
auto CurlSession::PROPFIND(OUString const& rInPath, Depth const depth,
                           ::std::vector<OUString> const& rPropertyNames,
                           ::std::vector<DAVResource>& o_rResources,
                           DAVRequestEnvironment const& rEnv) -> void
{
    ::std::pair<::std::vector<OUString> const&, ::std::vector<DAVResource>&> const args(
        rPropertyNames, o_rResources);
    return CurlProcessor::PropFind(*this, rInPath, depth, &args, nullptr, rEnv);
}
...
auto CurlSession::PROPFIND(OUString const& rInPath, Depth const depth,
                           ...
                           DAVRequestEnvironment const& rEnv) -> void
{
    return CurlProcessor::PropFind(*this, rInPath, depth, nullptr, &o_rResourceInfos, rEnv);
}
                            ...
                            DAVRequestEnvironment const& rEnv) -> void
{
    ...
    CurlProcessor::ProcessRequest(*this, rInPath, &rEnv, ::std::move(pList), nullptr,
                                  &xRequestInStream, nullptr);
}
                       ...
                       DAVResource& io_rResource, DAVRequestEnvironment const& rEnv) -> void
{
    ...
    CurlProcessor::ProcessRequest(*this, rInPath, &rEnv, nullptr, nullptr, nullptr, &headers);
}
    ...
    -> uno::Reference<io::XInputStream>
{
...
#endif
    uno::Reference<io::XInputStream> const xRet(io::Pipe::create(m_xContext));
    uno::Reference<io::XOutputStream> const xOut(xRet, uno::UNO_QUERY);
    assert(xRet.is());
    assert(xOut.is());
    ...
    CurlProcessor::ProcessRequest(*this, rInPath, &rEnv, nullptr, &xOut, nullptr, nullptr);
                      ...
                      DAVRequestEnvironment const& rEnv) -> void
{
    ...
    CurlProcessor::ProcessRequest(*this, rInPath, &rEnv, nullptr, &rxOutStream, nullptr, nullptr);
}
    ...
    -> uno::Reference<io::XInputStream>
{
    ...
    CurlProcessor::ProcessRequest(*this, rInPath, &rEnv, nullptr, &xOut, nullptr, &headers);
                      ...
                      DAVRequestEnvironment const& rEnv) -> void
{
    ...
    CurlProcessor::ProcessRequest(*this, rInPath, &rEnv, nullptr, &rxOutStream, nullptr, &headers);
}
                      ...
                      DAVRequestEnvironment const& rEnv) -> void
{
    ...
    OUString const token(g_Init.LockStore.getLockToken(rInPath));
    if (!token.isEmpty())
    {
        OString const utf8If("If: <" + ::rtl::OUStringToOString(rInPath, RTL_TEXTENCODING_ASCII_US)
                             + "> (<" + ::rtl::OUStringToOString(token, RTL_TEXTENCODING_ASCII_US)
                             + ">)");
    ...
    ::comphelper::ScopeGuard const gg([&]() {
        auto rc = curl_easy_setopt(m_pCurl.get(), CURLOPT_UPLOAD, 0L);
        assert(rc == CURLE_OK);
        (void)rc;
    });

    auto rc = curl_easy_setopt(m_pCurl.get(), CURLOPT_UPLOAD, 1L);
    assert(rc == CURLE_OK);
    (void)rc;

    CurlProcessor::ProcessRequest(*this, rInPath, &rEnv, ::std::move(pList), nullptr, &rxInStream,
                                  nullptr);
}
                       ...
                       DAVRequestEnvironment const& rEnv) -> uno::Reference<io::XInputStream>
{
    ...
    OString const utf8ContentType(
        "Content-Type: " + ::rtl::OUStringToOString(rContentType, RTL_TEXTENCODING_ASCII_US));
    pList.reset(curl_slist_append(pList.release(), utf8ContentType.getStr()));
    if (!pList)
    {
        throw uno::RuntimeException("curl_slist_append failed");
    }
    OString const utf8Referer("Referer: "
                              + ::rtl::OUStringToOString(rReferer, RTL_TEXTENCODING_ASCII_US));
    ...
    CurlProcessor::ProcessRequest(*this, rInPath, &rEnv, ::std::move(pList), &xOut, &rxInStream,
                                  nullptr);
                       ...
                       DAVRequestEnvironment const& rEnv) -> void
{
    ...
    CurlProcessor::ProcessRequest(*this, rInPath, &rEnv, ::std::move(pList), &rxOutStream,
                                  &rxInStream, nullptr);
}
...
auto CurlSession::MKCOL(OUString const& rInPath, DAVRequestEnvironment const& rEnv) -> void
{
    ...
    CurlProcessor::ProcessRequest(*this, rInPath, &rEnv, nullptr, nullptr, nullptr, nullptr);
}

auto CurlProcessor::MoveOrCopy(CurlSession& rSession, OUString const& rInPathSource,
                               ::std::u16string_view const rInPathDestination,
                               ...
                               char const* const pMethod) -> void
{
    ...
    OString const utf8Destination(
        "Destination: " + ::rtl::OUStringToOString(rInPathDestination, RTL_TEXTENCODING_ASCII_US));
    ::std::unique_ptr<curl_slist, deleter_from_fn<curl_slist_free_all>> pList(
    ...
    CurlProcessor::ProcessRequest(rSession, rInPathSource, &rEnv, ::std::move(pList), nullptr,
                                  nullptr, nullptr);
}
...
{
    return CurlProcessor::MoveOrCopy(*this, rInPathSource, rInPathDestination, rEnv, isOverwrite,
                                     "COPY");
}
...
auto CurlProcessor::ProcessLock(
    CurlSession& rSession, OUString const& rInPath, DAVRequestEnvironment const* const pEnv,
    ::std::unique_ptr<curl_slist, deleter_from_fn<curl_slist_free_all>> pRequestHeaderList,
    css::uno::Reference<css::io::XInputStream> const* const pxRequestInStream)
    -> ::std::vector<::std::pair<ucb::Lock, sal_Int32>>
{
    ::comphelper::ScopeGuard const gg([&]() {
        auto rc = curl_easy_setopt(rSession.m_pCurl.get(), CURLOPT_CUSTOMREQUEST, nullptr);
    ...
    CurlProcessor::ProcessRequest(rSession, rInPath, pEnv, ::std::move(pRequestHeaderList),
    ...
    SAL_WARN_IF(acquiredLocks.empty(), "ucb.ucp.webdav.curl", "could not get LOCK for " << rInPath);
    ...
    for (auto const& rLock : acquiredLocks)
    {
        ...
        {
            SAL_WARN("ucb.ucp.webdav.curl",
                     "LOCK timeout already expired when receiving LOCK response for " << rInPath);
            lockExpirationTimeSeconds = 0;
        }
        else
        {
            lockExpirationTimeSeconds = endTime.Seconds + rLock.Timeout - elapsedSeconds;
        }
                       ...
                       DAVRequestEnvironment const& rEnv) -> void
{
    ...
    auto const acquiredLocks
        = CurlProcessor::ProcessLock(*this, rInPath, &rEnv, ::std::move(pList), &xRequestInStream);

    for (auto const& rAcquiredLock : acquiredLocks)
    {
        g_Init.LockStore.addLock(rInPath, rAcquiredLock.first.LockTokens[0], this,
                                 rAcquiredLock.second);
        SAL_INFO("ucb.ucp.webdav.curl", "created LOCK for " << rInPath);
    }
}

auto CurlProcessor::Unlock(CurlSession& rSession, OUString const& rInPath,
                           DAVRequestEnvironment const* const pEnv) -> void
{
    ...
    CurlProcessor::ProcessRequest(rSession, rInPath, pEnv, ::std::move(pList), nullptr, nullptr,
                                  nullptr);
}
...
{
    CurlProcessor::Unlock(*this, rInPath, &rEnv);
    g_Init.LockStore.removeLock(rInPath);
}

auto CurlSession::NonInteractive_LOCK(OUString const& rInPath,
                                      sal_Int32& o_rLastChanceToSendRefreshRequest) -> bool
{
    ...
    OString const utf8If("If: (<" + ::rtl::OUStringToOString(token, RTL_TEXTENCODING_ASCII_US)
                         + ">)");
    pList.reset(curl_slist_append(pList.release(), utf8If.getStr()));
    if (!pList)
    {
    ...
    {
        auto const acquiredLocks
            = CurlProcessor::ProcessLock(*this, rInPath, nullptr, ::std::move(pList), nullptr);

        SAL_WARN_IF(1 < acquiredLocks.size(), "ucb.ucp.webdav.curl",
                    "multiple locks acquired on refresh for " << rInPath);
        if (!acquiredLocks.empty())
        {
            o_rLastChanceToSendRefreshRequest = acquiredLocks.begin()->second;
        }
        SAL_INFO("ucb.ucp.webdav.curl", "NonInteractive_LOCK succeeded on " << rInPath);
        return true;
    }
    catch (...)
    {
        SAL_INFO("ucb.ucp.webdav.curl", "NonInteractive_LOCK failed on " << rInPath);
        return false;
    }
}

auto CurlSession::NonInteractive_UNLOCK(OUString const& rInPath) -> void
{
    try
    {
        CurlProcessor::Unlock(*this, rInPath, nullptr);
        SAL_INFO("ucb.ucp.webdav.curl", "NonInteractive_UNLOCK succeeded on " << rInPath);
        // TODO: why does serf not remove it from the lock store now???
    }
    catch (...)
    {
        SAL_INFO("ucb.ucp.webdav.curl", "NonInteractive_UNLOCK failed on " << rInPath);
    }
}
