...
struct Headers
{
    ...
    CURL* pCurl;
    Headers(CURL* const i_pCurl)
        : pCurl(i_pCurl)
    {
    }
};

auto GetResponseCode(Headers const& rHeaders) -> ::std::optional<long>
{
...
struct DownloadTarget
{
    ...
    Headers const& rHeaders;
    DownloadTarget(uno::Reference<io::XOutputStream> const& i_xOutStream, Headers const& i_rHeaders)
...
struct UploadSource
{
    ...
    Headers const& rHeaders;
    UploadSource(uno::Reference<io::XInputStream> const& i_xInStream, Headers const& i_rHeaders)
...
static size_t header_callback(char* const buffer, size_t const size, size_t const nitems,
                              void* const userdata)
{
    auto* const pHeaders(static_cast<Headers*>(userdata));
    //assert(pHeaders);
    ...
    ) -> void
{
    ...
    do
    {
        ...
        Headers headers(rSession.m_pCurl.get());
                       ...
                       DAVResource& io_rResource, DAVRequestEnvironment const& rEnv) -> void
{
    ...
    CurlProcessor::ProcessRequest(*this, rInPath, rEnv, nullptr, nullptr, &headers);
}
    ...
    -> uno::Reference<io::XInputStream>
{
    ...
    CurlProcessor::ProcessRequest(*this, rInPath, rEnv, &xOut, nullptr, nullptr);
                      ...
                      DAVRequestEnvironment const& rEnv) -> void
{
    ...
    CurlProcessor::ProcessRequest(*this, rInPath, rEnv, &rxOutStream, nullptr, nullptr);
}
    ...
    -> uno::Reference<io::XInputStream>
{
    ...
    CurlProcessor::ProcessRequest(*this, rInPath, rEnv, &xOut, nullptr, &headers);
                      ...
                      DAVRequestEnvironment const& rEnv) -> void
{
    ...
    CurlProcessor::ProcessRequest(*this, rInPath, rEnv, &rxOutStream, nullptr, &headers);
}
                      ...
                      DAVRequestEnvironment const& rEnv) -> void
{
    ...
    ::std::unique_ptr<curl_slist, deleter_from_fn<curl_slist_free_all>> const pList(
    ...
    ::comphelper::ScopeGuard const gg([&]() {
        ...
        assert(rc == CURLE_OK);
        (void)rc;
        // note: must be run *before* pList is freed
        rc = curl_easy_setopt(m_pCurl.get(), CURLOPT_HTTPHEADER, nullptr);
        assert(rc == CURLE_OK);
        (void)rc;
    });

    auto rc = curl_easy_setopt(m_pCurl.get(), CURLOPT_HTTPHEADER, pList.get());
    assert(rc == CURLE_OK);
    (void)rc;
    ...
    CurlProcessor::ProcessRequest(*this, rInPath, rEnv, nullptr, &rxInStream, nullptr);
}
                       ...
                       DAVRequestEnvironment const& rEnv) -> uno::Reference<io::XInputStream>
{
    ...
    assert(rc == CURLE_OK);
    (void)rc;
    rc = curl_easy_setopt(m_pCurl.get(), CURLOPT_POST, 1L);
    assert(rc == CURLE_OK);
    (void)rc;
    ...
    CurlProcessor::ProcessRequest(*this, rInPath, rEnv, &xOut, &rxInStream, nullptr);
                       ...
                       DAVRequestEnvironment const& rEnv) -> void
{
    ...
    CurlProcessor::ProcessRequest(*this, rInPath, rEnv, &rxOutStream, &rxInStream, nullptr);
}
                               ...
                               char const* const pMethod) -> void
{
    ...
    auto rc = curl_easy_setopt(rSession.m_pCurl.get(), CURLOPT_HTTPHEADER, pList.get());
    assert(rc == CURLE_OK);
    (void)rc;
    rc = curl_easy_setopt(rSession.m_pCurl.get(), CURLOPT_CUSTOMREQUEST, pMethod);
    assert(rc == CURLE_OK);
    (void)rc;

    CurlProcessor::ProcessRequest(rSession, rInPathSource, rEnv, nullptr, nullptr, nullptr);
}
...
auto CurlSession::DESTROY(OUString const& rInPath, DAVRequestEnvironment const& rEnv) -> void
{
    ...
    CurlProcessor::ProcessRequest(*this, rInPath, rEnv, nullptr, nullptr, nullptr);
}
