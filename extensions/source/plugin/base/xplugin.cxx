...
PluginInputStream::~PluginInputStream()
{
    ...
    if (xPlugin.is() && pPlugin)
    {
        ...
        auto & rIS = pPlugin->getInputStreams();
        rIS.erase( std::remove(rIS.begin(), rIS.end(), this), rIS.end() );
        ...
        if( m_pPlugin )
        {
                ...
                // or the plugin is already disposing
            {
                ...
                m_pPlugin->getPluginComm()->NPP_SetWindow( m_pPlugin );
                auto & rIS2 = m_pPlugin->getInputStreams();
                rIS2.erase( std::remove(rIS2.begin(), rIS2.end(), this), rIS2.end() );
            }
            else
...
void PluginInputStream::setMode( sal_Int32 nMode )
{
    ...
    if (m_nMode == -1)
    {
        auto & rIS = m_pPlugin->getInputStreams();
        rIS.erase( std::remove(rIS.begin(), rIS.end(), this), rIS.end() );
        m_pPlugin = NULL;
        m_wPlugin.clear();
    }
...
PluginOutputStream::~PluginOutputStream()
{
    ...
    auto & rIS = m_pPlugin->getOutputStreams();
    rIS.erase( std::remove(rIS.begin(), rIS.end(), this), rIS.end() );
}
