...
public:
    ADsUserAccess(const css::uno::Reference<css::uno::XComponentContext>& xContext)
    {
        try
        {
            ...
            // If this user is an AD user, then all the above will succeed even without an active
            // connection to the domain, and m_sUserDN will be correctly initialized, but the
            // following call to ADsGetObject will fail.
...
private:
    ...
    void CacheData(const css::uno::Reference<css::uno::XComponentContext>& xContext)
    {
        try
        {
            ...
            const OUString sCachedData
                = "user=" + m_sUserDN + sNul + givenname + "=" + m_sGivenName + sNul + sn + "="
                  + m_sSn + sNul + initials + "=" + m_sInitials + sNul + street + "=" + m_sStreet
                  + sNul + l + "=" + m_sCity + sNul + st + "=" + m_sState + sNul + postalcode + "="
                  + m_sPostalCode + sNul + c + "=" + m_sCountry + sNul + o + "=" + m_sOrganization
                  + sNul + title + "=" + m_sTitle + sNul + homephone + "=" + m_sHomePhone + sNul
                  + telephonenumber + "=" + m_sTelephoneNumber + sNul + facsimiletelephonenumber
                  + "=" + m_sFaxNumber + sNul + mail + "=" + m_sMail;
    ...
    void GetCachedData(const css::uno::Reference<css::uno::XComponentContext>& xContext)
    {
        ...
        if (aMap["user"] != m_sUserDN)
            throw css::uno::RuntimeException();

        m_sGivenName = aMap[givenname];
        m_sSn = aMap[sn];
        m_sInitials = aMap[initials];
        m_sStreet = aMap[street];
        m_sCity = aMap[l];
        m_sState = aMap[st];
        m_sPostalCode = aMap[postalcode];
        m_sCountry = aMap[c];
        m_sOrganization = aMap[o];
        m_sTitle = aMap[title];
        m_sHomePhone = aMap[homephone];
        m_sTelephoneNumber = aMap[telephonenumber];
        m_sFaxNumber = aMap[facsimiletelephonenumber];
        m_sMail = aMap[mail];
    }
