...
static int doc_saveAs(LibreOfficeKitDocument* pThis, const char* sUrl, const char* pFormat, const char* pFilterOptions)
{
    ...
    try
    {
        ...
        if ((aIndex = aFilterOptions.indexOf(",PDFVer=")) >= 0)
        {
            ...
            if (sPdfVer.equalsIgnoreAsciiCaseAscii("PDF/A-1b"))
                pdfVer = 1;
            else if (sPdfVer.equalsIgnoreAsciiCaseAscii("PDF/A-2b"))
                pdfVer = 2;
            else if (sPdfVer.equalsIgnoreAsciiCaseAscii("PDF/A-3b"))
                pdfVer = 3;
            else if (sPdfVer.equalsIgnoreAsciiCaseAscii("PDF-1.5"))
                pdfVer = 15;
            else if (sPdfVer.equalsIgnoreAsciiCaseAscii("PDF-1.6"))
                pdfVer = 16;
            else
            {
