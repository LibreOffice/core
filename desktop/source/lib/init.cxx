...
static void doc_iniUnoCommand(const SfxViewShell* pViewShell, const OUString& aUnoCommand)
{
    ...
    if (aUnoCommand.endsWith("LanguageStatus"))
    {
        ...
        uno::Reference< css::linguistic2::XSpellChecker1 > xSpell( xLangSrv.is() ? xLangSrv->getSpellChecker() : nullptr, uno::UNO_QUERY );
        uno::Sequence< sal_Int16 > aLangs(xSpell.is() ? xSpell->getLanguages() : uno::Sequence< sal_Int16>() );

        aBuffer.append(aUnoCommand);
        aBuffer.append("?[");
        for ( sal_Int32 itLang = 0; itLang < aLangs.getLength(); itLang++ )
        {
            aBuffer.append("\"" + SvtLanguageTable::GetLanguageString((LanguageType)aLangs[itLang]) + "\"");
            if (itLang + 1 != aLangs.getLength())
                aBuffer.append(",");
        }
        aBuffer.append("]");
    }
