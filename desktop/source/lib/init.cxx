...
static void setupSidebar(bool bShow)
{
    ...
    if (pViewFrame)
    {
        ...
        {
            SAL_DEBUG("Create sidebar");
            pViewFrame->SetChildWindow(SID_SIDEBAR, false /* create it */, true /* focus */);
            SAL_DEBUG("Create sidebar: done");
        }
        SAL_DEBUG("Set sidebar on or off: " << bShow);
        pViewFrame->ShowChildWindow(SID_SIDEBAR, bShow);
        SAL_DEBUG("Set sidebar on or off done");
...
{
    VclPtr<Window> xRet;
    SAL_DEBUG("find the sidebar window");
    setupSidebar(true);
    SfxViewShell* pViewShell = SfxViewShell::Current();
    SfxViewFrame* pViewFrame = pViewShell? pViewShell->GetViewFrame(): nullptr;
    if (!pViewFrame)
    ...
    if (!pWin) {
        SAL_DEBUG("No docking / child window");
        return xRet;
    }
    xRet = pWin;
    return xRet;
...
static void doc_postUnoCommand(LibreOfficeKitDocument* pThis, const char* pCommand, const char* pArguments, bool bNotifyWhenFinished)
{
    ...
    } if (gImpl && aCommand == ".uno:SidebarShow") {
        setupSidebar(true);
        return;
    } if (gImpl && aCommand == ".uno:SidebarHide") {
        setupSidebar(false);
        return;
    }
