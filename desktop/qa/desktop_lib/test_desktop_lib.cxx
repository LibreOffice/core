...
void DesktopLOKTest::testNoTiledAnnotations()
{
    ...
    for (boost::property_tree::ptree::value_type& rComment : aTree.get_child("comments"))
    {
        ...
        if (rComment.second.get<int>("id") == 2)
        {
            CPPUNIT_ASSERT(!rComment.second.get<std::string>("textRange").empty());
        }
        // This is a reply comment
        else if (rComment.second.get<int>("id") == 3)
        {
            CPPUNIT_ASSERT_EQUAL(2, rComment.second.get<int>("parent"));
        }
    }
...
void DesktopLOKTest::testCommentsCallbacks()
{
    ...
    // comments in document: 1,2->3 (2 is parent of 3)

    // Add a new comment
    pDocument->pClass->postUnoCommand(pDocument, ".uno:InsertAnnotation",
          "{ \"Text\": { \"type\": \"string\", \"value\": \"Additional comment\" }, \"Author\": { \"type\": \"string\", \"value\": \"LOK User1\" } }", false);
    Scheduler::ProcessEventsToIdle();
    // comment state: 1,2->3,4
    ...
    pDocument->pClass->postUnoCommand(pDocument, ".uno:ReplyComment",
          "{ \"Id\": { \"type\": \"long\", \"value\": \"1\" }, \"Text\": { \"type\": \"string\", \"value\": \"Reply comment\" } }", false);
    Scheduler::ProcessEventsToIdle();
    // comment state: 1->5,2->3,4

    // We received a LOK_CALLBACK_COMMENT callback with comment 'Add' action and linked to its parent comment
    CPPUNIT_ASSERT_EQUAL(std::string("Add"), aView1.m_aCommentCallbackResult.get<std::string>("action"));
    CPPUNIT_ASSERT_EQUAL(std::string("Add"), aView2.m_aCommentCallbackResult.get<std::string>("action"));
    CPPUNIT_ASSERT_EQUAL(1, aView1.m_aCommentCallbackResult.get<int>("parent"));
    CPPUNIT_ASSERT_EQUAL(1, aView2.m_aCommentCallbackResult.get<int>("parent"));
    ...
    pDocument->pClass->postUnoCommand(pDocument, ".uno:DeleteComment",
          "{ \"Id\": { \"type\": \"long\", \"value\":  \"3\" } }", false);
    Scheduler::ProcessEventsToIdle();
    // comment state: 1->5,2,4
    ...
    CPPUNIT_ASSERT_EQUAL(3, aView1.m_aCommentCallbackResult.get<int>("id"));
    CPPUNIT_ASSERT_EQUAL(3, aView2.m_aCommentCallbackResult.get<int>("id"));

    // Reply to comment again
    pDocument->pClass->postUnoCommand(pDocument, ".uno:ReplyComment",
          "{ \"Id\": { \"type\": \"long\", \"value\": \"2\" }, \"Text\": { \"type\": \"string\", \"value\": \"Reply comment again\" } }", false);
    Scheduler::ProcessEventsToIdle();
    // comment state: 1->5,2->6,4

    // We received a LOK_CALLBACK_COMMENT callback with comment 'Add' action and linked to its parent comment
    CPPUNIT_ASSERT_EQUAL(std::string("Add"), aView1.m_aCommentCallbackResult.get<std::string>("action"));
    CPPUNIT_ASSERT_EQUAL(std::string("Add"), aView2.m_aCommentCallbackResult.get<std::string>("action"));
    CPPUNIT_ASSERT_EQUAL(2, aView1.m_aCommentCallbackResult.get<int>("parent"));
    CPPUNIT_ASSERT_EQUAL(2, aView2.m_aCommentCallbackResult.get<int>("parent"));
    ...
    // .uno:ViewAnnotations returns total of 5 comments (1->5,2->6,4)
    boost::property_tree::ptree aTree;
    char* pJSON = pDocument->m_pDocumentClass->getCommandValues(pDocument, ".uno:ViewAnnotations");
    std::stringstream aStream(pJSON);
    free(pJSON);
