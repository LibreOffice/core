...
#define CPPUNIT_DUMP_ASSERT_NUMBERS_EQUAL(aActual) \
    if(isInDumpMode()) \
        writeActual(OUString::number(aActual), OUString(#aActual)); \
    else \
        { \
            OString sTestFile = OUStringToOString(getTestFileName(), RTL_TEXTENCODING_UTF8); \
            CPPUNIT_ASSERT_EQUAL_MESSAGE(OString("Failing test file is: " + sTestFile).getStr(), readExpected(#aActual), OUString::number(aActual)); \
        }
...
#define CPPUNIT_DUMP_ASSERT_DOUBLES_EQUAL(aActual, EPS_) \
    if(isInDumpMode()) \
        ...
        { \
            OString sTestFile = OUStringToOString(getTestFileName(), RTL_TEXTENCODING_UTF8); \
            CPPUNIT_ASSERT_DOUBLES_EQUAL_MESSAGE(OString("Failing test file is: " + sTestFile).getStr(), readExpectedDouble(#aActual), aActual, EPS_); \
        }

#define CPPUNIT_DUMP_ASSERT_STRINGS_EQUAL(aActual) \
    if(isInDumpMode()) \
        writeActual(aActual, OUString(#aActual)); \
    else \
    { \
        OString sTestFile = OUStringToOString(getTestFileName(), RTL_TEXTENCODING_UTF8); \
        CPPUNIT_ASSERT_EQUAL_MESSAGE(OString("Failing test file is: " + sTestFile).getStr(), readExpected(#aActual), aActual); \
    }
...
protected:
    ...
    {
        return OUString("/chart2/qa/extras/chart2dump/reference/") + getTestName().toAsciiLowerCase() + "/";
    }
    ...
    void writeActual(const OUString& sActualValue, const OUString& sCheck)
    {
        ...
        m_aDumpFile << OUStringToOString(sActualValue, RTL_TEXTENCODING_UTF8).getStr() << "\n";      // Write out the checked value, will be used as reference later
    }
    ...
    for (const OUString& sTestFile : aTestFiles)
    {
        ...
        for (const OUString& sAxisShapeName : aAxisShapeNames)
        {
            uno::Reference<drawing::XShape> xXAxis = getShapeByName(xShapes, sAxisShapeName,
                ...
                {
                    uno::Reference<drawing::XShapes> xShapes(rXShape, uno::UNO_QUERY);
                    CPPUNIT_ASSERT(xShapes.is());
                    uno::Reference<drawing::XShape> xChildShape(xShapes->getByIndex(0), uno::UNO_QUERY);
...
DECLARE_DUMP_TEST(ColumnChartTest, Chart2DumpTest, false)
{
    ...
    for (const OUString& sTestFile : aTestFiles)
    {
        ...
        for (size_t nSeries = 0; nSeries < nSeriesCount; ++nSeries)
        {
            ...
            util::Color aSeriesColumnFillColor;
