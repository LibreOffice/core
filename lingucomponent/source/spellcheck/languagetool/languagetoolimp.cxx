...
static PropertyValue lcl_MakePropertyValue(const OUString rName, uno::Any& rValue)
{
    return PropertyValue(rName, -1, rValue, css::beans::PropertyState_DIRECT_VALUE);
}

static Sequence< PropertyValue > lcl_GetLineColorPropertyFromErrorId(const std::string& rErrorId)
{
    uno::Any aColor;
    if (rErrorId == "TYPOS")
    {
        aColor = uno::Any(COL_LIGHTRED);
    }
    else if (rErrorId == "STYLE")
    {
        aColor = uno::Any(COL_LIGHTBLUE);
    }
    else
    {
        // Same color is used for other errorId's such as GRAMMAR, TYPOGRAPHY..
        aColor = uno::Any(COL_ORANGE);
    }
    Sequence< PropertyValue > aProperties {
        lcl_MakePropertyValue("LineColor", aColor)
    };
    return aProperties;
}
...
Sequence< Locale > SAL_CALL LanguageToolGrammarChecker::getLocales()
{
    ...
    auto pArray = m_aSuppLocales.getArray(); // this is the fix
    int i = 0;
...
ProofreadingResult SAL_CALL LanguageToolGrammarChecker::doProofreading( const OUString& aDocumentIdentifier, const OUString& aText, const Locale& aLocale, sal_Int32 nStartOfSentencePosition, sal_Int32 nSuggestedBehindEndOfSentencePosition, const Sequence< PropertyValue >& aProperties )
{
    ...
    xRes.nBehindEndOfSentencePosition = std::min(xRes.nStartOfNextSentencePosition, aText.getLength());
    ...
    const std::string response_body = makeHttpRequest(checkerURL, HTTP_METHOD::HTTP_POST, postData, http_code);
...
void LanguageToolGrammarChecker::parseProofreadingJSONResponse(ProofreadingResult& rResult, std::string_view aJSONBody)
{
    ...
    for (auto it1 = matches.begin(); it1 != matches.end(); it1++, i++)
    {
        ...
        for (auto it2 = replacements.begin(); it2 != replacements.end() && j < MAX_SUGGESTIONS_SIZE; it2++, j++)
        {
            ...
            pSuggestions[j] = OUString(replacementStr.c_str(), replacementStr.length(), RTL_TEXTENCODING_UTF8);
        }
    }
