...
<!--
   ...
   last change: $Author: rt $ $Date: 2005-03-29 12:47:33 $
...
exclude-result-prefixes="aml dt html o ss SL v w10 w wx x">
			...
			<config:config-item-set config:name="view-settings">
				...
				<config:config-item-map-indexed config:name="Views">
					<config:config-item-map-entry>
						...
						<xsl:if test="../ss:Worksheet/x:WorksheetOptions/x:DoNotDisplayGridlines">
	...
	<xsl:template name="font-declaration">
		<office:font-face-decls>
			<xsl:if test="/ss:Workbook/ss:Styles/ss:Style/ss:Font">
				<xsl:for-each select="/ss:Workbook/ss:Styles/ss:Style/ss:Font[
					generate-id(.) =
					generate-id(key('same-named-font', concat(@ss:FontName, @x:Family))[1])]">
						...
						<xsl:if test="@ss:FontName">
							<xsl:attribute name="style:name">
								<xsl:call-template name="encode-as-nc-name">
									<xsl:with-param name="string" select="@ss:FontName" />
								</xsl:call-template>
							</xsl:attribute>
			...
			<xsl:for-each select="/ss:Workbook/ss:Worksheet/ss:Table/ss:Row/ss:Cell/descendant::html:Font[@html:Face][
					generate-id(.) =
					generate-id(key('same-named-face', concat(@html:Face, @x:Family))[1])]">
					<xsl:sort select="name()" />

				<xsl:element name="style:font-face">
					<xsl:attribute name="style:name">
						<xsl:call-template name="encode-as-nc-name">
							<xsl:with-param name="string" select="@html:Face" />
						</xsl:call-template>
					</xsl:attribute>
					<xsl:attribute name="svg:font-family"><xsl:value-of select="@html:Face" /></xsl:attribute>
					<xsl:attribute name="style:font-family-generic"><xsl:call-template name="translate-font-family-generic"><xsl:with-param name="family" select="@x:Family" /></xsl:call-template></xsl:attribute>
				</xsl:element>
			</xsl:for-each>
		</office:font-face-decls>
	</xsl:template>
	...
	<xsl:template match="ss:Styles">
		<office:styles>
			<xsl:apply-templates select="ss:Style[@ss:Name]" />
			<xsl:apply-templates select="ss:Style/ss:NumberFormat[@ss:Format]" />
			<!-- if ConditionalFormatting exists,it should generate some styles for style:style -->
			<xsl:if test="../ss:Worksheet/x:ConditionalFormatting">
				<xsl:call-template name="CondFormat_office_style" />
			</xsl:if>
		</office:styles>
		<office:automatic-styles>
			<xsl:apply-templates select="../ss:Worksheet/ss:Table" />
			<xsl:apply-templates select="ss:Style[not(@ss:Name)]" />
			<xsl:apply-templates select="ss:Style/ss:Font[@ss:VerticalAlign]" />
			<!-- applying to ss:Data (but *, as also ss:Data nested in ss:Comments -->
			<xsl:apply-templates select="../ss:Worksheet/ss:Table/ss:Row/ss:Cell/*[descendant-or-self::*[namespace-uri()='http://www.w3.org/TR/REC-html40']]" />
			<xsl:apply-templates select="../ss:Worksheet/x:WorksheetOptions/x:PageSetup//@x:Data" />
			...
			<xsl:call-template name="create-page-master">
				<xsl:with-param name="worksheetoptions" select="../ss:Worksheet/x:WorksheetOptions" />
			</xsl:call-template>
		</office:automatic-styles>
	...
	<xsl:template name="CondFormat_office_style">
		...
		<xsl:for-each select="../ss:Worksheet/x:ConditionalFormatting">
			...
			<xsl:for-each select="x:Condition">
				...
				<!-- has the default style as parent -->
	...
	<xsl:template name="CondFormat_automatic_style">
		<!-- translate the ConditionalFormatting condition,including map,formalu,etc -->
		<xsl:for-each select="../ss:Worksheet/x:ConditionalFormatting">
			<xsl:variable name="table-pos" select="count(../preceding-sibling::ss:Worksheet)+1" />
			<xsl:variable name="conditions" select="count(preceding-sibling::x:ConditionalFormatting)+1" />
			<!-- has the default style as parent -->
			<xsl:element name="style:style">
				...
				<xsl:for-each select="x:Condition">
					...
					<xsl:element name="style:map">
						...
						<xsl:attribute name="style:base-cell-address"><xsl:value-of select="concat(../../@ss:Name,'.',$base-cell-address)" /></xsl:attribute>
					</xsl:element>
				</xsl:for-each>
			</xsl:element>
		</xsl:for-each>
				...
				<xsl:choose>
					<xsl:when test="$style-type-name='number:text-style'" >
						<xsl:choose>
							<xsl:when test="$isNumberTextElementOpened">
								<number:text-content />
								<number:text><xsl:copy-of select="$numberTextValue" /></number:text>
					...
					<xsl:when test="$style-type-name='number:number-style'">
						<xsl:choose>
							...
							<xsl:otherwise>
								<number:text><xsl:copy-of select="$numberTextValue" /><xsl:call-template name="get-number-text-content">
										<xsl:with-param name="style-type-name" select="$style-type-name" />
										<xsl:with-param name="number-format-unit" select="$number-format-unit" />
										<xsl:with-param name="unit-pos" select="$unit-pos" />
										<xsl:with-param name="condition-pos" select="$condition-pos" />
	...
	<xsl:template name="create-number-format-content">
		...
		<xsl:choose>
			...
			<xsl:when test="starts-with( $posed-number-format-unit, '\') or
							starts-with( $posed-number-format-unit, '*') or
							starts-with( $posed-number-format-unit, '_') or
							starts-with( $posed-number-format-unit, '&quot;')">
				<xsl:call-template name="get-number-text-content">
					<xsl:with-param name="style-type-name" select="$style-type-name" />
					<xsl:with-param name="number-format-unit" select="$number-format-unit" />
					<xsl:with-param name="unit-pos" select="$unit-pos" />
				...
				<xsl:choose>
					<xsl:when test="contains( $valid-number-format-string, '/')">
						<xsl:element name="number:fraction">
							<xsl:attribute name="number:min-integer-digits"><xsl:value-of select="string-length( substring-before($valid-number-format-string, '/') ) - string-length(translate( substring-before($valid-number-format-string, '/'), '0', '') )" /></xsl:attribute>
							<xsl:if test="contains( $valid-number-format-string, ',')">
								<xsl:attribute name="number:grouping">true</xsl:attribute>
							</xsl:if>
							<xsl:attribute name="number:min-numerator-digits"><xsl:value-of select="string-length( substring-before($valid-number-format-string, '/') ) - string-length(translate( substring-before($valid-number-format-string,'/'), '?', '') )" /></xsl:attribute>
							<xsl:attribute name="number:min-denominator-digits"><xsl:value-of select="string-length(substring-after($valid-number-format-string, '/') )" /></xsl:attribute>
					...
					<xsl:when test="contains( $valid-number-format-string, '%')">
						<xsl:element name="number:number">
							<xsl:choose>
								<xsl:when test="contains( $valid-number-format-string, '.')">
									<xsl:attribute name="number:decimal-places"><xsl:value-of select="string-length( substring-before( substring-after( $valid-number-format-string, '.'), '%') ) - string-length( translate( substring-before( substring-after( $valid-number-format-string, '.'), '%'), '0', '') )" /></xsl:attribute>
									<xsl:attribute name="number:min-integer-digits"><xsl:value-of select="string-length( substring-before($valid-number-format-string, '.') ) - string-length(translate( substring-before($valid-number-format-string, '.'), '0', '') )" /></xsl:attribute>
								</xsl:when>
								<xsl:otherwise>
							...
							<xsl:choose>
								<xsl:when test="contains( $valid-number-format-string, ',') and (substring( $valid-number-format-string,string-length($valid-number-format-string)) = ',')">
									...
									<xsl:attribute name="number:display-factor"><xsl:value-of select="$display-factor" /></xsl:attribute>
									<xsl:attribute name="number:grouping">true</xsl:attribute>
								</xsl:when>
								<xsl:when test="contains( $valid-number-format-string, ',')">
									<xsl:attribute name="number:grouping">true</xsl:attribute>
					...
					<xsl:when test="contains( $valid-number-format-string, 'E') or contains ($valid-number-format-string, 'e')">
						<xsl:element name="number:scientific-number">
							<xsl:choose>
								<xsl:when test="contains( $valid-number-format-string, '.')">
									<xsl:attribute name="number:decimal-places"><xsl:value-of select="string-length( substring-before( substring-after( $valid-number-format-string, '.'), 'E') ) - string-length( translate( substring-before( substring-after( $valid-number-format-string, '.'), 'E'), '0', '') )" /></xsl:attribute>
									<xsl:attribute name="number:min-integer-digits"><xsl:value-of select="string-length( substring-before($valid-number-format-string, '.') ) - string-length(translate( substring-before($valid-number-format-string, '.'), '0', '') )" /></xsl:attribute>
								</xsl:when>
								<xsl:otherwise>
									<xsl:attribute name="number:decimal-places">0</xsl:attribute>
									<xsl:attribute name="number:min-integer-digits"><xsl:value-of select="string-length( substring-before($valid-number-format-string, 'E') ) - string-length(translate( substring-before($valid-number-format-string, 'E'), '0', '') )" /></xsl:attribute>
								</xsl:otherwise>
							</xsl:choose>
							<xsl:attribute name="number:min-exponent-digits"><xsl:value-of select="string-length( substring-after( $valid-number-format-string, 'E') ) - string-length( translate( substring-after( $valid-number-format-string, 'E'), '0', '') )" /></xsl:attribute>
							<!-- deal with number:embedded-text -->
							<xsl:call-template name="create-number-format-embedded-text">
								<xsl:with-param name="adapted-number-format-unit" select="$posed-number-format-unit" />
								<xsl:with-param name="valid-number-format-string" select="$valid-number-format-string" />
					...
					<xsl:otherwise>
						...
						<xsl:element name="number:number">
							<xsl:choose>
								<xsl:when test="contains( $valid-number-format-string, '.')">
									<xsl:attribute name="number:decimal-places"><xsl:value-of select="string-length( substring-after( $valid-number-format-string, '.') ) - string-length( translate( substring-after( $valid-number-format-string, '.'), '0', '') )" /></xsl:attribute>
									<xsl:attribute name="number:min-integer-digits"><xsl:value-of select="string-length( substring-before($valid-number-format-string, '.') ) - string-length(translate( substring-before($valid-number-format-string, '.'), '0', '') )" /></xsl:attribute>
								</xsl:when>
								<xsl:otherwise>
									<xsl:attribute name="number:decimal-places">0</xsl:attribute>
									<xsl:attribute name="number:min-integer-digits"><xsl:value-of select="string-length( $valid-number-format-string ) - string-length(translate( $valid-number-format-string, '0', '') )" /></xsl:attribute>
								</xsl:otherwise>
							</xsl:choose>
							<xsl:choose>
								<xsl:when test="contains( $valid-number-format-string, ',') and (substring( $valid-number-format-string,string-length($valid-number-format-string)) = ',')">
			...
			<xsl:when test="starts-with( $posed-number-format-unit, 'ggg')">
				<xsl:element name="number:era">
					...
					<xsl:if test="string-length($calendar-type-name) &gt; 0">
						<xsl:attribute name="number:calendar"><xsl:value-of select="$calendar-type-name" /></xsl:attribute>
					</xsl:if>
				</xsl:element>
				<xsl:call-template name="create-number-format-content">
					<xsl:with-param name="style-type-name" select="$style-type-name" />
			...
			<xsl:when test="starts-with( $posed-number-format-unit, 'ss')">
				...
				<xsl:element name="number:seconds">
					...
					<xsl:if test="$decimal-places &gt; 0">
						<xsl:attribute name="number:decimal-places"><xsl:value-of select="$decimal-places" /></xsl:attribute>
					</xsl:if>
				</xsl:element>
				...
				<xsl:element name="style:page-layout-properties">
					...
					<xsl:choose>
						<xsl:when test="x:PageSetup/x:Layout/@x:StartPageNumber">
							<xsl:attribute name="style:first-page-number"><xsl:value-of select="x:PageSetup/x:Layout/@x:StartPageNumber" /></xsl:attribute>
						</xsl:when>
						<xsl:otherwise>
	...
	<xsl:template name="style-style-content">
		...
		<xsl:element name="style:table-cell-properties">
			<xsl:if test="ss:Alignment">
				...
				<xsl:if test="ss:Alignment/@ss:Indent">
					<xsl:attribute name="fo:padding-left">
						<!-- Indent is ten times of a point -->
			...
			<xsl:if test="ss:Interior">
				<xsl:choose>
					<xsl:when test="ss:Interior/@ss:Pattern = 'Solid'">
						<xsl:if test="ss:Interior/@ss:Color">
							<xsl:attribute name="fo:background-color"><xsl:value-of select="ss:Interior/@ss:Color" /></xsl:attribute>
						</xsl:if>
					</xsl:when>
					<xsl:otherwise>
						<xsl:if test="ss:Interior/@ss:PatternColor">
							<xsl:variable name="pattern-value">
								<xsl:call-template name="cell-pattern-color">
									<xsl:with-param name="pattern" select="concat('0.',substring-after(./ss:Interior/@ss:Pattern,'y'))" />
									<xsl:with-param name="color-value" select="./ss:Interior/@ss:Color" />
									<xsl:with-param name="pattern-color-value" select="./ss:Interior/@ss:PatternColor" />
								</xsl:call-template>
							</xsl:variable>
			...
			<xsl:if test="ss:Font">
				<xsl:choose>
					<xsl:when test="ss:Font/@ss:FontName">
						<xsl:attribute name="style:font-name"><xsl:value-of select="ss:Font/@ss:FontName" /></xsl:attribute>
						<xsl:attribute name="style:font-name-asian"><xsl:value-of select="ss:Font/@ss:FontName" /></xsl:attribute>
						<xsl:attribute name="style:font-name-complex"><xsl:value-of select="ss:Font/@ss:FontName" /></xsl:attribute>
					</xsl:when>
					<xsl:otherwise>
				...
				<xsl:choose>
					<xsl:when test="ss:Font/@ss:Color">
						<xsl:attribute name="fo:color"><xsl:value-of select="ss:Font/@ss:Color" /></xsl:attribute>
					</xsl:when>
					<xsl:otherwise>
				...
				<xsl:choose>
					<xsl:when test="ss:Font/@ss:Size">
						<xsl:attribute name="fo:font-size"><xsl:value-of select="concat( ss:Font/@ss:Size, 'pt')" /></xsl:attribute>
						<xsl:attribute name="style:font-size-asian"><xsl:value-of select="concat( ss:Font/@ss:Size, 'pt')" /></xsl:attribute>
						<xsl:attribute name="style:font-size-complex"><xsl:value-of select="concat( ss:Font/@ss:Size, 'pt')" /></xsl:attribute>
					</xsl:when>
					<xsl:otherwise>
	...
	<xsl:template match="ss:Style">
		...
		<!-- style:default-style is meant for application defaults
			...
			<xsl:otherwise>
				<xsl:element name="style:style">
					<xsl:attribute name="style:name"><xsl:value-of select="@ss:ID" /></xsl:attribute>
					<xsl:if test="@ss:Parent">
						<xsl:attribute name="style:parent-style-name"><xsl:value-of select="@ss:Parent" /></xsl:attribute>
					</xsl:if>
					<xsl:if test="ss:NumberFormat/@ss:Format">
						<xsl:attribute name="style:data-style-name"><xsl:value-of select="concat( @ss:ID, 'F')" /></xsl:attribute>
					</xsl:if>
					<xsl:call-template name="style-style-content" />
				</xsl:element>
			</xsl:otherwise>
	...
	<xsl:template match="ss:Border">
		...
		<xsl:attribute name="{$position}"><xsl:value-of select="concat( $width, ' ', $linestyle, ' ', $color)" /></xsl:attribute>
	...
	<xsl:template match="ss:Table">
		...
		<xsl:if test="ss:Row">
			<xsl:call-template name="get-row-style-name">
				...
				<xsl:with-param name="row-count" select="count(./ss:Row)" />
	...
	<xsl:template name="get-row-style-name">
		...
		<xsl:if test="$currentRow &lt; ($row-count + 1)">
			<xsl:variable name="span-value">
				<xsl:choose>
					<xsl:when test="./ss:Row[position() = $currentRow]/@ss:Index">
						<xsl:choose>
							<xsl:when test="./ss:Row[position() = $currentRow]/@ss:Span">
								<xsl:value-of select="./ss:Row[position() = $currentRow]/@ss:Index - $lastrowpos+ ./ss:Row[position() = $currentRow]/@ss:Span" />
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="0" />
							</xsl:otherwise>
					...
					<xsl:otherwise>
						<xsl:choose>
							<xsl:when test="./ss:Row[position() = $currentRow]/@ss:Span">
								<xsl:value-of select="./ss:Row[position() = $currentRow]/@ss:Span + 1" />
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="0" />
							</xsl:otherwise>
			...
			<xsl:variable name="current-index">
				<xsl:choose>
					<xsl:when test="./ss:Row[position() = $currentRow]/@ss:Index">
						<xsl:value-of select="./ss:Row[position() = $currentRow]/@ss:Index - 1" />
					</xsl:when>
					<xsl:otherwise>
			...
			<xsl:variable name="row-break">
				<xsl:choose>
					...
					<xsl:otherwise>
						<xsl:if test="../x:PageBreaks/x:RowBreaks[(x:RowBreak/x:Row &gt; $lastrowpos) and (x:RowBreak/x:Row &lt; ($lastrowpos + $span-value))]">
							<xsl:value-of select="1" />
						</xsl:if>
					</xsl:otherwise>
				</xsl:choose>
			...
			<xsl:if test="$row-break = 1">
				<style:style style:name="{concat('rob', $table-pos, '-',$currentRow)}" style:family="table-row">
					<xsl:element name="style:table-row-properties">
						<xsl:choose>
							<xsl:when test="./ss:Row[position() = $currentRow]/@ss:Height">
								<xsl:attribute name="style:row-height">
									<xsl:call-template name="convert2cm">
										<xsl:with-param name="value" select="concat(./ss:Row[position() = $currentRow]/@ss:Height,'pt')" />
									</xsl:call-template>
									<xsl:text>cm</xsl:text>
								</xsl:attribute>
							</xsl:when>
						...
						<xsl:choose>
							<xsl:when test="./ss:Row[position() = $currentRow]/@ss:AutoFitHeight = '0'">
								<xsl:attribute name="style:use-optimal-row-height">false</xsl:attribute>
							</xsl:when>
							<xsl:otherwise>
								<xsl:choose>
									<xsl:when test="./ss:Row[position() = $currentRow]/@ss:Height &gt; 0">
										<xsl:attribute name="style:use-optimal-row-height">false</xsl:attribute>
									</xsl:when>
									<xsl:otherwise>
			...
			<style:style style:name="{concat('ro', $table-pos, '-',$currentRow)}" style:family="table-row">
				<xsl:element name="style:table-row-properties">
					...
					<xsl:choose>
						<xsl:when test="./ss:Row[position() = $currentRow]/@ss:AutoFitHeight">
							<xsl:choose>
								<xsl:when test="./ss:Row[position() = $currentRow]/@ss:AutoFitHeight = '0'">
									<xsl:attribute name="style:use-optimal-row-height">false</xsl:attribute>
								</xsl:when>
					...
					<xsl:with-param name="lastrowpos">
						<xsl:choose>
							<xsl:when test="./ss:Row[position() = $currentRow]/@ss:Index">
								<xsl:choose>
									<xsl:when test="./ss:Row[position() = $currentRow]/@ss:Span">
										<xsl:value-of select="./ss:Row[position() = $currentRow]/@ss:Index + ./ss:Row[position() = $currentRow]/@ss:Span" />
									</xsl:when>
									<xsl:otherwise>
										<xsl:value-of select="./ss:Row[position() = $currentRow]/@ss:Index" />
									</xsl:otherwise>
								</xsl:choose>
							</xsl:when>
							<xsl:otherwise>
								<xsl:choose>
									<xsl:when test="./ss:Row[position() = $currentRow]/@ss:Span">
										<xsl:value-of select="$lastrowpos + ./ss:Row[position() = $currentRow]/@ss:Span + 1" />
									</xsl:when>
									<xsl:otherwise>
	...
	<xsl:template match="ss:Worksheet">
		<xsl:element name="table:table">
			<xsl:attribute name="table:name"><xsl:value-of select="@ss:Name" /></xsl:attribute>
			<!-- other attributes aren't suitable to apply yet -->
			<xsl:if test="ss:Table">
				<xsl:attribute name="table:style-name"><xsl:value-of select="concat( 'ta', count(preceding-sibling::ss:Worksheet)+1)" /></xsl:attribute>
				...
				<xsl:if test="descendant::ss:NamedRange[@ss:Name = 'Print_Area' and contains( @ss:RefersTo, '!R')]">
					...
					<xsl:attribute name="table:print-ranges"><xsl:value-of select="translate( $referto, '=', '$')" /></xsl:attribute>
				</xsl:if>
				<xsl:variable name="table-pos">
					<xsl:value-of select="count(../preceding-sibling::ss:Worksheet)+1" />
				</xsl:variable>
				<xsl:choose>
					...
					<xsl:otherwise>
						<!-- No absolute column number (ss:ExpandedColumnCount) is available use the nodes as approximiation -->
						<xsl:call-template name="create-columns">
				...
				<xsl:choose>
					<xsl:when test="./ss:Table/ss:Row">
						<xsl:call-template name="create-rows">
							<xsl:with-param name="lastrowpos" select="0" />
							<xsl:with-param name="row-count" select="count(ss:Table/ss:Row)" />
							<xsl:with-param name="currentRow" select="1" />
					...
					<xsl:otherwise>
						...
						<xsl:choose>
									...
									<xsl:if test="$number-repeated &gt; 0">
										<xsl:element name="table:table-row">
											<xsl:if test="ancestor::ss:Worksheet/ss:Table/@ss:StyleID">
												<xsl:attribute name="table:default-cell-style-name"><xsl:value-of select="ancestor::ss:Worksheet/ss:Table/@ss:StyleID" /></xsl:attribute>
											</xsl:if>
							...
							<xsl:otherwise>
								...
								<xsl:element name="table:table-row">
									<xsl:choose>
										<xsl:when test="ss:Table/@ss:StyleID">
											<xsl:attribute name="table:style-name"><xsl:value-of select="ss:Table/@ss:StyleID" /></xsl:attribute>
										</xsl:when>
										<xsl:otherwise>
						...
						<xsl:if test="./ss:Table/@ss:StyleID">
							<table:table-row table:style-name="ro1" table:number-rows-repeated="32000" table:default-cell-style-name="{./ss:Table/@ss:StyleID}">
								<table:table-cell table:number-columns-repeated="256" />
							</table:table-row>
						</xsl:if>
					</xsl:otherwise>
			...
			<xsl:otherwise>
				<xsl:element name="table:table-column">
					...
					<xsl:if test="ss:Table/@ss:ExpandedColumnCount and ss:Table/@ss:ExpandedColumnCount > 0">
						<xsl:attribute name="table:number-columns-repeated"><xsl:value-of select="ss:Table/@ss:ExpandedColumnCount" /></xsl:attribute>
					</xsl:if>
   ...
   Example:
	...
	First three columns (ss:Index - 1) are written out empty.
	...
	-->
	<xsl:key match="/Workbook/Worksheet/PageBreaks/ColBreaks/ColBreak" name="ColBreak" use="Column" />
	...
	<xsl:template name="create-rows">
		...
		<xsl:choose>
			<xsl:when test="$currentRow &lt; ($row-count + 1)">
				<xsl:variable name="span-value">
					<xsl:choose>
						<xsl:when test="./ss:Table/ss:Row[position() = $currentRow]/@ss:Index">
							<xsl:choose>
								<xsl:when test="./ss:Table/ss:Row[position() = $currentRow]/@ss:Span">
									<xsl:value-of select="./ss:Table/ss:Row[position() = $currentRow]/@ss:Index - $lastrowpos + ./ss:Table/ss:Row[position() = $currentRow]/@ss:Span" />
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="./ss:Table/ss:Row[position() = $currentRow]/@ss:Index - $lastrowpos" />
								</xsl:otherwise>
							</xsl:choose>
						</xsl:when>
						<xsl:otherwise>
							<xsl:choose>
								<xsl:when test="./ss:Table/ss:Row[position() = $currentRow]/@ss:Span">
									<xsl:value-of select="./ss:Table/ss:Row[position() = $currentRow]/@ss:Span + 1" />
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="1" />
								</xsl:otherwise>
				...
				<xsl:variable name="current-index">
					<xsl:choose>
						<xsl:when test="./ss:Table/ss:Row[position() = $currentRow]/@ss:Index">
							<xsl:value-of select="./ss:Table/ss:Row[position() = $currentRow]/@ss:Index" />
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="$lastrowpos + 1" />
						</xsl:otherwise>
						...
						<xsl:with-param name="lastrowpos">
							<xsl:choose>
								<xsl:when test="./ss:Table/ss:Row[position() = $currentRow]/@ss:Index">
									<xsl:choose>
										<xsl:when test="./ss:Table/ss:Row[position() = $currentRow]/@ss:Span">
											<xsl:value-of select="./ss:Table/ss:Row[position() = $currentRow]/@ss:Index + ./ss:Table/ss:Row[position() = $currentRow]/@ss:Span" />
										</xsl:when>
										<xsl:otherwise>
											<xsl:value-of select="./ss:Table/ss:Row[position() = $currentRow]/@ss:Index" />
										</xsl:otherwise>
								...
								<xsl:otherwise>
									<xsl:choose>
										<xsl:when test="./ss:Table/ss:Row[position() = $currentRow]/@ss:Span">
											<xsl:value-of select="$lastrowpos + ./ss:Table/ss:Row[position() = $currentRow]/@ss:Span + 1" />
										</xsl:when>
										<xsl:otherwise>
											<xsl:value-of select="$lastrowpos + 1" />
										</xsl:otherwise>
			...
			<xsl:when test="$currentRow = $row-count">
				...
				<xsl:if test="./x:PageBreaks/x:RowBreaks/x:RowBreak/x:Row &gt; ($last-pos - 1) or ./x:ConditionalFormatting">
					<xsl:variable name="condition-row-max">
						<xsl:call-template name="condition-row-col-pos-max">
							<xsl:with-param name="condition-pos-str" select="$condition-pos-str" />
							<xsl:with-param name="last-value" select="0" />
				...
				<!-- generates some special tags for whole column style. here write a fix number count 32000 for rows -->
				<xsl:if test="./ss:Table/ss:Column[@ss:StyleID] or ./ss:Table[@ss:StyleID]">
	...
	<xsl:template name="create-row">
		<!-- dealing the every row from lastrowpos to Index or ss:Span(if exists) one by one -->
		...
		<xsl:param name="condition-pos-str" />
		<xsl:element name="table:table-row">
			<!-- generates default-cell-style-name tag if appearance of StyleID in tag of Table -->
			<xsl:if test="./ss:Table[@ss:StyleID]">
				<xsl:attribute name="table:default-cell-style-name"><xsl:value-of select="./ss:Table/@ss:StyleID" /></xsl:attribute>
			</xsl:if>
			<xsl:choose>
				<xsl:when test="./x:PageBreaks/x:RowBreaks/x:RowBreak/x:Row = ($index-value + $times - 1)">
					<xsl:choose>
			...
			<xsl:if test="./ss:Table/ss:Row[position() = $row-value]/@ss:Hidden = '1'">
				<xsl:choose>
					<xsl:when test="./ss:Table/ss:Row[position() = $row-value]/@ss:Span">
			...
			<xsl:choose>
				...
				<xsl:when test="not(./ss:Table/ss:Row[position() = $row-value]/*)">
					<!-- XML OASIS does not allow empty rows -->
					<table:table-cell />
				</xsl:when>
				<xsl:otherwise>
					<xsl:choose>
						...
						<xsl:when test="$index-value + $times = $current-index">
							<xsl:apply-templates select="ss:Table/ss:Row[position() = $row-value]" mode="selected">
								<xsl:with-param name="row-pos" select="$index-value + $times" />
								<xsl:with-param name="condition-pos-str" select="$condition-pos-str" />
							</xsl:apply-templates>
						</xsl:when>
						<!-- non empty row with an Index skipping rows -->
						<xsl:otherwise>
							<!-- XML OASIS does not allow empty rows -->
							<table:table-cell />
						</xsl:otherwise>
	...
	<xsl:template name="get-row-beyond-last">
		...
		<xsl:element name="table:table-row">
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<!-- OASIS XML does not allow empty rows -->
					<table:table-cell />
				</xsl:otherwise>
			</xsl:choose>
		</xsl:element>
	...
	<xsl:template name="get-cell-span-in">
		...
		<xsl:variable name="style-name">
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:choose>
						<xsl:when test="../ss:Cell[position() = position() - 1]/@ss:StyleID">
							<xsl:value-of select="../ss:Cell[position() = position() - 1]/@ss:StyleID " />
						</xsl:when>
	...
	<xsl:template name="get-cell-condition-in">
		<!-- generates the style code for current tag of Cell one by one. the attribute name differ with different starts(such as ConditionalFormatting, DataValidation) -->
	...
	<xsl:template match="ss:Row" mode="selected">
		<xsl:param name="row-pos" />
		<xsl:param name="condition-pos-str" />
	...
	<xsl:template match="ss:Cell" mode="selected">
		...
		<xsl:variable name="col-pos">
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:choose>
						<xsl:when test="@ss:MergeAcross">
							<xsl:value-of select="1 + @ss:MergeAcross + $col-pos-before" />
						</xsl:when>
						<xsl:otherwise>
		...
		<xsl:variable name="next-cell" select="following-sibling::ss:Cell[1]" />
		<!-- Multiple empty cells with the same style will be moved to one cell with a 'table:number-columns-repeated' attribute -->
		<xsl:variable name="cell-is-repeatable" select="not(current()/*) and not($next-cell/*) and not($next-cell/text()) and (@ss:StyleID = $next-cell/@ss:StyleID) and not($next-cell/@ss:Index)" />
		<xsl:if test="$col-repeated = 1">
			<!-- In Open Document nothing comparable to Index exists,
			the missing cells have to be created -->
			<xsl:if test="@ss:Index != ($col-pos-before + 1)">
				<xsl:call-template name="get-cell-span-in">
					<xsl:with-param name="row-pos" select="$row-pos" />
			...
			<xsl:element name="table:table-cell">
				<!-- possibly override current column position with new ss:Index -->
			...
			<xsl:if test="@ss:MergeAcross">
				<xsl:element name="table:covered-table-cell">
					<xsl:if test="@ss:MergeAcross &gt; 1">
						<xsl:attribute name="table:number-columns-repeated"><xsl:value-of select="@ss:MergeAcross" /></xsl:attribute>
					</xsl:if>
				</xsl:element>
			</xsl:if>
		</xsl:if>

		<!-- Do not use the following block, if the cell had been repeated earlier -->
		<!--<xsl:if test="not($cell-is-repeatable) or $col-repeated != 1">-->
		<xsl:choose>
	...
	<xsl:template match="ss:Cell" mode="skip">
		...
		<xsl:variable name="next-cell" select="following-sibling::ss:Cell[1]" />
		<!-- Multiple empty cells with the same style will be moved to one cell with a 'table:number-columns-repeated' attribute -->
		<xsl:variable name="cell-is-repeatable" select="not($next-cell/*) and not($next-cell/text()) and (@ss:StyleID = $next-cell/@ss:StyleID) and not($next-cell/@ss:Index)" />
		<xsl:choose>
	...
	<xsl:template name="create-table-cell-attributes">
			...
			<xsl:otherwise>
				<xsl:attribute name="table:style-name">
					<xsl:choose>
						...
						<xsl:when test="../@ss:StyleID and ../@ss:StyleID != ''">
							<xsl:value-of select="../@ss:StyleID" />
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="'Default'" />
		...
		<xsl:if test="@ss:MergeAcross or @ss:MergeDown">
			<xsl:choose>
				<xsl:when test="@ss:MergeAcross">
					<xsl:attribute name="table:number-columns-spanned"><xsl:value-of select="@ss:MergeAcross + 1" /></xsl:attribute>
				</xsl:when>
				<xsl:otherwise>
			...
			<xsl:choose>
				<xsl:when test="@ss:MergeDown">
					<xsl:attribute name="table:number-rows-spanned"><xsl:value-of select="@ss:MergeDown+1" /></xsl:attribute>
				</xsl:when>
				<xsl:otherwise>
		...
		<xsl:if test="ss:Data">
			...
			<xsl:choose>
				<xsl:when test="ss:Data/@ss:Type = 'Number'">
					...
					<xsl:attribute name="office:value"><xsl:value-of select="ss:Data" /></xsl:attribute>
				</xsl:when>
				<xsl:when test="ss:Data/@ss:Type = 'DateTime'">
					<xsl:choose>
						<xsl:when test="(contains( $data-format, 'Date') or contains($data-format,'y') or contains($data-format,'g') or contains($data-format,'d') or contains($data-format,'e') or starts-with( substring( ss:Data, 11), 'T00:00:00.000' ) ) and (not (contains( $data-format, 'Time') ) )">
							...
							<xsl:attribute name="office:date-value"><xsl:value-of select="substring(ss:Data, 1, 19)" /></xsl:attribute>
						</xsl:when>
						<xsl:otherwise>
							<xsl:attribute name="office:value-type">time</xsl:attribute>
							<xsl:attribute name="office:time-value"><xsl:value-of select="concat('P',substring(ss:Data, 11, 3), 'H', substring(ss:Data, 15, 2), 'M', substring(ss:Data, 18,2), ',', substring-after( ss:Data, '.'), '000S')" /></xsl:attribute>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:when>
			...
			<style:style style:name="{concat(ancestor::ss:Cell/@ss:StyleID,'T',count(preceding::ss:Data[child::html:*]), '_', position())}" style:family="text">
				<xsl:element name="style:text-properties">
					<xsl:if test="ancestor-or-self::html:Font/@html:Face">
						<xsl:attribute name="style:font-name"><xsl:value-of select="ancestor-or-self::html:Font/@html:Face" /></xsl:attribute>
					</xsl:if>
					<xsl:if test="ancestor-or-self::html:Font/@html:Size">
						<xsl:attribute name="fo:font-size"><xsl:value-of select="concat(ancestor-or-self::html:Font/@html:Size,'pt')" /></xsl:attribute>
						<xsl:attribute name="style:font-size-asian"><xsl:value-of select="concat(ancestor-or-self::html:Font/@html:Size,'pt')" /></xsl:attribute>
						<xsl:attribute name="style:font-size-complex"><xsl:value-of select="concat(ancestor-or-self::html:Font/@html:Size,'pt')" /></xsl:attribute>
					</xsl:if>
					<xsl:if test="ancestor-or-self::html:Font/@html:Color">
						<xsl:attribute name="fo:color"><xsl:value-of select="ancestor-or-self::html:Font/@html:Color" /></xsl:attribute>
					</xsl:if>
	...
	<xsl:template name="create-header-footer-style">
		...
		<xsl:choose>
			...
			<xsl:when test="starts-with($current-style-data,'&amp;')">
				<xsl:element name="style:style">
					<xsl:attribute name="style:name"><xsl:value-of select="concat($style-name-header,$index)" /></xsl:attribute>
	...
	<xsl:template name="process-header-footer-style-properties">
		...
		<xsl:choose>
			...
			<xsl:when test="starts-with($current-style-data,'&amp;&quot;')">
				<xsl:attribute name="style:font-name"><xsl:value-of select="substring-before(substring-after($current-style-data,'&amp;&quot;'),',')" /></xsl:attribute>
			...
			<xsl:when test="starts-with($current-style-data,'&amp;0') or starts-with($current-style-data,'&amp;1') or starts-with($current-style-data,'&amp;2') or starts-with($current-style-data,'&amp;3') or starts-with($current-style-data,'&amp;4') or starts-with($current-style-data,'&amp;5') or starts-with($current-style-data,'&amp;6') or starts-with($current-style-data,'&amp;7') or starts-with($current-style-data,'&amp;8') or starts-with($current-style-data,'&amp;9')">
				...
				<xsl:attribute name="fo:font-size"><xsl:value-of select="concat(substring($current-style-data,2,$font-size-length),'pt')" /></xsl:attribute>
				<xsl:call-template name="process-header-footer-style-properties">
					<xsl:with-param name="style-data" select="$style-data" />
					<xsl:with-param name="current-pos" select="$current-pos + 1 + $font-size-length" />
				</xsl:call-template>
	...
	<xsl:template name="translate-expression">
		...
		<xsl:param name="cell-row-pos" /> <!-- the position in row (vertical) of cell -->
		<xsl:param name="cell-column-pos" /> <!-- the position in column (horizontal of cell -->
		<xsl:param name="expression" /> <!-- recomposed expression containing cell positions after every conversion -->
			...
			<xsl:when test="$range-type = 1">
				...
				<xsl:call-template name="translate-expression">
					...
					<xsl:with-param name="return-value">
						<!-- join converted string with current being converted cell position string, and return it -->
						<xsl:choose>
	...
	<xsl:template match="ss:Comment" mode="body">
		<xsl:element name="office:annotation">
			...
			<xsl:if test="@ss:Author">
				<xsl:element name="dc:creator"><xsl:value-of select="@ss:Author" /></xsl:element>
			</xsl:if>
			<xsl:if test="ss:Data">
				<text:p>
					<xsl:call-template name="create-data-content">
	...
	<xsl:template name="Names">
		<xsl:if test="//ss:NamedRange">
			<table:named-expressions>
				<xsl:for-each select="//ss:NamedRange">
					<xsl:choose>
						...
						<xsl:otherwise>
							...
							<xsl:element name="table:named-expression">
								...
								<xsl:attribute name="table:base-cell-address">
									<xsl:call-template name="encode-as-cell-address">
										<xsl:with-param name="string" select="'$Sheet1.$A$1'" />
									</xsl:call-template>
								</xsl:attribute>
								<xsl:attribute name="table:expression"><xsl:value-of select="substring( @ss:RefersTo, 2)" /></xsl:attribute>
							</xsl:element>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:for-each>
	...
	<xsl:template name="transform-advanced-filter">
		...
		<xsl:element name="table:database-range">
			...
			<xsl:attribute name="table:target-range-address"><xsl:value-of select="$target-range" /></xsl:attribute>
			<xsl:attribute name="table:name"><xsl:value-of select="concat($target-range, '.filter')" /></xsl:attribute>
			<xsl:element name="table:filter">
				<xsl:attribute name="table:condition-source-range-address"><xsl:value-of select="$condition-range" /></xsl:attribute>
				<xsl:element name="table:filter-condition">
					...
					<xsl:attribute name="table:value"></xsl:attribute>
					<xsl:attribute name="table:operator"></xsl:attribute>
				</xsl:element>
			</xsl:element>
		</xsl:element>
	</xsl:template>
	<xsl:template match="x:AutoFilter">
		<!-- for AutoFilter -->
		<xsl:element name="table:database-range">
			<xsl:attribute name="table:name"><xsl:value-of select="concat(../@ss:Name,'_',@x:Range)" /></xsl:attribute>
			...
			<xsl:attribute name="table:target-range-address"><xsl:value-of select="concat('$',../@ss:Name,'.',$range)" /></xsl:attribute>
			...
			<xsl:element name="table:filter-condition">
				...
				<xsl:choose>
					<xsl:when test="./x:AutoFilterColumn[position() = $item-pos]/@x:Type = 'TopPercent'">
						<xsl:attribute name="table:operator"><xsl:value-of select="'top value'" /></xsl:attribute>
						<xsl:attribute name="table:value"><xsl:value-of select="./x:AutoFilterColumn[position() = $item-pos]/@x:Value" /></xsl:attribute>
					</xsl:when>
					<xsl:when test="./x:AutoFilterColumn[position() = $item-pos]/@x:Type = 'Top'">
						<xsl:attribute name="table:operator"><xsl:value-of select="'top values'" /></xsl:attribute>
						<xsl:attribute name="table:value"><xsl:value-of select="./x:AutoFilterColumn[position() = $item-pos]/@x:Value" /></xsl:attribute>
					</xsl:when>
					<xsl:when test="./x:AutoFilterColumn[position() = $item-pos]/@x:Type = 'TopPercent'">
						<xsl:attribute name="table:operator"><xsl:value-of select="'top percent'" /></xsl:attribute>
						<xsl:attribute name="table:value"><xsl:value-of select="./x:AutoFilterColumn[position() = $item-pos]/@x:Value" /></xsl:attribute>
					</xsl:when>
					<xsl:when test="./x:AutoFilterColumn[position() = $item-pos]/@x:Type = 'Bottom'">
						<xsl:attribute name="table:operator"><xsl:value-of select="'bottom values'" /></xsl:attribute>
						<xsl:attribute name="table:value"><xsl:value-of select="./x:AutoFilterColumn[position() = $item-pos]/@x:Value" /></xsl:attribute>
					</xsl:when>
					<xsl:when test="./x:AutoFilterColumn[position() = $item-pos]/@x:Type = 'BottomPercent'">
						<xsl:attribute name="table:operator"><xsl:value-of select="'bottom percent'" /></xsl:attribute>
						<xsl:attribute name="table:value"><xsl:value-of select="./x:AutoFilterColumn[position() = $item-pos]/@x:Value" /></xsl:attribute>
					</xsl:when>
	...
	<xsl:template match="x:Sorting">
		...
		<xsl:if test="contains(./x:Sort, 'Column')">
			<xsl:element name="table:database-range">
				...
				<xsl:attribute name="table:target-range-address"><xsl:value-of select="concat(../@ss:Name, '.', $left-column, '1:', ../@ss:Name, '.', $right-column, '32000')" /></xsl:attribute>
				<xsl:attribute name="table:name"><xsl:value-of select="concat(../@ss:Name, '.sort')" /></xsl:attribute>
				<xsl:attribute name="table:contains-header"><xsl:value-of select="'false'" /></xsl:attribute>
				<xsl:element name="table:sort">
					<xsl:for-each select="./x:Sort">
						<xsl:element name="table:sort-by">
							<xsl:attribute name="table:field-number"><xsl:value-of select="0" /></xsl:attribute>
							<xsl:attribute name="table:data-type"><xsl:value-of select="'automatic'" /></xsl:attribute>
							<xsl:variable name="after-sort" select="following-sibling::*" />
							<xsl:if test="name($after-sort[position() = 1]) = 'Descending'">
								<xsl:attribute name="table:order"><xsl:value-of select="'descending'" /></xsl:attribute>
							</xsl:if>
							<xsl:choose>
								<xsl:when test="position() = 1">
									<xsl:attribute name="table:field-number"><xsl:value-of select="$first-sort-num - $min-left-num" /></xsl:attribute>
								</xsl:when>
								<xsl:when test="position() = 2">
									<xsl:attribute name="table:field-number"><xsl:value-of select="$second-sort-num - $min-left-num" /></xsl:attribute>
								</xsl:when>
								<xsl:when test="position() = 3">
									<xsl:attribute name="table:field-number"><xsl:value-of select="$third-sort-num - $min-left-num" /></xsl:attribute>
								</xsl:when>
							</xsl:choose>
						</xsl:element>
					</xsl:for-each>
		...
		<xsl:element name="table:calculation-settings">
			...
			<xsl:element name="table:iteration">
				...
				<xsl:if test="/ss:Workbook/x:ExcelWorkbook/x:MaxIterations">
					<xsl:attribute name="table:steps"><xsl:value-of select="/ss:Workbook/x:ExcelWorkbook/x:MaxIterations" /></xsl:attribute>
				</xsl:if>
				<xsl:if test="/ss:Workbook/x:ExcelWorkbook/x:MaxChange">
					<xsl:attribute name="table:maximum-difference"><xsl:value-of select="/ss:Workbook/x:ExcelWorkbook/x:MaxChange" /></xsl:attribute>
				</xsl:if>
			</xsl:element>
		</xsl:element>
	</xsl:template>
	<xsl:template match="x:DataValidation">
		...
		<xsl:element name="table:content-validation">
			...
			<xsl:element name="table:help-message">
				<xsl:attribute name="table:title"><xsl:value-of select="./x:InputTitle" /></xsl:attribute>
				<xsl:attribute name="table:display"><xsl:choose><xsl:when test="./x:InputHide"><xsl:value-of select="'false'" /></xsl:when><xsl:otherwise><xsl:value-of select="'true'" /></xsl:otherwise></xsl:choose></xsl:attribute>
				<xsl:element name="text:p">
					<xsl:value-of select="./x:InputMessage" />
				</xsl:element>
			</xsl:element>
			<xsl:element name="table:error-message">
				<xsl:attribute name="table:message-type"><xsl:choose><xsl:when test="./x:ErrorStyle= 'Info'"><xsl:value-of select="'information'" /></xsl:when><xsl:when test="./x:ErrorStyle= 'Warn'"><xsl:value-of select="'warning'" /></xsl:when><xsl:otherwise><xsl:value-of select="'stop'" /></xsl:otherwise></xsl:choose></xsl:attribute>
				<xsl:attribute name="table:title"><xsl:value-of select="./x:ErrorTitle" /></xsl:attribute>
				<xsl:attribute name="table:display"><xsl:choose><xsl:when test="./x:ErrorHide"><xsl:value-of select="'false'" /></xsl:when><xsl:otherwise><xsl:value-of select="'true'" /></xsl:otherwise></xsl:choose></xsl:attribute>
				<xsl:element name="text:p">
