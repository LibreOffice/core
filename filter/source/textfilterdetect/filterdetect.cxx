...
bool IsHTMLStream( const uno::Reference<io::XInputStream>& xInStream )
{
    ...
    if ( GetHTMLToken( OStringToOUString( sHeader.copy( nStartOfTagIndex, i - nStartOfTagIndex ),
...
OUString SAL_CALL PlainTextFilterDetect::detect(uno::Sequence<beans::PropertyValue>& lDescriptor) throw (uno::RuntimeException)
{
    ...
    {
        if (aDocService == CALC_DOCSERVICE)
            aMediaDesc[utl::MediaDescriptor::PROP_FILTERNAME()] <<= OUString(CALC_TEXT_FILTER);
        else if (aDocService == WRITER_DOCSERVICE)
            aMediaDesc[utl::MediaDescriptor::PROP_FILTERNAME()] <<= OUString(WRITER_TEXT_FILTER);
