...
sal_uInt16 SdrEdgeObj::ImpCalcEscAngle(SdrObject* pObj, const Point& rPt) const
{
    ...
    if (bxMitt && byMitt) return SDRESC_ALL; 
    if (bDiag) {  
        sal_uInt16 nRet=0;
...
XPolygon SdrEdgeObj::ImpCalcObjToCenter(const Point& rStPt, long nEscAngle, const Rectangle& rRect, const Point& rMeeting) const
{
    ...
    Point aP3(aP2);
    if ((bLks && rMeeting.X()>aP2.X()) || (bRts && rMeeting.X()<aP2.X())) { 
        if (rMeeting.Y()<aP2.Y()) {
            aP3.Y()=rRect.Top();
    ...
    if ((bObn && rMeeting.Y()>aP2.Y()) || (bUnt && rMeeting.Y()<aP2.Y())) { 
        if (rMeeting.X()<aP2.X()) {
            aP3.X()=rRect.Left();
    ...
    sal_uIntPtr* pnQuality, SdrEdgeInfoRec* pInfo) const
{
    ...
    if (eKind==SDREDGE_ONELINE) {
        ...
        aXP[2]=rPt2;
        aXP[3]=rPt2;
        if (bRts1) aXP[1].X()=aBewareRect1.Right();  
        if (bObn1) aXP[1].Y()=aBewareRect1.Top();    
        if (bLks1) aXP[1].X()=aBewareRect1.Left();   
        if (bUnt1) aXP[1].Y()=aBewareRect1.Bottom(); 
        if (bRts2) aXP[2].X()=aBewareRect2.Right();  
        if (bObn2) aXP[2].Y()=aBewareRect2.Top();    
        if (bLks2) aXP[2].X()=aBewareRect2.Left();   
        if (bUnt2) aXP[2].Y()=aBewareRect2.Bottom(); 
        if (pnQuality!=NULL) {
    ...
    sal_uInt16 nIntersections=0;
    {
        ...
        if (nMainCase==1) { 
            if (bVer1) aMeeting.X()=(aPt1.X()+aPt2.X()+1)/2; 
            if (bHor1) aMeeting.Y()=(aPt1.Y()+aPt2.Y()+1)/2; 
                    ...
                    if (aBewR1.Right()>aBewR2.Left()) {
                        ...
                        */
                        ...
                        if (!bCase29Direct) {
                            ...
                            long nWant1=aBewR1.Right()-aBndR1.Right(); 
                            long nWant2=aBndR2.Left()-aBewR2.Left();   
                            long nSpace=aBndR2.Left()-aBndR1.Right(); 
                    ...
                    if (aBewR1.Bottom()>aBewR2.Top()) {
                        ...
                        */
                        ...
                        if (!bCase29Direct) {
                            ...
                            long nWant1=aBewR1.Bottom()-aBndR1.Bottom(); 
                            long nWant2=aBndR2.Top()-aBewR2.Top();   
                            long nSpace=aBndR2.Top()-aBndR1.Bottom(); 
                            long nGet1=BigMulDiv(nWant1,nSpace,nWant1+nWant2);
                            long nGet2=nSpace-nGet1;
                            if (bUnt1) { 
    ...
    bool bHorzE1=aEP1.Y()==aXP1[nXP1Anz-2].Y(); 
    bool bHorzE2=aEP2.Y()==aXP2[nXP2Anz-2].Y(); 
