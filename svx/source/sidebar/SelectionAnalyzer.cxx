...
EnumContext::Context SelectionAnalyzer::GetContextForSelection_SC (const SdrObjectVector& rSelection)
{
    EnumContext::Context eContext = EnumContext::Context_Unknown;

    switch (rSelection.size())
    {
        case 0:
        ...
        {
            // Multi selection.
            switch (GetInventorTypeFromSelection(rSelection))
            {
                case SdrInventor:
                {
                    const sal_uInt16 nObjId (GetObjectTypeFromSelection(rSelection));
                    if (nObjId == 0)
...
sal_uInt32 SelectionAnalyzer::GetInventorTypeFromSelection (const SdrObjectVector& rSelection)
{
    const sal_uLong nMarkCount (rSelection.size());
...
sal_uInt16  SelectionAnalyzer::GetObjectTypeFromSelection (const SdrObjectVector& rSelection)
{
    const sal_uLong nMarkCount (rSelection.size());
