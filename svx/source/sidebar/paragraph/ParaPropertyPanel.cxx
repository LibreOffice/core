...
void ParaPropertyPanel::VertStateChanged(sal_uInt16 nSID, SfxItemState eState, const SfxPoolItem* pState)
{
    ...
    else
    {
        ...
        if(eState >= SFX_ITEM_DEFAULT && dynamic_cast< const SfxBoolItem*>(pState))
        {
            const bool aBool = static_cast< const SfxBoolItem*>(pState)->GetValue();
...
{
    if(eState >= SFX_ITEM_DEFAULT && dynamic_cast< const SvxColorItem* >(pState))
    {
...
{
    if(eState >= SFX_ITEM_DEFAULT && dynamic_cast< const SfxUInt16Item* >(pState))
    {
        sal_uInt16 nValue = static_cast< const SfxUInt16Item* >(pState)->GetValue();
