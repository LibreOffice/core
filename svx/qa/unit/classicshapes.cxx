...
CPPUNIT_TEST_FIXTURE(ClassicshapesTest, testTdf130076Flip)
{
    ...
    for (sal_uInt8 nPageIndex = 0; nPageIndex < 2; ++nPageIndex)
    {
        ...
        CPPUNIT_ASSERT_MESSAGE("Could not get the shape properties", xShapeProps.is());
        CPPUNIT_ASSERT_MESSAGE("Could not get the shape properties", xShapeProps2.is());
        if (angle1 != goodAngle1)
        {
            sErrors += "page " + OUString::number(nPageIndex) + " expected vertical flip starting angle "
                       + OUString::number(goodAngle1) + " actual " + OUString::number(angle1) + "\n";
        }
        if (angle2 != goodAngle2)
        {
            sErrors += "page " + OUString::number(nPageIndex) + " expected horizontal flip starting angle "
                       + OUString::number(goodAngle2) + " actual " + OUString::number(angle2) + "\n";
        }
    }
    CPPUNIT_ASSERT_EQUAL(OUString(), sErrors);
}
