...
void GalleryObjTest::TestCreateTheme() {
    ...
    comphelper::DirectoryHelper helperObj;
    CPPUNIT_ASSERT_MESSAGE("Could not create temporary directory", helperObj.dirExists( aGalleryURL ) );
    ...
    CPPUNIT_ASSERT_MESSAGE("Could not find .thm file inside it", helperObj.fileExists( aGalleryURL + "/" +  myThemeName + ".thm" ) );
    CPPUNIT_ASSERT_MESSAGE("Could not find .sdv file inside it", helperObj.fileExists( aGalleryURL + "/" +  myThemeName + ".sdv" ) );
}
...
void GalleryObjTest::TestDeleteTheme() {
    ...
    CPPUNIT_ASSERT_MESSAGE("Could not remove theme, theme found even after trying to remove", !pGallery->HasTheme(myThemeName) );

    // Check that files do not exist
    comphelper::DirectoryHelper helperObj;
    CPPUNIT_ASSERT_MESSAGE("Found .thm file inside it after deletion", !helperObj.fileExists(aGalleryURL + "/" +  myThemeName + ".thm") );
    CPPUNIT_ASSERT_MESSAGE("Found .sdv file inside it after deletion", !helperObj.fileExists(aGalleryURL + "/" +  myThemeName + ".sdv") );
    CPPUNIT_ASSERT_MESSAGE("Found .sdg file inside it after deletion", !helperObj.fileExists(aGalleryURL + "/" +  myThemeName + ".sdg") );
    CPPUNIT_ASSERT_MESSAGE("Found .str file inside it after deletion", !helperObj.fileExists(aGalleryURL + "/" +  myThemeName + ".str") );
}
...
void GalleryObjTest::TestSetThemeName() {
    ...
    CPPUNIT_ASSERT_MESSAGE( "Could not rename theme because old theme name still exists", !pGallery->HasTheme( myThemeName ) );
