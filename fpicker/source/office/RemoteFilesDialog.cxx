...
RemoteFilesDialog::RemoteFilesDialog( weld::Window* pParent, PickerFlags nBits )
    ...
    , m_xAddService_btn(m_xBuilder->weld_menu_button("add_service_btn"))
    ...
    , m_xListView_btn(m_xBuilder->weld_button("list_view"))
    , m_xIconView_btn(m_xBuilder->weld_button("icon_view"))
    , m_xFilter_lb(m_xBuilder->weld_combo_box("filter_lb"))
    , m_xName_ed(new AutocompleteEdit(m_xBuilder->weld_entry("filename_container")))
{
    ...
    else
    {
        ...
        m_xNewFolder->set_from_icon_name(BMP_FILEDLG_CREATEFOLDER);
...
//TODO    m_xAddService_btn->SetDelayMenu(true);
//TODO    m_xAddService_btn->SetDropDown(PushButtonDropdownStyle::SplitMenuButton);

    m_xAddService_btn->connect_clicked( LINK( this, RemoteFilesDialog, AddServiceHdl ) );
    m_xAddService_btn->connect_selected( LINK( this, RemoteFilesDialog, EditServiceMenuHdl ) );
...
{
    m_xFileView->SetSelectHdl(Link<weld::TreeView&,void>());
...
void RemoteFilesDialog::Resize()
{
    ...
    if( m_xFileView && m_xContainer )
    {
        Size aSize = m_xContainer->GetSizePixel();
        m_xFileView->SetSizePixel( aSize );
    }
...
//TODO        m_xAddService_btn->SetPopupMenu( nullptr );
    }
...
//TODO        m_xAddService_btn->SetPopupMenu( m_pAddMenu );
    }
#if 0
    else
        m_xAddService_btn->SetPopupMenu( nullptr );
#endif
...
void RemoteFilesDialog::EnableControls()
{
    if (m_xServices_lb->get_count() > 0)
    {
        ...
        {
            m_xAddService_btn->set_item_sensitive("change_password", false);
            ...
            try
            {
                if( m_xMasterPasswd->isPersistentStoringAllowed() )
                {
                    ...
                    if( nPos >= 0 )
                    {
                        ...
                        {
                            m_xAddService_btn->set_item_sensitive("change_password", true);
                        }
                    }
                }
            }
    ...
    m_xAddService_btn->set_sensitive(true);

//TODO    Invalidate(InvalidateFlags::Update);
}
...
void RemoteFilesDialog::DisableControls()
{
    m_xServices_lb->set_sensitive(false);
    m_xFilter_lb->set_sensitive(false);
    m_xAddService_btn->set_sensitive(false);
    m_xName_ed->set_sensitive(false);
    m_xContainer->set_sensitive(false);
    m_xOk_btn->set_sensitive(false);
...
IMPL_LINK_NOARG ( RemoteFilesDialog, AddServiceHdl, weld::Button&, void )
{
...
IMPL_LINK_NOARG( RemoteFilesDialog, DoubleClickHdl, weld::TreeView&, bool )
{
    if( m_xFileView->GetSelectionCount() )
    {
        SvTreeListEntry* pEntry = m_xFileView->FirstSelected();
...
IMPL_LINK_NOARG( RemoteFilesDialog, SelectHdl, weld::TreeView&, void )
{
    SvTreeListEntry* pEntry = m_xFileView->FirstSelected();
...
std::vector<OUString> RemoteFilesDialog::GetPathList() const
{
    ...
    sal_uLong nCount = m_xFileView->GetSelectionCount();
    SvTreeListEntry* pEntry = nCount ? m_xFileView->FirstSelected() : nullptr;
    ...
    while( pEntry )
    {
        ...
        pEntry = m_xFileView->NextSelected( pEntry );
    }
