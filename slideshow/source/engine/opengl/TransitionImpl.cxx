...
constexpr auto initPermutation(int y)
{
    ...
    for (int x = 0; x < 256; x++)
        a[x] = permutation256[(y + permutation256[x]) & 0xff];
    return a;
}

template <int... Is> constexpr auto initPermutationsTable(std::integer_sequence<int, Is...>)
{
    return std::array{ initPermutation(Is)... };
}

constexpr auto permutation2D = initPermutationsTable(std::make_integer_sequence<int, 256>{});
...
void initPermTexture(GLuint *texID)
{
    ...
    glTexImage2D( GL_TEXTURE_2D, 0, GL_RGBA, 256, 256, 0, GL_RED, GL_UNSIGNED_BYTE, permutation2D.data() );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST );
    CHECK_GL_ERROR();
}
