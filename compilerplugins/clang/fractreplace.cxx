...
void FractReplace::foundTypeFraction(const SourceLocation &begin,
    ...
    if (rewriter == nullptr || flags & Flag_EnableWarningsWhenRewrite) {
        report(DiagnosticsEngine::Warning,
               "need replace " + nametype + " with " + newtype + " [" + from +
                   "]",
               begin)
            << range;
    }
...
void FractReplace::foundMethod_GetNumerator(const std::string name,
    ...
    if (rewriter == nullptr || flags & Flag_EnableWarningsWhenRewrite)
        report(DiagnosticsEngine::Warning,
               "replace '" + name + "' method with numerator()", begin)
            << range;
    if (rewriter != nullptr)
...
void FractReplace::foundMethod_GetDenominator(const std::string name,
    ...
    if (rewriter == nullptr || flags & Flag_EnableWarningsWhenRewrite)
        report(DiagnosticsEngine::Warning,
               "replace '" + name + "' method with denominator()", begin)
            << range;
    if (rewriter != nullptr)
...
void FractReplace::foundMethod_IsValid(const std::string name,
                                       const SourceLocation &begin,
                                       const SourceRange &range) {
    // FIXME: this method need replace the variable.method() expression
    if (rewriter == nullptr || flags & Flag_EnableWarningsWhenRewrite)
        report(DiagnosticsEngine::Warning,
               "manual change is needed for replace 'IsValid' method - "
               "boost::rational<long> always is valid",
               begin)
            << range;
    if (rewriter != nullptr)
        replaceText(range, " true /* LOPLUGIN:FRACTREPLACE manual change is "
                           "needed for replace 'IsValid' method - "
                           "boost::rational<long> always is valid */ ");
}
