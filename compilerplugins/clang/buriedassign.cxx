...
public:
    ...
    virtual void run() override
    {
        ...
        if (fn == SRCDIR "/sal/rtl/bootstrap.cxx")
            return;
        if (fn == SRCDIR "/sal/rtl/digest.cxx")
            return;
        ...
        if (fn == SRCDIR "/sal/osl/unx/process.cxx")
            return;
        if (fn == SRCDIR "/registry/tools/options.cxx")
            return;
        if (fn == SRCDIR "/i18npool/source/textconversion/genconv_dict.cxx")
            return;
        if (fn == SRCDIR "/soltools/cpp/_eval.c")
            return;
...
bool BuriedAssign::VisitBinaryOperator(BinaryOperator const* binaryOp)
{
    ...
    {
        // Ignore chained normal assignment. Chained other kind of assignment is icky to read.
        if (binaryOp->getOpcode() == BO_Assign && rhs->getOpcode() == BO_Assign)
...
bool BuriedAssign::VisitCXXOperatorCallExpr(CXXOperatorCallExpr const* cxxOper)
{
    ...
    {
        // Ignore chained normal assignment. Chained other kind of assignment is icky to read.
        if (cxxOper->getOperator() == OO_Equal && rhs->getOperator() == OO_Equal)
            m_handled.insert(rhs);
    }
