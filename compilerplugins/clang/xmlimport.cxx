        ...
        for (auto it = cxxRecordDecl->ctor_begin(); it != cxxRecordDecl->ctor_end(); ++it)
        {
            ...
            for (auto paramIt = ctor->param_begin(); paramIt != ctor->param_end(); ++paramIt)
            {
                ...
                if (tc.Typedef("sal_uInt16"))
                    ...
                    foundInt16 = false;
                if (tc.LvalueReference().Const().Class("OUString") && pvd->getName() == "rLocalName")
                    foundConstructor = true;
            }
        }

        if (!foundConstructor)
            report(DiagnosticsEngine::Warning, "overrides startElement, but looks like a fastparser context class, no constructor that takes slowparser args",
                   compat::getBeginLoc(methodDecl))
                << methodDecl->getSourceRange();
    }
