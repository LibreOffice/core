...
private:
    ...
    typedef std::map<std::pair<VarDecl const*, QualType>, SourceLocation> MapType;
...
bool StaticDynamic::VisitCXXStaticCastExpr(CXXStaticCastExpr const* staticCastExpr)
{
    ...
    if (!varDecl)
        return true;
    staticCastVars.insert(
        { { varDecl, staticCastExpr->getTypeAsWritten() }, compat::getBeginLoc(staticCastExpr) });
    return true;
}
...
bool StaticDynamic::VisitCXXDynamicCastExpr(CXXDynamicCastExpr const* dynamicCastExpr)
{
    ...
    if (!varDecl)
        return true;
    auto it = staticCastVars.find({ varDecl, dynamicCastExpr->getTypeAsWritten() });
