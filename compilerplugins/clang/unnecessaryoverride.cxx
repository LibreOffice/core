...
public:
    ...
    virtual void run() override
    {
        ...
        if (strstr(dirname, "sd/source/ui/framework/factories") && strstr(fe->getName(), "ChildWindowPane.cxx"))
            return;
        if (strstr(dirname, "forms/source/component") && strstr(fe->getName(), "Date.cxx"))
            return;
        if (strstr(dirname, "forms/source/component") && strstr(fe->getName(), "Time.cxx"))
            return;
...
bool UnnecessaryOverride::VisitCXXMethodDecl(const CXXMethodDecl* methodDecl)
{
    ...
    if (aFileName.startswith(SRCDIR "/comphelper/source/property/propertycontainer.cxx"))
        return true;
    // not sure what is happening here
    if (aFileName.startswith(SRCDIR "/extensions/source/bibliography/datman.cxx"))
        return true;

    const CXXMethodDecl* overriddenMethodDecl = *methodDecl->begin_overridden_methods();
    if (methodDecl->getAccess() != overriddenMethodDecl->getAccess())
        return true;
