...
public:
    ...
    bool VisitCXXFunctionalCastExpr(CXXFunctionalCastExpr const* expr)
    {
        ...
        if (isa<InitListExpr>(expr->getSubExpr()))
            return true;
        if (isa<CXXStdInitializerListExpr>(expr->getSubExpr()))
            return true;
        ...
        if (t1->isRecordType())
        {
            auto recordDecl = dyn_cast_or_null<CXXRecordDecl>(t1->getAs<RecordType>()->getDecl());
            if (recordDecl && recordDecl->hasTrivialDestructor())
                ok = true;
        }
        if (!ok && t1.isCXX11PODType(compiler.getASTContext()))
            ok = true;
        if (!ok)
        {
