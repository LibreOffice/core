...
{
    return !type->getAs<TypedefType>() // ignore sal_Int64
           && type->isSpecificBuiltinType(BuiltinType::Kind::Long);
}

bool isLongArray(QualType type)
{
    ...
    auto const k = isLong(t->getElementType());
    if (k)
    {
        return k;
    }
    return isLongArray(t->getElementType());
}
...
void ToolsLong::run()
{
    ...
    for (auto const& dcl : parmVarDecls_)
    {
        ...
        if (!((compiler.getSourceManager().isInMainFile(compiler.getSourceManager().getSpellingLoc(
                   dyn_cast<FunctionDecl>(decl->getDeclContext())->getNameInfo().getLoc()))
...
bool ToolsLong::VisitParmVarDecl(ParmVarDecl const* decl)
{
    ...
    if (isExcludedFile(compiler.getSourceManager().getSpellingLoc(decl->getLocation())))
        return true;
    auto const fbk = isLong(decl->getType().getNonReferenceType());
    if (fbk)
    {
...
bool ToolsLong::VisitFunctionDecl(FunctionDecl const* decl)
{
    ...
    if (isExcludedFile(compiler.getSourceManager().getSpellingLoc(decl->getLocation())))
        return true;
    auto const fbk = isLong(decl->getReturnType().getNonReferenceType());
    if (fbk && !(decl->isDeletedAsWritten() && isa<CXXConversionDecl>(decl)))
    {
...
bool ToolsLong::isExcludedFile(SourceLocation spellingLocation) const
{
    ...
    return loplugin::isSamePathname(f, SRCDIR "/include/cppu/")
           || loplugin::isSamePathname(f, SRCDIR "/include/cppuhelper/")
           || loplugin::isSamePathname(f, SRCDIR "/include/osl/")
           || loplugin::isSamePathname(f, SRCDIR "/include/rtl/")
           || loplugin::isSamePathname(f, SRCDIR "/include/sal/")
           || loplugin::isSamePathname(f, SRCDIR "/include/salhelper/")
           || loplugin::isSamePathname(f, SRCDIR "/include/typelib/")
           || loplugin::isSamePathname(f, SRCDIR "/bridges/")
           || loplugin::isSamePathname(f, SRCDIR "/cppu/")
           || loplugin::isSamePathname(f, SRCDIR "/cppuhelper/")
           || loplugin::isSamePathname(f, SRCDIR "/rtl/")
           || loplugin::isSamePathname(f, SRCDIR "/sal/")
           || loplugin::isSamePathname(f, SRCDIR "/salhelper/");
}

loplugin::Plugin::Registration<ToolsLong> X("toolslong", false);
}
