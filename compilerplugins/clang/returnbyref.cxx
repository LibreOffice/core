...
bool ReturnByRef::VisitCXXMethodDecl(const CXXMethodDecl * functionDecl) {
    ...
    */
    ...
    const Stmt* nextStmt = *returnStmt->child_begin();
    if (dyn_cast<ParenExpr>(nextStmt) != nullptr) {
        nextStmt = *nextStmt->child_begin();
    }
    ...
    nextStmt = *unaryOperator->child_begin();
    if (dyn_cast<ParenExpr>(nextStmt) != nullptr) {
        nextStmt = *nextStmt->child_begin();
    }
    ...
    nextStmt = *memberExpr->child_begin();
    if (dyn_cast<ParenExpr>(nextStmt) != nullptr) {
        nextStmt = *nextStmt->child_begin();
    }
