...
{
    if (BaseDefinition && loplugin::TypeCheck(BaseDefinition).Class("SfxPoolItem")) {
        return false;
    }
    return true;
}
...
bool isDerivedFromSfxPoolItem(const CXXRecordDecl *decl) {
    if (!decl)
        return false;
    if (loplugin::TypeCheck(decl).Class("SfxPoolItem"))
        return true;
...
bool isDerivedFromSwMsgPoolItem(const CXXRecordDecl *decl) {
    if (!decl)
        return false;
    if (loplugin::TypeCheck(decl).Class("SwMsgPoolItem"))
        return true;
    if (!decl->hasDefinition()) {
        return false;
    }
...
bool SfxPoolItem::VisitCXXRecordDecl(const CXXRecordDecl* decl)
{
    ...
    if (tc.Class("SfxEnumItem") || tc.Class("SfxAllEnumItem"))
        return true;

    // the new field is only used for reading and writing to storage
    if (tc.Class("SvxCharSetColorItem"))
        return true;
