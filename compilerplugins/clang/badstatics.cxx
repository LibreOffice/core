...
public:
    ...
    bool VisitVarDecl(VarDecl const*const pVarDecl)
    {
        ...
        if (pVarDecl->hasGlobalStorage()) {
            auto const type(pVarDecl->getType().getUnqualifiedType().getCanonicalType());
            if (   type.getAsString() == "class Image"
                || type.getAsString() == "class Bitmap"
                || type.getAsString() == "class BitmapEx"
               )
            {
