    ...
    )
{
    if (BaseDefinition) {
        ...
        if (tc.Class("Dialog") || tc.Class("SfxPoolItem")) {
            return false;
        }
    }
    return true;
...
bool UnusedVariableCheck::VisitVarDecl( const VarDecl* var )
    ...
    if( CXXRecordDecl* type = var->getType()->getAsCXXRecordDecl())
        ...
        if( !warn_unused )
            ...
            if( tc.Class("string").Namespace("std")
                || tc.Class("basic_string").Namespace("std")
                || tc.Class("list").Namespace("std")
                || tc.Class("list").Namespace("__debug").Namespace("std")
                || tc.Class("vector").Namespace("std")
