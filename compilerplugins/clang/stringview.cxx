...
/**
    ...
    a string_view.
    TODO handle OString/OUStringBuffer/OStringBuffer similarly
*/
...
bool StringView::VisitCallExpr(CallExpr const* callExpr)
{
    ...
    for (unsigned i = 0; i < len; ++i)
    {
        ...
        if (!loplugin::TypeCheck(paramDecl->getType()).Typedef("u16string_view").StdNamespace())
            continue;
        ...
        if (memberCallExpr2->getMethodDecl()->getName() != "copy")
            continue;
        report(DiagnosticsEngine::Warning, "rather than copy, pass with a view using substr()",
