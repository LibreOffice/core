...
static bool containsStaticTypeMethod(const CXXRecordDecl* x)
{
    for (auto it = x->method_begin(); it != x->method_end(); ++it) {
        auto i = *it;
        if ( i->isStatic() && i->getIdentifier() && i->getIdentifier()->isStr("static_type") ) {
            return true;
        }
    }
    return false;
...
{
    qt = qt.getDesugaredType(compiler.getASTContext()); // because some places use typedefs around uno::Reference
    if (loplugin::TypeCheck(qt).Class("Reference").Namespace("uno").Namespace("star").Namespace("sun").Namespace("com").GlobalNamespace()) {
        ...
        const CXXRecordDecl* x = rArg.getAsType()->getAsCXXRecordDecl()->getDefinition();
        if (x && x->isThisDeclarationADefinition()) {
            if (!containsStaticTypeMethod(x)) {
                report(
