...
bool ComparisonWithConstant::rewrite(const BinaryOperator * binaryOp) {
    ...
    // invert the operator
    StringRef originalOpcodeStr = binaryOp->getOpcodeStr();
    StringRef replaceOpcodeStr;
    if (originalOpcodeStr == "==" || originalOpcodeStr == "!=") {
        replaceOpcodeStr = originalOpcodeStr;
    }
    else if (originalOpcodeStr == ">") {
        replaceOpcodeStr = "<";
    }
    else if (originalOpcodeStr == "<") {
        replaceOpcodeStr = ">";
        ...
        throw originalOpcodeStr;
    }
    unsigned n = Lexer::MeasureTokenLength( binaryOp->getOperatorLoc(), compiler.getSourceManager(), compiler.getLangOpts());
    if (!replaceText(binaryOp->getOperatorLoc(), n, replaceOpcodeStr)) {
        return false;
    }
    return true;
}
