...
class SimplifyDynamicCast:
    public RecursiveASTVisitor<SimplifyDynamicCast>, public loplugin::Plugin
{
public:
    explicit SimplifyDynamicCast(loplugin::InstantiationData const & data): Plugin(data) {}
    ...
    if (!dynamicCastExpr)
    {
        if (auto binaryOp = dyn_cast<BinaryOperator>(condExpr))
        {
            if (binaryOp->getOpcode() == BO_NE)
                dynamicCastExpr = dyn_cast<CXXDynamicCastExpr>(binaryOp->getLHS()->IgnoreParenImpCasts());
        }
    }
    Decl const * subExprDecl = nullptr;
    if (dynamicCastExpr)
    {
        auto subExprDeclRefExpr = dyn_cast<DeclRefExpr>(dynamicCastExpr->getSubExpr()->IgnoreParenImpCasts());
...
bool SimplifyDynamicCast::VisitCXXStaticCastExpr(CXXStaticCastExpr const * staticCastExpr)
{
    ...
    if (dynamicCastSubExprVec[idx] != subExprDecl->getDecl())
        return true;
    report(
        DiagnosticsEngine::Warning, "simplify, use var in if",
        staticCastExpr->getLocStart())
        << staticCastExpr->getSourceRange();
    auto ifStmt = ifVec[idx];
    report(
        DiagnosticsEngine::Note, "if here",
        ifStmt->getLocStart())
