...
bool ConstParams::VisitFunctionDecl(FunctionDecl * functionDecl)
{
    ...
    if (aFileName.startswith(SRCDIR "/sal/")
        || aFileName.startswith(WORKDIR "/bridges/")
        || aFileName.startswith(WORKDIR "/")
        || aFileName.startswith(SRCDIR "/unoidl/source/sourceprovider-parser.y")) {
        return true;
    }
    // TODO some hard cases, mostly to do with pointer types and containers
    if (aFileName.startswith(SRCDIR "/l10ntools/source/xmlparse.cxx")
        || aFileName.startswith(SRCDIR "/cppu/source/uno/lbmap.cxx")
        || aFileName.startswith(SRCDIR "/comphelper/source/misc/threadpool.cxx")) {
        return true;
    }
...
bool ConstParams::checkIfCanBeConst(const Stmt* stmt)
{
    ...
    if (isa<UnaryOperator>(parent)) {
        ...
        if (isa<VarDecl>(declStmt->getSingleDecl())) {
            // TODO could do better here, but would require tracking the target
            return false;
        }
        std::cout << "dump4" << std::endl;
        parent->dump();
    } else if (isa<ReturnStmt>(parent)) {
        // TODO could do better here, check if the return type is const
        return false;
