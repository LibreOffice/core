...
bool ConstParams::VisitFunctionDecl(FunctionDecl * functionDecl)
{
    ...
    if (functionDecl->getLocation().isMacroID())
        return true;
    // ignore lex/yacc callback
    if (functionDecl->getIdentifier() && functionDecl->getName() == "yyerror")
        return true;
    // some function callbacks
    if (functionDecl->getIdentifier() && functionDecl->getName() == "PDFSigningPKCS7PasswordCallback")
        return true;
    if (functionDecl->getIdentifier() && functionDecl->getName() == "VCLExceptionSignal_impl")
        return true;
