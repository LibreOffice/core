...
static bool areTypesEqual(QualType lhs, QualType rhs)
{
    ...
    // this is the carve-out exception for the "struct S {...} T" idiom we use in the UNO code
    if (isa<ElaboratedType>(lhsType))
        if (auto rhsTypedef = dyn_cast<TypedefType>(rhsType))
            if (areTypesEqual(lhs, rhsTypedef->desugar()))
                return true;
    if (auto lhsTypedef = dyn_cast<TypedefType>(lhsType))
        if (isa<ElaboratedType>(rhsType))
            if (areTypesEqual(lhsTypedef->desugar(), rhs))
                return true;
