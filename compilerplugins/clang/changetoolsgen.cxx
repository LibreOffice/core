...
bool ChangeToolsGen::VisitCXXMemberCallExpr(CXXMemberCallExpr const* call)
{
    ...
    std::string unarySetPrefix = "Move";
    ...
    {
        if (!ChangeUnaryOperator(unaryOp, call, methodName, unarySetPrefix))
    ...
    {
        if (!ChangeBinaryOperatorPlusMinus(binaryOp, call, methodName, unarySetPrefix))
...
bool ChangeToolsGen::ChangeUnaryOperator(UnaryOperator const* unaryOp,
                                         CXXMemberCallExpr const* call,
                                         std::string const& methodName,
                                         std::string const& unarySetPrefix)
{
    // Look for expressions like
    //    aRect.Left()++;
    //    ++aRect.Left();
