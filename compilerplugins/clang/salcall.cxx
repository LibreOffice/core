...
bool SalCall::VisitFunctionDecl(FunctionDecl const* decl)
{
    ...
    // it looks like it is returning the location of the LNG_DLLPUBLIC macro.
    ...
    SourceLocation startLoc = decl->getReturnTypeSourceRange().getBegin();
    ...
    //    if (decl->getIdentifier() && decl->getName() == "capitalType")
    //    {
    //        std::cout << "xxxx " << (long)p1 << " " << (long)p2 << " " << (int)(p2-p1) << std::endl;
    //        std::cout << "  " << std::string(p1, 80) << std::endl;
    //        report(DiagnosticsEngine::Warning, "jhjkahdashdkash", decl->getLocation())
    //            << decl->getSourceRange();
    //    }
    //
    ...
    if (rewrite(startLoc.getLocWithOffset(found - p1)))
        return true;
    ...
    if (canonicalDecl != decl)
        report(DiagnosticsEngine::Note, "parent function decl here", canonicalDecl->getLocation())
...
bool SalCall::rewrite(SourceLocation locBegin)
{
    ...
    auto locEnd = locBegin.getLocWithOffset(9); // 9 = SAL_CALL plus one space
