...
bool RenderContext::TraverseFunctionDecl(const FunctionDecl * pFunctionDecl)
{
    ...
    if (pCXXMethodDecl) {
        if (loplugin::TypeCheck(pCXXMethodDecl->getParent()).Class("OutputDevice"))
            return true;
    }
    // we are only currently interested in methods where the first parameter is RenderContext
    if (pFunctionDecl->getNumParams() == 0)
        return true;
    if ( loplugin::TypeCheck(pFunctionDecl->getParamDecl( 0 )->getType()).Class("RenderContext") ) {
        return true;
    }
    mbChecking = true;
...
bool RenderContext::VisitCXXMemberCallExpr(const CXXMemberCallExpr* pCXXMemberCallExpr)
{
    ...
    if (!loplugin::TypeCheck(pCXXRecordDecl).Class("OutputDevice")) {
        return true;
    }
