...
private:
    ...
    std::unordered_set<const Stmt*> chainedIfSet;
};
...
bool Indentation::VisitIfStmt(IfStmt const* ifStmt)
{
    ...
    if (auto chainedIf = dyn_cast_or_null<IfStmt>(ifStmt->getElse()))
        chainedIfSet.insert(chainedIf);
    if (chainedIfSet.find(ifStmt) != chainedIfSet.end())
        return true;
