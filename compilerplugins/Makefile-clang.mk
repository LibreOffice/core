...
CLANGCONFIG=$(CLANGDIR)/bin/llvm-config
...
$(CLANGINDIR)/sharedvisitor/sharedvisitor.cxx: $(CLANGOUTDIR)/sharedvisitor/generator$(CLANG_EXE_EXT)
	...
	$(CLANGOUTDIR)/sharedvisitor/generator$(CLANG_EXE_EXT) $(shell grep -l "LO_CLANG_SHARED_PLUGINS" $(CLANGINDIR)/*.cxx) > $(CLANGINDIR)/sharedvisitor/sharedvisitor.cxx

CLANGTOOLLIBS = -lclangTooling -lclangAST -lclangFrontend -lclangBasic -lLLVM $(shell $(CLANGCONFIG) --ldflags)
# Path to the clang system headers (no idea if there's a better way to get it).
CLANGTOOLDEFS = -DCLANGSYSINCLUDE=$(shell $(CLANGCONFIG) --libdir)/clang/$(shell $(CLANGCONFIG) --version)/include
$(CLANGOUTDIR)/sharedvisitor/generator$(CLANG_EXE_EXT): $(CLANGINDIR)/sharedvisitor/generator.cxx
	...
	$(QUIET)$(COMPILER_PLUGINS_CXX) $(CLANGCXXFLAGS) $(CLANGWERROR) $(CLANGDEFS) $(CLANGTOOLDEFS) $(CLANGINCLUDES) -DBUILDDIR=$(BUILDDIR) -I$(BUILDDIR)/config_host $< -o $@ -MMD -MT $@ -MP -MF $(CLANGOUTDIR)/sharedvisitor/generator.d $(CLANGTOOLLIBS)

$(CLANGOUTDIR)/sharedvisitor/generator$(CLANG_EXE_EXT): $(CLANGOUTDIR)/sharedvisitor
