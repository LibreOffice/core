...
void Test::testMetaGenerator()
{
    ...
    struct {
        OUString const generator;
        OUString const buildId;
    ...
    for (size_t i = 0; i < SAL_N_ELEMENTS(tests); ++i)
    {
        ...
        SvXMLMetaDocumentContext::setBuildId(tests[i].generator, xInfoSet);
        if (tests[i].buildId.getLength())
        {
            CPPUNIT_ASSERT_EQUAL(tests[i].buildId, xInfoSet->getPropertyValue("BuildId").get<OUString>());
        }
        else
        {
