    ...
    SAL_THROW_EXTERN_C()
{
    ...
    if( pTDR )
    {
        ...
        if( pTDR->pType )
        {
            ...
            if (((void *)pTDR != (void *)*ppNewDescription) && 
                (!pTDR->pType->pWeakRef || 
    ...
    SAL_THROW_EXTERN_C()
{
    if (pAssignable && pFrom)
    {
        ...
        if (eAssignable == typelib_TypeClass_ANY) 
            return sal_True;
        if (eAssignable == eFrom)
        {
            if (type_equals( pAssignable, pFrom )) 
            {
                return sal_True;
            }
            else
