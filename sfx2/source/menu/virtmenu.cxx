...
SfxVirtualMenu::~SfxVirtualMenu()
{
    ...
    // Delete addons popup menu as it was generated by ourself
    PopupMenu* pRuntimeMenu = pSVMenu->GetPopupMenu( SID_ADDONS );
    if ( pRuntimeMenu )
    {
        delete pRuntimeMenu;
        pSVMenu->SetPopupMenu( SID_ADDONS, 0 );
    }
    ...
    if ( bIcons )
    {
        ...
        for ( USHORT nPos=0; nPos < nCount; ++nPos )
        {
            ...
            if ( pMenu->GetItemType( nPos ) != MENUITEM_SEPARATOR )
            {
                ...
                {
                    Image aImage = GetImage( aXFrame, aImageId, FALSE, bIsHiContrastMode );
                    if ( !!aImage )
                    {
                        bImageSet = sal_True;
                ...
                if ( !bImageSet )
                {
                    ...
                    Image aImage = GetImage( aXFrame, aMenuItemCommand, FALSE, bIsHiContrastMode );
                    if ( !aImage )
                        aImage = aAddonOptions.GetImageFromURL( aMenuItemCommand, FALSE, bIsHiContrastMode );
    ...
    if ( pMenu == pSVMenu )
    {
        ...
        if ( !pParent )
            pAddonsMenu = pSVMenu->GetPopupMenu(SID_ADDONLIST);
                  ...
                  pSVMenu == pParent->pAddonsMenu &&
                  framework::AddonsOptions().HasAddonsMenu() &&
                  !pMenu->GetPopupMenu( SID_ADDONS ) )
        {
            ...
            pMenu->SetPopupMenu( SID_ADDONS, aConf.CreateAddonMenu( xFrame ));

            if ( SvtMenuOptions().IsMenuIconsEnabled() )
            {
                SfxModule* pModule = pBindings->GetDispatcher()->GetFrame()->GetObjectShell()->GetModule();
                pSVMenu->SetItemImage( SID_ADDONS, pBindings->GetImageManager()->GetImage( SID_ADDONS, pModule, FALSE, IsHiContrastMode() ) );
            }
        }
