...
namespace
{
void fixmtf(GDIMetaFile& tmpMtf)
{
    static int i = 0;
    i++;
    //GDIMetaFile tmpMtf(aMtf);

    //clipMetafileContentAgainstOwnRegions(tmpMtf);
    ...
    /*OUString wmfFileName("newmetafile-" + OUString::number(i) + ".wmf");
    SvFileStream aStream(wmfFileName, StreamMode::STD_READWRITE | StreamMode::TRUNC);

    //ConvertGDIMetaFileToEMF(tmpMtf, aStream);
    bool convertresult = ConvertGDIMetaFileToWMF(tmpMtf, aStream, nullptr, false);
    aStream.Close();

    // Read back in
    tmpMtf.Clear();
    aStream.Open("newmetafile-" + OUString::number(i) + ".wmf", StreamMode::STD_READ);
    convertresult = ReadWindowMetafile(aStream, tmpMtf);
    aStream.Close();

    aStream.Open("newmetafile-" + OUString::number(i) + ".svm",
                 StreamMode::STD_READWRITE | StreamMode::TRUNC);
    //tmpMtf.dumpAsXml();
    tmpMtf.Write(aStream);*/
}
}
