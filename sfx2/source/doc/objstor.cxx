...
//???? dv           SetFileName( aDirEntry.GetFull() );
        ...
        if (    bOnlineUpdateEnabled
            /*!!!&&  pMedium->GetInteractionHandler().is()*/
            &&  !SFX_APP()->Get_Impl()->bODFVersionWarningLater )
        {
            ...
            if ( xSet.is() )
            {
                // the new property "ODFVersion" is a temporary solution
                // because at the moment the "Generator" property is not set correctly
                uno::Any aAny;
                try
                {
                    aAny = xSet->getPropertyValue( DEFINE_CONST_UNICODE("ODFVersion") );
                }
                catch( const uno::Exception& )
                {
                    // Custom Property "ODFVersion" not exists
                }
                ...
                aAny = xSet->getPropertyValue( DEFINE_CONST_UNICODE("Generator") );
                if ( !bFound && ( aAny >>= sTemp ) )
                {
                    String sGenerator( sTemp );
                    String sWorkStamp = sGenerator.GetToken( 1, '$' ).GetToken( 1, '/' );
                    sal_Int32 nWorkStamp = sWorkStamp.ToInt32();
                    if ( nWorkStamp > SUPD )
                    {
                        // document is generated by an newer office
                        sfx2::NewerVersionWarningDialog aDlg( NULL );
