...
namespace
{
void collectAllWindows(vcl::Window* pParent, std::vector<vcl::Window*>& rAllChildren)
{
    if (!pParent)
        return;
    rAllChildren.push_back(pParent);

    vcl::Window* pChild = pParent->GetWindow(GetWindowType::FirstChild);
    while (pChild)
    {
        collectAllWindows(pChild, rAllChildren);
        pChild = pChild->GetWindow(GetWindowType::Next);
    }
}

std::vector<vcl::Window*> collectAllChildren(vcl::Window* pParent)
{
    std::vector<vcl::Window*> aAllChildren;
    vcl::Window* pChild = pParent->GetWindow(GetWindowType::FirstChild);
    while (pChild)
    {
        collectAllWindows(pChild, aAllChildren);
        pChild = pChild->GetWindow(GetWindowType::Next);
    }
    return aAllChildren;
}
    ...
    if (!mpWindow)
    {
        if (const css::uno::Reference<css::frame::XController>& xController = GetXController())
        {
            if (const css::uno::Reference<css::frame::XFrame> xFrame = xController->getFrame())
            {
                ...
                if (vcl::Window* pParent = VCLUnoHelper::GetWindow(xDockerWin))
                {
                    vcl::Window* pChildWin = pParent->GetWindow(GetWindowType::FirstChild);
                    while (pChildWin)
                    {
                        if (pChildWin->IsChart())
                        {
                            mpWindow = pChildWin;
                            break;
                        }
                        pChildWin = pChildWin->GetWindow(GetWindowType::Next);
                    }
                    if (!mpWindow)
                    {
                        auto a = collectAllChildren(pParent);
                        for (auto pWindow : a)
                        {
                            if (pWindow->IsStarMath())
                            {
                                mpWindow = pWindow;
                                break;
                            }
