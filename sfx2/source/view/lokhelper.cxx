...
void SfxLokHelper::notifyDialog(const vcl::DialogID& rDialogID,
                                const OUString& rAction,
                                const std::vector<vcl::LOKPayloadItem>& rPayload)
{
    if (SfxLokHelper::getViewsCount() <= 0 || rDialogID == 0)
        return;

    SfxViewShell* pViewShell = SfxViewShell::GetFirst();
    OString aPayload = OString("{ \"dialogId\": \"") + OString::number(rDialogID) + OString("\"");
...
void SfxLokHelper::notifyDialogChild(const vcl::DialogID& rDialogID, const OUString& rAction, const Point& rPos)
{
    if (SfxLokHelper::getViewsCount() <= 0 || rDialogID == 0)
        return;

    SfxViewShell* pViewShell = SfxViewShell::GetFirst();
    const OString aPayload = OString("{ \"dialogId\": \"") + OString::number(rDialogID) +
