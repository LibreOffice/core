...
{
    // The DocId must always be set, so use the current one.
    SfxViewShell* pViewShell = SfxViewShell::Current();
    if (pViewShell == nullptr)
        return -1;

    // Create a view of the first frame. Assumes a single document.
    SfxViewFrame* pViewFrame = SfxViewFrame::GetFirst();

    return createView(pViewFrame, pViewShell->GetDocId());
}
...
int SfxLokHelper::createView(int nDocId)
{
    ...
    // Find a frame with the given DocId.
