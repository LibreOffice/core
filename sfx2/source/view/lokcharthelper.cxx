...
inline tools::Rectangle lcl_TwipsToHMM( const tools::Rectangle& rRect )
{
    return tools::Rectangle(lcl_TwipsToHMM(rRect.TopLeft()), lcl_TwipsToHMM(rRect.GetSize()));
}
...
bool LokChartHelper::setTextSelection(int nType, int nX, int nY)
{
    ...
    if (rChartBBox.IsInside(Point(nX, nY)))
    {
        ...
        if (xDispatcher.is())
        {
            ...
            aArgs[0].Value <<= nType;
            aArgs[1].Value <<= nChartWinX;
            aArgs[2].Value <<= nChartWinY;
