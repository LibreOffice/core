...
public:
    ...
    {
        auto const& xSubClasses = mxClass->getSuperclasses();
        return xSubClasses.getLength() > 2
               || (xSubClasses.getLength() == 1 && !isXInterface(xSubClasses[0]));
    }
    ...
    void fillChildren(std::unique_ptr<weld::TreeView>& rTree,
                      const weld::TreeIter* pParent) override
    {
        for (auto const& xSuper : mxClass->getSuperclasses())
        {
