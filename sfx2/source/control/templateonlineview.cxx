    ...
    for (auto it = keys.begin(), ite = keys.end(); it != ite; ++it)
    {
        ...
        if(value.type() == orcus::json_node_t::array)
        {
            for (size_t i = 0; i < value.child_count(); ++i)
            {
                ...
                OUString sPath, sId, sAuthor, sTitle, sUrl, sDescription;
                ...
                for (auto templateIter = aTemplateParams.begin(); templateIter != aTemplateParams.end(); ++templateIter)
                {
                    ...
                    if(templateVal == (const char*) "path")
                    {
                        sPath = OUString::createFromAscii(prop.string_value().get());
                    }
                    else if(templateVal == (const char*)"id")
                    {
                        sId = OUString::createFromAscii(prop.string_value().get());
                    }
                    else if(templateVal == (const char*)"author")
                    {
                        sAuthor = OUString::createFromAscii(prop.string_value().get());
                    }
                    else if(templateVal == (const char*)"title")
                    {
                        sTitle = OUString::createFromAscii(prop.string_value().get());
                    }
                    else if(templateVal == (const char*)"url")
                    {
                        sUrl = OUString::createFromAscii(prop.string_value().get());
                    }
                    else if(templateVal == (const char*)"description")
                    {
                        sDescription = OUString::createFromAscii(prop.string_value().get());
                    }
                }

                //Call TemplateOnlineView::AppendItem() to populate each template as it is parsed
                AppendItem(sTitle, sId + " by " + sAuthor, sPath, sUrl, sAuthor , sDescription, BitmapEx ( SfxResId( SFX_THUMBNAIL_TEXT ) ));
            }
        }
    }
}
