...
ListView::ListView(std::unique_ptr<weld::TreeView> xTreeView)
    ...
    , mnSortColumn(-2)
{
    std::vector<int> aWidths = { 140, 110, 110, 130, 80 };
                          ...
                          const OUString& rPath, bool bDefault)
{
    ...
    AppendRow(sImage, pItem->maTitle, pItem->maSubtitle, pItem->maApplication,
            pItem->maDisplayModify, pItem->maDisplaySize, pItem->maDisplayPath,
            pItem->maId);
...
void ListView::AppendRow(const OUString& rImage, const OUString& rTitle, const OUString& rSubtitle,
                            ...
                            const OUString& rPath, const OUString& rId)
{
...
void ListView::UpdateRow(int nIndex, const OUString& rImage, const OUString& rTitle, const OUString& rSubtitle,
                            const OUString& rApplication, const OUString& rModify, const OUString& rSize,
                            const OUString& rPath, const OUString& rId)
{
    ...
    mxTreeView->set_text(nIndex, rPath, COLUMN_PATH);
...
void ListView::sortColumn(const int col)
{
                    ...
                    std::unique_ptr<ListViewItem> const & pItemB)
    {
        ...
        switch (col)
        {
            ...
            case COLUMN_PATH:
            {
                OUString sPathA = pItemA->maPath;
                OUString sPathB = pItemB->maPath;
...
void ListView::updateIsDefaultColumn()
{
    for(const auto& pItem : mListViewItems)
    {
        bool bDefault = SfxTemplateLocalView::IsDefaultTemplate(pItem->maPath);
