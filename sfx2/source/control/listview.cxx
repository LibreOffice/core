...
static OUString getFileSize(const OUString& rURL);
static OUString getFileModifyTime(const OUString& rURL);
                          ...
                          const OUString& rPath, bool isDefault)
{
    ...
    if (isDefault)
        mxTreeView->set_image(*xIter, BMP_DEFAULT, COLUMN_IMG_ISDEFAULT);
    else
...
void ListView::updateIsDefaultColumn()
{
    ...
    mxTreeView->all_foreach([&](weld::TreeIter& rIter) {
        ...
        if (isDefault)
            mxTreeView->set_image(rIter, BMP_DEFAULT, COLUMN_IMG_ISDEFAULT);
        else
