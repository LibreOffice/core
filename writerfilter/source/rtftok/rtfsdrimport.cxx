                                 ...
                                 std::u16string_view aKey, const OUString& aValue) const
{
    ...
    if (obFitShapeToText.has_value() && xPropertySet.is())
    {
        xPropertySet->setPropertyValue(
            "SizeType",
            uno::makeAny(obFitShapeToText.value() ? text::SizeType::MIN : text::SizeType::FIX));
        xPropertySet->setPropertyValue("FrameIsAutomaticHeight",
                                       uno::makeAny(obFitShapeToText.value()));
    }
    ...
    if (obRelFlipV.has_value() && xPropertySet.is())
    {
        if (nType == ESCHER_ShpInst_Line)
        {
                ...
                && aPolyPolySequence.hasElements())
            {
                ...
                aTransformation.scale(1.0, obRelFlipV.value() ? -1.0 : 1.0);
