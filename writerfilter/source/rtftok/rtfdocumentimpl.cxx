        ...
        case Destination::SHAPEINSTRUCTION:
                     ...
                     && !(aState.bInShapeGroup && !aState.bInShape))
            {
                ...
                if (eType == RTFSdrImport::PICT) {
                    if (m_aStates.size() > 1) {
                        const auto aPreviousPropNo = m_aStates[m_aStates.size() -2].aShape.aProperties.size();
                        const auto &aProperties = aState.aShape.aProperties;
                        // check if \picprop does not have incorrect shape type
                        auto pIter = std::find_if(std::next(std::begin(aProperties), aPreviousPropNo), std::end(aProperties),
                                    [](const std::pair<OUString, OUString> &a) { return ((a.first == "shapeType")
