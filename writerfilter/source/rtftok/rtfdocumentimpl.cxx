...
void RTFDocumentImpl::enlargeSpaceSequence(OUString& rString)
{
    ...
    // not for field destination or for nested group
    if (!m_aStates.empty()
        && (m_aStates.top().getDestination() != Destination::NORMAL
            || &m_aStates.top().getDestinationText()
    ...
    if ((nPos = rString.indexOf(sDoubleSpace)) > -1 &&
