...
class XmlScriptTest : public test::BootstrapFixture, public unotest::MacrosTest, public XmlTestTools
{
    ...
    Reference<container::XNameContainer> importFile(const OUString& sFileName);
    void exportToFile(const OUString& sFileName,
...
Reference<container::XNameContainer> XmlScriptTest::importFile(const OUString& sFileName)
{
    // Read import file
    ...
    CPPUNIT_ASSERT_MESSAGE("File not read correctly", nBytes == nBytesRead);
    aFile.close();
...
void XmlScriptTest::exportToFile(const OUString& sURL,
                                 Reference<container::XNameContainer> const& xDialogModel)
{
    ...
    CPPUNIT_ASSERT_MESSAGE("File not read correctly", bytes.getLength() == nBytesWritten);
    aFile.close();
}
...
{
    // Import
    Reference<container::XNameContainer> xModel(importFile("test.xml"));
...
{
    // Import
    Reference<container::XNameContainer> xModel(importFile("EmptyPopupItems.xdl"));
    CPPUNIT_ASSERT(xModel.is());
