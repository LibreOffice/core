...
bool EnableWUService_Impl(bool bEnable, MSIHANDLE hInstall)
{
    WriteLog(hInstall, bEnable ? "Enabling" : "Disabling", "WU service");
    ...
    auto hService = Guard(
        OpenServiceW(hSCM.get(), L"wuauserv", SERVICE_QUERY_CONFIG | SERVICE_CHANGE_CONFIG));
    if (!hService)
        ThrowLastError("OpenServiceW");
    WriteLog(hInstall, "Obtained WU service handle");

    DWORD nCbRequired = 0;
    if (!QueryServiceConfigW(hService.get(), nullptr, 0, &nCbRequired))
    {
    ...
    if (!QueryServiceConfigW(hService.get(), pConfig, nCbRequired, &nCbRequired))
        ThrowLastError("QueryServiceConfigW");
    WriteLog(hInstall, "Obtained service config");

    DWORD eNewStartType = 0;
    if (bEnable && pConfig->dwStartType == SERVICE_DISABLED)
    {
        bConfigChanged = true;
        eNewStartType = SERVICE_DEMAND_START;
        WriteLog(hInstall, "Service is disabled, and requested to enable");
    }
    else if (!bEnable && pConfig->dwStartType != SERVICE_DISABLED)
    {
        bConfigChanged = true;
    ...
    if (bConfigChanged)
    {
        if (!ChangeServiceConfigW(hService.get(), SERVICE_NO_CHANGE, eNewStartType,
...
struct DisableServiceStruct
{
    bool mbDisable;
    MSIHANDLE mhInstall;
    ~DisableServiceStruct()
    {
        try
        {
            if (mbDisable)
                EnableWUService_Impl(false, mhInstall);
        }
...
typedef std::unique_ptr<DisableServiceStruct> DisableServiceGuard;
DisableServiceGuard EnableWUService(MSIHANDLE hInstall)
{
    return DisableServiceGuard(
        new DisableServiceStruct{ EnableWUService_Impl(true, hInstall), hInstall });
}
}
...
extern "C" UINT __stdcall InstallMSU(MSIHANDLE hInstall)
{
    try
    {
        ...
        auto aDisableServiceGuard(EnableWUService(hInstall));
