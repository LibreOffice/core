...
extern "C" __declspec(dllexport) UINT __stdcall InstallMSU(MSIHANDLE hInstall)
{
    ...
    try
    {
...

        {
            ...
            // since it wakes up when Windows Update service is stopped, we use a trick to stop
            // the service after some time (reasonably long: 120 s is more than enough) in the hope
            // that it will wake up the process (https://superuser.com/questions/1044528/).
