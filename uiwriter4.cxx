...
public:
    ...
    void testTableRemoveTextChangesOnly();
    void testTdf66405();
    ...
    CPPUNIT_TEST(testTableRemoveTextChangesOnly);
...
void SwUiWriterTest4::testTableRemoveTextChangesOnly()
{
    //createSwDoc(DATA_DIRECTORY, "tdf91292_paraBackground.docx");
    ...
    // Accepting again: 3 rows (Undo of TextChangesOnly is correct)
    rIDRA.AcceptAllRedline(/*bAccept=*/true);
    Scheduler::ProcessEventsToIdle();
    discardDumpedLayout();
    pXmlDoc = parseLayoutDump();
    ...
    // but deleting the extra text with change tracking, resulted tracked row deletion
    // again, because of missing removing of TextChangeOnly SwTabLine property
...
#if 0
    // Save it and load it back.
    reload("Office Open XML Text", "tableRemoveTextChangesOnly.docx");
...
//xxx
    // Accepting again: 3 rows (Undo of TextChangesOnly is correct)
    rIDRA = pDoc->getIDocumentRedlineAccess();
    rIDRA.AcceptAllRedline(/*bAccept=*/true);
    Scheduler::ProcessEventsToIdle();
    discardDumpedLayout();
    pXmlDoc = parseLayoutDump();
