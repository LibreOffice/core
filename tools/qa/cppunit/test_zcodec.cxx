...
void ZCodecTest::testMakeDummyFile()
{
    ...
    // Check size
    CPPUNIT_ASSERT_EQUAL(DUMMY_SIZE, pStream->GetSize());
}
...
void ZCodecTest::testGzCompressDecompress()
{
    ...
    auto nInitialCRC32 = rtl_crc32(0, pInitialStream->GetData(), pInitialStream->GetSize());
    ZCodec aCodec;
    aCodec.BeginCompression(ZCODEC_DEFAULT_COMPRESSION, true);
    // Set compression metadata for compressing a GZ file
    aCodec.SetCompressionMetadata("dummy.txt", 0, nInitialCRC32);
    aCodec.Compress(*pInitialStream, pCompressedStream);
    auto nCompressed = aCodec.EndCompression();
    // Check that the compression succeeded
    CPPUNIT_ASSERT_GREATER(0l, nCompressed);
    ...
    auto nDecompressed = aCodec.EndCompression();
    // Check that the decompression succeeded
    CPPUNIT_ASSERT_GREATER(0l, nDecompressed);
    auto nCompressedDecompressedCRC32
    ...
    CPPUNIT_ASSERT_EQUAL(nInitialCRC32, nCompressedDecompressedCRC32);
}
