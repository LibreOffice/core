...
{
    if (DS_DEVICE_OPENCL_DEVICE == device->eType)
    {
        /* Evaluating an OpenCL device */
        SAL_INFO("opencl.device", "Device: \"" << device->sDeviceName << "\" (OpenCL) evaluation...");
        cl_int clStatus;
        /* Check for 64-bit float extensions */
        size_t aDevExtInfoSize = 0;
        clStatus = clGetDeviceInfo(device->aDeviceID, CL_DEVICE_EXTENSIONS, 0, nullptr, &aDevExtInfoSize);
        DS_CHECK_STATUS(clStatus, "evaluateScoreForDevice::clGetDeviceInfo");

        std::unique_ptr<char[]> aExtInfo(new char[aDevExtInfoSize]);
        clStatus = clGetDeviceInfo(device->aDeviceID, CL_DEVICE_EXTENSIONS, sizeof(char) * aDevExtInfoSize, aExtInfo.get(), nullptr);
        DS_CHECK_STATUS(clStatus, "evaluateScoreForDevice::clGetDeviceInfo");
        ...
        else
        {
            ...
            cl_context  clContext = clCreateContext(nullptr, 1, &device->aDeviceID, nullptr, nullptr, &clStatus);
            DS_CHECK_STATUS(clStatus, "evaluateScoreForDevice::clCreateContext");
            cl_command_queue clQueue = clCreateCommandQueue(clContext, device->aDeviceID, 0, &clStatus);
            ...
            clStatus = clBuildProgram(clProgram, 1, &device->aDeviceID, buildOption, nullptr, nullptr);
            ...
            if (CL_SUCCESS != clStatus)
            {
                ...
                size_t length;
                char* buildLog;
                clStatus = clGetProgramBuildInfo(clProgram, device->aDeviceID, CL_PROGRAM_BUILD_LOG, 0, nullptr, &length);
                buildLog = static_cast<char*>(malloc(length));
                clGetProgramBuildInfo(clProgram, device->aDeviceID, CL_PROGRAM_BUILD_LOG, length, buildLog, &length);
            ...
            else
            {
                ...
                device->fTime = timerCurrent(&kernelTime);
                device->bErrors = false;
            }
    ...
    else
    {
        ...
        device->fTime = timerCurrent(&kernelTime);
        device->fTime *= fInterpretTailFactor;
        device->bErrors = false;
    }
    return DS_SUCCESS;
}
...
ds_status pickBestDevice(ds_profile* profile, int* bestDeviceIdx)
{
    ...
    *bestDeviceIdx = -1;
    ...
    for (unsigned int d = 0; d < profile->numDevices; d++)
    {
        ...
        if (device.eType == DS_DEVICE_OPENCL_DEVICE)
        {
    ...
    if (DS_DEVICE_OPENCL_DEVICE == profile->devices[*bestDeviceIdx].eType)
    {
        SAL_INFO("opencl.device", "Selected Device[" << *bestDeviceIdx << "]: " << profile->devices[*bestDeviceIdx].sDeviceName << "(OpenCL).");
    }
    else
    {
...
int matchDevice(ds_profile* profile, char* deviceName)
{
    ...
    {
        if ((std::string(profile->devices[d].sDeviceName.getStr())).find(deviceName) != std::string::npos) deviceMatch = d;
    }
...
void writeDevicesLog(std::unique_ptr<ds_profile>& rProfile, OUString sProfilePath, int nSelectedIndex)
{
    ...
    {
        if (rDevice.eType == DS_DEVICE_OPENCL_DEVICE)
        {
...
ds_device getDeviceSelection(const char* pProfilePath, bool bForceSelection)
{
    ...
    if (!bIsDeviceSelected || bForceSelection)
    {
        ...
        if (status != DS_SUCCESS)
        {
            ...
            selectedDevice.eType = DS_DEVICE_NATIVE_CPU;
            return selectedDevice;
        }

        /* Try reading scores from file */
        OUString sProfilePath = OUString::createFromAscii(pProfilePath);
        OUString sFilePath = sProfilePath + OUString("opencl_profile.xml");

        if (!bForceSelection)
        {
            status = readProfile(sFilePath, aProfile.get());
        }
        else
        {
        ...
        if (DS_SUCCESS != status)
        {
            ...
            status = profileDevices(aProfile.get(), DS_EVALUATE_ALL, evaluateScoreForDevice, static_cast<void*>(testData.get()), &numUpdates);

            if (DS_SUCCESS == status)
            {
                /* Write scores to file */
                status = writeProfile(sFilePath, aProfile.get());
                if (DS_SUCCESS == status)
                {
        ...
        /* Pick best device */
        int bestDeviceIdx;
        pickBestDevice(aProfile.get(), &bestDeviceIdx);

        /* Override if necessary */
        char* overrideDeviceStr = getenv("SC_OPENCL_DEVICE_OVERRIDE");
        if (nullptr != overrideDeviceStr)
        {
            int overrideDeviceIdx = matchDevice(aProfile.get(), overrideDeviceStr);
            if (-1 != overrideDeviceIdx)
            {
                ...
                if (DS_DEVICE_OPENCL_DEVICE == aProfile->devices[bestDeviceIdx].eType)
                {
