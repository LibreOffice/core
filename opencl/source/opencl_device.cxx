...
/* Evaluate devices */
ds_status evaluateScoreForDevice(ds_device* device, std::unique_ptr<LibreOfficeDeviceEvaluationIO>& testData)
{
...
ds_status profileDevices(ds_profile* profile, std::unique_ptr<LibreOfficeDeviceEvaluationIO>& pTestData)
{
    ...
    for (auto& rDevice : profile->devices)
    {
...
/* Pick best device */
ds_status pickBestDevice(ds_profile* profile, int& rBestDeviceIndex)
{
...
ds_device getDeviceSelection(OUString sProfilePath, bool bForceSelection)
{
    ...
    if (!bIsDeviceSelected || bForceSelection)
    {
        ...
        if (DS_SUCCESS != status)
        {
            ...
            /* Perform evaluations */
            status = profileDevices(aProfile.get(), testData);
