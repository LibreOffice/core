...
bool switchOpenCLDevice(const OUString &aDevice, bool bAutoSelect, bool bForceEvaluation, OUString& rOutSelectedDeviceVersionIDString)
{
    if (!canUseOpenCL() || fillOpenCLInfo().empty())
        return false;

    cl_device_id pDeviceId = findDeviceIdByDeviceString(std::u16string_view(aDevice), fillOpenCLInfo());
