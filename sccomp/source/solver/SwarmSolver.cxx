...
typedef cppu::WeakImplHelper<sheet::XSolver,
...
public:
    SwarmSolver()
        ...
        , mfResultValue(0.0)
    {
        registerProperty("NonNegative", PROP_NONNEGATIVE, 0, &mbNonNegative, cppu::UnoType<decltype(mbNonNegative)>::get());
        registerProperty("Integer",     PROP_INTEGER,     0, &mbInteger,     cppu::UnoType<decltype(mbInteger)>::get());
        registerProperty("Timeout",     PROP_TIMEOUT,     0, &mnTimeout,     cppu::UnoType<decltype(mnTimeout)>::get());
        registerProperty("Algorithm",   PROP_ALGORITHM,   0, &mnAlgorithm,   cppu::UnoType<decltype(mnAlgorithm)>::get());
    }

    virtual ~SwarmSolver() = default;
    ...
    virtual void SAL_CALL setDocument(const uno::Reference<sheet::XSpreadsheetDocument>& rDocument) override
    {
        mxDocument = rDocument;
    }

    virtual table::CellAddress SAL_CALL getObjective() override
    {
        return maObjective;
    }
    ...
    virtual void SAL_CALL setConstraints(const uno::Sequence<sheet::SolverConstraint>& rConstraints) override
    {
    ...
    virtual void SAL_CALL setMaximize(sal_Bool bMaximize) override
    {
    ...
    virtual uno::Sequence<double> SAL_CALL getSolution() override
    {
        return maSolution;
    }
    ...
    sal_Bool SAL_CALL supportsService(const OUString& rServiceName)
    {
        return cppu::supportsService(this, rServiceName);
    }
    uno::Sequence<OUString> SAL_CALL getSupportedServiceNames()
    {
...
SwarmSolver::getCell(const table::CellAddress& rPosition)
{
    uno::Reference<container::XIndexAccess> xSheets(mxDocument->getSheets(), uno::UNO_QUERY);
    uno::Reference<sheet::XSpreadsheet> xSheet(xSheets->getByIndex(rPosition.Sheet), uno::UNO_QUERY);
...
size_t SwarmSolver::getDimensionality()
{
...
template <typename SwarmAlgorithm>
class SwarmRunner
{
private:
...
public:
    ...
    std::vector<double> solve()
    {
        ...
        std::chrono::high_resolution_clock::time_point aStart;
        aStart = std::chrono::high_resolution_clock::now();

        std::chrono::high_resolution_clock::time_point aCurrent = aStart;

        int nLastChange = 0;

        while ((mrAlgorithm.getGeneration() - nLastChange) < constNumberOfGenerationsWithoutChange &&
               std::chrono::duration_cast<std::chrono::milliseconds>(aCurrent - aStart).count() < mfTimeout)
        {
            ...
            aCurrent = std::chrono::high_resolution_clock::now();
        }

        double fTime = std::chrono::duration_cast<std::chrono::milliseconds>(aCurrent - aStart).count();
        (void)fTime;
...
void SAL_CALL SwarmSolver::solve()
{
    ...
    for (size_t i = 0; i < aSolution.size(); i++)
    {
        maSolution[i] = aSolution[i];
    }

}

extern "C" SAL_DLLPUBLIC_EXPORT uno::XInterface * SAL_CALL
com_sun_star_comp_Calc_SwarmSolver_get_implementation(uno::XComponentContext *, uno::Sequence<uno::Any> const &)
{
