...
public:
    ...
    void initialize()
    {
        ...
        for (size_t i = 0; i < mnNumOfParticles; i++)
        {
            ...
            if (rParticle.mCurrentFitness > mfBestFitness)
            {
                ...
                std::copy(rParticle.mPosition.begin(), rParticle.mPosition.end(), maBestPosition.begin());
            }
        }
    }
    ...
    bool next()
    {
        ...
        for (Particle & rParticle : maSwarm)
        {
            ...
            {
                rParticle.mBestFitness = rParticle.mCurrentFitness;
                std::copy(rParticle.mPosition.begin(), rParticle.mPosition.end(), rParticle.mBestPosition.begin());
            }

            if (rParticle.mCurrentFitness > mfBestFitness)
            {
                if (std::abs(rParticle.mCurrentFitness - mfBestFitness) > constAcceptedPercision)
                {
                    bBestChanged = true;
