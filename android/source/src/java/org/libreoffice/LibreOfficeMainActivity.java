...
public class LibreOfficeMainActivity extends AppCompatActivity {
    ...
    private void createAlertDialogue() {
        String documentName = mInputFile.getName();
        ...
        builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                directoryPath = documentDirectory.getPath();
                File fromFile = new File(directoryPath, mInputFile.getName());
                File newFile = new File(directoryPath, input.getText().toString());
                boolean isRenamed = fromFile.renameTo(newFile);
                if (isRenamed) {
                    toolbarTop.setTitle(newFile.getName());
                    Toast.makeText(LibreOfficeMainActivity.this, R.string.new_file_created, Toast.LENGTH_SHORT).show();
                }
            }
        });
    ...
    protected void onStart() {
        ...
        super.onStart();
        if (isNewDocument){
            LOKitShell.sendLoadEvent(newDocumentType);
        }
        else {
            if (partIndex == -1)
