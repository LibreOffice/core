...
public class CalcHeadersView extends View implements View.OnTouchListener {
    ...
    private void updateHeaders(Canvas canvas) {
        ...
        boolean inRangeOfVisibleHeaders = false;
        for (int i = 1; i < mLabels.size(); i++) {
            if (mIsRow) {
                float top = -origin.y + zoom*mDimens.get(i-1);
                float bottom = -origin.y + zoom*mDimens.get(i);
                if (top <= getHeight() && bottom >= 0) {
                    inRangeOfVisibleHeaders = true;
                    new CalcHeaderCell(0f, top, getWidth(), bottom - top, mLabels.get(i), zoom).onDraw(canvas);
                } else {
                ...
                float left = -origin.x + zoom*mDimens.get(i-1);
                float right = -origin.x + zoom*mDimens.get(i);
                if (left <= getWidth() && right >= 0) {
                    new CalcHeaderCell(left, 0f, right - left, getHeight(), mLabels.get(i), zoom).onDraw(canvas);
                } else {
                    if (inRangeOfVisibleHeaders) {
                        break;
                    }
