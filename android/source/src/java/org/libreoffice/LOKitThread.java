...
class LOKitThread extends Thread {
    ...
    private TextSelectionEventListener textSelectionEventListener;

    LOKitThread(LibreOfficeMainActivity context) {
        mContext = context;
        textSelectionEventListener = mContext;
    ...
    private void onCopyPressed(final String mimeType) {
        LOKitShell.getMainHandler().post(new Runnable() {
            @Override
            public void run() {
                textSelectionEventListener.copySelectedText(mTileProvider.getTextSelection(mimeType));
            }
        });
    }
