...
static void
reportError(const std::string& rString)
{
...
lok_doc_view_signal_button(GtkWidget* pWidget, GdkEventButton* pEvent)
{
    ...
    case GDK_BUTTON_PRESS:
    {
        ...
        priv->m_pDocument->pClass->postMouseEvent(priv->m_pDocument, LOK_MOUSEEVENT_MOUSEBUTTONDOWN, pixelToTwip(pEvent->x, priv->m_fZoom), pixelToTwip(pEvent->y, priv->m_fZoom), nCount, MOUSE_LEFT, 0);
        break;
    }
    case GDK_BUTTON_RELEASE:
    {
        ...
        priv->m_pDocument->pClass->postMouseEvent(priv->m_pDocument, LOK_MOUSEEVENT_MOUSEBUTTONUP, pixelToTwip(pEvent->x, priv->m_fZoom), pixelToTwip(pEvent->y, priv->m_fZoom), nCount, MOUSE_LEFT, 0);
        break;
    }
    default:
        break;
...
lok_doc_view_signal_motion (GtkWidget* pWidget, GdkEventMotion* pEvent)
{
    ...
    priv->m_pDocument->pClass->postMouseEvent(priv->m_pDocument, LOK_MOUSEEVENT_MOUSEMOVE, pixelToTwip(pEvent->x, priv->m_fZoom), pixelToTwip(pEvent->y, priv->m_fZoom), 1, MOUSE_LEFT, 0);
...
lok_doc_view_open_document (LOKDocView* pDocView, char* pPath)
{
    ...
    {
        pDocView->priv->m_pDocument->pClass->initializeForRendering(pDocView->priv->m_pDocument, 0);
...
{
    pDocView->priv->m_pDocument->pClass->postUnoCommand(pDocView->priv->m_pDocument, pCommand, pArguments, false);
}
