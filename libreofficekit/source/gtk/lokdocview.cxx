...
static void
setClientViewInThread(gpointer data)
{
    GTask* task = G_TASK(data);
    LOKDocView* pDocView = LOK_DOC_VIEW(g_task_get_source_object(task));
    LOKDocViewPrivate& priv = getPrivate(pDocView);
    ...
    if (!priv->m_pDocument)
        return;
    priv->m_pDocument->pClass->setClientView(priv->m_pDocument,
...
lokThreadFunc(gpointer data, gpointer /*user_data*/)
{
    ...
    case LOK_SET_GRAPHIC_SELECTION:
        ...
        break;
    case LOK_SET_CLIENT_VIEW:
        setClientViewInThread(task);
        break;
    }
...
lok_doc_view_set_zoom (LOKDocView* pDocView, float fZoom)
{
    ...
    GTask* task = g_task_new(pDocView, NULL, NULL, NULL);
    LOEvent* pLOEvent = new LOEvent(LOK_SET_CLIENT_VIEW);
    ...
    if (error != NULL)
    {
        g_warning("Unable to call LOK_SET_CLIENT_VIEW: %s", error->message);
        g_clear_error(&error);
    }
    g_object_unref(task);
}
