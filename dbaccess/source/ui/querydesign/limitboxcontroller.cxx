...
void LimitBox::SetText( const OUString& rStr )
{
    ///Delete leading 0's
    sal_Int32 nIndex = 0;
    while( nIndex < rStr.getLength() &&
        rStr.iterateCodePoints( &nIndex )=='0' ){}
    const OUString sTemp = rStr.copy(nIndex-1);

    if ( m_bUseNativeNumbers )
    {
        ComboBox::SetText( AsciiToNatNum(sTemp) );
    }
    else
        ComboBox::SetText( sTemp );
...
bool LimitBox::CheckAndConvertData( OUString& io_rData ) const
{
    ...
    if( m_bUseNativeNumbers )
    {
        ///The data must contain only non-ascii digits.
        sal_Int32 nIndex = 0;
        while( nIndex < io_rData.getLength() )
        {
            sal_uInt32 nChar = io_rData.iterateCodePoints( &nIndex );
            if( nChar >= '0' && nChar <= '9' )
            {
                return false;
            }
