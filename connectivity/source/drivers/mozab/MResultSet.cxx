                                     ...
                                     MQueryExpression                     &queryExpression)
{
    ...
    else if (SQL_ISRULE(parseTree,like_predicate))
    {
        ...
        pColumn     = parseTree->getChild(0);                        
        pAtom       = pPart2->getChild(static_cast<sal_uInt32>(pPart2->count()-2));     
        pOptEscape  = pPart2->getChild(static_cast<sal_uInt32>(pPart2->count()-1));     
        (void)pOptEscape;
...
sal_Bool OResultSet::seekRow( eRowPosition pos, sal_Int32 nOffset )
{
    ...
    if ( nCurPos < (sal_Int32)m_pKeySet->get().size() ) 
    {
