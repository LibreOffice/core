...
BASEGFX_DLLPUBLIC B2DPolygon createWaveLinePolygon(basegfx::B2DRectangle const& rRectangle)
{
    ...
    float fWaveHeight = rRectangle.getHeight();
    // Wavelength depends on the wave height so it looks nice
    float fHalfWaveLength = fWaveHeight + 1.0f;
    float fWaveAmplitude = fWaveHeight / 2.0f;

    float fLastX = rRectangle.getMinX();
    float fBaseY = rRectangle.getMinY() + fWaveAmplitude;
    float fDirection = 1.0f;

    // In quadratic bezier the curve is 1/2 of the control height
    // so we need to compensate for that.
    float fHeightCompensation = 2.0f;

    aPolygon.append(basegfx::B2DPoint(fLastX, fBaseY));

    for (float fI = fHalfWaveLength; fI <= rRectangle.getWidth(); fI += fHalfWaveLength)
    {
        basegfx::B2DPoint aPoint(fLastX + fHalfWaveLength, fBaseY);
        basegfx::B2DPoint aControl(fLastX + (fHalfWaveLength / 2.0f),
        ...
        fDirection *= -1.0f; // fDirection iterates between 1 and -1
    }
