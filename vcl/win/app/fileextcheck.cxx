...
bool IsInstDir()
{
    ...
    return path.find(L"instdir\\program\\soffice.bin") != std::wstring::npos;
}
...
static bool IsDefaultAppInstalledInReg()
{
    ...
    if (RegOpenKeyExW(HKEY_LOCAL_MACHINE, keyPath, 0, KEY_READ, &hTestKey) == ERROR_SUCCESS)
    {
        ...
        for (DWORD i = 0; retCode == ERROR_SUCCESS; i++)
        {
            ...
            achValue[0] = '\0';
            retCode = RegEnumValueW(hTestKey, i, achValue, &cchValue, NULL, NULL, NULL, NULL);

            if (retCode == ERROR_SUCCESS)
            {
                std::wstring regApp(achValue);
                if (regApp.find(L"LibreOffice") != std::wstring::npos)
                {
                    bFoundDefault = true;
                    break;
                }
