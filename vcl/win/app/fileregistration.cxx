...
static bool IsDefaultAppInstalledInReg()
{
    ...
    if (rc == ERROR_SUCCESS)
    {
        ...
        if (GetModuleFileNameW(NULL, szProcPath, MAX_LONG_PATH))
        {
            ...
            {
                WCHAR szLongProcPath[MAX_LONG_PATH];

                if (GetLongPathNameW(szFullProcPath, szLongProcPath, MAX_LONG_PATH))
                {
                    WCHAR szLongRegPath[MAX_LONG_PATH];

                    if (GetLongPathNameW(szRegPath, szLongRegPath, MAX_LONG_PATH))
                    {
                        std::wstring longProcPath(szLongProcPath);

                        if (longProcPath.find(szLongRegPath) != std::wstring::npos)
                            return true;
                    }
                }
            }
...
void CheckFileExtRegistration(weld::Window* pDialogParent)
{
    ...
    for (std::map<LPCWSTR, LPCWSTR>::iterator it = formats.begin(); it != formats.end(); it++)
    {
        if (IsPathDefaultForClass(pAAR, it->first, it->second) == S_FALSE)
        {
            isNotDefault = true;
