...
public:
    ...
    virtual void Paint(vcl::RenderContext& rRenderContext, tools::Rectangle const&) override;
};

void ListFontsWin::Paint(vcl::RenderContext&, tools::Rectangle const&)
{
    ...
    for (int i = 0; i < pOutDev->GetFontFaceCollectionCount(); i++)
    {
        ...
        std::cout << aFont.GetFamilyName() << "\n\tFamily type: " << aFont.GetFamilyType()
                  ...
                  << (aFont.IsFullstopCentered() ? "yes" : "no")
                  // how do you output a degree?
                  << "\n\tOrientiation: " << toDegrees(aFont.GetOrientation()) << " degrees"
        ...
        for (auto feature : features)
        {
            ...
            std::cout << "\t"
                      << (feature.m_eType == vcl::font::FeatureType::OpenType ? "OpenType"
                                                                              : "Graphite")
                      << " Feature:\n\t\tId = { feature code = 0x" << std::hex << std::setfill('0')
                      << std::setw(4) << feature.m_aID.m_aFeatureCode << ", script code = 0x"
                      << feature.m_aID.m_aScriptCode << ", language code = 0x"
                      << feature.m_aID.m_aLanguageCode << " }\n";

            std::cout.copyfmt(init);

            std::cout << "\t\tCode: 0x" << std::hex << feature.m_aDefinition.getCode() << "\n";
            std::cout << "\t\tDescription: " << feature.m_aDefinition.getDescription() << "\n";
            std::cout << "\t\tType: "
                      << (feature.m_aDefinition.getType() == vcl::font::FeatureParameterType::BOOL
                              ? "BOOL"
                              : "ENUM")
                      << "\n";
            ...
            {
                for (auto param : feature.m_aDefinition.getEnumParameters())
                {
                    std::cout << "\t\t\t" << param.getDescription() << ": " << param.getCode()
                              << "\n";
                }
            }
...
int ListFonts::Main()
{
    ...
    {
        InitVCL();
        mpWin = VclPtr<ListFontsWin>::Create();
        mpWin->Show();
        Application::Execute();
        DeInitVCL();
    }
...
void vclmain::createApplication() { static ListFonts aApp; }
