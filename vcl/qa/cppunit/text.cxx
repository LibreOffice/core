...
{
    OUString sTestString("The quick brown fox\n jumped over the lazy dog");
    sTestString += reinterpret_cast<const sal_Unicode*>(u"العاشر");
    vcl::TextLayoutCache cache(sTestString.getStr(), sTestString.getLength());

    vcl::Run run1 = cache.runs[0];
    vcl::Run run2 = cache.runs[1];

    CPPUNIT_ASSERT_EQUAL(static_cast<unsigned long>(2), cache.runs.size());
