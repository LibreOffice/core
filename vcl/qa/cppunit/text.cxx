...
{
    OUString sTestString(
        reinterpret_cast<const sal_Unicode*>("The quick brown fox\n jumped over the lazy dog"));
    sTestString += u"العاشر";
    vcl::TextLayoutCache cache(sTestString.getStr(), sTestString.getLength());

    vcl::Run run1 = cache.runs[0];
    vcl::Run run2 = cache.runs[1];

    CPPUNIT_ASSERT_EQUAL(2UL, cache.runs.size());
    CPPUNIT_ASSERT_EQUAL(USCRIPT_LATIN, run1.nCode);
    CPPUNIT_ASSERT_EQUAL(static_cast<int32_t>(0), run1.nStart);
    CPPUNIT_ASSERT_EQUAL(static_cast<int32_t>(45), run1.nEnd);
    CPPUNIT_ASSERT_EQUAL(USCRIPT_ARABIC, run2.nCode);
    CPPUNIT_ASSERT_EQUAL(static_cast<int32_t>(45), run2.nStart);
    CPPUNIT_ASSERT_EQUAL(static_cast<int32_t>(51), run2.nEnd);
}
