...
public:
    ...
    CPPUNIT_TEST(testDrawBorder_printable);
    ...
    CPPUNIT_TEST(testDrawGradient_polygon_linear_printable);
    CPPUNIT_TEST(testDrawGradient_polygon_axial);
    CPPUNIT_TEST(testDrawGradient_polygon_axial_printable);
...
void VclOutdevTest::testDrawBorder()
{
    ...
    MetaAction* pAction = aMtf.GetAction(INITIAL_SETUP_ACTION_COUNT);
    CPPUNIT_ASSERT_EQUAL_MESSAGE("Not a border action", MetaActionType::BORDER, pAction->GetType());
    MetaBorderAction* pBorderAction = dynamic_cast<MetaBorderAction*>(pAction);

    pAction = pBorderAction->GetAction(0);
    ...
    pAction = pBorderAction->GetAction(1);
    ...
    pAction = pBorderAction->GetAction(2);
    ...
    pAction = pBorderAction->GetAction(3);
...
void VclOutdevTest::testDrawBorder_printable()
{
    ScopedVclPtrInstance<Printer> pPrinter;
    GDIMetaFile aMtf;
    aMtf.Record(pPrinter.get());

    pPrinter->DrawBorder(tools::Rectangle(Point(0, 0), Size(50, 60)));

    MetaAction* pAction = aMtf.GetAction(0);
    CPPUNIT_ASSERT_EQUAL_MESSAGE("Not a border container action", MetaActionType::BORDER,
                                 pAction->GetType());
    MetaBorderAction* pBorderAction = dynamic_cast<MetaBorderAction*>(pAction);

    pAction = pBorderAction->GetAction(0);
    CPPUNIT_ASSERT_EQUAL_MESSAGE("Not a line color action (black)", MetaActionType::LINECOLOR,
                                 pAction->GetType());
    MetaLineColorAction* pLineColorAction = dynamic_cast<MetaLineColorAction*>(pAction);
    CPPUNIT_ASSERT_EQUAL_MESSAGE("Not black", COL_BLACK, pLineColorAction->GetColor());

    pAction = pBorderAction->GetAction(1);
    CPPUNIT_ASSERT_EQUAL_MESSAGE("Not a rect action (black border)", MetaActionType::RECT,
                                 pAction->GetType());
    MetaRectAction* pRectAction = dynamic_cast<MetaRectAction*>(pAction);
    CPPUNIT_ASSERT_EQUAL_MESSAGE("Rectangle wrong", tools::Rectangle(Point(0, 0), Size(50, 60)),
                                 pRectAction->GetRect());
...
static void TestLinearStripes(MetaLinearGradientAction const* pLinearAction)
{
    size_t nIndex = 0;

    MetaAction* pAction = pLinearAction->GetAction(nIndex);
    CPPUNIT_ASSERT_EQUAL_MESSAGE("Not a comment action (start)", MetaActionType::COMMENT,
                                 pAction->GetType());
    nIndex++;

    pAction = pLinearAction->GetAction(nIndex);
    CPPUNIT_ASSERT_EQUAL_MESSAGE("Not a fill color action (start)", MetaActionType::FILLCOLOR,
                                 pAction->GetType());
    nIndex++;
    ...
    for (size_t i = 0; i < pLinearAction->GetSteps(); i++)
    {
        nIndex++;
        pAction = pLinearAction->GetAction(nIndex);
        CPPUNIT_ASSERT_EQUAL_MESSAGE("Not a fill color action", MetaActionType::FILLCOLOR,
    ...
    nIndex++;
    CPPUNIT_ASSERT_EQUAL_MESSAGE("More actions to be processed", pLinearAction->size(), nIndex);
}

static void TestAxialStripes(MetaLinearGradientAction const* pLinearAction)
{
    size_t nIndex = 0;
...
static void TestComplexStripes(MetaComplexGradientAction const* pComplexAction)
{
    size_t nIndex = 0;

    MetaAction* pAction = pComplexAction->GetAction(nIndex);
    CPPUNIT_ASSERT_EQUAL_MESSAGE("Not a comment action (Start)", MetaActionType::COMMENT,
                                 pAction->GetType());

    nIndex++;

    pAction = pComplexAction->GetAction(nIndex);
    CPPUNIT_ASSERT_EQUAL_MESSAGE("Not a fill color action", MetaActionType::FILLCOLOR,
                                 pAction->GetType());
    ...
    for (size_t i = 1; i < pComplexAction->GetSteps(); i++)
    {
        pAction = pComplexAction->GetAction(nIndex);
        CPPUNIT_ASSERT_EQUAL_MESSAGE("Not a polypolygon action", MetaActionType::POLYPOLYGON,
                                     pAction->GetType());
    ...
    nIndex++;
    CPPUNIT_ASSERT_EQUAL_MESSAGE("More actions to be processed", pComplexAction->size(), nIndex);
}
