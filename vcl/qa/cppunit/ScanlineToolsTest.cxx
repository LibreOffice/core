...
void ScanlineToolsTest::ScanlineTransformer_32_ARGB()
{
    ...
    for (Color aColor : aColors)
    {
        pScanlineTransformer->writePixel(aColor);
    }

    std::vector<sal_uInt8> aExpected{ 0,   10,  250, 120, 50,  30, 230, 110, 100, 50,
                                      210, 100, 150, 70,  190, 90, 200, 90,  170, 80 };

    CPPUNIT_ASSERT(aExpected == aScanLine);
}
...
void ScanlineToolsTest::ScanlineTransformer_24_BGR()
{
    ...
    std::vector<sal_uInt8> aExpected{ 120, 250, 10,  110, 230, 30,  100, 210,
...

{
    ...
    std::vector<sal_uInt8> aExpected{ 207, 15, 45, 31, 140, 54, 235, 69, 74, 93 };
...
void ScanlineToolsTest::ScanlineTransformer_8bit_Palette()
{
    ...
    CPPUNIT_ASSERT(aExpectedBytes == aScanLine);
...
void ScanlineToolsTest::ScanlineTransformer_4bit_Palette()
{
    ...
    std::vector<sal_uInt8> aExpected{ 0x01, 0x23, 0x45 };

    for (size_t i = 0; i < aScanLine.size(); ++i)
    {
        CPPUNIT_ASSERT_EQUAL(int(aExpected[i]), int(aScanLine[i]));
    }
...
void ScanlineToolsTest::ScanlineTransformer_1bit_Palette()
{
    ...
    std::vector<sal_uInt8> aExpected{
