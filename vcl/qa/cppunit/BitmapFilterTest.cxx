...
void BitmapFilterTest::testBlurCorrectness()
{
...

    {
        ...
        Rectangle aRectangle1(aMargin1, aMargin1, aSize.Width() - 1 - aMargin1,
                              aSize.Height() - 1 - aMargin1);

        Rectangle aRectangle2(aMargin2, aMargin2, aSize.Width() - 1 - aMargin2,
                              aSize.Height() - 1 - aMargin2);

        Rectangle aRectangle3(aSize.Width() / 2, aSize.Height() / 2, aSize.Width() / 2,
                              aSize.Height() / 2);
...
void BitmapFilterTest::testPerformance()
{
    ...
    Bitmap aBigBitmap(aSize, 24);
    {
        ...
        Rectangle aRectangle(aMargin, aMargin, aSize.Width() - 1 - aMargin,
    ...
    if (constWriteResultBitmap)
    {
        std::unique_ptr<SvFileStream> pStream(new SvFileStream(
            OUString("~/BlurBigPerformance.png"), StreamMode::WRITE | StreamMode::TRUNC));
        GraphicFilter& rFilter = GraphicFilter::GetGraphicFilter();
        rFilter.compressAsPNG(aBigBitmap, *pStream, 1);

        pStream.reset(new SvFileStream(OUString("~/BlurBigPerformance.txt"), StreamMode::WRITE));
