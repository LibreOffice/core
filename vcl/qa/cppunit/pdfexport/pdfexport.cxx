...
void PdfExportTest::testTdf118244_radioButtonGroup()
{
    ...
    for ( const auto& aElement : aDocument.GetElements() )
    {
        ...
        if ( pType && pType->GetValue() == "Btn" )
        {
            ...
            // Only one group, containing both buttons.
            if ( pKids )
            {
                ++nRadioGroups;
                CPPUNIT_ASSERT_EQUAL(static_cast<size_t>(2), pKids->GetElements().size());
            }
        }
    }
    CPPUNIT_ASSERT_EQUAL_MESSAGE("# of radio groups", sal_uInt32(1), nRadioGroups);
}
