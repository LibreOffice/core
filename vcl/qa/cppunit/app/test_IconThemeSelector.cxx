...
private:
    ...
    static std::vector<vcl::ThemeNameAndUrlToFile>
...
{
    std::vector<vcl::ThemeNameAndUrlToFile> r(3);
    r.at(0).mThemeName = "tango";
    r.at(1).mThemeName = "oxygen";
    r.at(2).mThemeName = vcl::IconThemeSelector::HIGH_CONTRAST_ICON_THEME_NAME;
    return r;
}
...
IconThemeSelectorTest::ThemeIsOverriddenByHighContrastMode()
{
    ...
    CPPUNIT_ASSERT_EQUAL_MESSAGE("'tango' theme is overridden by high contrast mode",
            vcl::IconThemeSelector::HIGH_CONTRAST_ICON_THEME_NAME, selected);
...
IconThemeSelectorTest::FirstThemeIsReturnedIfRequestedThemeIsNotFound()
{
    ...
    CPPUNIT_ASSERT_EQUAL_MESSAGE("'oxygen' theme is found", themes.front().mThemeName, selected);
}
...
{
    vcl::IconThemeSelector s;
    rtl::OUString selected = s.SelectIconTheme(std::vector<vcl::ThemeNameAndUrlToFile>(), "oxygen");
    CPPUNIT_ASSERT_EQUAL_MESSAGE("fallback is returned for empty input",
            vcl::IconThemeSelector::FALLBACK_ICON_THEME_NAME, selected);
}
...
IconThemeSelectorTest::DifferentHighContrastModesAreInequal()
{
    ...
    bool equal = (s1 == s2);
    CPPUNIT_ASSERT_EQUAL_MESSAGE("Different high contrast modes are detected as inequal", equal, false);
}
...
IconThemeSelectorTest::DifferentPreferredThemesAreInequal()
{
    ...
    bool equal = (s1 == s2);
    CPPUNIT_ASSERT_EQUAL_MESSAGE("Different preferred themes are detected as inequal", equal, false);
}
