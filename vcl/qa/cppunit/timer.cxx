...
class IdleSerializer : public Idle
{
    ...
    sal_Int32 &mrProcesed;
public:
    IdleSerializer( const sal_Char *pDebugName,
                    sal_Int32 nPosition, sal_Int32 &rProcesed )
        : Idle( pDebugName )
...
void TimerTest::testPriority()
{
    // scope, so tasks are deleted
    {
        ...
        Scheduler::ProcessEventsToIdle();
        CPPUNIT_ASSERT_EQUAL_MESSAGE( "Not all idles processed", 2, nProcessed );
    }
...
public:
    TestAutoIdleRR( sal_Int32 &rCount,
                    const sal_Char *pDebugName )
        : AutoIdle( pDebugName )
        , mrCount( rCount )
    {
        CPPUNIT_ASSERT_EQUAL( mrCount, 0 );
