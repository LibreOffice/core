...
Bitmap OutputDeviceTestPolyPolygonB2D::setupIntersectingRectangles()
{
    ...
    /*
        ...
        which in turn collased them to each other and enabling the fill color
        which in turn should work accordingly to the even-odd rule.
    */

    //Rect - 1
    aPolygon1.append(
        { maVDRectangle.Left() + nOffset + nFix, maVDRectangle.Top() + (nOffset - 1) + nFix });
    aPolygon1.append({ maVDRectangle.Right() - (nOffset + 2) + nFix,
                       maVDRectangle.Top() + (nOffset - 1) + nFix });
    aPolygon1.append({ maVDRectangle.Right() - (nOffset + 2) + nFix,
                       maVDRectangle.Bottom() - (nOffset + 8) + nFix });
    aPolygon1.append(
        { maVDRectangle.Left() + nOffset + nFix, maVDRectangle.Bottom() - (nOffset + 8) + nFix });
    aPolygon1.setClosed(true);
    aPolyPolygon.append(aPolygon1);

    //Rect - 2
    aPolygon2.append(
        { maVDRectangle.Left() + nOffset + nFix, maVDRectangle.Top() + (nOffset + 2) + nFix });
    aPolygon2.append({ maVDRectangle.Right() - (nOffset + 2) + nFix,
                       maVDRectangle.Top() + (nOffset + 2) + nFix });
    aPolygon2.append({ maVDRectangle.Right() - (nOffset + 2) + nFix,
                       maVDRectangle.Bottom() - (nOffset + 5) + nFix });
    aPolygon2.append(
        { maVDRectangle.Left() + nOffset + nFix, maVDRectangle.Bottom() - (nOffset + 5) + nFix });
    aPolygon2.setClosed(true);
    aPolyPolygon.append(aPolygon2);

    //Rect - 3
    aPolygon3.append(
        { maVDRectangle.Left() + nOffset + nFix, maVDRectangle.Top() + (nOffset + 5) + nFix });
    aPolygon3.append({ maVDRectangle.Right() - (nOffset + 2) + nFix,
                       maVDRectangle.Top() + (nOffset + 5) + nFix });
    aPolygon3.append({ maVDRectangle.Right() - (nOffset + 2) + nFix,
                       maVDRectangle.Bottom() - (nOffset + 2) + nFix });
    aPolygon3.append(
        { maVDRectangle.Left() + nOffset + nFix, maVDRectangle.Bottom() - (nOffset + 2) + nFix });
    aPolygon3.setClosed(true);
    aPolyPolygon.append(aPolygon3);

    //Rect - 4
    aPolygon4.append(
        { maVDRectangle.Left() + nOffset + nFix, maVDRectangle.Top() + (nOffset + 8) + nFix });
    aPolygon4.append({ maVDRectangle.Right() - (nOffset + 2) + nFix,
                       maVDRectangle.Top() + (nOffset + 8) + nFix });
    aPolygon4.append(
        { maVDRectangle.Right() - (nOffset + 2) + nFix, maVDRectangle.Bottom() - nOffset + nFix });
    aPolygon4.append(
        { maVDRectangle.Left() + nOffset + nFix, maVDRectangle.Bottom() - nOffset + nFix });
