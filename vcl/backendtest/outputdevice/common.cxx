...
TestResult OutputDeviceTestCommon::checkDropShape(Bitmap& rBitmap, bool aEnableAA)
{
    ...
    for (tools::Long i = 0; i < pAccess->Height(); i++)
    {
        for (tools::Long j = 0; j < pAccess->Width(); j++)
        {
            ...
            {
                if(aEnableAA)
                 checkValueAA(pAccess, i, j, constLineColor, nNumberOfQuirks, nNumberOfErrors, false);
                else
                 checkValue(pAccess, i, j, constLineColor, nNumberOfQuirks, nNumberOfErrors, false);
            }
            else
            {
                if(aEnableAA)
