...
void Qt5Clipboard::flushClipboard()
{
    ...
    pSalInst->RunInMainThread([&, this]() {
        ...
        const QMimeData* pMimeData = pClipboard->mimeData(m_aClipboardMode);
        assert(dynamic_cast<const Qt5MimeData*>(pMimeData));
        const Qt5MimeData* pQt5MimeData = static_cast<const Qt5MimeData*>(pMimeData);
...
css::uno::Reference<css::datatransfer::XTransferable> Qt5Clipboard::getContents()
{
    ...
    if (m_aContents.is())
    {
        assert(dynamic_cast<Qt5ClipboardTransferable*>(m_aContents.get()));
        const auto* pTrans = static_cast<Qt5ClipboardTransferable*>(m_aContents.get());
        if (pMimeData == pTrans->mimeData())
            return m_aContents;
    }
...
void Qt5Clipboard::handleChanged(QClipboard::Mode aMode)
{
    ...
    const bool bLostOwnership = !isOwner(m_aClipboardMode) && m_aOwner.is();
    if (bLostOwnership)
    {
