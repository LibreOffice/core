    ...
    , m_aClipboardMode(aMode)
{
    ...
    // DirectConnection guarantess the changed slot runs in the same thread then the QClipboard
...
css::uno::Reference<css::datatransfer::XTransferable> Qt5Clipboard::getContents()
{
    if (m_aContents.is())
        return m_aContents;
    return new Qt5ClipboardTransferable(m_aClipboardMode);
}
    ...
    const css::uno::Reference<css::datatransfer::clipboard::XClipboardOwner>& xClipboardOwner)
{
    ...
    // this will trigger QClipboard::changed / handleChanged
    if (m_aContents.is())
