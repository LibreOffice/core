...
Qt5Transferable::Qt5Transferable(QClipboard::Mode aMode)
    : m_aContent(*QApplication::clipboard()->mimeData(aMode))
{
}
...
std::vector<css::datatransfer::DataFlavor> Qt5Transferable::getTransferDataFlavorsAsVector()
{
    ...
    for (QString& rMimeType : m_aContent.formats())
    {
...
Qt5Transferable::getTransferData(const css::datatransfer::DataFlavor& rFlavor)
{
    ...
    {
        QString clipboardContent = m_aContent.text();
    ...
    {
        QString clipboardContent = m_aContent.html();
        std::string aStr = clipboardContent.toStdString();
        uno::Sequence<sal_Int8> aSeq(reinterpret_cast<const sal_Int8*>(aStr.c_str()),
                                     aStr.length());
        aRet <<= aSeq;
    }
    else if (rFlavor.MimeType.startsWith("image") && m_aContent.hasImage())
    {
        QImage image = qvariant_cast<QImage>(m_aContent.imageData());
        QByteArray ba;
        QBuffer buffer(&ba);
