...
Qt5MimeData::Qt5MimeData(const QMimeData& rMimeReference)
{
    ...
    if (pQt5MimeData && pQt5MimeData->m_aContents.is())
        m_aContents = pQt5MimeData->m_aContents;
    else
        deepCopy(*this, rMimeReference);
}

void Qt5MimeData::deepCopy(QMimeData& rMimeCopy, const QMimeData& rMimeReference)
{
    rMimeCopy.clear();
    for (QString& format : rMimeReference.formats())
    {
        QByteArray data = rMimeReference.data(format);
        ...
        rMimeCopy.setData(format, data);
    }
}

QMimeData* Qt5MimeData::deepCopy() const
{
    QMimeData* pMimeDataCopy = new QMimeData();
    deepCopy(*pMimeDataCopy, *this);
    return pMimeDataCopy;
}
...
void Qt5Clipboard::flushClipboard()
{
    ...
    assert(pQt5MimeData);
    if (pQt5MimeData)
        pClipboard->setMimeData(pQt5MimeData->deepCopy(), m_aClipboardMode);
}
