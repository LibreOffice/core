...
void stackBlurHorizontal(BlurSharedData const& rShared, long nStart, long nEnd)
{
    ...
    for (long y = nStart; y <= nEnd; y++)
    {
        ...
        // the first row pixel, we pretend to have processed a fake previous pixel, as if the row
        // was extended to the left with the same color as that of the first pixel.
        //                         position of *previous* pixel => V
        // (Shouldn't aArrays.maPositionTable rather be [0,0,0,0,0,0,0,1,2,3,4]?)
...
void stackBlurVertical(BlurSharedData const& rShared, long nStart, long nEnd)
{
    ...
    for (long x = nStart; x <= nEnd; x++)
    {
        ...
        // for the first pixel; aArrays.maWeightTable has [1,2,3,4,5,6,5,4,3,2,1]. Before looking at
        // the first column pixel, we pretend to have processed a fake previous pixel, as if the
