...
double Geometry::LogicToPixel(double n, tools::Long nDPI, tools::Long nMapNum,
                              tools::Long nMapDenom)
{
    assert(nDPI > 0);
    assert(nMapDenom != 0);
...
tools::Long Geometry::LogicHeightToDevicePixel(tools::Long nHeight) const
{
    ...
    return LogicToPixel(nHeight, GetDPIY(), GetMapScalingYNumerator(), GetMapScalingYDenominator());
}
...
float Geometry::FloatLogicHeightToDevicePixel(float fLogicHeight) const
{
    ...
    float fPixelHeight
        = (fLogicHeight * GetDPIY() * GetMapScalingYNumerator()) / GetMapScalingYDenominator();
    return fPixelHeight;
}
...
DeviceCoordinate Geometry::LogicWidthToDeviceCoordinate(tools::Long nWidth) const
{
    ...
    return LogicToPixel(static_cast<double>(nWidth), GetDPIX(), GetMapScalingXNumerator(),
                        GetMapScalingXDenominator());
#else
    return LogicToPixel(nWidth, GetDPIX(), GetMapScalingXNumerator(), GetMapScalingXDenominator());
#endif
...
double Geometry::LogicWidthToDeviceFontWidth(tools::Long nWidth) const
{
    if (!IsMapModeEnabled())
        return nWidth;
...
DevicePoint Geometry::LogicToDeviceFontCoordinate(const Point& rPoint) const
{
    if (!IsMapModeEnabled())
        return DevicePoint(rPoint.X() + GetXFrameOffset(), rPoint.Y() + GetYFrameOffset());

    return DevicePoint(
        LogicToPixel(static_cast<double>(rPoint.X() + GetMappingOffset().X()), GetDPIX(),
                     GetMapScalingXNumerator(), GetMapScalingXDenominator())
            + GetXFrameOffset() + GetXOffsetFromOriginInPixels(),
        LogicToPixel(static_cast<double>(rPoint.Y() + GetMappingOffset().Y()), GetDPIY(),
                     GetMapScalingYNumerator(), GetMapScalingYDenominator())
            + GetYFrameOffset() + GetYOffsetFromOriginInPixels());
}
