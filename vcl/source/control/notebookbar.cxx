    ...
    , eLastContext(vcl::EnumContext::Context::Context_Any)
{
    ...
    m_pTabControl->HideDisabledTabs();
}
...
{
    vcl::EnumContext::Context eCurrentContext = vcl::EnumContext::GetContextEnum(rEvent.ContextName);
    ...
    if (eLastContext != eCurrentContext)
    {
        for (int nChild = 0; nChild < m_pTabControl->GetChildCount(); ++nChild)
        {
            ...
            if (pPage->HasContext(eCurrentContext) || pPage->HasContext(vcl::EnumContext::Context::Context_Any))
                m_pTabControl->EnablePage(nChild + 1, true);
            else
                m_pTabControl->EnablePage(nChild + 1, false);

            if (pPage->HasContext(eCurrentContext) && eCurrentContext != vcl::EnumContext::Context::Context_Any)
