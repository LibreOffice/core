...
OUString EditUIObject::get_action(VclEventId nEvent) const
{
    ...
    else if(nEvent == VclEventId::EditModify)
    {
        //under working
        return "debugging here";
    }
    else
        return WindowUIObject::get_action(nEvent);
}
...
OUString ListBoxUIObject::get_action(VclEventId nEvent) const
{
    if (nEvent == VclEventId::ListboxSelect)
    {
        ...
        return "Select element with position " + OUString::number(nPos) +" in '" + mxListBox->get_id() +"' from" + get_top_parent(mxListBox)->get_id() ;
    }
...
OUString TabControlUIObject::get_action(VclEventId nEvent) const
{
    if (nEvent == VclEventId::TabpageActivate)
    {
        ...
        return "Choose Tab number " + OUString::number(mxTabControl->GetPagePos(nPageId)) +" from '" + mxTabControl->get_id() + "'" ;
    }
    else
        return WindowUIObject::get_action(nEvent);
}
