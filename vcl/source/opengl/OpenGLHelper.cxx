...
{
    static const sal_uInt32 MAX_CACHE_SIZE = 10;
    typedef std::deque<sal_uInt32> CacheIndex;
    typedef std::unordered_map<sal_uInt32, rtl::OString> CacheType;
    static CacheIndex aCacheIndex;
    static CacheType aCache;
    sal_uInt32 nHash = rtl_crc32(0, rFilename.getStr(), rFilename.getLength());
    CacheType::const_iterator it = aCache.find( nHash );
    if( it != aCache.end() )
    {
        return it->second;
    }
    ...
    if(aFile.open(osl_File_OpenFlag_Read) == osl::FileBase::E_None)
    {
        ...
        if( aCacheIndex.size() == MAX_CACHE_SIZE )
        {
            sal_uInt32 nOldestHash = aCacheIndex.front();
            aCacheIndex.pop_front();
            aCache.erase( nOldestHash );
        }
        aCache[nHash] = OString( content.get() );
        aCacheIndex.push_back( nHash );
        return aCache[nHash];
    }
    else
    {
