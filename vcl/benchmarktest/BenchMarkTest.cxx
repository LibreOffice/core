...
public:
    ...
    virtual ~BenchMarkTestWindow() override { disposeOnce(); }
    ...
    static void updateResults(vcl::RenderContext& rRenderContext, clock_t& aTimeTaken)
    {
        rRenderContext.SetTextColor(COL_YELLOW);
        rRenderContext.DrawText(Point(10, 10),
                                "Time Taken: " + OUString::number(float(aTimeTaken)) + " ms");
    }
                                                    ...
                                                    int nHeight)
    {
        ...
        aRectangle = aRegions[index++];
        {
            ...
            Bitmap aBitmap = aOutDevTest.setupMultiplePolygonsWithPolyPolygon(m_xTimeTaken);
            drawBitmapScaledAndCentered(aRectangle, aBitmap, rRenderContext);
        }
        updateResults(rRenderContext, m_xTimeTaken);
    }

    static void testComplexShape(vcl::RenderContext& rRenderContext, int nWidth, int nHeight)
    {
        tools::Rectangle aRectangle;
        size_t index = 0;
        ...
        {
            vcl::test::BenchMarkTests aOutDevTest;
            Bitmap aBitmap = aOutDevTest.setupComplexPolygon(m_xTimeTaken);
            drawBitmapScaledAndCentered(aRectangle, aBitmap, rRenderContext);
        }
        updateResults(rRenderContext, m_xTimeTaken);
    }
...
namespace
{
class BenchMarkTestApp : public Application
{
public:
    BenchMarkTestApp() {}
    ...
    virtual int Main() override
    {
        try
        {
            ScopedVclPtrInstance<BenchMarkTestWindow> aMainWindow;

            aMainWindow->SetText("BenchMark Tests");
            aMainWindow->Show();
...
void vclmain::createApplication() { static BenchMarkTestApp aApplication; }
