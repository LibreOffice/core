...
const unsigned char* FtFontInfo::GetTable( const char* pTag, sal_uLong* pLength ) const
{
    ...
    else if( nFormat != 0x00010000 && nFormat != T_true && nFormat != T_otto) 
        return NULL;
    ...
    mpLayoutEngine( NULL )
{
    ...
    if( mbArtBold )
    {
        ...
        static const int TT_CODEPAGE_RANGES1_CJKT = 0x3F0000; 
...
#define MKTAG(s) ((((((s[0]<<8)+s[1])<<8)+s[2])<<8)+s[3])
    ...
    sal_uLong nRequestedScript = 0;     
    sal_uLong nRequestedLangsys = 0;    
    

    if( aReqFeatureTagList.empty()) 
        return true;
    ...
    {
        const sal_uLong nScriptTag      = GetUInt( pScriptHeader+0 ); 
        ...
        {
            const sal_uLong nTag    = GetUInt( pScriptTable+0 );    
    ...
    {
        const sal_uLong nTag    = GetUInt( pFeatureHeader+0 ); 
