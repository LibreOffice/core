...
public:
...
#else
            GtkLayoutManager* pOrigLayout = gtk_widget_get_layout_manager(GTK_WIDGET(pNotebook));
            g_object_ref(pOrigLayout);
            m_pLayout = NOTIFYING_LAYOUT(g_object_new(notifying_layout_get_type(), nullptr));
            m_pLayout->m_pOrigManager = pOrigLayout;
            m_pLayout->m_aLink = LINK(this, GtkInstanceNotebook, SizeAllocateHdl);
            gtk_widget_set_layout_manager(GTK_WIDGET(pNotebook), GTK_LAYOUT_MANAGER(m_pLayout));
#endif
        }
    ...
    virtual ~GtkInstanceNotebook() override
    {
        ...
        if (m_pLayout)
        {
            ...
            GtkLayoutManager* pOrigLayout = m_pLayout->m_pOrigManager;
            gtk_widget_set_layout_manager(GTK_WIDGET(m_pNotebook), pOrigLayout);
            g_object_unref(pOrigLayout);
        }
#endif
