...
private:
    ...
    bool signal_test_expand_row(GtkTreeIter& iter)
    {
        ...
        GtkTreeModel *pModel = GTK_TREE_MODEL(m_pTreeStore);
        if (gtk_tree_model_get_n_columns(pModel) >= 6)
        {
            GtkTreeIter tmp;
            if (gtk_tree_model_iter_children(pModel, &tmp, &iter))
            {
                gboolean bOnDemand(false);
                gtk_tree_model_get(pModel, &tmp, 5, &bOnDemand, -1);
                if (bOnDemand)
                {
...
public:
                        ...
                        VirtualDevice* pImageSurface, const OUString* pExpanderName, bool bChildrenOnDemand) override
    {
        ...
        {
            GtkTreeIter subiter;
            insert_row(m_pTreeStore, subiter, &iter, -1, nullptr, "loading", nullptr, nullptr, nullptr);
            gtk_tree_store_set(m_pTreeStore, &subiter, 5, true, -1);
        }
        enable_notify_events();
    }
    ...
    virtual bool iter_children(weld::TreeIter& rIter) const override
    {
        ...
        if (ret)
        {
            ...
            gtk_tree_model_get(pModel, &rGtkIter.iter, 5, &ret, -1);
            return !ret;
        }
        return ret;
    }
