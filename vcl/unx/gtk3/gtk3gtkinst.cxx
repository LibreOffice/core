...
struct DialogRunner
{
       ...
       , m_pLoop(nullptr)
    {
        ...
        m_pFrameWindow = pFrame ? pFrame->GetWindow() : nullptr;
    }

    bool loop_is_running() const
    {
        return g_main_loop_is_running(m_pLoop);
    }
    ...
    static gint signal_delete(GtkDialog*, GdkEventAny*, gpointer data)
    {
        DialogRunner* pThis = static_cast<DialogRunner*>(data);
        pThis->loop_quit();
    ...
    {
        if (m_pFrameWindow)
            m_pFrameWindow->IncModalCount();
    }

    void dec_modal_count()
    {
        if (m_pFrameWindow)
            m_pFrameWindow->DecModalCount();
    }
