...
class GtkInstanceWindow : public GtkInstanceWidget, public virtual Hackery::Window
{
private:
    GtkWindow* m_pWindow;
...
public:
    ...
    virtual void set_transient_for(SalFrame* pParent) override
    {
    ...
    virtual void set_transient_for(Hackery::Window* pParent) override
    {
...
class GtkInstanceDialog : public GtkInstanceWindow, public virtual Hackery::Dialog
{
private:
    GtkDialog* m_pDialog;
public:
...
class GtkInstanceEntry : public GtkInstanceWidget, public virtual Hackery::Entry
{
private:
    GtkEntry* m_pEntry;
    gulong m_nSignalId;
...
class GtkInstanceTreeView : public GtkInstanceWidget, public virtual Hackery::TreeView
{
private:
...
public:
    GtkInstanceTreeView(GtkTreeView* pTreeView, bool bTakeOwnership)
        ...
        , m_nSignalId(g_signal_connect(gtk_tree_view_get_selection(pTreeView), "changed", G_CALLBACK(signalChanged), this))
    {
    }
    ...
    {
        g_signal_handler_disconnect(gtk_tree_view_get_selection(m_pTreeView), m_nSignalId);
    }
};


class GtkInstanceSpinButton : public GtkInstanceEntry, public virtual Hackery::SpinButton
{
private:
...
class GtkInstanceDrawingArea : public GtkInstanceWidget, public virtual Hackery::DrawingArea
{
private:
    GtkDrawingArea* m_pDrawingArea;
    gulong m_nDrawSignalId;
    gulong m_nSizeAllocateSignalId;
    static gboolean signalDraw(GtkWidget* pWidget, cairo_t* cr, gpointer widget)
    {
        GtkInstanceDrawingArea* pThis = static_cast<GtkInstanceDrawingArea*>(widget);
        guint width = gtk_widget_get_allocated_width(pWidget);
        guint height = gtk_widget_get_allocated_height(pWidget);
        pThis->signal_draw(cr, width, height);
        return false;
    }
    static void signalSizeAllocate(GtkWidget*, GdkRectangle* allocation, gpointer widget)
    {
        GtkInstanceDrawingArea* pThis = static_cast<GtkInstanceDrawingArea*>(widget);
        pThis->signal_size_allocate(Size(allocation->width, allocation->height));
    }
    void signal_size_allocate(const Size& rAllocation)
    {
        m_aSizeAllocateHdl.Call(rAllocation);
    }
    void signal_draw(cairo_t* cr, guint nWidth, guint nHeight)
    {
        double fXScale = 1.0, fYScale = 1.0;
#if CAIRO_VERSION >= CAIRO_VERSION_ENCODE(1, 14, 0)
        cairo_surface_get_device_scale(cairo_get_target(cr), &fXScale, &fYScale);
#endif
        std::vector<unsigned char> aBuffer(nHeight * fXScale * nWidth * fYScale * 4);
        memset(aBuffer.data(), 0xFF, aBuffer.size());

        ScopedVclPtrInstance<VirtualDevice> xDevice(nullptr, Size(1, 1), DeviceFormat::DEFAULT);
        const int nStride = cairo_format_stride_for_width(CAIRO_FORMAT_ARGB32, nWidth * fXScale);
        xDevice->SetOutputSizePixelScaleOffsetAndBuffer(Size(nWidth, nHeight), Fraction(1.0), Point(),
                                                        aBuffer.data());

        m_aDrawHdl.Call(*xDevice);

        cairo_surface_t *s = cairo_image_surface_create_for_data(aBuffer.data(), CAIRO_FORMAT_ARGB32,
                                                                 nWidth * fXScale, nHeight * fYScale, nStride);
#if CAIRO_VERSION >= CAIRO_VERSION_ENCODE(1, 14, 0)
        cairo_surface_set_device_scale(s, fXScale, fYScale);
#endif
...
public:
    ...
    virtual Hackery::Window* get_window(const OString &id) override
    {
        GtkWindow* pWindow = GTK_WINDOW(gtk_builder_get_object(m_pBuilder, id.getStr()));
        return new GtkInstanceWindow(pWindow, false);
    }

    virtual Hackery::Widget* get_widget(const OString &id) override
    {
    ...
    virtual Hackery::Entry* get_entry(const OString &id) override
    {
    ...
    virtual Hackery::TreeView* get_tree_view(const OString &id) override
    {
    ...
    virtual Hackery::DrawingArea* get_drawing_area(const OString &id) override
    {
...
Hackery::Builder* GtkInstance::CreateBuilder(const OUString &rUri)
{
...
Hackery::Dialog* GtkInstance::CreateMessageDialog(Hackery::Window *pParent, VclMessageType eMessageType, VclButtonsType eButtonsType, const OUString &rPrimaryMessage)
{
