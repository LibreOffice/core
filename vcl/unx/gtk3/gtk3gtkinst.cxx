...
private:
    ...
    GtkContainer* m_pParentWidget;
public:
    GtkInstanceBuilder(GtkContainer* pParent, const OUString& rUIRoot, const OUString& rUIFile)
        ...
        , m_pParentWidget(pParent)
    {
        ...
        g_slist_foreach(pObjectList, postprocess, &aUtf8HelpRoot);
    ...
    const OUString& get_help_root() const
    {
        return m_sHelpRoot;
    }
    ...
    void auto_add_parentless_widgets_to_container(GtkWidget* pWidget)
    {
        if (gtk_widget_get_toplevel(pWidget) == pWidget)
            gtk_container_add(m_pParentWidget, pWidget);
    }
    ...
    virtual Weld::Dialog* weld_dialog(const OString &id) override
    {
        ...
        if (!pDialog)
            return nullptr;
        gtk_window_set_transient_for(GTK_WINDOW(pDialog), GTK_WINDOW(m_pParentWidget));
    ...
    virtual Weld::Container* weld_container(const OString &id) override
    {
        ...
        return new GtkInstanceContainer(pContainer, false);
    }
    ...
    virtual Weld::Notebook* weld_notebook(const OString &id) override
    {
        ...
        return new GtkInstanceNotebook(pNotebook, false);
    }
...
void GtkInstanceWindow::help()
{
    ...
    pHelp->Start(OStringToOUString(sHelpId, RTL_TEXTENCODING_UTF8), xTemp ? xTemp.get() : this);
}

Weld::Builder* GtkInstance::CreateBuilder(Weld::Container* pParent, const OUString& rUIRoot, const OUString& rUIFile)
{
    GtkInstanceContainer* pParentContainer = dynamic_cast<GtkInstanceContainer*>(pParent);
    GtkContainer* pParentWidget = pParentContainer ? pParentContainer->getContainer() : nullptr;
    return new GtkInstanceBuilder(pParentWidget, rUIRoot, rUIFile);
}
