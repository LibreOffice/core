...
private:
    ...
    {
        for (size_t i = 0; i < m_aPages.size(); ++i)
        {
            const gchar* pStr = gtk_buildable_get_name(GTK_BUILDABLE(m_aPages[i]->getContainer()));
            OString aName(pStr, pStr ? strlen(pStr) : 0);
            if (aName == rIdent)
                return i;
        }
        return -1;
...
public:
    ...
    {
        int nRealPage = gtk_assistant_get_current_page(m_pAssistant);
        GtkContainer* pChild = GTK_CONTAINER(gtk_assistant_get_nth_page(m_pAssistant, nRealPage));
        for (size_t i = 0; i < m_aPages.size(); ++i)
        {
            if (m_aPages[i]->getContainer() == pChild)
                return i;
        }
        return -1;
    }
...
//        return gtk_assistant_get_n_pages(m_pAssistant);
        return m_aPages.size();
    }

    virtual void remove_page(int nPage) override
    {
        fprintf(stderr, "hide logical page %d\n", nPage);
        gtk_widget_hide(GTK_WIDGET(m_aPages[nPage]->getContainer()));
    }

    virtual OString get_page_ident(int nPage) const override
    {
        const GtkWidget* pWidget = GTK_WIDGET(m_aPages[nPage]->getContainer());
    ...
    {
        GtkWidget* pWidget = GTK_WIDGET(m_aPages[nPage]->getContainer());

        int nRealPage = -1;
        for (size_t i = 0; i < m_aPages.size(); ++i)
        {
            if (GTK_WIDGET(m_aPages[i]->getContainer()) == pWidget)
            {
                nRealPage = i;
                break;
            }
        ...
        if (gtk_widget_is_visible(pWidget))
            gtk_assistant_set_current_page(m_pAssistant, nRealPage);
        else
        {
            g_object_ref(pWidget);
            fprintf(stderr, "1 n pages %d\n", get_n_pages());
            OString sTitle(gtk_assistant_get_page_title(m_pAssistant, pWidget));
            gtk_assistant_remove_page(m_pAssistant, nRealPage);
            fprintf(stderr, "2 n pages %d\n", get_n_pages());
            gtk_assistant_append_page(m_pAssistant, pWidget);
            gtk_assistant_set_page_type(m_pAssistant, pWidget, GTK_ASSISTANT_PAGE_CUSTOM);
            gtk_assistant_set_page_title(m_pAssistant, pWidget, sTitle.getStr());
            gtk_widget_show(pWidget);
            fprintf(stderr, "3 n pages %d\n", get_n_pages());
            gtk_assistant_set_current_page(m_pAssistant, gtk_assistant_get_n_pages(m_pAssistant) - 1);
            g_object_unref(pWidget);
            fprintf(stderr, "current page is now %s\n", get_current_page_ident().getStr());
        }
    }
    ...
    virtual void set_page_title(const OString& rIdent, const OUString& rTitle) override
    {
        ...
        if (nIndex == -1)
            return;
        gtk_assistant_set_page_title(m_pAssistant, GTK_WIDGET(m_aPages[nIndex]->getContainer()),
