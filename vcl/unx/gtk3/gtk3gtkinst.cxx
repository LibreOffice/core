...
public:
    ...
    virtual float approximate_char_width() const override
    {
        ...
        gint nCharWidth = pango_font_metrics_get_approximate_char_width(pMetrics);
        gint nDigitWidth = pango_font_metrics_get_approximate_digit_width(pMetrics);
        pango_font_metrics_unref(pMetrics);

        return std::max(nCharWidth, nDigitWidth);
    }
...
public:
    ...
    {
        gtk_frame_set_label(m_pFrame, MapToGtkAccelerator(rText).getStr());
    }
...
public:
    ...
    virtual bool get_inconsistent() const
    {
...
public:
    ...
    virtual int find(const OUString& rText) const
    {
...
public:
    ...
    virtual void set_selection(const Selection& rSelection) override
    {
        GtkTextBuffer* pBuffer = gtk_text_view_get_buffer(m_pTextView);
        GtkTextIter start, end;
        gtk_text_iter_set_offset(&start, rSelection.Min());
        gtk_text_iter_set_offset(&end, rSelection.Max());
