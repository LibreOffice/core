...
void Settingsmanager::setFormulaOptionsFromControls(FORMULAOBJECT& f, const Reference<XControlContainer>& xControlContainer) {
  for (const auto& srec : settings) {
    if (!hasControl(xControlContainer, srec.second.controlName)) {
      MSG_INFO(1, "setFormulaOptionsFromControls: control does not exist: " << STR(srec.second.controlName)<< endline);
      continue;
    }
