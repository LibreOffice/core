    ...
    if (rMedium.IsStorage())
    {
        ...
        ErrCode nWarn
            = ReadThroughComponent(rMedium.GetStorage(), xModelComp, "meta.xml", xContext, xInfoSet,
                                   (bOASIS ? u"com.sun.star.comp.Math.XMLOasisMetaImporter"
                                           : u"com.sun.star.comp.Math.XMLMetaImporter"),
        ...
        nWarn = ReadThroughComponent(rMedium.GetStorage(), xModelComp, "settings.xml", xContext,
                                     xInfoSet,
                                     (bOASIS ? u"com.sun.star.comp.Math.XMLOasisSettingsImporter"
                                             : u"com.sun.star.comp.Math.XMLSettingsImporter"),
                                     m_bUseHTMLMLEntities);
        ...
        return ReadThroughComponent(rMedium.GetStorage(), xModelComp, "content.xml", xContext,
                                    xInfoSet, "com.sun.star.comp.Math.XMLImporter",
                                    m_bUseHTMLMLEntities);
    }
    else
    {
        SAL_WARN("starmath", "Failed to read file on the midle");
        return ERRCODE_IO_BROKENPACKAGE;
    }
}
else
{
    Reference<io::XInputStream> xInputStream = new utl::OInputStreamWrapper(rMedium.GetInStream());

    if (xStatusIndicator.is())
        xStatusIndicator->setValue(nSteps++);

    nError
        = ReadThroughComponent(xInputStream, xModelComp, xContext, xInfoSet,
                               "com.sun.star.comp.Math.XMLImporter", false, m_bUseHTMLMLEntities);
}
...
void SmMlImportContext::inheritStyle()
{
    ...
    m_pElement->setAttribute(m_pParent->getAttribute(SmMlAttributeValueType::MlMathcolor));
    m_pElement->setAttribute(m_pParent->getAttribute(SmMlAttributeValueType::MlMathsize));
    m_pElement->setAttribute(m_pParent->getAttribute(SmMlAttributeValueType::MlDir));
    m_pElement->setAttribute(m_pParent->getAttribute(SmMlAttributeValueType::MlDisplaystyle));
