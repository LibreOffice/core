...
$(call gb_ExternalProject_get_state_target,coinmp,build) :
	+$(call gb_ExternalProject_run,build,\
		./configure COIN_SKIP_PROJECTS="Data/Sample" \
			...
			$(if $(DISABLE_DYNLOADING),--disable-shared,$(if $(filter MACOSX,$(OS)),--prefix=/@.__________________________________________________OOO)) \
			--enable-dependency-linking F77=unavailable \
		&& $(MAKE) \
		$(if $(filter MACOSX,$(OS)),\
			&& $(PERL) $(SRCDIR)/solenv/bin/macosx-change-install-names.pl shl OOO \
				Cbc/src/.libs/libCbc.$(CBC_VERSION).dylib \
				Cbc/src/.libs/libCbcSolver.$(CBCSOLVER_VERSION).dylib \
				Cgl/src/.libs/libCgl.$(CGL_VERSION).dylib \
				Clp/src/.libs/libClp.$(CLP_VERSION).dylib \
				Clp/src/OsiClp/.libs/libOsiClp.$(OSICLP_VERSION).dylib \
				CoinMP/src/.libs/libCoinMP.$(COINMP_VERSION).dylib \
				CoinUtils/src/.libs/libCoinUtils.$(COINUTILS_VERSION).dylib \
				Osi/src/Osi/.libs/libOsi.$(OSI_VERSION).dylib \
		) \
	)
