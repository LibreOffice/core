...
$(call gb_ExternalProject_get_state_target,cairo,build) :
	...
	./configure \
		$(if $(debug),STRIP=" ") \
		$(if $(filter ANDROID IOS,$(OS)),CFLAGS="$(if $(debug),-g) $(ZLIB_CFLAGS) $(gb_VISIBILITY_FLAGS)"i) \
		...
		$(if $(filter ANDROID,$(OS)),FREETYPE_CFLAGS="-I$(call gb_UnpackedTarball_get_dir,freetype)/include") \
		$(if $(filter ANDROID,$(OS)),FONTCONFIG_CFLAGS="-I$(call gb_UnpackedTarball_get_dir,fontconfig)") \
