...
ifeq ($(OS),WNT)
$(call gb_ExternalProject_get_state_target,nss,build): CROSS_CC_DIR := $(subst /cl.exe,,$(subst /x64/,/x86/,$(CXX_X64_BINARY)))
$(call gb_ExternalProject_get_state_target,nss,build): HOST_CC_DIR := $(subst /cl.exe,,$(CXX_X64_BINARY))
$(call gb_ExternalProject_get_state_target,nss,build): $(call gb_ExternalExecutable_get_dependencies,python)
	@echo "$(CROSS_CC_DIR);$(HOST_CC_DIR);$(subst ;;,,$(subst $(CROSS_CC_DIR),,$(subst HostX86,HostX64,$(PATH))))"
	$(call gb_ExternalProject_run,build, \
		$(if $(filter X86_64,$(CPUNAME)),USE_64=1,$(if $(CXX_X64_BINARY), \
			export PATH="$(CROSS_CC_DIR);$(HOST_CC_DIR);$(subst ;;,,$(subst $(CROSS_CC_DIR),,$(subst HostX86,HostX64,$(PATH))))" &&)) \
