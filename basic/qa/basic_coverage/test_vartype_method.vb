...
Function doUnitTest as Integer
    ...
    assert( VarType(float) = V_SINGLE  , "VarType(float) is not V_SINGLE" )
    assert( VarType(flaot) = V_DOUBLE  , "VarType(flaot) is not V_DOUBLE" )
    ...
    assert( VarType(byteArray) = V_ARRAY+V_BYTE+5    , "VarType(byteArray) is not V_ARRAY+V_BYTE" ) ' fail here

    If FailedAssertion Then
        'Print "test_vartype_method.vb fails", message
        Exit Function
    EndIf
