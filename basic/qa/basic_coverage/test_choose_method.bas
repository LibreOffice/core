...
Sub catch_Choose_Errors
    ' When missing all parameters or missing all choices - tdf#148466
    AssertError("No arguments provided", 5)
    'AssertError("Missing expression(s)", 13) ' VBA use case
End Sub

Sub AssertError(caseName$, curErrNo As Long)
try: On Error Goto catch
    Dim errorTrapped As Integer : errorTrapped = 0
    Select Case curErrNo
        Case 5 : Choose()
        'Case 13 : Choose(2, "Moon", ,"Earth") ' VBA use case
    End Select
