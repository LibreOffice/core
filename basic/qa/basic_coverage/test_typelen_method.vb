...
Function doUnitTest as Integer
    ...
    assert( TypeLen(str)   = 0 , "TypeLen(str) is not 0" ) ' if empty
    assert( TypeLen(myErr) = 2 , "TypeLen(myErr) is not 2" )
    assert( TypeLen(bool)  = -1 , "TypeLen(bool) is not 1" )
    assert( TypeLen(var)   = 0 , "TypeLen(var) is not 0" ) ' if empty
    ...
    assert( TypeLen(string_)   = 5.7 , "TypeLen(string_) is not 0" ) ' fail here

    If FailedAssertion Then
        ' Print "test_typelen_method.vb fails", message
        Exit Function
    EndIf
...
Dim failedAssertion As Boolean, message As String
