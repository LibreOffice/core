...
public class PptSlidePlacementTests
{
    ...
    [Fact]
    public void AHiddenSlideIsLaidOutAndFlaggedButPrintsNoPage()
    {
        SlidePages pages = Layout("ppt-features.ppt");

        // Two answers, deliberately different, and the difference is the whole point of the test.
        //
        // The deck still *has* three slides and still says which one is hidden, because a caller
        // indexing a corpus wants that slide's text — the same reason extraction reports it.
        pages.Slides.Select(slide => slide.IsHidden).ShouldBe([false, false, true]);

        // But it prints two pages. LibreOffice's PDF export leaves a hidden slide out, and this
        // matched its output on `deck-features.pptx` end to end once the page sequence began
        // skipping them: three pages from both, where we had been emitting a fourth page carrying
        // "This slide is hidden" that the reference had no equivalent for.
