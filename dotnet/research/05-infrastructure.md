...
| **Text shaping / font metrics (used by EditEngine layout)** | **HarfBuzzSharp** for complex-script shaping (Arabic/Indic/etc. glyph shaping, ligatures, kerning) + **SkiaSharp** or **SixLabors.Fonts** for font metrics/glyph outlines depending on whether you also need rasterization (SkiaSharp) or metrics-only (SixLabors.Fonts is lighter-weight if you don't need to actually paint pixels). | LibreOffice itself increasingly uses HarfBuzz internally for complex shaping; matching that choice minimizes shaping-behavior divergence versus real Office/LO output. Pick SkiaSharp if any rendering/rasterization is in scope (e.g. producing thumbnails or a viewer), SixLabors.Fonts if only metrics/reflow (no pixels) are needed. |
...
| **PDF, images (out of scope here but adjacent)** | Not covered by this document's areas A-G, but noted for completeness per the requester's candidate list: **PdfPig** for PDF *reading*, **ImageSharp** for general raster image decode/encode where a pure-managed dependency is preferred over SkiaSharp's native binding. | Mentioned only because they appeared in the candidate list; genuinely out of scope for the container/encryption/text-engine/item-system infrastructure this document covers. |
...
| EditEngine (rich text layout) | **Yes** (bespoke) | HarfBuzzSharp + SkiaSharp/SixLabors.Fonts (shaping/metrics only) |
