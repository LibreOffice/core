...
decoders — good news for a C# port, since ImageSharp/SkiaSharp/System.Drawing.Codecs
...
| Common raster codecs (PNG/JPEG/GIF/WEBP/TIFF/BMP) | libpng/libjpeg-turbo/libwebp/libtiff via thin VCL wrappers | **SkiaSharp** (broadest format+color-space coverage, native codecs) and/or **ImageSharp** (pure managed, MIT-licensed, good for cases wanting to avoid native deps) | ✅ No gap; both libraries comfortably exceed what's needed here. Prefer SkiaSharp if you're already using it for the canvas layer (avoids a second image-decode dependency + potential color-management mismatches between two libraries). |
...
5. **Images**: SkiaSharp/ImageSharp cover the common raster formats fully; budget
