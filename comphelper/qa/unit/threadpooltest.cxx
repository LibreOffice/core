...
void ThreadPoolTest::testPreferredConcurrency() {
    ...
    CPPUNIT_ASSERT_EQUAL(nThreads, (sal_Int32)std::thread::hardware_concurrency());

    // Cap at 3, which should be less than the typical
    // modern CPU's core count.
    setenv("MAX_CONCURRENCY", "3", true);
    nThreads = comphelper::ThreadPool::getPreferredConcurrency();
    CPPUNIT_ASSERT_MESSAGE("Expected no more than 3 threads", nThreads <= 3);

    // Now set the limit > hardware.
