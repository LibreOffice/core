...
OPropertyArrayHelper2::OPropertyArrayHelper2(
    const css::uno::Sequence<css::beans::Property>& rPropSeq)
    : mbRightOrdered(false)
{
    std::vector<css::beans::Property> aProps
        = comphelper::sequenceToContainer<std::vector<css::beans::Property>>(rPropSeq);
    std::sort(aProps.begin(), aProps.end(), PropertyNameLess());
    maInfos = comphelper::containerToSequence(aProps);
    for (auto const& rProp : maInfos)
        maInfosMap.emplace(rProp.Name, rProp);
