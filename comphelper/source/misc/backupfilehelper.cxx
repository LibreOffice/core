...
namespace comphelper
{
    ...
    namespace {
        uno::Reference<XElement> lcl_getConfigElement(uno::Reference<XDocument> xDocument, const OUString& rPath,
    ...
    void BackupFileHelper::tryDisableHWAcceleration()
    {
        ...
        //uno::Reference< XElement > xRet = xRootElement->appendChild(xElemFooNs);
        ...
        /*uno::Reference< XNodeList > aList = xRootElement->getChildNodes();
        if (!aList.is())
            return;
        ...
        for (long a(0); a < nLength; a++)
        {
            const uno::Reference< XElement > aChild(aList->item(a), uno::UNO_QUERY);
            if (!aChild.is())
                continue;

            const OUString aTagName(aChild->getTagName());
            if (aTagName != "item")
                continue;

            const OUString aPath(aChild->getAttribute("oor:path"));

            if (aPath == "/org.openoffice.Office.Common/VCL")
