...
namespace
{
    ...
    bool write_sal_uInt32(oslFileHandle& rHandle, const sal_uInt32& rSource)
    {
        sal_uInt8 aArray[4];
        sal_uInt64 nBaseWritten(0);

        // write rSource
        aArray[0] = sal_uInt8((rSource & 0xff000000) >> 24);
        aArray[1] = sal_uInt8((rSource & 0x00ff0000) >> 16);
        aArray[2] = sal_uInt8((rSource & 0x0000ff00) >> 8);
        aArray[3] = sal_uInt8(rSource & 0x000000ff);
...
namespace
{
    ...
    public:
        ...
        sal_uInt32 getEntrySize() const
        {
            sal_uInt32 nSize(0);

            // FullSize, CRC32 and PackedSize
            nSize += 12;
                            ...
                            if (osl::File::E_None == aSourceFile->read(static_cast<void*>(aArray), 4, nBaseRead) && 4 == nBaseRead)
                            {
                                ...
                                if (nEntries >= 1 && nEntries <= 10)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        for (const auto& a : maPackedFileEntryVector)
                                        {
                                            nHeaderSize += a.getEntrySize();
                                        }
...
namespace comphelper
{
    ...
    bool BackupFileHelper::tryPush(bool bCompress)
    {
        ...
        if (splitBaseURL())
        {
            ...
            osl::Directory::createPath(getPackDirName(), nullptr);
    ...
    bool BackupFileHelper::fileExists(const OUString& rBaseURL) const
    {
        if (!rBaseURL.isEmpty())
        {
