...
public:
    ...
    virtual void execute() override
    {
        ...
        while ( ( pTask = waitForWork() ) )
        {
            ...
            {
                SAL_WARN("comphelper", "exception in thread worker: " << e.what());
            }
            catch (const css::uno::Exception &e)
            {
                SAL_WARN("comphelper", "exception in thread worker: " << e.Message);
            }
            delete pTask;
...
#else
    // 5 minute timeout in production so the app eventually throws some kind of error
    if (maTasksComplete.wait(TimeValue { 5*60, 0 }) == osl_cond_result_timeout))
