...
std::string curlGet(const OString& rURL)
{
    ...
    if (http_code != 200)
    {
        SAL_WARN("cui.settings", "Download failed. Error code: " << http_code);
    }

    if (cc != CURLE_OK)
    {
        SAL_WARN("cui.settings", "curl error: " << cc);
    }
    ...
    if (!m_bDirectURL)
    {
        ...
        else
        {
            ...
            for (const auto & personaInfo : personaInfos)
            {
                ...
                if (!bResult)
                {
                    SAL_INFO("cui.settings", "Couldn't get the preview file. Skipping: " << aPreviewFile);
                    continue;
                }
