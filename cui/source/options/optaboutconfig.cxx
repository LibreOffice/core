...
CuiAboutConfigTabPage::CuiAboutConfigTabPage(weld::Window* pParent)
    ...
    , m_vectorOfModified()
{
    m_xPrefBox->set_size_request(m_xPrefBox->get_approximate_digit_width() * 48,
...
IMPL_LINK_NOARG( CuiAboutConfigTabPage, SearchHdl_Impl, weld::Button&, void)
{
    ...
    m_xPrefBox->freeze();
#if 0
    bool sortMode = m_xPrefBox->GetModel()->GetSortMode();
    sal_uInt16 sortedCol = m_xPrefBox->GetSortedCol();

    if( sortMode != SortNone )
        m_xPrefBox->SortByCol( 0xFFFF );
#endif
...
#if 0
    if( sortMode != SortNone )
        m_xPrefBox->SortByCol(sortedCol, sortMode == SortAscending);
#endif
...
void CuiAboutConfigTabPage::InsertEntry(const prefBoxEntry& rEntry)
{
    ...
    index = 0;
    std::shared_ptr<weld::TreeIter> xParentEntry(m_xPrefBox->make_iterator());
    std::shared_ptr<weld::TreeIter> xGrandParentEntry;
    ...
    do
    {
        ...
        if (bStartOk)
        {
            do
            {
                if (m_xPrefBox->get_text(0) == sParentName)
                {
                    hasEntry = true;
                    break;
                }
        ...
        if (xGrandParentEntry)
            m_xPrefBox->expand_row(*xGrandParentEntry);
        xGrandParentEntry = xParentEntry;
    ...
    m_xPrefBox->expand_row(*xParentEntry);
}

IMPL_LINK(CuiAboutConfigTabPage, ExpandingHdl_Impl, const weld::TreeIter&, rEntry, bool)
{
    if (UserData *pUserData = reinterpret_cast<UserData*>(m_xPrefBox->get_id(rEntry).toInt64()))
