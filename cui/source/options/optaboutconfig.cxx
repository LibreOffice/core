...
{
    return LogicToPixel(Size(150,500),MAP_APPFONT);
}
...
void CuiAboutConfigTabPage::AddToModifiedVector( boost::shared_ptr< Prop_Impl > rProp )
{
...
uno::Sequence< OUString > commaStringToSequence( const OUString& rCommaSepString )
{
    ...
    return comphelper::containerToSequence( tempVector );
}
            ...
            if( ret == RET_OK )
            {
                ...
                if ( sPropertyType == OUString("short"))
                {
                    ...
                    else if( sPropertyType == OUString("[]short") )
                    {
                        ...
                        uno::Sequence< sal_Int16 > seqShort( seqStr.getLength() );
                        //convert all strings to appropriate type
                        for( int i = 0; i < seqStr.getLength(); ++i )
                        {
                    ...
                    {
                        uno::Sequence< OUString > seqStrLong;
                        seqStrLong = commaStringToSequence( sNewValue );

                        uno::Sequence< sal_Int32 > seqLong( seqStrLong.getLength() );
                        for( int i = 0; i < seqStrLong.getLength(); ++i )
                        {
                    ...
                    {
                        uno::Sequence< OUString > seqStrHyper;
                        seqStrHyper = commaStringToSequence( sNewValue );
                        uno::Sequence< sal_Int64 > seqHyper( seqStrHyper.getLength() );
                        for( int i = 0; i < seqStrHyper.getLength(); ++i )
                        {
                    ...
                    {
                        uno::Sequence< OUString > seqStrDoub;
                        seqStrDoub = commaStringToSequence( sNewValue );
                        uno::Sequence< double > seqDoub( seqStrDoub.getLength() );
                        for( int i = 0; i < seqStrDoub.getLength(); ++i )
                        {
                    ...
                    {
                        uno::Sequence< OUString > seqStrFloat;
                        seqStrFloat = commaStringToSequence( sNewValue );
                        uno::Sequence< sal_Int16 > seqFloat( seqStrFloat.getLength() );
                        for( int i = 0; i < seqStrFloat.getLength(); ++i )
                        {
                    ...
                    {
                        pProperty->Value = uno::makeAny( commaStringToSequence( sNewValue ) );
                    }
                    else //unknown
                        throw uno::Exception();
