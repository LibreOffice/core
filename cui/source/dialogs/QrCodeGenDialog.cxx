...
OUString QrCodeGenDialog::ConvertToSVGFormat(ZXing::BitMatrix bitmatrix)
{
    OUStringBuffer sb,sbPath;
    ...
    for(int i = 0; i < height; ++i)
    {
        ...
        for(int j = 0; j < width; ++j)
        {
            if(row.get(j))
            {
                sbPath.append(" M"+OUString::number(j)+","+OUString::number(i)+"h1v1h-1z");
            }
        }
    }
    sb.append("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
    "<svg xmlns=\"http://www.w3.org/2000/svg\" version=\"1.1\" viewBox=\"0 0 "
    + OUString::number(width) + " " + OUString::number(height) + "\" stroke=\"none\">\n"
    "<style type=\"text/css\">\n"
    ".black {fill:#000000;}\n"
    "</style>\n";
    "<path class=\"black\"  d=\"" + sbPath.toString() + "\"/>\n"
    "</svg>\n");
    return sb.toString();
}
