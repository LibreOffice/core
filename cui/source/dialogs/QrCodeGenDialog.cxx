...
void QrCodeGenDialog::Apply()
{
    ...
    GenerateQRCode(aQRCode.Payload, aQRCode.ErrorCorrection, aQRCode.Border);
...
std::string QrCodeGenDialog::ConvertToSVGFormat(ZXing::BitMatrix bitmatrix)
{
    std::ostringstream sbPath,sb;
    int width = bitmatrix.width();
    int height = bitmatrix.height();
    ZXing::BitArray row(width);
    for(int i = 0; i < height; ++i)
    {
        row = bitmatrix.getRow(i, row);
        for(int j = 0; j < width; ++j)
        {
            if(row.get(j))
            {
                sbPath << " M"+j+","+i+"h1v1h-1z";
            }
        }
    }
    sb << "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n";
    sb << "<svg xmlns=\"http://www.w3.org/2000/svg\" version=\"1.1\" viewBox=\"0 0 " +
    std::to_string(width) + " " + std::to_string(height) + "\" stroke=\"none\">\n";
    sb << "<style type=\"text/css\">\n";
    sb << ".black {fill:#000000;}\n";
    sb << "</style>\n";
    sb << "<path class=\"black\"  d=\"" + sb.str() + "\"/>\n";
    sb << "</svg>\n";
...
OUString QrCodeGenDialog::GenerateQRCode(OUString aQRText, tools::Long aQRECC, int aQRBorder)
{
    ...
    ZXing::BitMatrix bitmatrix = writer.encode(ZXing::TextUtfEncoding::FromUtf8(QRText), 300, 300);
    std::string SvgString = ConvertToSVGFormat(bitmatrix);

    return OUString::createFromAscii(SvgString.c_str());
#else
    (void)aQRText;
    (void)aQRECC;
    (void)aQRBorder;
