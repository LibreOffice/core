    ...
    , m_xSpinBorder(m_xBuilder->weld_spin_button("edit_border"))
{
    ...
    int aQrCodeBorder;
    xProps->getPropertyValue("QrCode.Border") >>= aQrCodeBorder;
...
void QrCodeGenDialog::Apply()
{
    ...
    if (!aQrText.isEmpty())
        xShapeProps->setPropertyValue("QrCode.Text", Any(aQrText));
    // ECC Type, there was a if argument to pass, Currently removing the condition
    xShapeProps->setPropertyValue("QrCode.ECC", Any(aQrECC));
    if (aQrBorder > 0)
        xShapeProps->setPropertyValue("QrCode.Border", Any(aQrBorder));
