    ...
    , m_xSpinBorder(m_xBuilder->weld_spin_button("edit_border"))
{
    ...
    if (!bEditExisting)
    {
        //Debugging shows all the four buttons are active simultaneously, but
        for (auto& a : m_xECC)
            a->connect_toggled(LINK(this, QrCodeGenDialog, ECCHdl));
        return;
    }
...
void QrCodeGenDialog::Apply()
{
    ...
    bool bHighECCActive(m_xECC[3]->get_active());
...
void QrCodeGenDialog::GetErrorCorrection(long ErrorCorrection)
{
    switch (ErrorCorrection)
    {
        ...
        {
            m_xECC[1]->connect_toggled(LINK(this, QrCodeGenDialog, ECCHdl));
            m_aECCSelect = ErrorCorrection;
            break;
        }
        case css::drawing::QRCodeErrorCorrection::QUARTILE:
        {
            m_xECC[2]->connect_toggled(LINK(this, QrCodeGenDialog, ECCHdl));
            m_aECCSelect = ErrorCorrection;
            break;
        }
        case css::drawing::QRCodeErrorCorrection::HIGH:
        {
            m_xECC[3]->connect_toggled(LINK(this, QrCodeGenDialog, ECCHdl));
            m_aECCSelect = ErrorCorrection;
            break;
        }
        case css::drawing::QRCodeErrorCorrection::LOW:
        {
            m_xECC[0]->connect_toggled(LINK(this, QrCodeGenDialog, ECCHdl));
            m_aECCSelect = ErrorCorrection;
            break;
        }
    }
...
IMPL_LINK(QrCodeGenDialog, ECCHdl, weld::ToggleButton&, rButton, void)
{
    for (int i = 0; i <= 3; ++i)
    {
        if (&rButton == m_xECC[i].get())
        {
            m_xECC[i]->set_active(m_xECC[i]->get_active());
            break;
        }
    }
}
