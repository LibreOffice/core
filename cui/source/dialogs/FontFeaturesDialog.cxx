...
void FontFeaturesDialog::fillGrid(std::vector<vcl::font::Feature> const& rFontFeatures)
{
    ...
    for (vcl::font::Feature const& rFontFeature : rFontFeatures)
    {
        ...
        {
            aCurrentItem.m_xText->set_grid_left_attach(nGridPositionX);
            aCurrentItem.m_xText->set_grid_top_attach(nGridPositionY);
            ...
            aCurrentItem.m_xCombo->set_grid_left_attach(nGridPositionX + 1);
            aCurrentItem.m_xCombo->set_grid_top_attach(nGridPositionY);
            aCurrentItem.m_xCombo->connect_changed(aComboBoxSelectHandler);
            aCurrentItem.m_xCombo->show();
        }
        else
        {
            aCurrentItem.m_xCheck->set_grid_left_attach(nGridPositionX);
            aCurrentItem.m_xCheck->set_grid_top_attach(nGridPositionY);
