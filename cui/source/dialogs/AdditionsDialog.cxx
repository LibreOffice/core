...
#define MAX_ITEM_IN_LIST 30 // To use in the API.
...
std::vector<AdditionInfo> SearchAndParseThread::CreateInfoVectorToLoading(sal_uInt32 startNumber)
{
    std::vector<AdditionInfo> additionInfos;
    for (sal_uInt32 i = startNumber; i < m_pAdditionsDialog->m_nMaxItemCount; i++)
    {
...
void SearchAndParseThread::LoadImage(const OUString& rPreviewFile,
                                     const AdditionsItem& rCurrentItem)
{
...
void SearchAndParseThread::UpdateUI(const std::vector<AdditionInfo>& aAdditionInfos)
{
    ...
    for (auto& additionInfo : aAdditionInfos)
    {
        ...
        rDialog->m_aAdditionsListUIElements.emplace_back(rDialog->m_xContentGrid.get());
        AdditionsItem& aCurrentItem = rDialog->m_aAdditionsListUIElements.back();
        ...
        if (m_pAdditionsDialog->m_nCurrentListItemCount == rDialog->m_nMaxItemCount)
        {
    ...
    , m_xLabelProgress(m_xBuilder->weld_label("labelProgress"))
{
    ...
    m_nMaxItemCount = MAX_ITEM_IN_LIST; // Dialog initialization item count
...
void AdditionsDialog::ClearList()
{
    ...
    for (auto& item : this->m_aAdditionsListUIElements)
    {
        item.m_xContainer->hide();
    }
    this->m_aAdditionsListUIElements.clear();
}
...
IMPL_LINK_NOARG(AdditionsItem, ShowMoreHdl, weld::Button&, void)
{
    ...
    m_pParentDialog->m_nMaxItemCount += MAX_ITEM_IN_LIST;
