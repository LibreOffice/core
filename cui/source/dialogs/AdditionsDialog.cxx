...
void SearchAndParseThread::UpdateUI()
{
    //Get Preview Files
    for (size_t i = m_pAdditionsDialog->m_nCurrentListItemCount;
         i < m_pAdditionsDialog->m_nMaxItemCount; i++)
    {
        AdditionInfo& additionInfo = m_pAdditionsDialog->m_aAllExtensionsVector[i];
        if (!m_bExecute)
            return;
...
void SearchAndParseThread::execute()
{
    ...
    /*
    if (m_pAdditionsDialog->m_aAllExtensionsVector.empty())
    {
