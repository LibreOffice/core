...
void SearchAndParseThread::Search()
{
    ...
    CheckInstallButtonsLabels();
}

void SearchAndParseThread::CheckInstallButtonsLabels()
{
            ...
            if (extensionVersion != nullptr)
            {
                ...
                for (auto& rInfo : m_pAdditionsDialog->m_aAdditionsItems)
                {
                    ...
                    if (!currentExtensionName.isEmpty()
                        && !textSearch.searchForward(sExtensionDownloadURL))
                    {
                        continue;
                    }
                    else
                    {
                        rInfo.m_xButtonInstall->set_label("Uninstall");
                    }
                }
            }
        }
