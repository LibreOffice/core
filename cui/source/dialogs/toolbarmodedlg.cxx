...
static const char* const ModeNames[9] = { "Default",
                                          "Single",
                                          "Sidebar",
                                          "notebookbar.ui",
...
static const char* const ImageNames[9] = { "default.png",
                                           "single.png",
                                           "sidebar.png",
                                           "notebookbar.png",
...
static int GetCurrentModeID()
{
    ...
    if (pViewFrame)
    {
        ...
        if (aAppNode.isValid())
        {
            ...
            for (int i = 0; i < 9; i++)
                if (aModeName == OUString::createFromAscii(ModeNames[i]))
                    return i;
        }
    }
    return -1;
    ...
    , m_pInfoLabel(m_xBuilder->weld_label("lbInfo"))
{
    ...
    if (pMod == SfxApplication::GetModule(SfxToolsModule::Writer))
        ...
        m_pApply->set_label(aLabel.replaceFirst("%MODULE", "Draw/Impress"));
...
void ToolbarmodeDialog::UpdateImage(const char* aFileName)
{
    // load image
    OUString aURL("$BRAND_BASE_DIR/$BRAND_SHARE_SUBDIR/toolbarmode/");
    OUString sFileName = OUString::createFromAscii(aFileName);
...
IMPL_LINK(ToolbarmodeDialog, OnApplyClick, weld::Button&, rButton, void)
{
    ...
    for (int i = 0; i < 9; i++)
    {
        if (m_pRadioButtons[i]->get_active())
        {
            sCmd = OUString::createFromAscii(ModeNames[i]);
            break;
        }
    }
