...
static OUString GetCurrentApp()
{
    ...
    {
        const css::uno::Reference<css::frame::XFrame> xCurrentFrame
            = pViewFrame->GetFrame().GetFrameInterface();
        const css::uno::Reference<css::uno::XComponentContext> xContext
            = comphelper::getProcessComponentContext();
        const css::uno::Reference<css::frame::XModuleManager> xModuleManager
...
{
    OSL_ASSERT(SAL_N_ELEMENTS(m_pRadioButtons) == SAL_N_ELEMENTS(TOOLBARMODES_ARRAY));
...
IMPL_LINK(ToolbarmodeDialog, OnApplyClick, weld::Button&, rButton, void)
{
    ...
    if ((&rButton == m_pApplyAll.get()) && (!sCmd.isEmpty()))
    {
        ...
        if (SfxViewFrame* pViewFrame = SfxViewFrame::Current())
        {
            ...
            if (sCurrentApp != "Writer") //dispatch command does not work if entry has been set before
