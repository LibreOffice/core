    ...
    , m_xModel(xModel)
{
    ...
    if (!bIsSignatureLine)
    {
        SAL_WARN("cui", "No signature line selected!");
        return;
    }
    ...
    /* bool bCanAddComment;
    xProps->getPropertyValue("SignatureLineCanAddComment") >>= bCanAddComment;
    m_xEditComment->show(bCanAddComment); */

    // Read properties from selected signature line
    OUString aSignatureLineId;
    xProps->getPropertyValue("SignatureLineId") >>= aSignatureLineId;
    SAL_DEBUG("aSignatureLineId: " << aSignatureLineId);
