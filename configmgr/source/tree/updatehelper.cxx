...
void OAdjustUpdateAction::handle(ValueChange& _rChange)
{
    if (checkTree())
    {
        ...
        else
        {
            ...
            data::TreeSegment aNewTree = data::TreeSegment::createNew(base_ptr(pNode));
...
//--------------------------------------------------------------------------
    ...
    void TreeUpdate::handle(AddNode& aAddNode)
    {
        ...
        if (m_pCurrentSubtree)
        {
            if (aAddNode.isReplacing())
            {
                ...
                aAddNode.takeReplacedTree( data::TreeSegment::createNew(aOldNode) );
            }

            memory::Pointer pInsertedTree( m_pCurrentSubtree->addChild(aAddNode.getNewTree().cloneData()) );
            aAddNode.setInsertedAddress( data::TreeAddress(pInsertedTree) );
    ...
    void TreeUpdate::handle(RemoveNode& aRemoveNode)
    {
        ...
        if (m_pCurrentSubtree)
        {
            ...
            aRemoveNode.takeRemovedTree( data::TreeSegment::createNew(aOldNode) );
...
//--------------------------------------------------------------------------
// -----------------------------------------------------------------------------
    struct OBuildChangeTreeForward : NodeAction
    {
    protected:
        SubtreeChange&  m_rChangeList;
        INode const*    m_pCacheNode;

    public:
        OBuildChangeTreeForward(SubtreeChange& rList, INode const* pNode)
                : m_rChangeList(rList)
                , m_pCacheNode(pNode)
        {
        }

        virtual void handle(ValueNode const& _nNode)
        {
            OUString aNodeName = _nNode.getName();
            ISubtree const* pTree = m_pCacheNode->asISubtree();
            OSL_ENSURE(pTree, "OBuildChangeTree::handle : node must be a inner node!");
            if (pTree)
            {
                INode const* pChild = pTree->getChild(aNodeName);
                ValueNode const* pValueNode = pChild ? pChild->asValueNode() : NULL;
                OSL_ENSURE(pValueNode, "OBuildChangeTree::handle : node must be a value node!");
        ...
        virtual void handle(ISubtree const& _rSubtree)
        {
            ...
            ISubtree const* pSubtreeInCache = m_pCacheNode->asISubtree();
            OSL_ENSURE(pSubtreeInCache, "OBuildChangeTree::handle : node must be a inner node!");
            if (pSubtreeInCache)
            {
                INode const* pChild = pSubtreeInCache->getChild(aNodeName);
                ...
                if (pChild)
                {
                    OSL_ENSURE(pChild->asISubtree(), "OBuildChangeTree::handle : node must be a inner node!");
                    // generate a new change

                    std::auto_ptr<SubtreeChange> pChange( new SubtreeChange(_rSubtree) );
                    OBuildChangeTreeForward aNextLevel(*pChange, pChild);
                ...
                else
                {
                    ...
                    OSL_ENSURE(m_rChangeList.isSetNodeChange(), "Found newly added node in group");

                    auto_ptr<INode> pSubtree( _rSubtree.clone() );
                    data::TreeSegment aNewTree = data::TreeSegment::createNew(pSubtree);

                    auto_ptr<Change> pAdd(new AddNode(aNewTree, aNodeName, _rSubtree.isDefault()));
...
// -----------------------------------------------------------------------------
    ...
    struct OBuildChangeTreeBackward : NodeAction
    {
    protected:
        SubtreeChange&  m_rChangeList;
        INode const *   m_pCacheNode;
    public:
        OBuildChangeTreeBackward(SubtreeChange& rList, INode const* pNode)
                : m_rChangeList(rList)
                , m_pCacheNode(pNode)
        {
        }
            ...
            if (pSubtreeInCache)
            {
                ...
                if (pChild)
                {
                    ...
                    if (pChange && pChange->ISA(SubtreeChange))
                    {
                        ...
                        OBuildChangeTreeBackward aNextLevel(*pSubtreeChange, pChild);
                        aNextLevel.applyToChildren(_rSubtree);
                    }
                }
                else
...
// apply a set of changes to the target tree, return true, if there are changes found
    ...
    {
        INode const * pCachedNode = _aExistingData.rawAddress();
    // create the differences
        OBuildChangeTreeForward aNewChangeTree(_rResultingUpdateTree, pCachedNode);
        _aNewData.forEachChild(aNewChangeTree);

        OBuildChangeTreeBackward aNewChangeTree2(_rResultingUpdateTree, & _aNewData);
        if (ISubtree const * pCachedTree = pCachedNode->asISubtree())
            aNewChangeTree2.applyToChildren(*pCachedTree);
        else
            OSL_ASSERT(false);
