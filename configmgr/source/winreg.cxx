...
void dumpWindowsRegistryKey(HKEY hKey, OUString aKeyName, oslFileHandle aFileHandle)
{
    ...
    if(RegOpenKeyExW(hKey, aKeyName.getStr(), 0, KEY_READ, &hCurKey) == ERROR_SUCCESS)
    {
        ...
        else if(nValues)
        {
            ...
            wchar_t* pValueName = new (std::nothrow) wchar_t[nLongestValueNameLen + 1];
            wchar_t* pValue = new (std::nothrow) wchar_t[nLongestValueLen + 1];
            ...
            if(pValueName && pValue)
            {
                ...
                {
                    DWORD nValueNameLen = nLongestValueNameLen;
                    DWORD nValueLen = nLongestValueLen;
                ...
                OString aPath = OUStringToOString(aKeyName.replaceAll("\\","/").copy(0, aLastSeparator), RTL_TEXTENCODING_UTF8);
                OString aProp = OUStringToOString(aKeyName.copy(aLastSeparator + 1), RTL_TEXTENCODING_UTF8);
...
bool dumpWindowsRegistry(OUString& rFileURL)
{
    HKEY hKey;
    ...
    switch (osl::FileBase::createTempFile(0, &aFileHandle, &rFileURL)) {
