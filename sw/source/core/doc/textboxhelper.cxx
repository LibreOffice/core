...
void SwTextBoxHelper::create(SwFrameFormat* pShape)
{
    ...
    {
        uno::Reference<text::XTextCursor> xCur = xTextDocument->getText()->createTextCursor();
        xCur->gotoStart(false);
        xTextContentAppend->insertTextContentWithProperties(
            xTextFrame, uno::Sequence<beans::PropertyValue>(), xCur->getStart());
    }
    ...
    do
    {
        switch (pItem->Which())
        {
            case RES_VERT_ORIENT:
            {
                ...
                // so let's correct it. (Later it have to be fixed correctly.)
                if (rShape.GetAnchor().GetAnchorId() == RndStdIds::FLY_AT_PAGE
                    && aOrient.GetRelationOrient() == text::RelOrientation::FRAME)
                {
                    aOrient.SetRelationOrient(text::RelOrientation::PAGE_FRAME);
                }
