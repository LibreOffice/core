...
{
    assert(pDrawObject);
    if (m_pTextBoxes.empty() || m_bUnderDestruction)
        return;
...
{
    // First disable the delete calls from ~SwFrameFormat()
    m_bUnderDestruction = true;

    // Delete all textboxes from the main frame format table from the doc
    // Keep in mind, the adresses of them will be still registered in the node!
    for (const auto& rTextBox : m_pTextBoxes)
        m_pOwnerShapeFormat->GetDoc()->getIDocumentLayoutAccess().DelLayoutFormat(
            rTextBox.m_pTextBoxFormat);

    // After removing them from doc, remove the entries (adresses) from the node.
    m_pTextBoxes.clear();
}
