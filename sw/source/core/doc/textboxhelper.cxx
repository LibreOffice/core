...
{
    auto xFrame = uno::Reference<beans::XPropertySet>(
                      pShape->FindRealSdrObject()->getWeakUnoShape().get(), uno::UNO_QUERY)
                      ->getPropertyValue(UNO_NAME_TEXT_BOX_CONTENT)
                      .get<uno::Reference<text::XTextFrame>>();
    if (xFrame)
    {
...
uno::Any SwTextBoxHelper::queryInterface(const SwFrameFormat* pShape, const uno::Type& rType)
{
    ...
    //else if (rType == cppu::UnoType<css::beans::XPropertySet>::get())
    //{
    //    lcl_queryInterface<beans::XPropertySet>(pShape, aRet);
    //}
        ...
        if (xShapeProps)
        {
            ...
            if (xFrameProps)
            {
                if (!pShapeFormat)
                {
                    ...
                    else if (auto pShape = dynamic_cast<SwXGroupShape*>(xShape.get()))
                        pShapeFormat = pShape->GetFrameFormat();
                }
