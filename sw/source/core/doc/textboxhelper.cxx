...
void SwTextBoxNode::Clone(SwDoc* pDoc, SwFrameFormat* o_pTarget, bool bSetAttr,
                          bool bMakeFrame) const
{
    ...
    Clone_Impl(pDoc, o_pTarget, m_pOwnerShapeFormat->FindSdrObject(), o_pTarget->FindSdrObject(),
               bSetAttr, bMakeFrame);

    m_bIsCloningInProgress = false;
}

void SwTextBoxNode::Clone_Impl(SwDoc* pDoc, SwFrameFormat* o_pTarget, SdrObject* pSrcObj,
                               SdrObject* pDestObj, bool bSetAttr, bool bMakeFrame) const
{
    if (!pSrcObj || !pDestObj)
        return;
    ...
    if (pSrcList && pDestList)
    {
        ...
        for (auto i = 0; i < pSrcList->GetObjCount(); ++i)
        {
            Clone_Impl(pDoc, o_pTarget, pSrcList->GetObj(i), pDestList->GetObj(i), bSetAttr,
                       bMakeFrame);
        }
        return;
    }
    ...
    if (!pSrcList && !pDestList)
    {
        if (auto pSrcFormat = GetTextBox(pSrcObj))
            if (auto pTargetFormat = pDoc->getIDocumentLayoutAccess().CopyLayoutFormat(
                    *pSrcFormat, pSrcFormat->GetAnchor(), bSetAttr, bMakeFrame))
            {
