    ...
    if (isTextBoxShapeHasValidTextFrame(pShape))
    {
        if (auto pFormat = getOtherTextBoxFormat(pShape, RES_DRAWFRMFMT))
        {
            if (auto xFrame = SwXTextFrame::CreateXTextFrame(*pFormat->GetDoc(), pFormat))
                ...
                {
                    sErrMsg = "UNO-Exeption: " + e.Message;
                }
            else
...
bool SwTextBoxHelper::changeAnchor(SwFrameFormat* pShape)
{
    if (isTextBoxShapeHasValidTextFrame(pShape))
    {
        if (auto pFormat = getOtherTextBoxFormat(pShape, RES_DRAWFRMFMT))
        {
            const SwFormatAnchor& OldAnch = pFormat->GetAnchor();
            const SwFormatAnchor& NewAnch = pShape->GetAnchor();

            const auto OldCnt = OldAnch.GetContentAnchor();
            const auto NewCnt = NewAnch.GetContentAnchor();

            const sal_uInt16 OldPageNum = OldAnch.GetPageNum();
            const sal_uInt16 NewPageNum = NewAnch.GetPageNum();

            if (isAnchorTypeDiffetrent(pShape))
            {
                try
                {
                    uno::Reference<beans::XPropertySet> const xPropertySet(
                        ...
                        uno::UNO_QUERY);
                    if (OldCnt && NewAnch.GetAnchorId() == RndStdIds::FLY_AT_PAGE
                        && NewAnch.GetPageNum())
                    {
                        ...
                        xPropertySet->setPropertyValue(UNO_NAME_ANCHOR_PAGE_NO,
                                                       uno::Any(NewAnch.GetPageNum()));
                    }
                    else if (OldAnch.GetAnchorId() == RndStdIds::FLY_AT_PAGE && NewCnt)
                    {
                        if (NewAnch.GetAnchorId() == RndStdIds::FLY_AS_CHAR)
                        {
                            ...
                            aPos.SetAnchor(NewCnt);
                            pFormat->SetFormatAttr(aPos);
                        }
                        else
                        {
                            uno::Any aValue(mapAnchorType(NewAnch.GetAnchorId()));
                            xPropertySet->setPropertyValue(UNO_NAME_HORI_ORIENT_RELATION,
                                                           uno::Any(text::RelOrientation::FRAME));
                            xPropertySet->setPropertyValue(UNO_NAME_ANCHOR_TYPE, aValue);
                            pFormat->SetFormatAttr(NewAnch);
                        }
                    }
                    else
                    {
...
std::optional<bool> SwTextBoxHelper::isAnchorTypeDiffetrent(SwFrameFormat* pShape)
{
...
bool SwTextBoxHelper::isTextBoxValid(SwFrameFormat* pShape) { return false; }

bool SwTextBoxHelper::isTextBoxValid(uno::Reference<drawing::XShape> xShape) { return false; }
