...
void SwTxtNode::CopyCollFmt( SwTxtNode& rDestNd )
{
    ...
    {
        // Special case for our Break-Attribute
        ...
        SwPaM *const pCpyRange ) const
{
    ...
    // Adjust position (in case it was moved / in the other node)
    rPos.nContent.Assign( rPos.nNode.GetNode().GetCntntNode(),
...
{
    // First search all Flys together, sort them according to their ordering number,
    ...
    for (::std::set< _ZSortFly >::const_iterator it=aSet.begin() ; it != aSet.end(); ++it )
    {
        ...
        // Check recursion: copy content in its own frame, then
        // don't copy it.
