...
void SwCommentRuler::Command(const CommandEvent& rCEvt)
{
    ...
    if (!mpViewShell->GetPostItMgr() || !mpViewShell->GetPostItMgr()->HasNotes()
        || !GetCommentControlRegion().HasInside(aMousePos))
...
void SwCommentRuler::MouseMove(const MouseEvent& rMEvt)
{
    ...
    mbIsHighlighted = GetCommentControlRegion().HasInside(aMousePos);
...
{
    Point aMousePos = rMEvt.GetPosPixel();
    if (!rMEvt.IsLeft() || IsTracking() || !GetCommentControlRegion().HasInside(aMousePos))
    {
