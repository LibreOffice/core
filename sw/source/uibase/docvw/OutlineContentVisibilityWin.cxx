...
void SwOutlineContentVisibilityWin::Set()
{
    ...
    if (!rOutlineNodes.Seek_Entry(
            static_cast<SwNode*>(const_cast<SwTextNode*>(pTextNode)),
            &m_nOutlinePos)) // don't think this ever happens, just making sure
    {
        assert(false);
        return;
    }
    ...
    Point aPxPt(GetEditWin()->GetOutDev()->LogicToPixel(
        Point(GetFrame()->IsRightToLeft() ? aFrameAreaRect.Right() : aFrameAreaRect.Left(),
              aFrameAreaRect.Center().getY())));
