...
{
    FontMetric aFontMetric = GetFontMetric(GetFont());
    Size aBoxSize = Size(BUTTON_WIDTH, aFontMetric.GetLineHeight() + TEXT_PADDING * 2);
    SetSizePixel(aBoxSize);

    SetPopupMenu(m_pPopupMenu);
    m_pPopupMenu->SetDeactivateHdl(LINK(this, SwOutlineContentVisibilityWin, HideHandler));
...
void SwOutlineContentVisibilityWin::Set()
{
    ...
    size_t nPos;
    rSh.GetNodes().GetOutLineNds().Seek_Entry(static_cast<SwNode *>(const_cast<SwTextNode*>(pTextNode)), &nPos);
    assert(nPos != SwOutlineNodes::npos);
    m_nOutlinePos = nPos;

    // determine need for toggle all menu item
    m_pPopupMenu->ShowItem(m_pPopupMenu->GetItemId("toggleall"), false);
    ...
    if (m_nOutlinePos + 1 < nOutlineNodesCount && rSh.getIDocumentOutlineNodesAccess()->getOutlineLevel(m_nOutlinePos + 1) > nLevel)
        m_pPopupMenu->ShowItem(m_pPopupMenu->GetItemId("toggleall"), true);
...
IMPL_LINK_NOARG(SwOutlineContentVisibilityWin, HideHandler, Menu *, bool)
{
    Fade(false);
    GrabFocusToDocument();
    return false;
