...
void UnfloatTableButton::MouseButtonDown(const MouseEvent& /*rMEvt*/)
{
    ...
    // If we move the table before the first text node, we need to clear RES_PAGEDESC attribute
    // of the text node otherwise LO will create a page break after the table
    if(pTextFrame->GetTextNodeFirst() && pTextFrame == GetPageFrame()->Lower()->GetLower())
    {
        SfxItemSet aSet(
