...
void DocxSdrExport::startDMLAnchorInline(const SwFrameFormat* pFrameFormat, const Size& rSize)
{
    ...
    **/
    const long MAX_INTEGER_VALUE = SAL_MAX_INT32 / 635; // TwipsToEMU(MAX_INTEGER_VALUE) is max safe

    OString aWidth(OString::number(TwipsToEMU(std::clamp(rSize.Width(), 0L, MAX_INTEGER_VALUE))));
    OString aHeight(OString::number(TwipsToEMU(std::clamp(rSize.Height(), 0L, MAX_INTEGER_VALUE))));
