...
void lclMovePositionWithRotation(awt::Point& aPos, const Size& rSize, Degree100 nRotation100)
{
    ...
    // position used in LO. This method converts LOs aPos to the position uses by MSO.
    // code from ImplEESdrWriter::ImplFlipBoundingBox (filter/source/msfilter/eschesdo.cxx)

    // rSize has to be size of the logicRect of the object. For calculating the diff, we build a rectangle
    // with left|top A = (-fWidthHalf | -fHeightHalf) and right|top B = (fWidthHalf | -fHeightHalf).
    // Because y-axis points down, the rotation matrix R is
    //  fcos fsin
    // -fsin fcos
    // Left of rectangle S = X-coord of R * A, Top of rectangle S = Y-coord of R * B

    // use nRotation in [0;9000], for to have only one case (A and B) and not four cases.
...
bool lcl_IsRotateAngleValid(const SdrObject& rObj)
{
    ...
    // have been incorporated. Here only OBJ_GRUP and OBJ_LINE are used. Others (e.g. OBJ_MEASURE) can
    // be added if needed, but I think they do not appear here.
...
void lcl_calculateBoundRawEffectExtent(sal_Int32& rLeft, sal_Int32& rRight, sal_Int32& rTop,
                                  sal_Int32& rBottom, const SdrObject& rObj)
{
    // This method calculates the extent needed to bring the current bound rectangle of LibreOffice to
    // the base rectangle used by Word. The 'base rectangle' is the unrotated shape rectangle, maybe
    // having swapped width and height depending on rotation angle. MS rotates around shape center,
    // LO around left/top edge. Thus logic rectangle of LO is not directly usable as 'base rectangle'.
    ...
    double fRotation = lcl_IsRotateAngleValid(rObj)
                       ? toDegrees(NormAngle36000(rObj.GetRotateAngle())) : 0.0;
    if ((fRotation >= 45.0 && fRotation < 135.0) || (fRotation >= 225.0 && fRotation < 315.0))
    {
        fMSOLeft = fCenterX - fHalfHeight;
        fMSORight = fCenterX + fHalfHeight;
        fMSOTop = fCenterY - fHalfWidth;
        fMSOBottom = fCenterY + fHalfWidth;
    }
    else
    {
        fMSOLeft = fCenterX - fHalfWidth;
        fMSORight = fCenterX + fHalfWidth;
        fMSOTop = fCenterY - fHalfHeight;
        fMSOBottom = fCenterY + fHalfHeight;
    }
    // The boundRect does not contain wrap "distance from text" values, but fat stroke and effects.
    tools::Rectangle aLOBoundRect = rObj.GetCurrentBoundRect();
    rLeft = fMSOLeft - aLOBoundRect.Left();
    rRight = aLOBoundRect.Right() - fMSORight;
    rTop = fMSOTop - aLOBoundRect.Top();
    rBottom = aLOBoundRect.Bottom() - fMSOBottom;
    // Result values might be negative, e.g for a custom shape 'Arc'.
    return;
}

void lcl_calculateSnapRawEffectExtent(sal_Int32& rLeft, sal_Int32& rRight, sal_Int32& rTop,
                                  sal_Int32& rBottom, const SdrObject& rObj)
{
    // LO uses the snap rectangle as the default place for "as char" (MSO 'inline') anchored objects.
    // Word reserves an area that covers the unrotated rectangle or - in the case of width-height swap
    // - the 90° rotated rectangle. It places the rotated object in this area so that the center points
    // match. Effects like shadow or thick lines are not considered. This method calculates the
    // effectExtent needed in Word to render the object the same way as LibreOffice.

    tools::Rectangle aLOSnapRect = rObj.GetSnapRect();
    double fCenterX = (aLOSnapRect.Left() + aLOSnapRect.Right()) / 2.0;
    double fCenterY = (aLOSnapRect.Top() + aLOSnapRect.Bottom()) / 2.0;
    double fHalfWidth = rObj.GetLogicRect().getWidth() / 2.0;
    double fHalfHeight = rObj.GetLogicRect().getHeight() / 2.0;
    double fMSOLeft;
    double fMSORight;
    ...
    rLeft = fMSOLeft - aLOSnapRect.Left();
    rRight = aLOSnapRect.Right() - fMSORight;
    rTop = fMSOTop - aLOSnapRect.Top();
    rBottom = aLOSnapRect.Bottom() - fMSOBottom;
    return;
}
...
void makeSingleDistZeroAndExtentNonNegative(sal_Int64& rDist, sal_Int32& rExt)
{
    if (static_cast<double>(rDist) + static_cast<double>(rExt) >= static_cast<double>(SAL_MAX_INT32))
        rExt = SAL_MAX_INT32;
...
void lcl_makeDistZeroAndExtentNonNegative(sal_Int64& rDistT, sal_Int64& rDistB, sal_Int64& rDistL,
                                      ...
                                      sal_Int32& rRightExt, sal_Int32& rBottomExt)
{
    makeSingleDistZeroAndExtentNonNegative(rDistL, rLeftExt);
    makeSingleDistZeroAndExtentNonNegative(rDistT, rTopExt);
    makeSingleDistZeroAndExtentNonNegative(rDistR, rRightExt);
    makeSingleDistZeroAndExtentNonNegative(rDistB, rBottomExt);
}
...
void DocxSdrExport::startDMLAnchorInline(const SwFrameFormat* pFrameFormat, const Size& rSize)
{
    ...
    // All values in Twips, change to EMU immediately before writing out.
    ...
    if ((!pObj) || (pObj && (pObj->GetObjIdentifier() == SwFlyDrawObjIdentifier)))
    {
        ...
        // ToDo LineWidth is not correct for frames. We need border.
        // sal_Int32 nHalfStrokeWidth = pObj->GetMergedItem(XATTR_LINEWIDTH).GetValue() / 2;
        // nLeftExt += nHalfStrokeWidth;
        // nRightExt += nHalfStrokeWidth;
        // nTopExt += nHalfStrokeWidth;
        // nBottomExt += nHalfStrokeWidth;
    }
    else // other objects than frames. pObj exists.
    {
        ...
        if (isAnchor)
        {
            lcl_calculateBoundRawEffectExtent(nLeftExt, nRightExt, nTopExt, nBottomExt, *pObj);
        ...
        {
            lcl_calculateSnapRawEffectExtent(nLeftExt, nRightExt, nTopExt, nBottomExt, *pObj);
    ...
    else // inline
    {
        ...
        // ToDo: Preparation needed for effectExtent?
    }
    ...
    **/
    sal_uInt64 cx
        = TwipsToEMU(std::clamp(rSize.Width() + nWidthDiff, tools::Long(0), tools::Long(SAL_MAX_INT32)));
    OString aWidth(OString::number(std::min(cx, sal_uInt64(SAL_MAX_INT32))));
    sal_uInt64 cy
        = TwipsToEMU(std::clamp(rSize.Height() + nHeightDiff, tools::Long(0), tools::Long(SAL_MAX_INT32)));
    ...
    m_pImpl->getSerializer()->singleElementNS(
        XML_wp, XML_effectExtent, XML_l, OString::number(TwipsToEMU(nLeftExt)), XML_t, OString::number(TwipsToEMU(nTopExt)),
        XML_r, OString::number(TwipsToEMU(nRightExt)), XML_b, OString::number(TwipsToEMU(nBottomExt)));
