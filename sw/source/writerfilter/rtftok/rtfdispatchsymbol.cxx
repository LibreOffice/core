    ...
    switch (nKeyword)
    {
        ...
        case RTFKeyword::PAR:
        {
            ...
            if (auto const pCurrentBuffer{ m_aStates.top().getCurrentBuffer() })
            {
                auto const isTable{ std::find_if(
                                        ...
                                        [](auto const& it) {
                                            return ::std::get<0>(it) == BUFFER_PAR
                                                   || (::std::get<0>(it) == BUFFER_PROPS
                ...
                if (m_aStates.top().getDestination() != Destination::SHAPETEXT)
                {
                    ...
                    pCurrentBuffer->push_back(Buf_t(BUFFER_PAR, pValue, nullptr));
                }
        ...
        case RTFKeyword::NESTCELL:
        {
            ...
            if (auto const pCurrentBuffer{ m_aStates.top().getCurrentBuffer() })
            {
                ...
                if (iter == pCurrentBuffer->end() || ::std::get<0>(*iter) == BUFFER_PROPS)
                {
