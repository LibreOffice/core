...
public:
    ...
    static SwFrameFormat* getShapeFormat(css::uno::Reference<drawing::XShape> xShape);

    /// Copy shape attributes to the text frame
    static void updateTextBoxMargin(SdrObject* pObj);

    /// Sets the sorrround to through for the textframe of the given shape,
    /// not to interfere with the layout. Returns true on success.
    static bool setWrapThrough(SwFrameFormat* pShape);

    /// Sets the sorrround to through for the textframe of the given shape,
    /// not to interfere with the layout. Returns true on success.
    static bool setWrapThrough(css::uno::Reference<drawing::XShape> xShape);
    ...
    /// Sets the anchor of the associated textframe of the given shape, and
    /// returns true on success.
    static bool changeAnchor(css::uno::Reference<drawing::XShape> xShape);
    ...
    /// Does the positioning for the associated textframe of the shape, and
    /// returns true on success.
    static bool doTextBoxPositioning(css::uno::Reference<drawing::XShape> xShape);

    /// Returns true if the anchor different for the  given shape, and the
    /// associated textbrame of the given shape.
    ...
    static std::optional<bool> isAnchorTypeDiffetrent(SwFrameFormat* pShape);

    /// Returns true if the given shape has a valid textframe.
    static bool isTextBoxShapeHasValidTextFrame(SwFrameFormat* pShape);
    /// Returns true if the given shape has a valid textframe.
    static bool isTextBoxShapeHasValidTextFrame(css::uno::Reference<drawing::XShape> xShape);

    /// Returns true if the the textbox valid, otherwise false for example
    /// the frame outside the shape.
    static bool isTextBoxValid(SwFrameFormat* pShape);
    /// Returns true if the the textbox valid, otherwise false for example
    /// the frame outside the shape.
    static bool isTextBoxValid(css::uno::Reference<drawing::XShape> xShape);
