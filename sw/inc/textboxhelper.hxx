...
public:
    ...
    /// like groupshapes, it will destroy only that textbox what belongs
     ...
     *
     * If we have more textboxes for this format (groupshape), that one will be
     ...
     *                 the textbox what belongs to the pFomat will only be inspected.
     */
...
/// Textboxes are basically texframe + shape pairs. This means one shape have one frame.
/// This is not enought for groupshapes, beacuse they have only one shape-format and
/// can have many frame-formats. This class provides if there is a groupshape for example,
...
class SwTextBoxNode
{
    ...
    // This is the pointer to the shapeformat, which have this node
    // (and the textboxes)
...
public:
    ...
    //     pNewTextBox: The newly created textbox fotmat what will be added to the shape.
    ...
    // This will return with the frameformat of the textbox what belongs
    ...
    // If this is a groupshape, that returns true.
    bool IsGroupTextBox() const;
    // This returns with tha shape what this class belongs to.
