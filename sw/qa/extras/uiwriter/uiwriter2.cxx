...
uno::Reference<text::XTextRange> getAssociatedTextRange(uno::Any object)
{
    // possible cases:
    ...
    try
    {
        ...
        if (xIndexAccess.is() && xIndexAccess->getCount() > 0)
        {
            ...
            {
                uno::Reference<text::XTextRange> range
    ...
    try
    {
        ...
        if (xTextContent.is())
        {
            uno::Reference<text::XTextRange> range = xTextContent->getAnchor();
            if (range.is())
                return range;
        }
    }
    ...
    try
    {
        uno::Reference<text::XTextRange> range(object, uno::UNO_QUERY_THROW);
        if (range.is())
            return range;
    }
    catch (const uno::Exception&)
