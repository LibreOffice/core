...
void SwUiWriterTest2::testUnfloatButton()
{
    ...
    for (const OUString& aTestFile : aTestFiles)
    {
        ...
        if (sTestFileName == "unfloatable_small_floating_table.docx")
            pAnchored = pWrtShell->GetLayout()->GetLower()->GetLower()->GetLower()->GetNext()->GetDrawObjs();
        else
        ...
        CPPUNIT_ASSERT_EQUAL_MESSAGE(sFailureMessage.getStr(), static_cast<size_t>(1), pAnchored->size());
        SwAnchoredObject* pAnchoredObj = (*pAnchored)[0];
...
void SwUiWriterTest2::testUnfloating()
{
    ...
    // Before unfloating we have only one page with a fly frame
    {
        ...
        CPPUNIT_ASSERT_EQUAL(SwFrameType::Txt, pWrtShell->GetLayout()->GetLower()->GetLower()->GetLower()->GetType());
        const SwSortedObjs* pAnchored = pWrtShell->GetLayout()->GetLower()->GetLower()->GetLower()->GetDrawObjs();
        CPPUNIT_ASSERT(pAnchored);
        CPPUNIT_ASSERT_EQUAL(static_cast<size_t>(1), pAnchored->size());
        SwAnchoredObject* pAnchoredObj = (*pAnchored)[0];
    ...
    CPPUNIT_ASSERT_EQUAL(SwFrameType::Page, pWrtShell->GetLayout()->GetLower()->GetNext()->GetType());
    CPPUNIT_ASSERT(pWrtShell->GetLayout()->GetLower()->GetNext()->GetNext() == nullptr);
    CPPUNIT_ASSERT_EQUAL(SwFrameType::Tab, pWrtShell->GetLayout()->GetLower()->GetLower()->GetLower()->GetType());
    CPPUNIT_ASSERT_EQUAL(SwFrameType::Tab, pWrtShell->GetLayout()->GetLower()->GetNext()->GetLower()->GetLower()->GetType());
}
