...
CPPUNIT_TEST_FIXTURE(SwUiWriterTest6, testCrashOnExit)
{
    ...
    uno::Reference<frame::XController> xContorller(xModel->getCurrentController(), uno::UNO_QUERY);
    CPPUNIT_ASSERT(xContorller);
    uno::Reference<view::XSelectionSupplier> xSelection(xContorller, uno::UNO_QUERY);
    CPPUNIT_ASSERT(xSelection);
    const bool& rSuccess = xSelection->select(uno::Any(xShape));
    CPPUNIT_ASSERT(rSuccess && xSelection->getSelection().hasValue());
