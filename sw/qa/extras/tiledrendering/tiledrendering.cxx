...
CPPUNIT_TEST_FIXTURE(SwTiledRenderingTest, testViewCursorCleanup)
{
    ...
    KitHelper::destroyView(nView2);
    Scheduler::ProcessEventsToIdle();
    CPPUNIT_ASSERT_EQUAL(static_cast<std::size_t>(1), KitHelper::getViewsCount(0));
...
CPPUNIT_TEST_FIXTURE(SwTiledRenderingTest, testSemiTransparent)
{
    ...
    ScopedVclPtrInstance<VirtualDevice> pDevice(DeviceFormat::WITHOUT_ALPHA);
    pDevice->SetBackground(Wallpaper(COL_TRANSPARENT));
    pDevice->SetOutputSizePixelScaleOffsetAndKitBuffer(Size(nCanvasWidth, nCanvasHeight),
            1.0, Point(), aPixmap.data());
...
CPPUNIT_TEST_FIXTURE(SwTiledRenderingTest, testHyperlink)
{
    ...
    SwWrtShell* pWrtShell = getSwDocShell()->GetWrtShell();
    setupCOKitViewCallback(pWrtShell->GetSfxViewShell());
    m_callbackWrapper.setKitViewId(KitHelper::getView(*pWrtShell->GetSfxViewShell()));
    SwShellCursor* pShellCursor = pWrtShell->getShellCursor(false);
