...
DECLARE_OOXMLIMPORT_TEST(testTdf133647_2, "tdf133647-2.docx")
{
    /* Tests document attached to the original bug report */
    uno::Reference<text::XTextFieldsSupplier> xTextFieldsSupplier(mxComponent, uno::UNO_QUERY);
    uno::Reference<container::XEnumerationAccess> xFieldsAccess(xTextFieldsSupplier->getTextFields());
    uno::Reference<container::XEnumeration> xFields(xFieldsAccess->createEnumeration());
    ...
    /* Formula that doesn't contain a cell range */
    uno::Reference<text::XTextField> xEnumerationAccess1(xFields->nextElement(), uno::UNO_QUERY);
    CPPUNIT_ASSERT_EQUAL(OUString("250*0.75*4/11*6.3"), xEnumerationAccess1->getPresentation(true).trim());
    CPPUNIT_ASSERT_ASSERTION_FAIL(CPPUNIT_ASSERT_EQUAL(OUString("** Expression is faulty **"), xEnumerationAccess1->getPresentation(false).trim()));

    /* SUM formula that revealed the bug */
    uno::Reference<text::XTextField> xEnumerationAccess(xFields->nextElement(), uno::UNO_QUERY);
    CPPUNIT_ASSERT_EQUAL(OUString("SUM(<D2:D5>)"), xEnumerationAccess->getPresentation(true).trim());
    CPPUNIT_ASSERT_ASSERTION_FAIL(CPPUNIT_ASSERT_EQUAL(OUString("** Expression is faulty **"), xEnumerationAccess->getPresentation(false).trim()));
}
