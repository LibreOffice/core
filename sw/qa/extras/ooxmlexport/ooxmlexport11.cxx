...
DECLARE_OOXMLEXPORT_TEST(testTdf112118, "tdf112118.docx")
{
    ...
    typedef std::initializer_list<StyleSettings> StyleList;
    ...
    // 1482 + 847 + 212 = 2541
    for (const auto& style : StyleList{ { "Standard", 847, 1482 }, { "Converted1", 1482, 847 } })
    {
        uno::Reference<beans::XPropertySet> xStyle(xStyles->getByName(style.sName), uno::UNO_QUERY_THROW);
        for (const auto& side : StringList{ "Top", "Left", "Bottom", "Right" })
        {
            ...
            CPPUNIT_ASSERT_EQUAL(sal_Int16(212), aBorder.OuterLineWidth);
            CPPUNIT_ASSERT_EQUAL(sal_Int16(0), aBorder.InnerLineWidth);
            CPPUNIT_ASSERT_EQUAL(sal_Int16(0), aBorder.LineDistance);

            sal_Int32 nMargin = getProperty<sal_Int32>(xStyle, side + "Margin");
            CPPUNIT_ASSERT_EQUAL(style.nMargin, nMargin);

            sal_Int32 nBorderDistance = getProperty<sal_Int32>(xStyle, side + "BorderDistance");
            CPPUNIT_ASSERT_EQUAL(style.nBorderDistance, nBorderDistance);
        }
    }
}
