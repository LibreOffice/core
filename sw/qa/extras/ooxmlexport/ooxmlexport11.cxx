...
bool lcl_nearEqual(const sal_Int32 nNumber1, const sal_Int32 nNumber2, sal_Int32 nMaxDiff = 5)
{
...
DECLARE_OOXMLEXPORT_TEST(testTdf119952_negativeMargins, "tdf119952_negativeMargins.docx")
{
    ...
    // LibreOffice unable to display that, so when importing negative magins,
    // the header (or footer) converted to a flyframe, anchored to the header..
    // that can overlap with the body, and will apperar like in the word.
    // This cnversion modify the docment [like replacing header text with a textbox...]
    // so exporting back to docx will result a similar looking but different document.
    xmlDocUniquePtr pDump = parseLayoutDump();
