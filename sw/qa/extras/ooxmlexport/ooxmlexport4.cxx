...
// Pages 4 and 5 OUGHT to use Second Section Header, but currently don't.  Page 6 does.
// The first header (defined on page 3) ONLY is shown IF the section happens to start on a new page (which it doesn't in this document).
    ...
    while( nPages > 0 )
    {
        ...
        CPPUNIT_ASSERT( xHeaderText->getString() != OUString("Third section - First page header. No follow defined") );
        if( nPages <= 3 )
            CPPUNIT_ASSERT_EQUAL( xHeaderText->getString(), OUString("First section header") );
        else
            CPPUNIT_ASSERT( xHeaderText->getString() == OUString("First section header") || xHeaderText->getString() == OUString("Second section header") );
