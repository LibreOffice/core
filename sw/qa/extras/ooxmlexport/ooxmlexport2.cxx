...
CPPUNIT_TEST_FIXTURE(Test, testMathMso2k7)
{
    ...
    CHECK_FORMULA("{left (x\"+\"a right )} ^ {n} \"=\" sum from {k\"=\"0} to {n} {left (stack {n # "
                  "k} right ) {x} ^ {k} {a} ^ {n\"-\"k}}",
                  getFormula(getRun(getParagraph(2), 1)));
    CHECK_FORMULA(
        u"{left (1\"+\"x right )} ^ {n} \"=\"1\"+\" {nx} over {1!} \"+\" {n left (n\"-\"1 right ) {x} ^ {2}} over {2!} \"+\"\u2026",
    ...
    CHECK_FORMULA(
        u"f left (x right ) \"=\" {a} rsub {0} \"+\" sum from {n\"=\"1} to {\u221E} {left ({a} rsub {n} cos {n\u03C0x} over {L} \"+\" {b} rsub {n} sin {n\u03C0x} over {L} right )}",
...
{
    CHECK_FORMULA(
        u"iiint from {V} to <?> {\"div\" \"F\"}  dV\"=\" llint from {S} to <?> {\"F\" \u2219 \"n \" dS}",
        getFormula(getRun(getParagraph(1), 1)));
}
...
CPPUNIT_TEST_FIXTURE(Test, testTdf162070Export)
{
    ...
    CPPUNIT_ASSERT_EQUAL(OUString(u"{P} rsub {\"abs\"} \"~\" {B} rsub {0} \u00B2"), getFormula(getRun(getParagraph(1), 1)));
}
