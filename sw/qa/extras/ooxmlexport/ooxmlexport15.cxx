                         ...
                         "tdf133070_testRelativeAnchorHeightFromBottomMarginHasFooter.docx")
{
    ...
    // Note: the MSO Word margin = LO Writer margin + LO Writer footer height.
                         ...
                         "tdf133070_testRelativeAnchorHeightFromBottomMarginNoFooter.docx")
{
    ...
    if (mbExported)
        return;
    // tdf#133070 The height was set relative from bottom margin, but this was handled relative from page height.
    // Note: the MSO Word margin = LO margin + LO footer height.
    // In this case the footer does not exist, so MSO Word margin and LO Writer margin are the same.
    xmlDocUniquePtr pXmlDoc = parseLayoutDump();
    const sal_Int32 nAnchoredHeight
        = getXPath(pXmlDoc, "//SwAnchoredDrawObject/bounds", "height").toInt32();
    CPPUNIT_ASSERT_EQUAL(static_cast<sal_Int32>(1147), nAnchoredHeight);
