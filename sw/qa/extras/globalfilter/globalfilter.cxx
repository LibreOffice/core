...
void Test::testMultipleIdenticalGraphics()
{
    ...
    for (OUString const & rFilterName : aFilterNames)
    {
        if (mxComponent.is())
            mxComponent->dispose();
        OUString sDocumentFilename("/sw/qa/extras/globalfilter/data/multiple_identical_graphics.odt");
        mxComponent = loadFromDesktop(m_directories.getURLFromSrc(sDocumentFilename), "com.sun.star.text.TextDocument");
        ...
        for (auto xGraphic : aGraphics)
        {
            aGfxLinkAddresses.emplace_back(sal_Int64(Graphic(xGraphic).GetSharedGfxLink().get()));
            printf ("%ld\n", aGfxLinkAddresses.back());
        }
