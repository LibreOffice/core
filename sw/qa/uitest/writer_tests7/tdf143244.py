...
class tdf143244(UITestCase):
    def test_tdf143244(self):
        ...
        for i in range (0,3):
            ...
            sHexCustomText = get_state_as_dict(hex_custom)["Text"]

            xOKBtn = xDialog.getChild("ok")
            self.ui_test.close_dialog_through_button(xOKBtn)

            if i == 0:
                self.assertEqual(sHexCustomText, "bee3d3")
            if i == 1:
                self.assertEqual(sHexCustomText, "ffffff")
            if i == 2:
                self.assertEqual(sHexCustomText, "dddddd")
