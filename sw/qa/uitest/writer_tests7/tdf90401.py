...
class tdf90401(UITestCase):
   ...
   def test_tdf90401_remove_personal_info(self):
        ...
        with TemporaryDirectory() as tempdir:
            ...
            xWriterDoc = self.xUITest.getTopFocusWindow()
            self.ui_test.wait_until_child_is_available('author')
            xAuthor = xWriterDoc.getChild('author')
            self.ui_test.wait_until_property_is_updated(xAuthor, 'DisplayText', '2')
            state = get_state_as_dict(xWriterDoc.getChild('author'))
            # This was 'Unknown Author'
            self.assertEqual(state['DisplayText'], '2')

            self.ui_test.wait_until_child_is_available('date')
            xDate = xWriterDoc.getChild('date')
            self.ui_test.wait_until_property_is_updated(xDate, 'DisplayText', '(no date)')
            state = get_state_as_dict(xWriterDoc.getChild('date'))
            # This was the actual date
            self.assertEqual(state['DisplayText'], '(no date)')
            ...
            self.assertEqual(state['SelectEntryText'], '1\t01/01/1970 00:00:00\t')
