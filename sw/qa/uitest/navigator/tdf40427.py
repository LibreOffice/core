...
class tdf40427(UITestCase):
  ...
  def test_tdf40427(self):
    ...
    nSectionsIndex = 6
    xSections = xContentTree.getChild(str(nSectionsIndex))
    while not get_state_as_dict(xSections)['Text'] == 'Sections':
      # If entries in Navigator are expanded, Sections may be at a greater index
      nSectionsIndex += 1
      xSections = xContentTree.getChild(str(nSectionsIndex))
