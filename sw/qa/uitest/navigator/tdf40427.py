...
class tdf40427(UITestCase):
  ...
  def test_tdf40427(self):
    ...
    nSections = 12
    self.assertEqual(nSections, len(xSections.getChildren()))

    sectionNames = []
    for i in range(nSections):
      sectionNames.append(get_state_as_dict(xSections.getChild(str(i)))['Text'])
    # Without the fix in place, this would fail with
    #   AssertionError: Lists differ: ['SectionZ', 'SectionY', 'SectionT2', 'SectionT1', 'SectionT0'[87 chars]onC'] != ['SectionZ', 'SectionB', 'SectionF3', 'SectionFinF3', 'Section[87 chars]onA']
    self.assertEqual(['SectionZ',
                      'SectionY',
                      'SectionT2',
                      'SectionT1',
                      'SectionT0',
