...
# works locally and linux_gcc_release_64, but fails at linux_clang_dbgutil_64. Markus: Actually after a round of debugging I think the problem is actually that the test depends on the used font. Therefore, if the font is not available or not selected by default the test fails. 
