...
class TestXScriptProvider(unittest.TestCase):
    ...
    def setUp(self):
        x_master_script_provider_factory = create_master_script_provider_factory()
        self.x_script_provider = x_master_script_provider_factory.createScriptProvider("")

    def tearDown(self):
        del self.x_script_provider

    def test_get_script_application(self):
        # getScript for built-in StarBasic function
        x_script = self.x_script_provider.getScript(
            "vnd.sun.star.script:Tools.Misc.CreateNewDocument?language=Basic&"
            "location=application")

        self.assertIsNotNone(x_script, "xScript was not loaded")
    ...
    def test_get_script_document(self):
        ...
        x_master_script_provider_factory = create_master_script_provider_factory()
        x_script_provider = x_master_script_provider_factory.createScriptProvider(x_doc)

        x_script = x_script_provider.getScript(
    ...
    def test_get_script_invalid_uri(self):
        ...
        with self.assertRaises(ScriptFrameworkErrorException):
            self.x_script_provider.getScript("invalid URI, isn't it?")

    def test_get_script_not_found(self):
        # getScript fails when script not found
        with self.assertRaises(ScriptFrameworkErrorException):
            self.x_script_provider.getScript(
                "vnd.sun.star.script:NotExisting.NotExisting.NotExisting?"
                "language=Basic&location=document")

def create_master_script_provider_factory():
    x_service_manager = uno.getComponentContext().ServiceManager

    return x_service_manager.createInstanceWithContext(
