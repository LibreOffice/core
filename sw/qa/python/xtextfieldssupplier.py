...
class TestXTextFieldsSupplier(unittest.TestCase):
    ...
    def test_getTextFields(self):
        ...
        for fieldType, xField in zip(fieldTypesList, xFieldEnum):
            self.assertTrue(xField.supportsService(fieldType), "field " + xField.getPresentation(True) + " does not support " + fieldType + " service!")
    ...
    def test_getTextFieldMasters(self):
        ...
        for masterName in masterNames:
            self.assertTrue(xFieldMasters.hasByName(masterName), "TextFieldMaster has no element " + masterName)
            xMaster = xFieldMasters.getByName(masterName)
            self.assertIsNotNone(xFieldMasters.getByName(masterName), "can't get " + masterName + " from TextFieldMaster")
        ...
        self.assertFalse(xFieldMasters.hasByName(invalidMasterName), "TextFieldMaster has element " + invalidMasterName)
