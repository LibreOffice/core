...
class TestXStyleLoader(unittest.TestCase):
    ...
    def test_loadStyleFromStream(self):
        ...
        simpleFileAccess = xServiceManager.createInstance("com.sun.star.ucb.SimpleFileAccess")
        xInputStream = simpleFileAccess.openFileRead(pathlib.Path(makeCopyFromTDOC("xstyleloader.odt")).as_uri())

        p1 = PropertyValue(Name="InputStream", Value=xInputStream)
        p2 = PropertyValue(Name="LoadTextStyles", Value=True)

        #print(dir(xDoc))
        xDoc.loadStylesFromURL("private:stream", [p1, p2])

        styles = xDoc.getStyleFamilies()
        self.assertTrue(styles.hasByName("Test_Template"))


        xDoc.close(True)
        xDocWithStyle.close(True)
