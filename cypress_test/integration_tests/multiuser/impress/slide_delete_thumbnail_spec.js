...
describe(['tagmultiuser'], 'Multiuser slide delete thumbnails', function() {
	...
	// Map each slide's part number to the thumbnail image it currently shows,
	...
	it('another view deleting a slide does not misplace the thumbnails', function() {
		...
		cy.wait(2000);
		cy.then(function() {
			win1.__before = thumbnailsByPart(win1);
			win1.__deletedPart = String(win1.app.impress.partList[1].part);
		...
		cy.wrap(null).should(function() {
			...
			var after = thumbnailsByPart(win1);

			// The deleted slide's thumbnail is gone.
			expect(after).to.not.have.property(win1.__deletedPart);
			...
			Object.keys(before).forEach(function(part) {
				if (part === win1.__deletedPart)
					return;
				expect(after, 'slide ' + part + ' still has a thumbnail').to.have.property(part);
			});
			...
			Object.keys(after).forEach(function(shownPart) {
				Object.keys(before).forEach(function(otherPart) {
					if (shownPart === otherPart)
						return;
					expect(after[shownPart],
						'slide ' + shownPart + ' must not show slide ' + otherPart + ' image')
						.to.not.equal(before[otherPart]);
				});
			});
		});
	});
