...
describe(['tagdesktop'], 'Accessibility Impress Notebookbar Tests', { testIsolation: false }, function () {
    var tabs;
    var allTabNames;
    var visitedTabNames;
    var win;
    ...
    before(function () {
        ...
        cy.then(function () {
            ...
            cy.log('Available tabs:', allTabNames); // Check your browser console!
            visitedTabNames = [];
        });
    });
    ...
    it('Notebookbar tab: Master (context)', function () {

        cy.then(function () {
            win.app.map.sendUnoCommand('.uno:SlideMasterPage');
        });

        selectAndValidateTab(findTab('MasterPage'));

        // exit master view mode
        cy.then(function () {
