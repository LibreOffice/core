 ...
 *  $Revision: 1.9 $
 *
 *  last change: $Author: hr $ $Date: 2007-01-03 11:38:33 $
 *
...
extern "C" void SAL_CALL rtl_createUuid( sal_uInt8 *pTargetUUID ,
                                         ...
                                         sal_Bool )
{
    rtlRandomError err = rtl_Random_E_None;
    rtlRandomPool pool = rtl_random_createPool();
    if (pool == NULL) {
        err = rtl_Random_E_Unknown;
    } else {
        if (pPredecessorUUID != NULL) {
            err = rtl_random_addBytes(pool, pPredecessorUUID, 16);
        }
        if (err == rtl_Random_E_None) {
            err = rtl_random_getBytes(pool, pTargetUUID, 16);
        }
