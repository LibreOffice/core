...
class tdf114724(UITestCase):
    ...
    def test_track_headings_outline(self):
        ...
        self.assertEqualWithTimeout(xNavigatorPanel, {"selectioncount": "1", "selectedtext": "Headings"})
        for _ in range(0,3):
            xWriterEdit.executeAction("TYPE", mkPropertyValues({"KEYCODE": "DOWN"}))

        self.assertEqualWithTimeout(xNavigatorPanel, {"selectioncount": "1", "selectedtext": "HEADING 4"})

        for _ in range(0,3):
            xWriterEdit.executeAction("TYPE", mkPropertyValues({"KEYCODE": "UP"}))

        self.assertEqualWithTimeout(xNavigatorPanel, {"selectioncount": "1", "selectedtext": "HEADING 1"})
