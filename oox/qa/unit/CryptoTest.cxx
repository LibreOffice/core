...
void CryptoTest::testStandard2007()
{
    oox::core::Standard2007Engine aEngine;
    {
        ...
        oox::BinaryXOutputStream aBinaryEncryptionInfoOutputStream(new utl::OSeekableOutputStreamWrapper(aEncryptionInfo), false);
    ...
    {
        oox::BinaryXInputStream aBinaryInputStream(new utl::OSeekableInputStreamWrapper(aUnencryptedInput), true);
        oox::BinaryXOutputStream aBinaryOutputStream(new utl::OSeekableOutputStreamWrapper(aEncrypted), true);

        aEncrypted.WriteUInt32(aUnencryptedInput.GetSize());
        aEncrypted.WriteUInt32(0U);

        aEngine.encrypt(aBinaryInputStream, aBinaryOutputStream);
        aBinaryOutputStream.close();
        aBinaryInputStream.close();

        const sal_uInt8* pData = static_cast<const sal_uInt8*>(aEncrypted.GetData());
        sal_uInt64 nSize = aEncrypted.GetSize();
    ...
    aEncrypted.Seek(STREAM_SEEK_TO_BEGIN);
    SvMemoryStream aUnencryptedOutput;

    {
        oox::BinaryXInputStream aBinaryInputStream(new utl::OSeekableInputStreamWrapper(aEncrypted), true);
        oox::BinaryXOutputStream aBinaryOutputStream(new utl::OSeekableOutputStreamWrapper(aUnencryptedOutput), true);
