    ...
    mrOleStorage(rOleStorage)
{
    ...
    for (const auto & sStreamName : aStreamNames)
    {
        ...
        SAL_WARN_IF(nStreamSize != nReadBytes, "oox", "Read unxepected amount of bytes from stream");
...
bool DocumentDecryption::readEncryptionInfo()
{
    ...
    if (xDataSpaceMap.is())
    {
        ...
        {
            aDataSpaceStream.skip(4); // DataSpaceMapEntry Length
            ...
            for (sal_uInt32 j = 0; j < aReferenceComponentCount; j++)
            {
                ...
                SAL_WARN_IF(refComponentType !=0 && refComponentType != 1,
                    "oox", "Encrypted package does not contain DataSpaceMap");

                sal_uInt32 aReferenceComponentNameLength = aDataSpaceStream.readuInt32();
                // Component name is not used by our code, so just skip it
                // It can be read by aDataSpaceStream.readUnicodeArray(aReferenceComponentNameLength / 2)
                // once required
                aDataSpaceStream.skip(aReferenceComponentNameLength);
