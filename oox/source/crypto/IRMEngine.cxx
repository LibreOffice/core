...
bool IRMEngine::readEncryptionInfo(uno::Reference<io::XInputStream>& rxInputStream)
{
    ...
    if (mInfo.license.getLength() && mInfo.license[0] != 0x0ef) // BOM is missing?
    {
...
void IRMEngine::writeEncryptionInfo(oox::ole::OleStorage& rOleStorage)
{
    ...
    aDataSpaceMapStream.WriteInt32(1); // References component type TODO
    ...
    {
        aDataSpaceMapStream.WriteInt16(0);
    }
    ...
    {
        aDataSpaceInfoStream.WriteInt16(0);
    }
    ...
    {
        aTransformInfoStream.WriteInt16(0);
    }
    ...
    aTransformInfoStream.WriteInt32(mInfo.license.getLength()); // LicenseLength
    aTransformInfoStream.writeArray<sal_Char>(mInfo.license.getStr(), mInfo.license.getLength());
                        ...
                        sal_uInt32 nSize)
{
    ...
    do
    {
        sal_uInt32 readBytes = aInputStream.readArray(decryptedBuffer, *blockSize);
        lastBlock = readBytes != *blockSize;
        DWORD bytes = 0;
        hr = IpcEncrypt(key, blockNo, lastBlock, (PBYTE)decryptedBuffer, *blockSize,
                        (PBYTE)encryptedBuffer, *blockSize, &bytes);
        ...
        aOutputStream.writeArray(encryptedBuffer, bytes);
