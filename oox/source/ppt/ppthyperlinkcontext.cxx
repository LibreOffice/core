            ...
            if (!sAction.isEmpty())
            {
                ...
                if (sAction.matchIgnoreAsciiCase(sPPAction))
                {
                    ...
                    else if (aPPAction.match("hlinksldjump"))
                    {
                        ...
                        {
                            sal_Int32 nLength = 1;
                            sal_Int32 nIndex2 = sHlinkSlide.getLength();
                            while ((nIndex2 + nLength) < sURL.getLength())
                            {
                                sal_Unicode nChar = sURL[nIndex2 + nLength];
                                if ((nChar < '0') || (nChar > '9'))
                                    break;
                                nLength++;
                            }
                            sal_Int32 nPageNumber = sURL.copy(nIndex2, nLength).toInt32();
                            if (nPageNumber)
