        ...
        m_pDMLexporter->GetFS()->singleElementNS(
            ...
            OUString("val " + bXAxis
                         ? OUString::number(std::lround(
                               *GetOOXMLHandlePointAdjustmentRatio(m_HandleValues[index]).first
                               * nMaxVal))
                         : OUString::number(std::lround(
...
bool DMLPresetShapeExporter::WriteShapeWithAVlist()
{
    ...
    const long nConstOfMaxDegreeOf60th = 21600000;
    try
    {
        ...
        {
            auto aPoint1 = GetAdjustmentPointYValue(0);
            auto nMaxVal = m_xShape->getSize().Height
                           / std::min(m_xShape->getSize().Height, m_xShape->getSize().Width)
                           * 50000;
            long nVal1
        ...
        {
            int nVal1 = ((m_AdjustmentValues[0].Value.get<double>() - m_xShape->getSize().Width / 2)
                         * 360000);
            int nVal2
                = ((m_AdjustmentValues[1].Value.get<double>() - m_xShape->getSize().Height / 2)
                   * 360000);
