 ...
 * style id (in ppt/slides/slidex.xml) but have not
 * corresponding style defination (ppt/tableStyles.xml).
 * So we should create that styles here for this case.
 * There are 74 predefined styles and many different
 * variables. A style map was created by examining all
 * 74 style properties. And table styles has coded due
 * to that map. You can see that map in
 * oox/predefined-styles-map.ods. We should define all
 * of that variables to keep the code readeable and change
 * something easily when some styles changed.
 */
