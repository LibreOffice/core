...
#if ASCARCH
#if defined(_AIX)
   if( fread( (char *)&flhdr, sizeof(flhdr), 1, f ) != sizeof(flhdr) ) return (-1);
   if( strncmp(flhdr.fl_magic,AIAMAG, SAIAMAG) != 0 ) return(-1);
   fseek(f, atol(flhdr.fl_fstmoff), 0 ); /* postition to first member */
#else
   if( fread( magic, sizeof(magic), 1, f ) != sizeof(magic) ) return( -1 );
   if( strncmp(magic, ARMAG, SARMAG) != 0 ) return( -1 );
#endif
#else
   if( fread( (char*)&word, sizeof(word), 1, f ) != sizeof(word) ) return( -1 );
