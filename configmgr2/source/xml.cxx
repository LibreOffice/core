    ...
    rtl::OUString const & path, rtl::OUString * name)
{
    ...
    if (path[i - 1] == ']') {
        ...
        OSL_ASSERT(j > 0);
        xml::decodeXml(path, j + 1, i - 2, name);
...
rtl::Reference< Node > parseXcsGroup(
    int layer, Data const & data, rtl::OUString const & componentName,
    xmlDocPtr doc, xmlNodePtr node, rtl::OUString const & templateName);

void parseXcsGroupContent(
    int layer, Data const & data, rtl::OUString const & componentName,
    xmlDocPtr doc, xmlNodePtr node, rtl::Reference< GroupNode > const & group)
{
    xmlNodePtr p(skipBlank(node->xmlChildrenNode));
    if (isOorElement(p, "info")) {
        p = skipBlank(p->next);
    ...
    for (; p != 0; p = skipBlank(p->next)) {
        ...
        if (isOorElement(p, "prop")) {
            ...
            member = parseXcsGroup(
                layer, data, componentName, doc, p, rtl::OUString());
    ...
    rtl::Reference< GroupNode > group(
        new GroupNode(
            ...
            getBooleanAttribute(
                ...
                false),
            templateName));
    parseXcsGroupContent(layer, data, componentName, doc, node, group);
    return group.get();
}

xmlNodePtr parseXcsTemplates(
    int layer, Data * data, rtl::OUString const & componentName, xmlDocPtr doc,
    xmlNodePtr node)
{
    OSL_ASSERT(data != 0);
    ...
    for (; p != 0; p = skipBlank(p->next)) {
        ...
        if (isOorElement(p, "group")) {
            ...
            tmpl = parseXcsGroup(layer, *data, componentName, doc, p, tmplName);
        } else if (isOorElement(p, "set")) {
            tmplName = fullTemplateName(
                componentName, getNameAttribute(doc, p));
        ...
        if (!data->templates.insert(NodeMap::value_type(tmplName, tmpl)).second)
        {
            throw css::uno::RuntimeException(
                (rtl::OUString(
                    RTL_CONSTASCII_USTRINGPARAM(
...
xmlNodePtr parseXcsComponent(
    int layer, Data * data, xmlDocPtr doc, rtl::OUString const & component,
    xmlNodePtr node)
{
    OSL_ASSERT(data != 0);
    ...
    parseXcsGroupContent(layer, *data, component, doc, node, comp);
    if (!data->components.insert(NodeMap::value_type(component, comp.get())).
        second)
    {
        throw css::uno::RuntimeException(
            (rtl::OUString(
...
void parseXcsContent(int layer, Data * data, xmlDocPtr doc, xmlNodePtr root) {
    XmlString package(
        xmlGetNsProp(
    ...
    p = skipBlank(parseXcsTemplates(layer, data, comp, doc, p));
    p = skipBlank(parseXcsComponent(layer, data, doc, comp, p));
    if (p != 0) {
        throw css::uno::RuntimeException(
            (rtl::OUString(
                RTL_CONSTASCII_USTRINGPARAM(
    ...
    if (GroupNode * group = dynamic_cast< GroupNode * >(node.get())) {
             ...
             p = skipBlank(p->next))
        {
            if (isOorElement(p, "prop")) {
                ...
                if (modifications) {
                    if (localized == 0) {
                        data->addModification(pathPrefix + name);
                    } else {
                        rtl::OUString path(
                            pathPrefix + name +
                ...
                parseXcuNode(
                    layer, data, componentName, doc, p, subgroup,
             ...
             p = skipBlank(p->next))
        {
            ...
            rtl::Reference< Node > tmpl(data->getTemplate(layer, templateName));
            if (!tmpl.is()) {
                throw css::uno::RuntimeException(
                    (rtl::OUString(
                        RTL_CONSTASCII_USTRINGPARAM(
            ...
            case OPERATION_MODIFY:
                ...
                parseXcuNode(
                    layer, data, componentName, doc, p, j->second,
                    inheritedFinalized || finalizedLayer < layer, false,
                    rtl::OUString());
                break;
            case OPERATION_REPLACE:
                if (!inheritedFinalized && finalizedLayer >= layer) {
                    ...
                    parseXcuNode(
                        layer, data, componentName, doc, p, member, false,
                        false, rtl::OUString());
            ...
            if (modifications) {
                data->addModification(
                    pathPrefix + Data::createSegment(templateName, name));
            }
        }
    } else {
        throw css::uno::RuntimeException(
...
void parseXcuContent(int layer, Data * data, xmlDocPtr doc, xmlNodePtr root) {
    XmlString package(
        xmlGetNsProp(
            root, xmlString("package"),
            xmlString("http://openoffice.org/2001/registry")));
    ...
    rtl::Reference< Node > node(Data::findNode(layer, data->components, comp));
    if (!node.is()) {
        throw css::uno::RuntimeException(
            (rtl::OUString(
                RTL_CONSTASCII_USTRINGPARAM(
    ...
    parseXcuNode(
        layer, data, comp, doc, root, node, finalizedLayer < layer, false,
        rtl::OUString());
}
...
void parseXcsFile(rtl::OUString const & url, int layer, Data * data) {
    ...
    parseXcsContent(layer, data, doc.doc, root);
}
...
void parseXcuFile(rtl::OUString const & url, int layer, Data * data) {
    ...
    parseXcuContent(layer, data, doc.doc, root);
}
    ...
    xmlDocPtr doc, int layer, Dependencies const & dependencies, Data * data)
{
    ...
    xmlNodePtr p(skipBlank(root->xmlChildrenNode));
    while (p != 0 && xmlStrEqual(p->name, xmlString("dependency")) &&
           p->ns == 0)
    {
        XmlString text(xmlNodeListGetString(doc, p->xmlChildrenNode, 1));
        if (dependencies.find(fromXmlString(text.str)) == dependencies.end()) {
            return false;
        }
        p = skipBlank(p->next);
    }
    for (; p != 0; p = skipBlank(p->next)) {
        ...
        {
            parseXcsContent(layer, data, doc, p);
                   ...
                   xmlStrEqual(
                       ...
                       xmlString("http://openoffice.org/2001/registry")))
        {
            parseXcuContent(layer + 1, data, doc, p);
        } else {
            throw css::uno::RuntimeException(
...
void parseModFile(rtl::OUString const & url, Data * data) {
         ...
         p = skipBlank(p->next))
    {
        if (xmlStrEqual(p->name, xmlString("item")) && p->ns == 0) {
            ...
            parseXcuNode(
                NO_LAYER, data, componentName, doc.doc, p, node,
