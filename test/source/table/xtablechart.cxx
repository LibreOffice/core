...
void XTableChart::testSetGetRanges()
{
    ...
    CPPUNIT_ASSERT_MESSAGE("No ranges found", aRanges.getLength() != 0);
    CPPUNIT_ASSERT_EQUAL_MESSAGE("Wrong CellRangeAddress", table::CellRangeAddress(1, 0, 0, 10, 10),
                                 aRanges[0]);
    aRanges[0].EndRow = 7;
    ...
    CPPUNIT_ASSERT_EQUAL(nLength, nLength2);

    for (auto i = 0; i < nLength; i++)
    {
        CPPUNIT_ASSERT_EQUAL_MESSAGE("Successfully able to Set Values", aRanges[i], aRanges2[i]);
    }
}
}
