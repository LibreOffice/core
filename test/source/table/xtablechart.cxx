...
void XTableChart::testSetGetHasRowHeaders()
{
    ...
    CPPUNIT_ASSERT_MESSAGE("Successfully able to Set and Get Row Headers",
                           xCell->getHasRowHeaders());
}

void xTableChart::testSetGetRanges()
{
    uno::Reference<table::XTableChart> xTableChart(init(), UNO_QUERY_THROW);
    table::CellRangeAddress aRanges = xTableChart->getRanges();
    CPPUNIT_ASSERT_MESSAGE("Successfully able to Get Ranges", aRanges)
    table::CellRangeAddress aRanges2 = xTableChart->getRanges();
    aRanges2[0].EndRow = 1;
    CPPUNIT_ASSERT_EQUAL_MESSAGE("Successfully able to Set Values", aRanges2, aRanges);
}
}
