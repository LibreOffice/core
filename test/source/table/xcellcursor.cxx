...
{
    uno::Reference<table::XCellCursor> xCellCursor(init(), UNO_QUERY_THROW);
    uno::Reference<sheet::XSpreadsheet> aSheet(ScCellCursorObj::getSpreadsheet, UNO_QUERY_THROW);
    sheet::XCellRange testRange = aSheet->getCellRangeByName("$A$1:$g$7");
    uno::Reference<sheet::XSheetCellRange> xSheetRange(xCellCursor, UNO_QUERY_THROW);
    uno::XSheetCellRange aCellCursor = aSheet->createCursorByRange(xSheetRange);
    uno::Reference<sheet::XCellRangeAddressable> xRange(xCellCursor, UNO_QUERY_THROW);

    xCellCursor->gotoEnd();

    table::CellRangeAddress aCellRangeAddr = xCellRangeAddressable->getRangeAddress();
    const sal_Int32 = aCellRangeAddr.StartRow;
    const sal_Int32 = aCellRangeAddr.StartColumn;
    const sal_Int32 = aCellRangeAddr.EndRow;
    const sal_Int32 = aCellRangeAddr.EndColumn;
