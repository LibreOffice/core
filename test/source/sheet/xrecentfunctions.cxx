...
void XRecentFunctions::testGetRecentFunctionIds()
{
    ...
    uno::Sequence<int> aIds = xRecentFunctions->getRecentFunctionIds();
...
void XRecentFunctions::testSetRecentFunctionIds()
{
    ...
    // empty list
    uno::Sequence<int> aIds;
    xRecentFunctions->setRecentFunctionIds(aIds);

    aIds = xRecentFunctions->getRecentFunctionIds();
    CPPUNIT_ASSERT_MESSAGE("Unable to set Ids (empty list)", 0 == aIds.getLength());

    // max. size list
    aIds.realloc(nMaxNumber);
    std::srand(std::time(0));
    int nStartIdx = (std::rand() % nMaxNumber) + 1;
    ...
    aIds = xRecentFunctions->getRecentFunctionIds();
    CPPUNIT_ASSERT_MESSAGE("Unable to set Ids (max. size list)", nMaxNumber == aIds.getLength());
}
...
void XRecentFunctions::testGetMaxRecentFunctions()
{
    ...
    CPPUNIT_ASSERT_MESSAGE("Unable to execute getMaxRecentFunctions()",
                           0 != xRecentFunctions->getMaxRecentFunctions());
}
}
