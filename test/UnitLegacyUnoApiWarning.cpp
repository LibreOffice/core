...
/*
 * Unit test for the notice shown when an embedded script touches the legacy
...
/// Test suite for the legacy UNO API notice, with default configuration.
class UnitLegacyUnoApiWarning : public UnitWSD
{
    TestResult testWarningShownForLegacyApiUse();

public:
    UnitLegacyUnoApiWarning()
        : UnitWSD("UnitLegacyUnoApiWarning")
    {
    }
...
UnitBase::TestResult UnitLegacyUnoApiWarning::testWarningShownForLegacyApiUse()
{
    try
    {
...
void UnitLegacyUnoApiWarning::invokeWSDTest()
{
    UnitBase::TestResult result = testWarningShownForLegacyApiUse();
    exitTest(result);
}

UnitBase* unit_create_wsd(void) { return new UnitLegacyUnoApiWarning(); }
