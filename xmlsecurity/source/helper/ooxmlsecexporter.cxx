...
bool OOXMLSecExporter::Impl::isOOXMLBlacklist(const OUString& rStreamName)
{
    static const std::initializer_list<OUStringLiteral> vBlacklist
        = { "/%5BContent_Types%5D.xml", "/docProps/app.xml", "/docProps/core.xml",
            ...
            "/_xmlsignatures" };
    // Just check the prefix, as we don't care about the content type part of the
    // stream name.
    return std::find_if(
               vBlacklist.begin(), vBlacklist.end(),
               [&](const OUStringLiteral& rLiteral) { return rStreamName.startsWith(rLiteral); })
           != vBlacklist.end();
}
...
void OOXMLSecExporter::Impl::writeManifestReference(const SignatureReferenceInformation& rReference)
{
    ...
    // Transforms
    if (rReference.ouURI.endsWith("?ContentType=application/"
...
void OOXMLSecExporter::Impl::writeSignatureInfo()
{
    ...
    m_xDocumentHandler->startElement(
        ...
        uno::Reference<xml::sax::XAttributeList>(new SvXMLAttributeList()));
    m_xDocumentHandler->characters("9"); // This is what MSO 2016 writes, though
        // [MS-OFFCRYPTO] doesn't document what
        // the value means.
