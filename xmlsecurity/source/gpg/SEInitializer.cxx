...
{
    SAL_DEBUG("SEInitializerGpg::SEInitializerGpg");
...
{
    SAL_DEBUG("SEInitializerGpg::createSecurityContext");
    try
    {
        /* Build XML Security Context */
        Reference< XXMLSecurityContext > xSecCtx = xml::crypto::XMLSecurityContext::create(m_xContext);

        Reference< XSecurityEnvironment > xSecEnv = xml::crypto::SecurityEnvironment::create(m_xContext);
        Reference< XUnoTunnel > xSecEnvTunnel(xSecEnv, uno::UNO_QUERY_THROW);
        SecurityEnvironmentGpg* pSecEnv = reinterpret_cast<SecurityEnvironmentGpg*>(
            sal::static_int_cast<sal_uIntPtr>(xSecEnvTunnel->getSomething(SecurityEnvironmentGpg::getUnoTunnelId() )));
...
{
    SAL_DEBUG("SEInitializer_getImplementationName");
...
uno::Sequence< OUString > SAL_CALL SEInitializer_getSupportedServiceNames(  )
    throw (uno::RuntimeException)
{
    SAL_DEBUG("SEInitializer_getSupportedServiceNames");
...
uno::Reference< uno::XInterface > SAL_CALL SEInitializer_createInstance( const uno::Reference< lang::XMultiServiceFactory > & rxMSF)
    throw( uno::Exception )
{
    SAL_DEBUG("SEInitializer_createInstance");
