...
bool PDFSigningTest::sign(const OUString& rInURL, const OUString& rOutURL, size_t nOriginalSignatureCount)
{
    ...
    // Sign it and write out the result.
    {
        ...
        for (auto& cert : aCertificates)
        {
            ...
            {
                SAL_WARN("test.xmlsecurity.pdfio", "Skipping a certificate that is not yet valid or already not valid");
            }
            else
            {
                ...
                if (!bSignResult)
                {
                    ...
                    {
                        SAL_WARN("test.xmlsecurity.pdfio", "Skipping a certificate without a private key");
