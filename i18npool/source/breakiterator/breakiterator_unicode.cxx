        ...
        const LineBreakUserOptions& /*rOptions*/ ) throw(uno::RuntimeException)
{
    ...
    while (GlueSpace) {
        if (line.aBreakIterator->preceding(nStartPos + 1) == nStartPos) { 
            ...
            line.aBreakIterator->preceding(nStartPos + 1); 

            sal_Int32 nStartPosWordEnd = nStartPos;
            while (line.aBreakIterator->current() < nStartPosWordEnd && u_ispunct((sal_uInt32)Text[nStartPosWordEnd])) 
                nStartPosWordEnd --;
            ...
            while (nStartPosWordEnd < Text.getLength() && (u_ispunct((sal_uInt32)Text[nStartPosWordEnd]))) 
                nStartPosWordEnd ++;
