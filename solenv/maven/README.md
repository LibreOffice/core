...
This file documents the prerequites and workflow to upload LibreOffice
...
`Buck` driven build. That why Maven must be installed too. This document
only descibes how to install `Buck` and other pre-requisities to upload
the artifacts to maven Central. To be able to upload the API to Maven,
Central, access must be granted to LibreOffice project on OSSRH.
