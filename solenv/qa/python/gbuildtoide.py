...
class CheckGbuildToIdeModules(unittest.TestCase):
    def setUp(self):
        ...
        shutil.copyfile(os.path.join(os.environ['SRCDIR'], 'config_host.mk'), os.path.join(self.tempsrc, 'config_host.mk'))
        shutil.copyfile(os.path.join(os.environ['SRCDIR'], 'config_host_lang.mk'), os.path.join(self.tempsrc, 'config_host_lang.mk'))
        shutil.copyfile(os.path.join(os.environ['SRCDIR'], 'Repository.mk'), os.path.join(self.tempsrc, 'Repository.mk'))
        shutil.copyfile(os.path.join(os.environ['SRCDIR'], 'RepositoryExternal.mk'), os.path.join(self.tempsrc, 'RepositoryExternal.mk'))
        shutil.copyfile(os.path.join(os.environ['SRCDIR'], 'RepositoryFixes.mk'), os.path.join(self.tempsrc, 'RepositoryFixes.mk'))
        shutil.copytree(os.path.join(os.environ['SRCDIR'], 'config_host'),  os.path.join(self.tempsrc, 'config_host'))
        shutil.copytree(os.path.join(os.environ['SRCDIR'], 'solenv'),  os.path.join(self.tempsrc, 'solenv'))

    def tearDown(self):
        subprocess.check_call(['rm', '-rf', self.tempsrc])
        subprocess.check_call(['rm', '-rf', self.tempwork])
    ...
    def test_gbuildtoide(self):
        ...
        for module in modules:
            ...
            subprocess.check_call(['make', 'gbuildtoide', '--debug=v', 'WORKDIR=%s' % self.tempworkmixed])
