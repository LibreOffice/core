...
//ConstructPoEntry from GenPo
PoEntry::PoEntry(const GenPoEntry& rGenPo)
    : m_aGenPo( rGenPo )
    , m_sSourceFile( OString() )
    , m_sGroupId( OString() )
    , m_sLocalId( OString() )
...
OString ImplGetElement(const OString& rText, const sal_Int32 nToken,
                       const OString& rBefore, const sal_Char cAfter = '\0')
{
    OString sToken = rText.getToken(nToken,'\n');
    sal_Int32 nFirstIndex = sToken.indexOf(rBefore) + rBefore.getLength();
    sal_Int32 nAfterIndex = cAfter=='\0' || sToken.indexOf(cAfter)==-1 ?
                            sToken.getLength() :
                            sToken.lastIndexOf(cAfter);
    return sToken.copy( nFirstIndex,nAfterIndex-nFirstIndex );
}
...
PoHeader::PoHeader(const GenPoEntry& rGenPo)
    : m_aGenPo( rGenPo )
    , m_sExtractionSource( OString() )
    , m_sProjectIdVersion( OString() )
    , m_sReportMsgidBugsTo( OString() )
...
void PoHeader::SetMembers()
{
    if( !m_aGenPo.isNull() )
    {
        ...
        m_bTemplate = sTemp.indexOf("Language:")==-1;
        ...
        m_sCharset = ImplGetElement(sTemp,nToken,"=");
