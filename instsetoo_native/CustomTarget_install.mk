...
ifeq (ODK,$(filter ODK,$(BUILD_TYPE)))
	...
	$(call instsetoo_native_install_command, \
		$(foreach pkgformat,$(PKGFORMAT),\
			"$(if $(filter WNT,$(OS)),openoffice)$(COMMA)en-US$(COMMA)$(COMMA)$(COMMA)$(pkgformat)$(COMMA)$(if $(filter-out archive,$(pkgformat)),strip,nostrip)" \
			$(if $(filter ODK,$(BUILD_TYPE)), \
				"$(if $(filter WNT,$(OS)),sdkoo)$(COMMA)en-US$(COMMA)_SDK$(COMMA)$(COMMA)$(pkgformat)$(COMMA)nostrip") \
			$(if $(filter HELP,$(BUILD_TYPE))$(filter MACOSX,$(OS)), \
				$(foreach lang,$(gb_HELP_LANGS), \
					"$(if $(filter WNT,$(OS)),ooohelppack)$(COMMA)$(lang)$(COMMA)$(COMMA)-helppack$(COMMA)$(pkgformat)$(COMMA)nostrip" )) \
			$(if $(filter-out WNT,$(OS)), $(foreach lang,$(instsetoo_native_WITH_LANG), \
					"$(if $(filter WNT,$(OS)),ooolangpack)$(COMMA)$(lang)$(COMMA)$(COMMA)-languagepack$(COMMA)$(pkgformat)$(COMMA)nostrip" ) )))
