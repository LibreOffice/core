...
OUString GetString(const char* pResId)
{
    static const std::locale* s_pLocale = []() -> const std::locale* {
        // Add LO directory to DLL search path, needed in Translate::Create to initialize
        // boost::locale::generator; do that only for the duration of initialization, and
        ...
        struct Guard
        {
            ...
            Guard()
            {
                if (DWORD nBufSize = GetDllDirectoryW(0, nullptr))
                {
                    path = std::make_unique<wchar_t[]>(nBufSize);
                    GetDllDirectoryW(nBufSize, path.get());
                }
                SetDllDirectoryW(GetLOPath());
            }
            ~Guard()
            {
                SetDllDirectoryW(path.get()); // nullptr is OK here, restoring default search order
            }
        } sDLLPathGuard;

        // Initialize soffice bootstrap: see getIniFileName_Impl for reference
        OUString sPath = o3tl::toU(GetSofficeExe());
