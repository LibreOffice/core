                                           ...
                                           const OUString& sDocumentService)
{
    ...
    sofficePath = "D:\\lode\\dev\\core\\instdir\\program\\soffice.exe";
    OUString workDir("D:\\lode\\dev\\core\\instdir\\program");

    IObjectCollection* aCollection;
    result = CoCreateInstance(CLSID_EnumerableObjectCollection, NULL, CLSCTX_INPROC,
                              IID_PPV_ARGS(&aCollection));
    if (FAILED(result))
    {
        SAL_WARN("vcl", "Creating EnumerableObjectCollection failed.");
        return;
    }
    ...
    for (auto item : aJumpListItems)
    {
        ...
        IPropertyStore *pps;
        result = pShellLinkItem->QueryInterface(IID_PPV_ARGS(&pps));
        if (FAILED(result))
        {
            SAL_WARN("vcl", "Querying interface 'IPropertyStore' failed.");
            continue;
        }
        PROPVARIANT propvar;
        ...
        result = pShellLinkItem->SetDescription(
            reinterpret_cast<LPCSTR>(o3tl::toW(item.name.getStr())));
        if (FAILED(result))
        {
            SAL_WARN("vcl", "Setting description '" << item.name << "' failed.");
            continue;
        }
        result = pShellLinkItem->SetPath(reinterpret_cast<LPCSTR>(o3tl::toW(sofficePath.getStr())));
        if (FAILED(result))
        {
            SAL_WARN("vcl", "Setting path '" << sofficePath << "' failed.");
            continue;
        }
        result = pShellLinkItem->SetWorkingDirectory(reinterpret_cast<LPCSTR>(o3tl::toW(workDir.getStr())));
        if (FAILED(result))
        {
            SAL_WARN("vcl", "Setting workdir '" << workDir << "' failed.");
            continue;
        }
        result = pShellLinkItem->SetArguments(
            reinterpret_cast<LPCSTR>(o3tl::toW(item.arguments.getStr())));
        if (FAILED(result))
        {
        ...
        result = pShellLinkItem->SetIconLocation(
            reinterpret_cast<LPCSTR>(o3tl::toW(item.iconPath.getStr())), 0);
        if (FAILED(result))
        {
    ...
    result = aCollection->QueryInterface(IID_PPV_ARGS(&pObjectArray));
    if (FAILED(result))
    {
        SAL_WARN("vcl", "Querying interface 'IObjectArray' failed.");
        return;
    }
