...
namespace Item
{
    ...
    class CntMultiValue;
    typedef IBaseStaticHelper<CntMultiValue, IAdministrator_unsortedList> CntMultiValueStaticHelper;

    // if this should be based on faster IAdministrator_sort, changes needed would be:
    // - typedef IBaseStaticHelper<CntMultiValue, IAdministrator_sort> CntMultiValueStaticHelper;
...
namespace Item
{
    ...
    public:
        void checkCntMultiValue()
        {
            ...
            // to see the diff between IAdministrator_unsortedList and
            // IAdministrator_sort, see instructions in CntMultiValue above,
            // create version for IAdministrator_sort and raise this number.
        ...
        void checkCntMultiValueAtISet()
        {
            ...
            if(const ISet::StateAndItem<const CntMultiValue> Item(aSet->GetStateAndItem<const CntMultiValue>()); Item.IsSet())
            {
        ...
        void checkSimpleItemsAtISet()
        {
            ...
            ISet::SharedPtr aSet(ISet::Create(aModelSpecificIValues));
            const ISet::StateAndItem<const CntInt16> aActEmpty(aSet->GetStateAndItem<const CntInt16>());

            aSet->SetItem(CntInt16::Create(4));
            const ISet::StateAndItem<const CntInt16> aActA(aSet->GetStateAndItem<const CntInt16>());

            aSet->SetItem(CntInt16::GetDefault());
            const ISet::StateAndItem<const CntInt16> aActB(aSet->GetStateAndItem<const CntInt16>());

            aSet->SetItem(CntInt16::Create(12));
            const ISet::StateAndItem<const CntInt16> aActC(aSet->GetStateAndItem<const CntInt16>());

            aSet->SetItem(CntOUString::Create("Teststring - not really useful :-)"));
            const ISet::StateAndItem<const CntOUString> aActStr(aSet->GetStateAndItem<const CntOUString>());

            const ISet::StateAndItem<const CntInt16> ItemDADA(aSet->GetStateAndItem<const CntInt16>());

            if(const ISet::StateAndItem<const CntOUString> Item(aSet->GetStateAndItem<const CntOUString>()); Item.IsSet())
            {
                const ISet::IState aSAAState = Item.GetIState();
