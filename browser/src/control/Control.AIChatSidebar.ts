...
namespace cool {
	...
	export class AIChatSidebar {
		...
		private applyRatingBadge(): void {
			...
			if (!badge) return;
			const rating = this.getEthicalRating();
			badge.setAttribute('data-rating', rating.letter);
			// Ideal tooltip: Ethical AI rating: {0} ({1}). Based on open-source licensing, self-hosting, and training data.
			const aria = _(
				'Ethical AI rating: {0} ({1}). Based on open-source licensing and training data.',
			)
				.replace('{0}', rating.letter)
				.replace('{1}', rating.label);
		...
		// Ethical AI rating — coarse, model-name-keyed.
		// Criteria: open software, open weights,
		// open training data, renewable/on-device hosting. Score 4/4=A … 0-1/4=D.
		private static readonly MODEL_RATINGS: ReadonlyArray<{
			pattern: RegExp;
			letter: 'A' | 'B' | 'C' | 'D';
			labelKey: string;
		}> = [
			{ pattern: /^gpt[-_]/i, letter: 'D', labelKey: 'Poor' },
			{ pattern: /^o[1-9][-_]?/i, letter: 'D', labelKey: 'Poor' },
			{ pattern: /^claude[-_]/i, letter: 'D', labelKey: 'Poor' },
			{ pattern: /^gemini[-_]/i, letter: 'D', labelKey: 'Poor' },
			{ pattern: /mistral/i, letter: 'C', labelKey: 'Fair' },
			{ pattern: /llama/i, letter: 'B', labelKey: 'Good' },
			{ pattern: /qwen/i, letter: 'B', labelKey: 'Good' },
			{ pattern: /olmo/i, letter: 'A', labelKey: 'Best' },
			{ pattern: /pythia/i, letter: 'A', labelKey: 'Best' },
		];

		private getEthicalRating(): {
			letter: 'A' | 'B' | 'C' | 'D' | 'U';
			label: string;
		} {
			const model = app.map.aiModelName || '';
			if (model) {
				for (const entry of AIChatSidebar.MODEL_RATINGS) {
					if (entry.pattern.test(model))
						return { letter: entry.letter, label: _(entry.labelKey) };
				}
			}
			return { letter: 'U', label: _('Unknown') };
		}
