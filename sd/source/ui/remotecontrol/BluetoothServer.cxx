                        ...
                        const char *pPropertyName, bool *pBoolean )
{
    ...
    *pBoolean = false;
    ...
    dbus_message_iter_append_basic( &itIn, DBUS_TYPE_STRING, pProperties->maPath.getStr() );
...
setupBluez5Profile1(DBusConnection* pConnection, std::vector<Communicator*>* pCommunicators)
{
    ...
    if (bErr || dbus_error_is_set(&aError))
    {
        ...
        SAL_WARN("sdremote.bluetooth",
                 "If the above error message complains about \"security policies in the configuration file\", you will have to install  /etc/dbus-1/system.d/org.libreoffice.bluez.profile.conf");
        // TODO: add location?
        dbus_error_free(&aError);
        return;
    }
