    ...
    if(pSdrBaseHint)
    {
        switch(pSdrBaseHint->GetSdrHintKind())
        {
            case HINT_PAGEORDERCHG:
            {
                ...
                // page.
                SdDrawDocument* pDocument = dynamic_cast< SdDrawDocument* >(&rBroadcaster);

                if(pDocument)
                {
                    if(pDocument->GetMasterSdPageCount(PK_STANDARD) == pDocument->GetMasterSdPageCount(PK_NOTES))
                    {
                        AnalyzeUsedMasterPages(*pDocument);
                    }
                }
                break;
            }
