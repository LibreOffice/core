...
void SdTiledRenderingTest::callbackImpl(int nType, const char* pPayload)
{
    ...
    break;
    case LOK_CALLBACK_DOCUMENT_NOTIFY:
    {
        boost::property_tree::ptree aTree;
        std::stringstream aStream(pPayload);
        boost::property_tree::read_json(aStream, aTree);
        auto aInserted = aTree.get<std::string>("PageInserted", "");
        if (!aInserted.empty())
        {
            m_aPageList.push_back(std::atoi(aInserted.data()));
        }
        auto aDeleted = aTree.get<std::string>("PageDeleted", "");
        if (!aDeleted.empty())
        {
            m_aPageList.push_back(std::atoi(aDeleted.data()));
        }
    }
    break;
    }
