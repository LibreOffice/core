...
CPPUNIT_TEST_FIXTURE(SdUiImpressTest, testTdf167029PasteKeepsOutlineLevel)
{
    ...
    auto posName = [](PastePos ePos) -> std::string {
        switch (ePos)
        {
            case PastePos::AfterBullet:
                return "AfterBullet";
            case PastePos::InMiddle:
                return "InMiddle";
            case PastePos::AtEnd:
                return "AtEnd";
            case PastePos::IntoEmpty:
                return "IntoEmpty";
        }
        return "?";
    };
    auto checkCase = [this, &posName](PastePos ePos, bool bRtf) {
        ...
        {
            case PastePos::AfterBullet:
                aDest = ESelection(1, 0, 1, 0);
                break;
            case PastePos::InMiddle:
                aDest = ESelection(1, nLen / 2, 1, nLen / 2);
                break;
            case PastePos::AtEnd:
                aDest = ESelection(1, nLen, 1, nLen);
                break;
            case PastePos::IntoEmpty:
                aDest = ESelection(1, 0, 1, 0);
                break;
