...
CPPUNIT_TEST_FIXTURE(SdPNGExportTest, testTdf126319)
{
    ...
    Bitmap aBMP = aBMPEx.GetBitmap();
    {
        ...
        for (tools::Long nX = 1; nX < aSize.Width() - 1; ++nX)
        {
            ...
            // Without the fix in place, this test would have failed with
            // - Expected: Color: R:255 G:214 B:214 A:0
            // - Actual  : Color: R:77 G:0 B:0 A:0
            assertColorsAreSimilar(Color(0xFF, 0xD6, 0xD6), aColorBottom, 5);
        }
        ...
        for (tools::Long nY = 1; nY < aSize.Height() - 1; ++nY)
        {
            ...
            assertColorsAreSimilar(COL_LIGHTRED, aColorLeft, 5);
            assertColorsAreSimilar(Color(0xFF, 0xD6, 0xD6), aColorRight, 5);
        }
    }
}
