...
CPPUNIT_TEST_FIXTURE(SwLayoutWriter6, testBtlrCell)
{
    ...
    // x shifted from 1915 to 1911 after preferring Win metrics when hhea
    // and Win totals match in VCL font metric calculation
...
CPPUNIT_TEST_FIXTURE(SwLayoutWriter6, testCool15942_tableSplitRowReformat)
{
    ...
    OUString oLineHeight = getXPath(
        pXmlDoc, "//page[1]/body/tab[2]/row[2]/cell[2]/txt/infos/prtBounds", "height");
...
CPPUNIT_TEST_FIXTURE(SwLayoutWriter6, testCool15691_headerTextWrapsAroundImage)
{
    ...
    for (const auto* pFileName : { "Cool15691_headerTextWrapsAroundImage.docx",
                                   "Cool15691_headerTextWrapsAroundImageLargest.docx" })
    {
        createSwDoc(pFileName);
        xmlDocUniquePtr pXmlDoc = parseLayoutDump();
        CPPUNIT_ASSERT(pXmlDoc);
        ...
        for (int i = 1; i <= 2; ++i)
        {
            ...
            CPPUNIT_ASSERT_MESSAGE(OString(OString::Concat(pFileName) + ", paragraph "
                                           + OString::number(i) + ", gap "
                                           + OString::number(nGap))
                                       .getStr(),
                                   nGap >= 2268);
        }
