...
CPPUNIT_TEST_FIXTURE(SwUiWriterTest4, testTdf91292)
{
    ...
    xPropertySet->setPropertyValue(u"FillStyle"_ustr, cpo::uno::Any(drawing::FillStyle_NONE));
...
CPPUNIT_TEST_FIXTURE(SwUiWriterTest4, testTdf151548_tabNavigation2)
{
    ...
    SwXTextDocument* pTextDoc = getSwTextDoc();
    pTextDoc->postKeyEvent(COKitKeyEventType::DOWN, 32, KEY_SPACE);
...
CPPUNIT_TEST_FIXTURE(SwUiWriterTest4, testTdf151548_tabNavigation)
{
    ...
    for (auto it = pMarkAccess->getFieldmarksBegin(); it != pMarkAccess->getFieldmarksEnd(); ++it)
    {
        ...
        pTextDoc->postKeyEvent(COKitKeyEventType::DOWN, 32, KEY_SPACE); // toggle checkbox on
        pTextDoc->postKeyEvent(COKitKeyEventType::DOWN, 0, KEY_TAB); // move to next control
        Scheduler::ProcessEventsToIdle();
    }
    ...
    for (auto it = pMarkAccess->getFieldmarksBegin(); it != pMarkAccess->getFieldmarksEnd(); ++it)
    {
        ...
        CPPUNIT_ASSERT(pCheckBox->IsChecked());
        pTextDoc->postKeyEvent(COKitKeyEventType::DOWN, 32, KEY_SPACE); // toggle checkbox off
        Scheduler::ProcessEventsToIdle();
