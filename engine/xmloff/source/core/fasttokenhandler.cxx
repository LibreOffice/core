...
static const std::pair<cpo::uno::Sequence<sal_Int8>, OUString>& getNames(sal_Int32 nToken)
{
    static const auto saTokenNames = []()
    {
        static_assert(std::size(wordlist) == XML_TOKEN_COUNT);

        std::vector<std::pair<cpo::uno::Sequence<sal_Int8>, OUString>> names(std::size(wordlist));
    ...
    static const std::pair<cpo::uno::Sequence<sal_Int8>, OUString> EMPTY;
    return EMPTY;
}
...
// XFastTokenHandler
cpo::uno::Sequence< sal_Int8 > FastTokenHandler::getUTF8Identifier( sal_Int32 nToken )
{
...
sal_Int32 FastTokenHandler::getTokenFromUTF8( const cpo::uno::Sequence< sal_Int8 >& rIdentifier )
{
