...
std::span<const double> lclAsSpan(const cpo::uno::Sequence<double>& rValues)
{
...
CorrelationCircleChart::CorrelationCircleChart(
    const rtl::Reference<ChartType>& xChartTypeModel, sal_Int32 nDimensionCount)
...
std::pair<double, double> CorrelationCircleChart::getMinimumAndMaximumYInRange(
    double /*fMinimumX*/, double /*fMaximumX*/, sal_Int32 /*nAxisIndex*/)
{
    return { -1.0, 1.0 };
}
...
LegendSymbolStyle CorrelationCircleChart::getLegendSymbolStyle()
{
    return LegendSymbolStyle::Line;
}

cpo::uno::Any CorrelationCircleChart::getExplicitSymbol(const VDataSeries& rSeries,
    ...
    const rtl::Reference<SvxShapeGroupAnyD>& xTarget)
{
    ...
    const drawing::Direction3D aCircleSize(
        std::abs(aUpperRight.PositionX - aLowerLeft.PositionX),
        std::abs(aUpperRight.PositionY - aLowerLeft.PositionY), 0.0);

    rtl::Reference<SvxShapeCircle> xCircle
        = ShapeFactory::createCircle2D(xTarget, aOrigin, aCircleSize);
    if (xCircle.is())
    {
        xCircle->SvxShape::setPropertyValue(u"FillStyle"_ustr,
                                            cpo::uno::Any(drawing::FillStyle_NONE));
        xCircle->SvxShape::setPropertyValue(u"LineStyle"_ustr,
                                            cpo::uno::Any(drawing::LineStyle_SOLID));
        xCircle->SvxShape::setPropertyValue(u"LineColor"_ustr, cpo::uno::Any(nGuideColor));
    }
    ...
    const rtl::Reference<SvxShapeGroupAnyD>& xTextTarget)
{
    ...
    PlottingPositionHelper& rPosHelper
        = getPlottingPositionHelper(pSeries->getAttachedAxisIndex());
    m_pPosHelper = &rPosHelper;

    const drawing::Position3D aOrigin(rPosHelper.transformLogicToScene(0.0, 0.0, fLogicZ, false));
    const drawing::Position3D aTip(rPosHelper.transformLogicToScene(
        *oCorrelationX, *oCorrelationY, fLogicZ, false));
    ...
    if (pSymbolProperties && pSymbolProperties->Style != SymbolStyle_NONE)
    {
        ...
        if (pSymbolProperties->Style == SymbolStyle_STANDARD)
            ShapeFactory::createSymbol2D(xPointGroupShape_Shapes, aTip, aSymbolSize,
                                         pSymbolProperties->StandardSymbol,
    ...
    if (pSeries->getDataPointLabelIfLabel(nPointIndex))
    {
        ...
        switch (pSeries->getLabelPlacement(nPointIndex, m_xChartTypeModel,
        ...
        const awt::Point aScreenPosition2D(
        ...
        createDataLabel(xTextTarget, *pSeries, nPointIndex, *oCorrelationY, 1.0,
