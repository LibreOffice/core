...
$(call gb_ExternalProject_get_state_target,argon2,build):
	...
	$(call gb_ExternalProject_run,build,\
		$(gb_MSBUILD_CCACHE_ENV) MSBuild.exe vs2015/Argon2OptDll/Argon2OptDll.vcxproj \
			/p:SolutionDir="$(gb_UnpackedTarball_workdir)/argon2/" \
			$(gb_MSBUILD_CONFIG_AND_PLATFORM) $(gb_MSBUILD_DEPENDENCY_TRACKING) $(gb_MSBUILD_DEBUGINFO) $(gb_MSBUILD_CCACHE) \
			/p:PlatformToolset=$(VCTOOLSET) /p:VisualStudioVersion=$(VCVER) /ToolsVersion:Current \
			$(if $(filter 10,$(WINDOWS_SDK_VERSION)),/p:WindowsTargetPlatformVersion=$(UCRTVERSION)) \
		&& $(gb_MSBUILD_CCACHE_ENV) MSBuild.exe vs2015/Argon2OptTestCI/Argon2OptTestCI.vcxproj \
			/p:SolutionDir="$(gb_UnpackedTarball_workdir)/argon2/" \
			$(gb_MSBUILD_CONFIG_AND_PLATFORM) $(gb_MSBUILD_DEPENDENCY_TRACKING) $(gb_MSBUILD_DEBUGINFO) $(gb_MSBUILD_CCACHE) \
			/p:PlatformToolset=$(VCTOOLSET) /p:VisualStudioVersion=$(VCVER) /ToolsVersion:Current \
			$(if $(filter 10,$(WINDOWS_SDK_VERSION)),/p:WindowsTargetPlatformVersion=$(UCRTVERSION)) \
